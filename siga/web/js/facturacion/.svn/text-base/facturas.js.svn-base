 ...
 */
 ...
 function Cantidad(id)
 {
   ...
   var colrgo=col+7;
   var coltotal=col+8;
   var colporcrgo=col+11;
   var coldatrec=col+13;
   var coldatrec2=col+14;
   var colmarrec=col+9;
   ...
   var datrec=name+"_"+fil+"_"+coldatrec;
   var datrec2=name+"_"+fil+"_"+coldatrec2;
   var marrec=name+"_"+fil+"_"+colmarrec;
   ...
   else
   {
    ...
    if ($('fapedido_exeiva').checked==false) {
      if ($(datrec).value=="") {
       ...
       if (calrec>0) {
         $(datrec).value=$(datrec2).value;
         $(marrec).checked=true;
         recalcularecargosPed(id);
       }
     }else {
       recalcularecargosPed(id);
       calrec=toFloat(recargo);
     }
    }
...
function eliminarfechas(id)
  ...
  function llenargridfechas(id)
  {
    ...
    var id1=name+"_"+fil+"_1";
    var id2=name+"_"+fil+"_2";
    var id3=name+"_"+fil+"_3";
 ...
 function Precio(id)
 {
   ...
   var colrgo=col+3;
   var coltotal=col+4;
   var colporcrgo=col+7;
   var coldatrec=col+9;
   var coldatrec2=col+10;
   var colmarrec=col+5;
 ...
 function ajaxarticulos(e,id)
 {
   var aux = id.split("_");
    ...
    var colpre=col+6;
    var colpree=col+7;
    var colporrgo=col+13;
    var coldatrec=col+15;
    var coldatrec2=col+16;
    ...
    if (e.keyCode==13 || e.keyCode==9)
    {
      if ($(id).value!="")
      {
        new Ajax.Request(getUrlModulo()+'ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=3&cajtexmos='+descripcion+'&cajtexcom='+id+'&porrgo='+porrgo+'&datrec2='+datrec2+'&datrec='+$(datrec).value+'&codigo='+cod})
  ...
  function montoMarcados()
  {
	...
	while (fil<colart)
	{
      ...
      if ($(check).checked==true)
      {
        if ($(cant).value!="")
        {
          if ($('fafactur_calrecdes').value=='S')
            monto_marcados=monto_marcados + ((nprecio*ncant)-nmondesc);
          else  
      ...
      if ($(codrgo).value!="")
      {
	    ...
	    while (fil<regart)
	    {
	     ...
	     if ($('fafactur_trajo').value=='S')
	     {
            ...
            {
              if ($('fafactur_calrecdes').value=='S')
                 montoArt= montoArt - ((nprecio * ncant)-ndesc);
              else
                     ...
                     if ($(precio2)){
	                  ...
	                  new Ajax.Request(getUrlModuloAjax(), {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=6&articulo='+codart+'&recargo='+codrgo})
               	      if ($('fafactur_trajo').value=='S')
               	      {
               	        ...
               	        {
                          if ($('fafactur_calrecdes').value=='S')
                            monTot= monTot - ((nprecio2 * ncant2)-toFloat(mondesc));
                          else
      	         ...
      	         while (i<regart)
                 {
                  ...
                  new Ajax.Request(getUrlModuloAjax(), {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=6&articulo='+codart+'&recargo='+codrgo})
           	      if ($('fafactur_trajo').value=='S')
           	      {
           	        ...
           	        {
                      if ($('fafactur_calrecdes').value=='S')
                        monTot= monTot - ((nprecio2 * ncant2)-ndesc2);
                      else
           	            monTot= monTot - (nprecio2 * ncant2);
           	        }
           	        else
...
//var nf=parseInt($('fafactur_numfilas').value);
     ...
     while (fil<regart)
     {
      ...
      if ($(precio)){
       ...
       {
        if ($('fafactur_calrecdes').value=='S')
          calcularmonto= calcularmonto + ((nprecio * ncant)-ndesc);
        else 
  ...
  function calcularMontTotMod()
  {
    ...
    var descuento="ax_"+mifila+"_18";
      ...
      if ($(precio)){
       ...
       var ncant= toFloat(cant);
       var ndes= toFloat(descuento);

       if (($(precio).value!="" || $(precioe).value!="0,00") && $(cant).value!="")
       {
        if ($('fafactur_calrecdes').value=='S')
          calcularmonto= calcularmonto + ((nprecio * ncant)-ndes);
        else
          calcularmonto= calcularmonto + ( nprecio * ncant);
       }
       }
    ...
    if (monTot!=0)
    {
        ...
        var ndes2= toFloat(descto);
	      ...
	      if ($('fafactur_trajo').value=='S')
	      {
	       ...
	       {
           if ($('fafactur_calrecdes').value=='S')
             monTot= monTot - ((nprecio2 * ncant2)-ndes2);
           else
	           monTot= monTot - (nprecio2 * ncant2);
	       }
	     }
     ...
     if ($(codart)){
      ...
      if ($(check).checked==true)
      {
        ...
        if ($('fafactur_trajo').value=='S')
        {
         ...
         {
           if ($('fafactur_calrecdes').value=='S')
             monTot= monTot - redondear(((nprecio2 * ncant2)-ndesc2),2);
           else 
...
function retornaMontoMod2(codrgo)
 ...
 function aceptar1()
 {
   ...
   else alert('Debe seleccionar una Caja');
 }
  ...
  function cambios()
 {
   ...
   if ($('fafactur_tipref').value=='E')
   {
     $('fafactur_combo').readonly=true;
     $$('.botoncat')[5].hide();
     if ($('fafactur_manprecon').value=='S')
     {
        if (confirm("¿Desea Cargar Presupuestos por Contrato?"))
          $('fafactur_esprecon').value='S';
        else
          $('fafactur_esprecon').value='N';
     }
     $('pedid').hide();     
     $('despach').hide();
     $('presupu').show();
...
/*  function calcularTotalDescuento()
 ...
 function ajaxarticulosfactura(e,id)
 {
    ...
    var codalmcaja=$('fafactur_codalmcaj').value;
    var filagrid= fil;
    ...
    if (e.keyCode==13 || e.keyCode==9)
    {
      if ($(id).value!="")
      {
       if (londefart <= lonart)
       {
        ...
        //{
          new Ajax.Updater('lista',getUrlModuloAjax(), {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=11&cajtexmos='+descripcion+'&cajtexcom='+id+'&canent='+canent+'&docref='+docrefiere+'&ctaprove='+ctaprove+'&blanco2='+blanco2+'&unidad='+unidad+'&cantidad='+cantidad+'&existencia='+existencia+'&montrgo='+montrgo+'&cantot='+cantot+'&total='+total+'&filagrid='+filagrid+'&blanco='+blanco+'&precio='+precio+'&precio2='+precio2+'&codalmcaja='+codalmcaja+'&codigo='+cod})
          if (docrefiere=='V')
          {
            new Ajax.Updater(precio, getUrlModulo()+'ajax', {asynchronous:true, evalScripts:true, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=12&id='+$('id').value+'&precio2='+precio2+'&fecemi='+$('fafactur_fecfac').value+'&codigo='+cod});
 ...
 function despachos_pedido()
 {
   if ($('fafactur_rifpro').value!="")
   {
     ...
     var almacen=$('fafactur_codalm').value;
     var presupcon=$('fafactur_esprecon').value;
     var moneda=$('fafactur_tipmon').value;
     if (presupcon=='S')
       $('fechasper').show();
     else
       $('fechasper').hide();
     new Ajax.Updater('peddes',getUrlModuloAjax(), {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=13&tipref='+tipref+'&fechoy='+fechoy+'&fecdes='+fecdes+'&codalm='+almacen+'&fechas='+fechas+'&presupcon='+presupcon+'&moneda='+moneda+'&cedula='+cirif})
   }
   else
   {
 ...
 function ocul_ped_des()
 {
   ...
   if ($('fafactur_manprecon').value=='S' && $('fafactur_esprecon').value=='S')
   {
      if ($('fafactur_fecper1').value!='' && $('fafactur_fecper2').value!='')
      {
        new Ajax.Updater('articulos', '/facturacion'+getEnv()+'.php/fafactur/ajax', {asynchronous:true, evalScripts:true, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:Form.serialize(document.getElementById('sf_admin_edit_form'))}); return false;
      }else {
         alert_('Debe indicar el período');
      }
   }else {
     new Ajax.Updater('articulos', '/facturacion'+getEnv()+'.php/fafactur/ajax', {asynchronous:true, evalScripts:true, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:Form.serialize(document.getElementById('sf_admin_edit_form'))}); return false;
   }
 ...
 function gridarticulosfocus(filrow,colum,codigoref)
 {
   ...
   else
   {
    ...
    {
      alert('El Presupuesto ya fue Seleccionado');
    }
    else
    {
...
//     var oculid="ax"+filastot+"id";
   ...
   if (aux2[0]=='0')
     ...
     if ($('fafactur_tierecar').value=='S')
     {
       new Ajax.Request(getUrlModuloAjax(), {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=35&referencia='+$(codref).value+'&tipref='+$('fafactur_tipref').value+'&fila='+filastot+'&ida='+codart+'&codart='+$(codart).value})
     }
...
//var nf=parseInt($('fafactur_numfilas').value);
 ...
 if (am>0)
 {
   if ($(id).value!="")
   {
	   ...
	   {
	     if ($('fafactur_afeinvfac').value=='S'){
         $(exist).value=$(distot).value;
        var num2=toFloat(exist);
        if ($(blanco).value=='A' || num1<=num2 || $('fafactur_tipref').value=='V')
        {
          if ($(blanco).value=='A') {
          var canent=cantidadEntregarArt(fil,cod);
          if (num1<=(num3-canent))
        ...
        else
        {
         ...
         $(total).value="0,00";
           ...
           if ($(blanco).value=='A' || $('fafactur_tipref').value=='V')
        {
          if ($(blanco).value=='A') {
           if ($(id).value!="")
           {
  ...
  function ajustarDecimasRecargo()
  {
    ...
    while (fil<regart)
    {
      ...
      if ($(totart).value!="" && (ValidarNumeroV2VE(totart)==true))
      {
       ...
       {
         if ($('fafactur_calrecdes').value=='S')
           montot= montot + ((nprecio * ncant)-toFloat(desct));
         else
          montot= montot + (nprecio * ncant);
         acumrec=acumrec+nmonrec;
       }
       var nmondes= toFloat(desct);
       if (nmondes>0)
       {
        montot2= montot2 + (nprecio * ncant);
        acumdes=acumdes+nmondes;
      }
      }
      }
     fil++;
    ...
    if (montot2>0)
    {
        ...
        if (distrib2!="")
        {
          ...
          if (k==0) {
              if (ctipdesc=='M')
              var calculo2=toFloat2(cmontdesc)*toFloat2(ccantidad);
              else {
                if (ctipdesc=='P') {
                if (ctipret!='S')
                  var mondest=montot2;
                ...
                var calculo2=mondest*(toFloat2(cmontdesc)/100);
              }else var calculo2=0;
            }
...
function mostrargridrecargosPed(id)
 {
    var aux = id.split("_");
    var name=aux[0];
    var fil=parseInt(aux[1]);
    ...
    var chk="ax"+"_"+fil+"_12";
    if ($(chk).checked==true)
    {
     if ($(codart).value!="")
     {
        if ($('fapedido_nroped').value=='########') var nroped=''; else var nroped=$('fapedido_nroped').value;
        var articulo=$(codart).value;
        $('fapedido_filactrec').value=fil;

        new Ajax.Updater('divgrid_fargoped', getUrlModuloAjax(), {asynchronous:true, evalScripts:true, onComplete:function(request, json){AjaxJSON(request, json);distribuirRecargosenGridPed();$("datosRecarg").show();}, parameters:'ajax=12&articulo='+articulo+'&nroped='+nroped})
    }
    else
    {
      alert_("Debe introducir el Art&iacute;culo antes de los cargar los Recargos..");
  ...
  function distribuirRecargosenGridPed()
 {
     var j=$('fapedido_filactrec').value;
     var haydist="ax"+"_"+j+"_16";
     if ($(haydist).value!="")
      {
        var distrib=$(haydist).value;
        var aux2=distrib.split("!");
        ...
        calcularTotalRecargosPed();
      }
 }
 
function salvarmontorecargosPed()
{
  var cadena='';
  var fil=0;
  var ab=totalregistros2('cx',1,10);
  ...
  var moncos=0;
  var mifila=$('fapedido_filactrec').value;
  var infrecargos="ax"+"_"+mifila+"_16";
  var cant="ax"+"_"+mifila+"_3";
  var precio="ax_"+mifila+"_7";
  var precioe="ax_"+mifila+"_8";
  if ($(precio).value!="") {
  var index=$(precio).selectedIndex;
  //Obtener el valor del precio
  var moncos=toFloat2($(precio).options[index].text);
 }else {
  var moncos=toFloat(precioe);
 }
  var descto="ax"+"_"+mifila+"_9";
  var recargo="ax"+"_"+mifila+"_10";
  var total="ax"+"_"+mifila+"_11";

  $(infrecargos).value=cadena;
  $(recargo).value=$('fapedido_totrec').value;

  var recfil=toFloat('fapedido_totrec');
  var canti=toFloat(cant);
  var mondto=toFloat(descto);
  var monnet= canti*moncos;
...
function CalculoMontoRgoPed(e,id)
 {
   var aux = id.split("_");
   var name=aux[0];
   var fil=parseInt(aux[1]);
  ...
  function calcularTotalRecargosPed()
  {
    var mitot=0;
    var regrgo=totalregistros2('cx',1,10);
    var j=0;
    ...
    $('fapedido_totrec').value=format(mitot.toFixed(2),'.',',','.');

  }

function ajaxrecargosPed(e,id)
 {
   var aux = id.split("_");
   var name=aux[0];
   var fil=parseInt(aux[1]);
   var col=parseInt(aux[2]);
    ...
    var monto2=name+"_"+fil+"_"+colmonto2;
    var cod=$(id).value;
    var montot=retornaMontoModPed(cod);
    var montot1=retornaMontoModPed(cod);   
    var valmonto=$(monto).value;
    ...
    if (e.keyCode==13 || e.keyCode==9)
    {
      if ($(id).value!="")
      {
        if (!recargo_repetido(id))
        {
         new Ajax.Request(getUrlModuloAjax(), {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=13&cajtexmos='+nombre+'&cajtexcom='+id+'&recfij='+recfij+'&monto='+monto+'&cta='+cta+'&tipo='+tipo+'&monto2='+monto2+'&montot='+montot+'&montot1='+montot1+'&valmonto='+valmonto+'&codigo='+cod})
        }
        else
        {
         alert('El Recargo ya fue asignado');
  ...
  function calcularMontTotModPed()
  {
    var calcularmonto=0;
    var mifila=$('fapedido_filactrec').value;        
    var cant="ax"+"_"+mifila+"_3";
    ...
    var precioe="ax_"+mifila+"_8";
      ...
      if ($(precio).value!="") {
       ...
       if ( ($(precio).value!="" || $(precioe).value!="")  && $(cant).value!="")
       {
         calcularmonto= calcularmonto + ( nprecio * ncant);
       }
...
function retornaMontoModPed(codrgo)
 {
    var monTot=calcularMontTotModPed();
    if (monTot!=0)
    {
        var mifila=$('fapedido_filactrec').value;        
        var cant="ax"+"_"+mifila+"_3";
        ...
        if ($(codart)){
         if ($(precio).value!="") {
          ...
          var nprecio2=toFloat(precioe);
         }
        var ncant2= toFloat(cant);

        new Ajax.Request(getUrlModuloAjax(), {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=14&articulo='+codart+'&recargo='+codrgo})
        if ($('fapedido_trajo').value=='S')
        {
         if ($(cant).value!="" && ($(precio).value!="" || $(precioe).value!=""))
         {
           monTot= monTot - (nprecio2 * ncant2);
         }
       }
 ...
 function recalcularecargosPed(id)
  {
        var i=id.split('_');
        var fil=i[1];
        var id1="ax"+"_"+fil+"_12";
        var precio="ax_"+fil+"_7";
       var precioe="ax_"+fil+"_8";
        var cant="ax"+"_"+fil+"_3";
        var dest="ax"+"_"+fil+"_9";
        var recargo="ax"+"_"+fil+"_10";
        var total="ax"+"_"+fil+"_11";
        var infrecargos="ax"+"_"+fil+"_16";
 ...
 function marcarrecarg(id)
 {
    ...
    if ($(id).checked==false)
    {
        desmarcarfilaModPed(id);
    }else {
        aplicarRecanteriorPed(id)
    }
     monto_total();
 }   

 function desmarcarfilaModPed(id)
  {
      var aux = id.split("_");
      var name=aux[0];
      var fil=parseInt(aux[1]);      
      ...
      if ($(codart).value!="")
      {
       ...
       var recargo="ax"+"_"+fil+"_10";
       var total="ax"+"_"+fil+"_11";
       var haydistribucion="ax"+"_"+fil+"_16";
  ...
  function aplicarRecanteriorPed(ida)
  {
     var aux = ida.split("_");
     var name=aux[0];
     var fil=parseInt(aux[1]);

     var infrecargos="ax"+"_0_16";
     var distrib=$(infrecargos).value;
  ...
  function calcularPorcentajeCant(e,id){

    var regart=obtener_filas_grid('a',3);
    var colum=determinarReferenciaDoc($('fafactur_tipref').value);
    var fil=0;
    var porcen=toFloat(id);
    ...
    while (fil<regart)
    {
      ...
      var cal=num1*(porcen/100);

      $(cant).value=format(cal.toFixed(2),'.',',','.');
      if (colum==7)
         cansol(e,cant);
      else if (colum==8)
        canentregar(e,cant);
 ...
 function marcarDespachos()
  {
   var facdp=obtener_filas_grid('e',2);
   var fil=0;
    while (fil<facdp)
    {
       var check="ex_"+fil+"_1";
