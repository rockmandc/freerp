	...
	function botonAceptar()
	{
		...
		if ($('cajgrid1').value!='')
		{
			if ($('idlabel').value!='' && $('idlabel2').value=='')
			{
				...
				if (cod1=="NHMENEDA" || cod1=="MESF" || cod1=="ANOF" || cod1=="DIAF" || cod1=="ACUC" || cod1=="ACUPA" ||  cod1=="ACUMA" || cod1=="ACUMVA" ||cod1=="ACUMH" ||cod1=="ACUH"|| cod1=="PHIJO" || cod1=="MNIVF"  || cod1=="BNIVF" || cod1=="NHMAYEDA" || cod1=="SIMESDAD"|| cod1=="SINOMMESDAD"|| cod1=="CHDOCC"||cod1=='SHORAS' || cod1=='SDIAS' || cod1=='FECDIAS'|| cod1=='FECMES'|| cod1=='FECANNOS'|| cod1=='INTPRES'|| cod1=='DIAADIPRE' || cod1=='DIAADIFID' || cod1=="NHMENEDACIE" || cod1=="NHMED" || cod1=="NHIJO")
			...
			}else
			{
				if ($('idlabel2').value!='' && $('idlabel').value=='')
				{
					...
					if (cod1=='INTPRES' || cod1=='DIAADIPRE' || cod1=='DIAADIFID'  || cod1=="DJCO" || cod1=="DJF")
					    var cod = cod1+cod2.toUpperCase();
	  			    else
						var cod = cod1+cod2;
		...
		}else
		{
			if ($('idlabel').value!='' && $('idlabel2').value=='')
			{
				var cod1 = $('cajaux').value;
				var cod2 = $('idlabel').value;
				if (cod1=='INTPRES' || cod1=="MESF" || cod1=="ACUC" ||  cod1=="ACUMH" ||  cod1=="ACUH" ||  cod1=="ACUPA" ||  cod1=="ACUMA" || cod1=="ACUMVA" || cod1=="PHIJO" || cod1=="MNIVF" || cod1=="BNIVF" || cod1=='DIAADIPRE' || cod1=='DIAADIFID'  || cod1=="DJCO" || cod1=="DJF" || cod1=="NHIJO")
				    var cod = cod1+cod2.toUpperCase();
  			    else
					var cod = cod1+"("+cod2+")";
			...
			}else
			{
				if ($('idlabel2').value!='' && $('idlabel').value=='')
				{
					...
					var fecha = $('idlabel2').value;
					var cod2 = fecha.substr(0,2)+fecha.substr(3,2)+fecha.substr(6,4);
					if (cod1=='INTPRES' || cod1=="MESF" || cod1=='DIAADIPRE' || cod1=='DIAADIFID'  || cod1=="DJCO" || cod1=="DJF")
					    var cod = cod1+cod2.toUpperCase();
	  			    else
						var cod = cod1+cod2;
...
/*******************************Aqui entra cuando se esconde le grid**********************************/
		if ($('cajoculabel').value =='')
		{
			...
			if (dato=='STAB' || dato=='SC' || dato=='SESC' || dato=='CXHOR' || dato=='DIFSUECARCOL')
			{
			...
			if (dato != "SIC" &&  dato != "SARC" && dato != "SIM" && dato != "SC" && dato != "SESC" && dato != "SCAR" && dato != "CCAR" && dato != "PCAR" && dato != "TAF" && dato != "TDED" && dato != "NV" && dato != "NVP" && dato != "NL" && dato != "NLP" && dato != "NS" &&
			    dato != "AD" && dato != "DM" && dato != "DC" && dato != "DV" && dato != "DBV" && dato != "PV" && dato != "ADV" && dato != "ADF" && dato != "AM" && dato != "AMC" && dato != "AMF" && dato != "CATRAB" && dato != "CATRABMESNS" &&  dato != "NRETV" && dato != "CATRABTRI" && dato != "ESTRIM" &&
			    dato != "AC" && dato != "AA" && dato != "AAFM"  && dato != "AT" && dato != "ACON" && dato != "AET" && dato != "AAP" && dato != "AFD" && dato != "APUB" && dato != "CC" && dato != "ED" && dato != "EE"  && dato != "FFRAC" && dato != "FINT" && dato != "ACUC" && dato != "ACUPA" &&  dato != "ACUMH" &&  dato != "ACUH" && dato != "ACUMA" && dato != "ACUMVA" &&
			    dato != "MESF"  && dato != "DIAF" && dato != "ANOF" && dato != "NHGR" && dato != "NHIJ" && dato != "NHIJDIS"   && dato != "PPROF"  && dato != "MCES" &&
			    dato != "ADIC")
			{
...
/***************************Primero entra aqui cuando el grid esta activo************************/
   ...
   function validandoParametros(cod)
   {
		     	...
		     	if (cod=='FINT' || cod=='FFRAC' || cod=='SHORAS' || cod=='SDIAS')
		     	{
					...
					$('cajaux').value=cod;
					return true;
		     	}else if (cod=='STAB' || cod=='CTAB' || cod=='CXHOR' || cod=='SC' || cod=='SESC' || cod=='DIFSUECARCOL')
		     	{
		     	    $('grid1').hide();
					$('idfunciones').disabled=true;
					$('idempleados').disabled=true;
		     	...
		     	}else if (cod=='NHMAYEDA' || cod=='NHMENEDA' || cod=='NHMENEDACIE' || cod=='NHMED'  || cod=='NHIJO')
		     	{
		     	    $('grid1').hide();
					$('idfunciones').disabled=true;
					$('idempleados').disabled=true;
		     	...
		     	else if(cod=='CHDOCC')
		     	{
					...
					$('label1').innerHTML = "Introduzca el código del concepto para la funcion "+cod+":    ";
					$('textooculto1').show();
					$('idlabel').value='';
					$('cajaux').value=cod;
					$('idlabel').readOnly=false;
                 ...
                 else if (cod=='DJCO' || cod=='DJF')
		     	{
					...
					$('label1').innerHTML = "Introduzca el parametro para la funcion    "+cod+":    ";
					$('textooculto1').show();
					$('idlabel').value='';
					$('cajaux').value=cod;
					$('idlabel').readOnly=false;
				...
				else if (cod=='PHIJO')
		     	{
					...
					$('label1').innerHTML = "Introduzca el parametro para la funcion    "+cod+"XXXX:    ";
					$('textooculto1').show();
					$('idlabel').value='';
					$('cajaux').value=cod;
					$('idlabel').readOnly=false;
				...
				else if (cod=='ACUC' || cod=='ACUMH' || cod=='ACUH' || cod=='ACUMA' || cod=='ACUMVA' || cod=='ACUPA')
		     	{
		     	    $('grid1').hide();
					$('idfunciones').disabled=true;
					$('idempleados').disabled=true;
					...
					$('label1').innerHTML = "Introduzca el parametro para la funcion    "+cod+"XXX:    ";
					$('textooculto1').show();
					$('idlabel').value='';
					$('cajaux').value=cod;
					$('idlabel').readOnly=false;
		     	...
		     	else if (cod=='MNIVF'|| cod=='BNIVF')
		     	{
					...
					$('label1').innerHTML = "Introduzca el parametro para la funcion    "+cod+"X@YYY: Donde X es la tabla y YYY es el codigo de nivel    ";
					$('textooculto1').show();
					$('idlabel').value='';
					$('cajaux').value=cod;
					$('idlabel').readOnly=false;
    ...
    function condicionValor(id)
    {
        ...
        if ($(idnew).value!='' )
        {
        	...
        	if ($(idnew).value=="NS" || $(idnew).value=="NL" || $(idnew).value=="NLP" || $(idnew).value=="NVP" || $(idnew).value=="NV")
        	{
				$(id).setAttribute('maxlength','1');
        	}
        	if ($(idnew).value=="E08" || $(idnew).value=="AA" || $(idnew).value=="AAFM" || $(idnew).value=="AT" || $(idnew).value=="ACON" || $(idnew).value=="AET" || $(idnew).value=="AAP" || $(idnew).value=="APUB" || $(idnew).value=="CC" || $(idnew).value=="NHIJ" || $(idnew).value=="NHIJDIS" || $(idnew).value=="NHIJO" ||
        	$(idnew).value=="NHGR" || $(idnew).value=="MESF" || $(idnew).value=="DIAF" || $(idnew).value=="CATRAB" || $(idnew).value=="CATRABMESNS" || $(idnew).value=="NRETV" || $(idnew).value=="CATRABTRI" || $(idnew).value=="ESTRIM" || $(idnew).value=="AC" )
        	{
				$(id).setAttribute('maxlength','2');
        	}
        	if ($(idnew).value=="EE" || $(idnew).value=="AM" || $(idnew).value=="AMC" || $(idnew).value=="AMF" ||  $(idnew).value=="ANOF" )
        	{
				$(id).setAttribute('maxlength','4');
        	}
        	if ($(idnew).value=="AD" || $(idnew).value=="ED" || $(idnew).value=="DM" ||  $(idnew).value=="ADF" || $(idnew).value=="DC" || $(idnew).value=="ADV" || $(idnew).value=="DV" )
        	{
    ...
    function condicionValor2(e,id)
    {
        ...
        if($(id).value!='')
        {
        	...
        	if (($(idnew).value=="NS" || $(idnew).value=="NL" || $(idnew).value=="NLP" || $(idnew).value=="NVP" || $(idnew).value=="NV" ) &&  $(id).value!='')
        	{
				var num = parseInt($(id).value);
				if (isNaN(num))
				{
        	...
        	if (($(idnew).value=="E08" || $(idnew).value=="AA" || $(idnew).value=="AAFM" || $(idnew).value=="AT" || $(idnew).value=="ACON" || $(idnew).value=="AET" || $(idnew).value=="AAP" || $(idnew).value=="APUB" || $(idnew).value=="CC" || $(idnew).value=="NHIJ" || $(idnew).value=="NHIJDIS" || $(idnew).value=="NHIJO" ||
        	$(idnew).value=="NHGR" || $(idnew).value=="MESF" || $(idnew).value=="DIAF" || $(idnew).value=="CATRAB" || $(idnew).value=="CATRABMESNS" || $(idnew).value=="NRETV" || $(idnew).value=="CATRABTRI" || $(idnew).value=="ESTRIM" || $(idnew).value=="AC")&&  $(id).value!='')
        	{
        		var num = parseInt($(id).value);
				if (isNaN(num))
				{
			...
			if (($(idnew).value=="EE" || $(idnew).value=="AM" || $(idnew).value=="AMC" || $(idnew).value=="AMF" || $(idnew).value=="ANOF" )&&  $(id).value!='')
			{
				var num = parseInt($(id).value);
				if (isNaN(num))
				{
			...
			if (($(idnew).value=="AD" || $(idnew).value=="ED" || $(idnew).value=="DM" || $(idnew).value=="ADF" || $(idnew).value=="DC" || $(idnew).value=="ADV" || $(idnew).value=="DV")&&  $(id).value!='')
			{
				var num = parseInt($(id).value);
				if (isNaN(num))
				{
        	...
        	}else if ($(idnew).value!="E00" && $(idnew).value!="E02" && $(idnew).value!="CARG" && $(idnew).value!="TTRAB" && $(idnew).value!="CHDOC" && $(idnew).value!="CHDOCT" && $(idnew).value!="MCUOHCM" && $(idnew).value!="MCUOSFN" && $(idnew).value!="NTRAB")
        	{
