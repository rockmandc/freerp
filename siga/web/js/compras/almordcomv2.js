    ...
    if (valor!=null)
    {
        ...
        if (fecha!=null)
        {
          if (Validar_fecha(fecha))
          {
            ...
            new Ajax.Updater('anul', '/compras'+getEnv()+'.php/almordcomv2/salvaranu', {asynchronous:true, evalScripts:true, parameters:'ajax=1&valor='+valor+'&fecha='+fecha+'&ordcom='+document.getElementById('caordcom_ordcom').value+'&fecord='+document.getElementById('caordcom_fecord').value+'&fecserv='+$('fechaanuserv').value});
          }
          else
          {
...
{
  window.open('/tesoreria'+getEnv()+'.php/confincomgen/edit/?formulario=sf_admin/almordcomv2/confincomgen','menubar=no,toolbar=no,scrollbars=yes,width=1200,height=800,resizable=yes,left=1000,top=80')
}

function CatalogoGrid()
{
  var obj1=document.getElementById('caordcom_ordcom').value;
  window.open('/compras'+getEnv()+'.php/almordcomv2/anular?obj1='+obj1,'...','menubar=no,toolbar=no,scrollbars=yes,width=800,height=300,resizable=yes,left=500,top=80')
}
...
function verifica_presupuesto(e,id)
{
  ...
  if (e.keyCode==13 || e.keyCode==9)
  {
    if (codigo!="")
    {
      new Ajax.Request('/compras'+getEnv()+'.php/almordcomv2/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=13&bandera='+bandera+'&cajtexmos='+fila_monto+'&cajtexcom='+fila_monto+'&codigo='+codigo+'&moneda='+$('caordcom_tipmon').value+'&valmon='+$('caordcom_valmon').value+'&monto='+monto+'&fecha='+fecha})
    }
  }
}
...
function ajax_detalle_codigo_pre(e,id)
{
  if (!(verificar_codigo_repetido(id)))
  {
    ...
    if (e.keyCode==13 || e.keyCode==9)
    {
      if (codigo!="")
      {
        new Ajax.Request('/compras'+getEnv()+'.php/almordcomv2/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json),existencia_codigo_presupuestario(codigo_presupuestario,codigo,fila);}, parameters:'ajax=12&cajtexmos='+codigo_presupuestario+'&cajtexcom='+codigo_presupuestario+'&codigo='+codigo})
...
function ajaxdetalle(e,id)
{
  ...
  if (e.keyCode==13 || e.keyCode==9)
  {
    if ($(id).value!="")
    {
      new Ajax.Request('/compras'+getEnv()+'.php/almordcomv2/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=3&cajtexmos='+descripcion+'&cajtexcom='+id+'&unidad='+unidad+'&costo='+costo+'&partida='+partida+'&codigo='+cod+'&tipord='+$('caordcom_tipord').value})
    }
  }
}
...
function actualizar_sumatoria_total_cuando_esta_referida()
{
  ...
  $('caordcom_totorden').value=format(montotal2.toFixed(6),'.',',','.');
  $('caordcom_totrecargo').value=format(montotal3.toFixed(4),'.',',','.');
}
...
function actualizar_total_grid_detalle_datos(e,id,caldes)
{
  if (e.keyCode==13 || e.keyCode==9)
  {
    ...
    $(fila_total_costo_cantidad).value=format(total_cantidad_costo.toFixed(6),'.',',','.');//Math.round(total_cantidad_costo *100)/100;   
    $(fila_total_monto_por_cantidad).value=format(total_cantidad_por_monto.toFixed(6),'.',',','.');//Math.round(total_cantidad_por_monto  *100)/100;   
  }
}
...
function actualizar_sumatoria_detallada_orden()
{
  ...
  $('caordcom_totorden').value=format(total_cost_cant.toFixed(6),'.',',','.');
  $('caordcom_totrecargo').value=format(tot_recargo.toFixed(4),'.',',','.');
}
...
function actualizar_calculo_grid_recargo_por_fila()
{
  ...
  while (i < ($('numero_filas_recargos').value-1))
  {
    ...
    $(fila_resultado).value=format(montotal_recargo.toFixed(4),'.',',','.');
    i++;
  }
}
...
function actualizar_recargos_en_detalle_orden_por_fila()
{
  ...
  while (i < $('numero_filas_orden').value)
  {
    ...
    $(fila_grid_detalle_orden_total).value=format(total_completo_grid_detalle_orden.toFixed(6),'.',',','.');
...
function total_completo()
{
  ...
  while (i < $('numero_filas_orden').value)
  {
    ...
    var cantidad_art_total=toFloat(fila_cantidad)-toFloat(fila_cantidadaju);
    var costo_art_total=toFloat(fila_costo);
...
function mostrargridrecargos(id)
{
  ...
  if ($(chk).checked==true)
  {
    if ($(codart).value!="")
    {
      ...
      new Ajax.Updater('grid_recargo', getUrlModulo()+'recargos', {asynchronous:true, evalScripts:true, onComplete:function(request, json){AjaxJSON(request, json);distribuirRecargosenGrid(); Total_grid_recargos2(); $("recargos").show();$("botonesmarcar").hide();}, parameters:'articulo='+articulo+'&refsol='+refsol+'&codunidad='+codunidad+'&ordcom='+ordcom+'&tipcom='+tipcom+'&nuevo='+nuevo})
    }
    else
    {
...
function limpiardatos()
{
  if ($('caordcom_tipopro').value=='P'){
    ...
    while (f < $('numero_filas_orden').value)
    {
      ...
      $(filatotal).value=format(calculo.toFixed(6),'.',',','.');

      $(filacheck).value=false;
      $(filarecargo).value="0,0000";
      $(filadatrec).value="";
...
function generarEntrePDA()
{

  paramsA  = serializeGrid('a','0','2',$('ax_0_2'));
  paramsB  = serializeGrid('p','0','1',$('px_0_1'));

  params = '&proveedor='+$('caordcom_rifpro').value+'&'+paramsA+'&'+paramsB;

  new Ajax.Updater('gridentregapda', '/compras'+getEnv()+'.php/almordcomv2/entrepda', {
      asynchronous:true,
      evalScripts:true,
      onComplete:
          function(request, json){
...
function agregarfilaspda(id)
{
  ...
  var desart=name+'_'+fil+'_2';
  var fecha=name+'_'+fil+'_4';
  var tipo=name+'_'+fil+'_5';
  var dirori=name+'_'+fil+'_6';
  var cant2=name+'_'+fil+'_8';
  
  var ab=obtener_filas_grid('p',1)-1;
  
  var ncodart=name+'_'+ab+'_1';
  var ndesart=name+'_'+ab+'_2';
  var ncant=name+'_'+ab+'_3';
  var nfecha=name+'_'+ab+'_4';
  var ntipo=name+'_'+ab+'_5';
  var ndirori=name+'_'+ab+'_6';
  var ncant2=name+'_'+ab+'_8';
  
  if ($(ncodart).value!="")
  {
      NuevaFilaGrid('p');
      ab=ab+1;
      ncodart=name+'_'+ab+'_1';
      ndesart=name+'_'+ab+'_2';
      ncant=name+'_'+ab+'_3';
      fecha=name+'_'+ab+'_4';
      ntipo=name+'_'+ab+'_5';
      ndirori=name+'_'+ab+'_6';
      ncant2=name+'_'+ab+'_8';
  }
  
  $(ncodart).value=$(codart).value;
  $(ndesart).value=$(desart).value;
  $(ncant).value="0,00";
  $(nfecha).value=$(fecha).value;
  $(ntipo).value=$(tipo).value;
  $(ndirori).value=$(dirori).value;
  $(ncant2).value=$(cant2).value; 
}
...
function validartotal(id)
{
   ...
   var codart=name+'_'+fila+'_1';
   var desart=name+'_'+fila+'_2';   
   var cantot=name+'_'+fila+'_8';   
   
   var num0=toFloat(cantot);
   
   var articulo=$(codart).value+'_'+$(desart).value;

   var am=totalregistros2('px',1,obtener_filas_grid('p',1));
   var i=0;
   var acucan=0;
   while (i<am)
   {
    var codigo="px"+"_"+i+"_1";
    var descrip="px"+"_"+i+"_2";    
    var cant="px"+"_"+i+"_3";    
    
    if ($(codigo)){
    var articulo2=$(codigo).value+'_'+$(descrip).value;
   ...
   if (acucan>num0)
   {
       alert_('La Sumatoria Total de este Art&iacute;culo es mayor al Total por Art&iacute;culo');
       $(id).value='0,00';
   }
}
...
function Total_grid_recargos()
{
  i=0;
  monto_total_grid_recargo=0;
  var filrec=$('numero_filas_recargos').value-1;//totalregistros2('rx',1,10);
  while (i < (filrec))
  {
    var fila_monto_recargo_total = "rx_"+i+"_5";
    monto_recargo=0;
    //if  (!$(fila_monto_recargo_total)) break;
  ...
  $('totrecar').value=format(monto_total_grid_recargo.toFixed(4),'.',',','.');
}

function Total_grid_recargos2()
{
  i=0;
  monto_total_grid_recargo=0;
  var filrec=totalregistros2('rx',1,10)-1;
  while (i < (filrec))
  {
    var fila_monto_recargo_total = "rx_"+i+"_5";
    monto_recargo=0;
...
function CargarUnidades(id)
{
  i=id.split('_');
  fila=i[1];
  var unidad= "ax_"+fila+"_14";

  if ($('caordcom_manunialt').value=='S')
  {
      new Ajax.Updater(unidad, getUrlModuloAjax(), {asynchronous:true, evalScripts:true, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=20&codigo='+$(id).value});
  }      
}