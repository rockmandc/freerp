   ...
   var col=parseInt(aux[2]);
    ...
    if (e.keyCode==13 || e.keyCode==9)
    {
      if ($(id).value!="")
      {
        if (!art_repetido(id))
       {
          new Ajax.Request(getUrlModuloAjax(), {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=2&cajtexmos='+descripcion+'&cajunidad='+unidad+'&cajcosto='+costo+'&cajtexcom='+id+'&codigo='+cod})

          if ($('careqart_manunialt').value=='S')
          {
              new Ajax.Updater(unidad, getUrlModulo()+'ajax', {asynchronous:true, evalScripts:true, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=10&id='+$('id').value+'&codigo='+cod});
          }
       }
       else
       {
         if ($('careqart_novalcat').value=='S')
          alert_('El Art&iacute;culo ya esta repetido');
         else
...
function art_repetido(id)
 {
   ...
   var novalcat=$('careqart_novalcat').value;

   if ($(unidad).value!="" && novalcat!='S'){
    ...
    if (novalcat=='S') {
     ...
     while (i<am)
     {
      ...
      if ($(codigo))
      {
      var articulo_unidad2=$(codigo).value;
