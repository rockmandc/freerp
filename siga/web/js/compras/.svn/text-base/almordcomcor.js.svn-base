  ...
  function verifica_presupuesto(e,id)
  {
    ...
    var codigo=$(fila_codigo_presupuestario).value;
      ...
      if (e.keyCode==13 || e.keyCode==9)
      {
        if (codigo!="")
        {
        new Ajax.Request('/compras'+getEnv()+'.php/almordcom/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=13&bandera='+bandera+'&cajtexmos='+fila_monto+'&cajtexcom='+fila_monto+'&codigo='+codigo+'&moneda='+$('caordcom_tipmon').value+'&monto='+monto+'&fecha='+fecha})
        }
      }
  }
