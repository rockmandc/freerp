...
//<----------------------------Funciones solo del editform---------------------------------------->
      ...
      if (valor!=null)
      {
        ...
        if (fecha!=null)
        {
          if (Validar_fecha(fecha))
          {
            ...
            new Ajax.Updater('anul', '/compras'+getEnv()+'.php/almordcom/salvaranu', {asynchronous:true, evalScripts:true, parameters:'ajax=1&valor='+valor+'&fecha='+fecha+'&ordcom='+document.getElementById('caordcom_ordcom').value+'&fecord='+document.getElementById('caordcom_fecord').value+'&fecserv='+$('fechaanuserv').value});
        }
        else
        {
  ...
  {
    window.open('/tesoreria'+getEnv()+'.php/confincomgen/edit/?formulario=sf_admin/almordcom/confincomgen','menubar=no,toolbar=no,scrollbars=yes,width=1200,height=800,resizable=yes,left=1000,top=80')
  }

  function CatalogoGrid()
  {
    var obj1=document.getElementById('caordcom_ordcom').value;
    window.open('/compras'+getEnv()+'.php/almordcom/anular?obj1='+obj1,'...','menubar=no,toolbar=no,scrollbars=yes,width=800,height=300,resizable=yes,left=500,top=80')
  }
...
//<----------------------------Funciones solo del grid Detalle Orden---------------------------------------->
  ...
  function verifica_presupuesto(e,id)
  {
    ...
    var codigo=$(fila_codigo_presupuestario).value;
      ...
      if (e.keyCode==13 || e.keyCode==9)
      {
        if (codigo!="")
        {
        new Ajax.Request('/compras'+getEnv()+'.php/almordcom/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=13&bandera='+bandera+'&cajtexmos='+fila_monto+'&cajtexcom='+fila_monto+'&codigo='+codigo+'&moneda='+$('caordcom_tipmon').value+'&valmon='+$('caordcom_valmon').value+'&monto='+monto+'&fecha='+fecha})
        }
      }
  }
  ...
  function ajax_detalle_codigo_pre(e,id)
  {
   if (!(verificar_codigo_repetido(id)))
   {
      ...
      if (e.keyCode==13 || e.keyCode==9)
      {
        if (codigo!="")
        {
        new Ajax.Request('/compras'+getEnv()+'.php/almordcom/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json),existencia_codigo_presupuestario(codigo_presupuestario,codigo,fila);}, parameters:'ajax=12&cajtexmos='+codigo_presupuestario+'&cajtexcom='+codigo_presupuestario+'&codigo='+codigo})
        }
      }
  ...
  function ajaxdetalle(e,id)
  {
    ...
    if (e.keyCode==13 || e.keyCode==9)
    {
       if ($(id).value!="")
       {
        new Ajax.Request('/compras'+getEnv()+'.php/almordcom/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=3&cajtexmos='+descripcion+'&cajtexcom='+id+'&unidad='+unidad+'&costo='+costo+'&partida='+partida+'&codigo='+cod+'&tipord='+$('caordcom_tipord').value})
  ...
  function actualizar_total_grid_detalle_datos(e,id,caldes)
  {
      if (e.keyCode==13 || e.keyCode==9)
      {
        ...
        $(fila_total_costo_cantidad).value=Math.round(total_cantidad_costo *100)/100;   
        $(fila_total_monto_por_cantidad).value=Math.round(total_cantidad_por_monto  *100)/100;   
 ...
 function marcarTodo()
  {
   ...
   if (valorarticulo!="")
   {
    ...
    var mondto=toFloat(dest);
       ...
       var cantot=valcanord-valcanaju-valcanrec;
       var monuni=(moncos*cantot)-mondto;
