function totalizarMonto(e)
{
 ...
 function ajaxarticulos(e,id)
 {
    ...
    if (e.keyCode==13 || e.keyCode==9)
    {
      if ($(id).value!="")
      {
        ...
        {
          new Ajax.Request(getUrlModuloAjax(), {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json), totalizarMonto(e)}, parameters:'ajax=3&cajtexmos='+descripcion+'&monto='+monto+'&partida='+partida+'&cajtexcom='+id+'&valmonto='+valmonto+'&codigo='+cod})
        }
        else
        {
 ...
 function martodoscie()
 {
   var filas=obtener_filas_grid('b',2);
   var i=0;
   $('opciecaj_cadesel').value="";
   while (i<filas)
   {
     var check="bx_"+i+"_1";
     var refsal="bx_"+i+"_2";
     if ($(check)) {
       $(check).checked=true;
       $('opciecaj_cadesel').value=$('opciecaj_cadesel').value+"/"+$(refsal).value;
     }
     i++;
    }
 }

 function destodoscie()
 {
   var filas=obtener_filas_grid('b',2);
   var i=0;
   while (i<filas)
  ...
  function cargarimpcie()
 {
    if ($('opciecaj_cadesel').value!="")
    {
      new Ajax.Updater('divgrid', getUrlModuloAjax(), {asynchronous:true, evalScripts:true, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=1&codigo='+$('opciecaj_cadesel').value});
    }
 }

 function guardarseleccioncie()
 {   
   var filas=obtener_filas_grid('b',2);
   var i=0;
   $('opciecaj_cadesel').value="";
