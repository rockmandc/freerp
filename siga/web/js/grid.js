...
// grid = string
  function NuevaFilaGrid(grid)
  {
    ...
    if(ultima_fila){
      ...
      max = parseFloat(array_ultimo[1])+1;
      nuevo = $(grid+'_nuevo').value.replace(/INDEX/g,max);      
      new Insertion.After(ultima_fila,nuevo);  
    }else{
      max = 0;
      nuevo = $(grid+'_nuevo').value.replace(/INDEX/g,max);      
      new Insertion.Top($$('#tabla_'+ grid +' tbody')[0],nuevo);
    }
  ...
  function ValidarMontoGridv2(id, dec)
  {
     if (dec=="") dec=2;
    ...
    {
      calculo=0;
      $(id).value=format(calculo.toFixed(dec),'.',',','.');//'0,00';
      $(id).focus();
      $(id).select();
      return false;
    }
  ...
  function accionRemotaGrid(accion,grid,tipo,indice,adicionales,obj,update)
  {
    if(obj.value!=''){
      var param = null;
      ...
      for(i=0;i<adicionales.length;i++) {
         if ($(adicionales[i]).type=="radio")
             param += '&'+adicionales[i]+'='+$(adicionales[i]).checked;
         else 
             param += '&'+adicionales[i]+'='+$(adicionales[i]).value;           
      }
     if(update==''){
        new Ajax.Request(accion, {
          ...
          parameters:param
           ...
           new Ajax.Updater(update, accion, {
               asynchronous:true,
               evalScripts:true,
               onComplete:
                   function(request, json){
