...
{
  if ($('fctipesp_paresp').value!="")
    new Ajax.Request('/hacienda'+getEnv()+'.php/facesptip/ajax', {asynchronous:true, evalScripts:true, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:Form.serialize(document.getElementById('sf_admin_edit_form'))});
}
...
function formula(id)
{
    var seguir="si";
	var acum = 0;
	var cadena = $("fctipesp_paresp").value;
