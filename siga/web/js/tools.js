 ...
 */
 ...
 var expresionfloat =  /^(\d{1,3}\,){0,6}(\d{1,3})(\.\d{1,6})$/;
  ...
  function redondear(cantidad, decimales)
  {
    ...
    {
    var longa = fecha.length;
    var dia;
    var mes;
    var ano;

    if ((longa>=2) && (primerslap==false)) { dia=fecha.substr(0,2);
    ...
    function disableAllClass(form, classe, val) // Desahabilita todos los objetos menos los del arreglo
    {
      var objform = $(form)
...
function totalregistros(letra,posicion,filas)
  ...
  function obtenerColumna(id,valor,signo){
    ...
    if (signo=='-') colotro= col-valor;
    else if (signo=='+')  colotro= col+valor;
    else  colotro= col;
 ...
 */
 ...
 function compareNum(a,b)
 {
  ...
  if (longa==10)
  {
    dia=fecha.substr(0,2);
    longa = dia.length;
    if ((IsNumeric(dia)==true) && (dia<=31) && (dia!="00") && (longa==2))
    {
      mes=fecha.substr(3,2);
      longa = mes.length;
      if ((IsNumeric(mes)==true) && (mes<=12) && (mes!="00")  && (longa==2))
      {
        ano=fecha.substr(6,4);
        longa = ano.length;
        if ((IsNumeric(ano)==true)  && (ano<10000)  && (longa==4))
...
///FUNCIONES QUE USA EL CIDESA GENERATOR NO BORRAR
  ...
  function alert_(x){
   ...
   function disableAllObjetos_(obj,val) // Desahabilita todos los objetos menos los del arreglo
   {
     for(i=0;i<document.forms[0].elements.length;i++){
       if(document.forms[0].elements[i].type!='fieldset' && document.forms[0].elements[i].type!='hidden'){
 ...
 function getCeldav2(id, pos)
    ...
    function FloattoFloatVEd(monto,dec)
  {
	...
	//Carlos Ramirez funciona igual que la de php
	...
	//Carlos Ramirez funcion que se utiliza para actualizar los saldos totales del gridv2
	function actualizartotales(id,idtotal)
	{
		if($(idtotal))
		{
			...
			var total = 0;
			var val = 0;
			var letragrid = nomgrid.substring(0,1);
			var totfil = obtener_filas_grid(letragrid,1);
			for(i=0;i<totfil;i++)
			{
				val = $(nomgrid+'_'+i+'_'+col).value;
			    val = val.replace(".","");
				val = val.replace(",",".");
				if(parseFloat(val)!=0 && parseFloat(val)>0)
				  total = parseFloat(total) + parseFloat(val);
			}
			$(idtotal).value=number_format(total,2,',','.');
			//alert(id+'='+total+'--'+idtotal+'---'+$(idtotal).value);
		}
...
function totalregistros2(letra,posicion,filas)
  ...
  function validaFormatoCuenta(id,e){
    tecla=e.keyCode;
    if ((tecla==9 || tecla==51 || tecla==109 || tecla==16 || tecla==0 || tecla==8 || tecla==17 || tecla==37 || tecla==38 || tecla==39 || tecla==40) && tecla!=13){
      if (tecla==109){
        lon=$(id).value.length;
        valor=$(id).value.substr(lon-2,1);
        if (valor=='-'){
          lon=$(id).value.length;
          valor=$(id).value.substr(0,lon-1);
          $(id).value=valor;
        }
      }
    }
      ...
      lon=$(id).value.length;
      valora=$(id).value.substr(0,lon-1);
      if (valora=='-'){
      $(id).value=valora;}
    }
  }
...
function validaFechaMenorActual(fechaValidar) {
	
	var temp = fechaValidar.value;
	...
	if( temp == null || temp.length == 0 || /^\s+$/.test(temp) ) {
        ...
        var dia_actual = parseFloat(fecha.getDate());    //trae el dia actual del sistema
        var mes_actual = parseFloat(fecha.getMonth()+1); //se le suma 1 para que este entre (1,12)
        var ano_actual = parseFloat(fecha.getFullYear());//trae el año en formato yyyy

        var temp = fechaValidar.value;
	fragmentoTexto = temp.split('/');

	dia = parseFloat(fragmentoTexto[0]);// 
	mes = parseFloat(fragmentoTexto[1]);// 
	ano = parseFloat(fragmentoTexto[2]);// 

	if (ano != ano_actual ){

        	alert("Debe Ingresar una fecha anterior, favor corregir, gracias");
        	enviar=false;
		fechaValidar.value = "";
...
function completarCodigo(t,max,sin_numeral)
{
  if ($('id').value=='') {
  sin_numeral = (typeof sin_numeral == 'undefined') ? false: sin_numeral;
  if(trim(t.value)==''){
    if(!sin_numeral) t.value=trim(t.value).pad(max, "#",0);
  }else t.value=trim(t.value).pad(max, "0",0);
}
}

function entermontodecimal(e,id,dec)
   {
  if (e.keyCode==13)
   {
    if (validarnumero(id)==true)
     {
       var num=toFloat(id);
       document.getElementById(id).value=format(num.toFixed(dec),'.',',','.');
       }
      else
     {
      num=0;
...
function rayaenter2(e,tira)
 ...
 function obtener_filas_grid2(name_grid,numcolumn)
 {
  f=0;
  cont=0;
  a=0;
