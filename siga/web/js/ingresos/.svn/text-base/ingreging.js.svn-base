 ...
 */
 ...
 function calcularneto(){
   ...
   var am=obtener_filas_grid('a',1);//am=totalregistros('ax',1,10);
    ...
    if ($(id)) {
      ...
      if (net<0){
        ...
        $(desc).value='';
          ...
          $(monto).value=format(mon.toFixed(2),'.',',','.');
          $(recar).value=format(rec.toFixed(2),'.',',','.');
          $(desc).value=format(des.toFixed(2),'.',',','.');
      }
   }
  }
  if ($(dev).value=='S') {
   ...
   $('cireging_moning').value=format(acum1.toFixed(2),'.',',','.');
   $('cireging_monrec').value=format(acum2.toFixed(2),'.',',','.');
   $('cireging_mondes').value=format(acum3.toFixed(2),'.',',','.');
  ...
  function calculardcto(){
   ...
   var am=obtener_filas_grid('a',1);//totalregistros('ax',1,10);
   var fil=0;
   var dctototal=0;
  ...
  function anular()
  {
    if ($('cireging_staing').value!='N'){  //Si no esta nulo del documento
     if ($('cireging_staing').value!='L'){  //Si no esta liquidado     
      ...
      alert('Ya se encuentra Liquidado no se puede anular...');
    } 
   }else{
