...
[?php
  $fields = <?php echo $this->getParameterValue('model_class'); ?>Peer::getFieldNames();
  ...
  <div class="form-row" id="divusualms">
    [?php echo label_for('<?php echo $this->getSingularName()."[codalmusu]" ?>', __('Almacenes Por Usuario: '), 'class="required" Style="text-align:left; width:150px"') ?]
    <div class="content">
    [?php
      ...
      if($codalm == '') echo label_for('<?php echo $this->getSingularName()."[codalmusu]" ?>', __('Usuario sin Almacen Asociado'), 'class="required" Style="text-align:left; width:150px"').'<br><br><br>';
      else echo select_tag('<?php echo $this->getSingularName(); ?>[codalmusu]',options_for_select($almacenes,$<?php echo $this->getSingularName() ?>->getCodalmusu()!='' ? $<?php echo $this->getSingularName() ?>->getCodalmusu() : $codalm), ( ($<?php echo $this->getSingularName() ?>->getId()) ? 'disabled=true' : '').' style=width:500px');
    ?]
    </div>
  </div>
  </fieldset>
