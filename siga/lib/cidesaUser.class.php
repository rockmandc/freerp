 ...
 */
class cidesaUser extends sfGuardSecurityUser
{
  ...
  public function autenticate_native($nombre,$passwd,$codemp,$db='',$sinpass)
  {
     $valfeccad="";
     $valint="";
     $usualms="";
  ...
  if(!isset($sinpass)){
    ...
    if(!$sinpass) $c->add(UsuariosPeer::PASUSE,'md5'.md5(strtoupper($nombre).$passwd));
        ...
        if(trim($objUsuario[0]->getLoguse())==strtoupper($nombre)){
          ...
          if ($valint=='S')
          {
            ...
            sfContext::getInstance()->getLogger()->info(Constantes::Autenticacion.' Credenciales = '.$credenciales, 'info');
             ...
             {
              $b = new Criteria();
              $b->add(SegubausuPeer::LOGUSE,$nombre);
