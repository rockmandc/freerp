...
/////HECHA POR CARLOS OVIDIO RAMIREZ///////////////////////

function adodb_strtotime($fecha,$hr=0,$min=0,$sec=0,$is_gmt=false)
{
	//agregado
	$mon = @adodb_date("m",$fecha);
	$day = @adodb_date("d",$fecha);
	$year = @adodb_date("Y",$fecha);
	//
	...
	if (!defined('ADODB_TEST_DATES')) {
		...
		if (1971 < $year && $year < 2038
			|| !defined('ADODB_NO_NEGATIVE_TS') && (1901 < $year && $year < 2038)
				...
				return strtotime($mon."/".$day."/".$year);
			}
	}

	$gmt_different = ($is_gmt) ? 0 : adodb_get_gmt_strtotime($year,$mon,$day);
	...
	*/
	...
	if ($year >= 1970) {
		...
		$_total_date +=$day;//-1;//OJO REVISAR
		$ret = $_total_date * $_day_power + $hr * $_hour_power + $min * $_min_power + $sec ;//+ $gmt_different;//OJO REVISAR
		...
		$_day_time = $hr * $_hour_power + $min * $_min_power + $sec;
		//$_day_time = $_day_power - $_day_time;
		$ret = -( $_total_date * $_day_power + $_day_time );//- $gmt_different);//OJO REVISAR
		if ($ret < -12220185600) $ret += 10*86400; // if earlier than 5 Oct 1582 - gregorian correction
		else if ($ret < -12219321600) $ret = -12219321600; // if in limbo, reset to 15 Oct 1582.
	}
	//print " dmy=$day/$mon/$year $hr:$min:$sec => " .$ret;
...
function adodb_get_gmt_strtotime($y,$m,$d)
{
...
global $ADODB_DATETIME_CLASS;
	...
	if (isset($TZ)) return $TZ;
	$y = date('Y');
	$TZ = strtotime("12/02/".$y);//mktime(0,0,0,12,2,$y,0) - gmmktime(0,0,0,12,2,$y,0);
	}
