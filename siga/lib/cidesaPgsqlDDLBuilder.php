...
";
     ...
     * @see        parent::addIndices()
     */
    protected function addUnices(&$script) {
        $table = $this->getTable();
        $platform = $this->getPlatform();

        foreach ($table->getUnices() as $unique ) {
            $script .= "ALTER TABLE ".$this->quoteIdentifier($table->getName())." ADD CONSTRAINT ".$this->quoteIdentifier($unique->getName())." UNIQUE (".$this->getColumnList($unique->getColumns()).");
";
        }
    }
...
";
  ...
  ";
    ...
    protected function addColumn($colname, &$script, $notnull=true) {
        ...
        foreach ($table->getColumns() as $col) {
            if(strtolower($col->getName()) == strtolower($colname)) {
              ...
              if($col->getName()=='id') {
                  $line_sequence = "CREATE SEQUENCE ".strtolower($table->getSequenceName());
                  $lines = $this->getColumnDDL($col)." DEFAULT nextval('".(strtolower($table->getSequenceName()))."'::regclass)";
              }
              else $lines = $this->getColumnDDL($col);
...
ALTER TABLE ".$this->quoteIdentifier($table->getName())." ADD ".$lines.";
        ";
              ...
              $this->addColumnCommentsByColumn($colname, $script);
    ...
    protected function alterColumnVarchar($colname, &$script, $size)
    {
        ...
        $platform = $this->getPlatform();
                ...
                $script .= "
ALTER TABLE ".$this->quoteIdentifier($table->getName())." ALTER COLUMN ".$colname." TYPE character varying".$size.";
        ";

    }

    protected function alterColumnNumeric($colname, &$script, $size)
    {
        ...
        $platform = $this->getPlatform();
                ...
                $script .= "
ALTER TABLE ".$this->quoteIdentifier($table->getName())." ALTER COLUMN ".$colname." TYPE numeric".$size.";
        ";

    }    

    protected function alterColumnInteger($colname, &$script)
    {
        ...
        $platform = $this->getPlatform();
                ...
                $script .= "
ALTER TABLE ".$this->quoteIdentifier($table->getName())." ALTER COLUMN ".$colname." TYPE integer;
        ";

    }    

    protected function addTrimColumn(&$script) {

        $table = $this->getTable();
        $platform = $this->getPlatform();

        $script .= "
UPDATE ".$this->quoteIdentifier($table->getName())." SET ";
        ...
        foreach ($table->getColumns() as $col) {
            ...
            if(strstr($columntype,'VARCHAR')!='') {
                $trims[] = ' '.$col->getName()." = TRIM(".$col->getName().") ";
            }
        }
        $script .= implode(',',$trims).';';
    ...
    protected function addColumnCommentsByColumn($column, &$script) {
        ...
        foreach ($this->getTable()->getColumns() as $col) {
            if($column == $col->getName()) {
                  $vendor_info = $col->getVendorSpecificInfo();
                  if( isset($vendor_info['description']) && $vendor_info['description'] != '' ){
                    $script .= "
COMMENT ON COLUMN ".$this->quoteIdentifier($table->getName()).".".$this->quoteIdentifier($col->getName())." IS '".$platform->escapeText($vendor_info['description']) ."';
";
    ...
    protected function checkdatabase(&$script) {
        ...
        //print_r($remotas);exit;
        $locales = $this->getTable()->getColumns();
        $objtabla = $this->getTable();
        $tabla = $this->getTable()->getName();

        //if(strtolower($tabla)=='usuarios') print_r($remotas[strtolower($tabla)]);
        ...
        if(array_key_exists(strtolower($tabla), $remotas)) {
            ...
            foreach($locales as $col) {
                $columnname = $col->getName();
                if(!array_key_exists(strtolower($columnname),$remotas[strtolower($tabla)])) {
                    ...
                    if(strstr($columntype,'VARCHAR')!='') {
                        ...
                        if($domain->printSize()!='('.$remotas[strtolower($tabla)][$columnname]['size'].')')
                        {
                            $this->alterColumnVarchar($columnname,$script, $domain->printSize());
                        }
                    }else if(strstr($columntype,'NUMERIC')!='') {
                        $domain = $col->getDomain();

                        if(!array_key_exists('scale',$remotas[strtolower($tabla)][$columnname]))
                          $valor='('.$remotas[strtolower($tabla)][$columnname]['size'].',0)';
                        else
                          $valor='('.$remotas[strtolower($tabla)][$columnname]['size'].','.$remotas[strtolower($tabla)][$columnname]['scale'].')';
                    
                        if ($domain->printSize()!=$valor)
                        {
                            $this->alterColumnNumeric($columnname,$script, $domain->printSize());
                        }
                    }else if(strstr($columntype,'INTEGER')!='') {
                        $domain = $col->getDomain();
                      
                        if ($domain->getSqlType()!=$remotas[strtolower($tabla)][$columnname]['type'])
                        {
                            $this->alterColumnInteger($columnname,$script);
                        }
                    }
                    ...
                    $this->addColumnCommentsByColumn(strtolower($columnname), $script);

                }
            } // Foreach Columns

            // Reviso si la tabla tiene unices
            if(count($objtabla->getUnices())>0) {
                if(!array_key_exists('_uniques',$remotas[strtolower($tabla)])) {
                    $this->addUnices($script);
                }
            }
            ...
            // Reviso el comentario del nombre de la tabla
            //if($objtabla->getDescription()!='' && )
