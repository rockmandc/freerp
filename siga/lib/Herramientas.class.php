...
class Herramientas
{
   ...
   public static function Cargarcombo($tablas,$filtros_tablas,$filtros_variales,$campos_retornados)
  {
      ...
      if ($filtros_tablas[0]!='')
      {
        ...
        {
          eval('$c->add('.ucfirst(strtolower($tablas[0])).'Peer::'.strtoupper($filtros_tablas[$a]).','.chr(39).strtolower($filtros_variales[$a]).chr(39).');');
      ...
      while ($j<$col){
        ...
        elseif ($tipos[$pos-1]=="m") {
            if ($form->getRequestParameter($caja)=="0,00")
               ...
               $valor = Herramientas::toFloat($form->getRequestParameter($caja),$decimal[$pos-1]);
...
//print "</pre>";
          ...
          case 'opciones':
            ...
            foreach($confgrid[$confkey] as $key => $val){
              ...
              switch($key){
                ...
                case 'eliminar':
                  $metodo = 'setEliminar';
                  is_array($val) ?  $opciones->$metodo($val[0],$val[1]) : $opciones->$metodo($val) ;                  
                  break;
                default:
   ...
   * @return function  Retorna True si encuentra el Numero del correlativo de lo contrario false
   */
  public static function getCorrelativoMax($tabla,&$output)
  {
     $c = new Criteria();
     $clase = ucfirst(strtolower($tabla)).'Peer';
     eval('$reg = '.$clase.'::doCount($c);');
     if ($reg)
     {
        $output = $reg;
        return true;
  ...
  public static function grid_by_name_to_json($arra = array(), $grid='a', $extra='')
  {
    $json = '[';
    foreach($arra as $key0 => $value0){
      ...
      $index = 1;
      foreach($value0 as $key1 => $value1){
        $json .= '["'.$grid.'x_'.$key0.'_'.($index).'","'.$value1.'",""], ';
        $index++;
      }
    }
    return $json .= '["","",""]'.$extra.']';
 ...
 public static function CargarDatosGridv2(&$form,$obj,$arreglo = false)
  ...
  $datosvista = $form->getRequestParameter('grid'.$name);
    ...
    $tipos=$obj["tipos"];
    $oculta=$obj["oculta"];
    $comoindices=$obj["camposcomoindices"];
    $decimal=$obj["decimal"];
    ...
    while ($i<$fil) {
       ...
       if($comoindices){
         $cajchk1='grid'.$name.'['.$i.']['.(isset($campos[0]) ? strtolower($campos[0]) : 0 ).']';
         $cajchk2='grid'.$name.'['.$i.']['.(isset($campos[1]) ? strtolower($campos[1]) : 0 ).']';
         $cajchk3='grid'.$name.'['.$i.']['.(isset($campos[2]) ? strtolower($campos[2]) : 0 ).']';
         $cajchk4='grid'.$name.'['.$i.']['.(isset($campos[3]) ? strtolower($campos[3]) : 0 ).']';
         $cajchk5='grid'.$name.'['.$i.']['.(isset($campos[4]) ? strtolower($campos[4]) : 0 ).']';
       }else{
    ...
    if((trim($form->getRequestParameter($id))!="") and  $form->getRequestParameter($id)!="0,00") { // Modificación
      ...
      while ($j<$col){
        ...
        $caja='grid'.$name.'['.$i.']['.($comoindices ? strtolower($campos[($pos-1)]) : ($pos-1)).']';
        $tira1='$clase->set';
        $tira2='(';
        $tira3=');';
        ...
        if ($tipos[$pos-1]=="t"){
          ...
          $valor = self::toFloat($form->getRequestParameter($caja),$decimal[$pos-1]);
        }elseif ($tipos[$pos-1]=="e") {
          $valor = $form->getRequestParameter($caja);
        }elseif ($tipos[$pos-1]=="f"){
          $dateFormat = new sfDateFormat('es_VE');
          $str_fecha = $form->getRequestParameter($caja);
          ...
          if ($tipos[$pos-1]=="t") {
            ...
            if(!$arreglo){
                if($tipos[$pos-1]!="m") {
                    if($valor!="''") 
                        eval($tira1.$campos[$pos-1].$tira2.$valor.$tira3);
                }else eval($tira1.$campos[$pos-1].$tira2.$valor.$tira3);
            }else{
              $clase[strtolower($campos[$pos-1])] = $valor;
            }
            $j++;
...
*/
  ...
  public static function Monto_disponible_ejecucion($ano,$codigo,$perhas,&$mondis)
  {
     $valdisper12='';
     $varemp = sfContext::getInstance()->getUser()->getAttribute('configemp');
     if ($varemp)
       if(array_key_exists('generales',$varemp)) {
         if(array_key_exists('valdisper12',$varemp['generales']))
         {
           $valdisper12=$varemp['generales']['valdisper12'];
         }
       }
      ...
      coalesce(obtener_ejecucion(rtrim(codpre),'01','".$perhas."','".$ano."','TRA'),0) +
      coalesce(obtener_ejecucion(rtrim(codpre),'01','".$perhas."','".$ano."','ADI'),0) -
      ...
      from cpasiini where CodPre like '".$codigo."%' and anopre='".$ano."' and perpre='00'";
      if (Herramientas::BuscarDatos($sql,$result))
      {
        if ($result[0]['mondis']!='')
        {
 ...
 */
  ...
  public static function Monto_disponible($codigo,$fecmov='2012-00-01')
  {
     $montodisponible=0;
    $result=array();
    ...
    if (Herramientas::BuscarDatos($sql,$result))
    {
      ...
      $resul2 = array ();
      $sql1 = "SELECT nomabr, lonniv FROM CPNIVELES WHERE consec <= (select nivdis from cpdefniv limit 1) ORDER BY CONSEC";
      if (Herramientas::BuscarDatos($sql1, $resul2))
      {
        ...
        for ($a = 0; $a < count($resul2); $a++)
        {
          ...
          $longitud = $longitud + $resul2[$a]['lonniv'] + 1;
        }
      $longitud = $longitud -1;
        $var = substr($codigo, 0, $longitud);
        $mes = substr($fecmov, 5, 2);
    if (Herramientas::Monto_disponible_ejecucion($ano,$var,$mes,$mondis))
  ...
  public static function getCodempFromCedemp()
  {
    ...
    $usuarioid=sfContext::getInstance()->getUser()->getAttribute('usuario_id');
    $sql='select cedemp as cedemp from "SIMA_USER".usuarios where id=\''.$usuarioid.'\'';
     Herramientas::BuscarDatos($sql,$result);
     if (count($result)>0)
     {
        $cedemp=$result[0]["cedemp"];
        $c = new Criteria();
  ...
  public static function setvalSecuencia($seq, $valor) {

        $result = true;

        try {
            $sql = "ALTER SEQUENCE " . $seq . "  RESTART " . $valor;
            H::insertarRegistros($sql);
        } catch (Exception $ex) {
            // Creamos la secuencia
            $sql = "CREATE SEQUENCE $seq;";
    ...
    public static function getvalSecuencia($seq) {

        $result = true;

        try {
            $sql = "SELECT last_value as val from " . $seq;
            if (Herramientas::BuscarDatos($sql, $result)) {
                return $result[0]['val'];
            }else
                return 0;
...
public static function getXx_vacio($tabla,$filtros,$variables,$result)
    {
      if (!empty($variables))
      {
        $c = new Criteria();
    ...
    public static function EliminarGrid($clasemodelo, $tabla, $campo)
    {
        $c = new Criteria();
        eval('$c->add('.  ucfirst($tabla).'Peer::'.  strtoupper($campo).',$clasemodelo->get'.  ucfirst($campo).'());');
        eval('$per = '. ucfirst($tabla).'Peer::doSelect($c);');
        foreach($per as  $r)
            $r->delete();
    }

    public static function Buscar_CodigoHijoGeneral($codigo,$tabla,$campo)
    {
      $sql = "Select * from ".$tabla." where ".$campo." Like '". $codigo. "%'";
         if (Herramientas::BuscarDatos($sql,$result))
         {
          if (count($result)>1)
               return true;
    ...
    public static function bisiesto($ano){
      if ($ano%4!=0)
        $bisiesto=false;
      else
        if ($ano%400==0)
  ...
  public static function search_files( $dir , &$files, $criteria = '*')
  {
    if (is_dir($dir))
    {
      if ($gd = opendir($dir))
      {
            while (($file = readdir($gd)) !== false)
        {
          if ( $file != '.' AND $file != '..'  )
          {
            ...
            if ( is_dir( $dir.'/'.$file ) )
            {
              self::search_files( $dir.'/'.$file , $files, $criteria);
            }
            else
            {
              // Ready File
              if ( is_file( $dir.'/'.$file)  )
              {
                $print=false;
                if($criteria!='*'){
                  if(strstr($file,$criteria)) $print=true;
                }else $print=true;
                if($print) $files[] = $dir.'/'.$file;
  ...
  public static function f_name($file){

    $arr_file = explode('/',$file);

    $file = $arr_file[count($arr_file)-1];

    return explode('.',$file);

  }

  public static function d_name($file){

    $arr_file = explode('/',$file);
    $dir = '';
    foreach($arr_file as $i => $f){
      if(!($i==(count($arr_file)-1))) $dir .= $f.'/';
    }
    return $dir;

  }
  
      public static function BuscarIndice($arreglo1,$codiva)
    {
        $j=0;
        $indice="";
        if (count($arreglo1)>0)
        {
            ...
            foreach($arreglo1 as $confkey => $confval)
            {
                $codigo=split("_",$arreglo1[$confkey]);
                $codaux=$codigo[0];
               if ($codiva==$codaux)
               {
                   $indice=$confkey;
   ...
   { 
    ...
    public static function getCodPreDis($codpre){
      ...
      if ($cpdefniv)
      {
        $longitud = 0;
        $c = new Criteria();
        $c->add(CpnivelesPeer::CONSEC,$cpdefniv->getNivdis(),Criteria::LESS_EQUAL);
        $cpniveles = CpnivelesPeer::doSelect($c);
        

        foreach($cpniveles as $niv){
          //$longitud = $longitud + strlen($niv->getNomabr()) + 1;
          $longitud = $longitud + $niv->getLonniv() + 1;
        }


        $longitud = $longitud -1;
        $var = substr($codpre, 0, $longitud);
        return $var;
      }else return $codpre;
    }
    ...
    public static function hourdiff($hour_1 , $hour_2 , $formated=false)
    {
        ...
        $cal=(string)($result_explode[1]*60)/10;
        $cal2='0.'.$cal;
        $cal3=(float)$cal2;
        $cal3=round($cal3,2);
        $cal3=substr($cal3,2,strlen($cal3));
        

        return $result_explode[0].":".$cal3; 
        } 
        else 
        { 
        return $result; 
  ...
  public static function toDateUS($fecbarra)
  {
   $fecha=split('/',$fecbarra);
   if(count($fecha)>1) $fecraya=$fecha[2]."-".$fecha[1]."-".$fecha[0];
   else $fecraya = $fecbarra;
   return $fecraya;
  }
  
  public static function Puede_Eliminar($usuario,$form,$codemp)
  {
      $c = new Criteria();
      $c->add(ApliUserPeer::LOGUSE,$usuario);
      $c->add(ApliUserPeer::NOMOPC,$form);
      $c->add(ApliUserPeer::CODEMP,$codemp);
      $result=ApliUserPeer::doSelectOne($c);
      if ($result){
          if ($result->getPriuse()=='15')
              return true;
          else
              return false;
          ...
          $c2 = new Criteria();
          $c2->add(ApliUserPeer::LOGUSE,$usuario);
          $c2->add(ApliUserPeer::NOMOPC,'admin');
          $c2->add(ApliUserPeer::CODEMP,$codemp);
          $result2=ApliUserPeer::doSelectOne($c2);
          if ($result2)
              return true;
          else
              return false;       
  ...
  public static function ComprobanteAnulado($numcom)
  {
      $w= new Criteria();
      $w->add(ContabcPeer::NUMCOM,$numcom);
      $reg=ContabcPeer::doSelectOne($w);
      if ($reg)
      {
          if ($reg->getStacom()=='N')
                  return true;
  ...
  public static function url_to_array($url)
  {
    $assoc_array = array();
    $arr = explode('/', $url);
    if(count($arr)>1){
  ...
  public static function array_to_ul($array) {
    $out='<ul class="nav nav-list">';
    foreach($array as $key => $elem){
        if(!is_array($elem)){
                $out=$out."<li>$key:[$elem]</li>";
        }
        else $out=$out.'<li class="nav-header">'.$key.self::array_to_ul($elem)."</li>";
    }
    $out=$out."</ul>";
    return $out;
  }
  ...
  */
  ...
  public static function Repetido3($grid, $codact, $col, &$i) { //Busca en otro grid
    $repetido = false;
    $i = 0;
    while ($i < count($grid)) {
      $codact2 = $grid[$i][$col];
...
public static function gbversion()
{
  $Name="";
  $Version=""; 
...
public static function ObtenerExistenciaArticulo($codart='', $codalm='', $codubi=''){
  $result=array();
  if ($codalm!='')
    $sql="select coalesce(sum(exiact),0) as existencia from caartalmubi where codart='".$codart."' and codalm='".$codalm."'";
  else if ($codalm!='' && $codubi!='')
    $sql="select coalesce(exiact,0) as existencia from caartalmubi where codart='".$codart."' and codalm='".$codalm."'and codubi='".$codubi."'";
  else
    $sql="select coalesce(sum(exiact),0) as existencia from caartalmubi where codart='".$codart."'";
  if (Herramientas::BuscarDatos($sql,$result))
  {
  ...
  public static function VerificarFormatoPadrePartida($codigo)
  {
    $c = new Criteria();
  ...
  public static function ObtenerMesenNumero($mes)
  {
    if($mes=='ENERO')  return $mes='01';
    if($mes=='FEBRERO')  return $mes='02';
    if($mes=='MARZO')  return $mes='03';
    if($mes=='ABRIL')  return $mes='04';
    if($mes=='MAYO')  return $mes='05';
    if($mes=='JUNIO')  return $mes='06';
    if($mes=='JULIO')  return $mes='07';
    if($mes=='AGOSTO')  return $mes='08';
    if($mes=='SEPTIEMBRE')  return $mes='09';
    if($mes=='OCTUBRE')  return $mes='10';
    if($mes=='NOVIEMBRE')  return $mes='11';
    if($mes=='DICIEMBRE')  return $mes='12';
  }

  public static function dateadde($interval, $number, $date) 
 {
  ...
  public static function getObtener_FormatoCategoriaIngresos()
  {
    $result=array();
    $sql = "Select rupcat, ruppar, forpre From CiDefNiv";
    $i=1;
    if (Herramientas::BuscarDatos($sql,$result))
    {
      $categoria = $result[0]['rupcat'];
  ...
  {
    $usuario_id = sfContext::getInstance()->getUser()->getAttribute('usuario_id', null);
    if($usuario_id){
      $c = new Criteria();
      $c->add(UsuariosPeer::ID, $usuario_id);
      return UsuariosPeer::doSelectone($c);
    }else return false;
  }  

  public static function EnPapiro($coddoc){
    $c = new Criteria();
    $c->add(PapiroPeer::CODDOC, $coddoc);
    return PapiroPeer::doSelectOne($c);
  }
