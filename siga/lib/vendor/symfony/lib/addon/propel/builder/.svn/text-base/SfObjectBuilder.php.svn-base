...
";
      }else if (!$created_user && in_array($clo, array('created_by_user')))
      {
        $created_user = true;
        $date_script .= "
    if (\$this->isNew() && !\$this->isColumnModified(".$this->getColumnConstant($col)."))
    {
      \$this->set".$col->getPhpName()."(sfContext::getInstance()->getUser()->getAttribute('loguse','Sin Usuario'));
    }
";
      }else if (!$updated_user && in_array($clo, array('updated_by_user')))
      {
        $updated_user = true;
        $date_script .= "
    if (\$this->isModified() && !\$this->isColumnModified(".$this->getColumnConstant($col)."))
