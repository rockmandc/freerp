...
abstract class BaseNpcargos extends BaseObject  implements Persistent {
  ...
  public function getRetcar($val=false)
  {

    if($val) return number_format($this->retcar,2,',','.');
    else return $this->retcar;

  }
  
  public function getCanpmix($val=false)
  {

    if($val) return number_format($this->canpmix,2,',','.');
    else return $this->canpmix;
  ...
  public function getId()
  {
    ...
    if ($this->codniv !== $v) {
        $this->codniv = $v;
        $this->modifiedColumns[] = NpcargosPeer::CODNIV;
      }
  
	} 
	
	public function setRetcar($v)
	{

    if ($this->retcar !== $v) {
        $this->retcar = Herramientas::toFloat($v);
        $this->modifiedColumns[] = NpcargosPeer::RETCAR;
      }
  
	} 
	
	public function setCanpmix($v)
	{

    if ($this->canpmix !== $v) {
        $this->canpmix = Herramientas::toFloat($v);
        $this->modifiedColumns[] = NpcargosPeer::CANPMIX;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->canvhom = $rs->getInt($startcol + 17);

      $this->canvmuj = $rs->getInt($startcol + 18);

      $this->codniv = $rs->getString($startcol + 19);

      $this->retcar = $rs->getFloat($startcol + 20);

      $this->canpmix = $rs->getFloat($startcol + 21);
  ...
  public function get($m, $a)
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 19:
				...
				break;
			case 20:
				return $this->getRetcar();
				break;
			case 21:
				return $this->getCanpmix();
				break;
			case 22:
				return $this->getId();
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[19] => $this->getCodniv(),
			$keys[20] => $this->getRetcar(),
			$keys[21] => $this->getCanpmix(),
			$keys[22] => $this->getId(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 19:
				...
				break;
			case 20:
				$this->setRetcar($value);
				break;
			case 21:
				$this->setCanpmix($value);
				break;
			case 22:
				$this->setId($value);
				break;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[19], $arr)) $this->setCodniv($arr[$keys[19]]);
		if (array_key_exists($keys[20], $arr)) $this->setRetcar($arr[$keys[20]]);
		if (array_key_exists($keys[21], $arr)) $this->setCanpmix($arr[$keys[21]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NpcargosPeer::CODNIV)) $criteria->add(NpcargosPeer::CODNIV, $this->codniv);
		if ($this->isColumnModified(NpcargosPeer::RETCAR)) $criteria->add(NpcargosPeer::RETCAR, $this->retcar);
		if ($this->isColumnModified(NpcargosPeer::CANPMIX)) $criteria->add(NpcargosPeer::CANPMIX, $this->canpmix);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setRetcar($this->retcar);

		$copyObj->setCanpmix($this->canpmix);
