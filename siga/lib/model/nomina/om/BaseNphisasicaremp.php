...
abstract class BaseNphisasicaremp extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->codttrab !== $v) {
        $this->codttrab = $v;
        $this->modifiedColumns[] = NphisasicarempPeer::CODTTRAB;
      }
    ...
    if ($this->codtipemp !== $v) {
        $this->codtipemp = $v;
        $this->modifiedColumns[] = NphisasicarempPeer::CODTIPEMP;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->codttrab = $rs->getString($startcol + 15);

      $this->codtipemp = $rs->getString($startcol + 16);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[15] => $this->getCodttrab(),
			$keys[16] => $this->getCodtipemp(),
			$keys[17] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[15], $arr)) $this->setCodttrab($arr[$keys[15]]);
		if (array_key_exists($keys[16], $arr)) $this->setCodtipemp($arr[$keys[16]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NphisasicarempPeer::CODTTRAB)) $criteria->add(NphisasicarempPeer::CODTTRAB, $this->codttrab);
		if ($this->isColumnModified(NphisasicarempPeer::CODTIPEMP)) $criteria->add(NphisasicarempPeer::CODTIPEMP, $this->codtipemp);
