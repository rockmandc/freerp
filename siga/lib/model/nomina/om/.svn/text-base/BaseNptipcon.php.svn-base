...
abstract class BaseNptipcon extends BaseObject  implements Persistent {
  ...
  public function getTrimfis()
  {

    return trim($this->trimfis);

  }
  
  public function getFecfin108($format = 'Y-m-d')
  {

    if ($this->fecfin108 === null || $this->fecfin108 === '') {
      return null;
    } elseif (!is_int($this->fecfin108)) {
            $ts = adodb_strtotime($this->fecfin108);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [fecfin108] as date/time value: " . var_export($this->fecfin108, true));
      }
    } else {
      $ts = $this->fecfin108;
    }
  ...
  public function getApliregant()
  {

    return trim($this->apliregant);
  ...
  public function getId()
  {
    ...
    if ($this->mesini !== $v) {
        $this->mesini = $v;
        $this->modifiedColumns[] = NptipconPeer::MESINI;
      }
  
	} 
	
	public function setTrimfis($v)
	{

    if ($this->trimfis !== $v || $v === 'S') {
        $this->trimfis = $v;
        $this->modifiedColumns[] = NptipconPeer::TRIMFIS;
      }
  
	} 
	
	public function setFecfin108($v)
	{

    if ($v !== null && !is_int($v)) {
      $ts = adodb_strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [fecfin108] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    if ($this->fecfin108 !== $ts) {
      $this->fecfin108 = $ts;
      $this->modifiedColumns[] = NptipconPeer::FECFIN108;
    }

	} 
	
	public function setApliregant($v)
	{

    if ($this->apliregant !== $v || $v === '') {
        $this->apliregant = $v;
        $this->modifiedColumns[] = NptipconPeer::APLIREGANT;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->mesini = $rs->getInt($startcol + 12);

      $this->trimfis = $rs->getString($startcol + 13);

      $this->fecfin108 = $rs->getDate($startcol + 14, null);

      $this->apliregant = $rs->getString($startcol + 15);
  ...
  public function get($m, $a)
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 12:
				...
				break;
			case 13:
				return $this->getTrimfis();
				break;
			case 14:
				return $this->getFecfin108();
				break;
			case 15:
				return $this->getApliregant();
				break;
			case 16:
				return $this->getId();
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[12] => $this->getMesini(),
			$keys[13] => $this->getTrimfis(),
			$keys[14] => $this->getFecfin108(),
			$keys[15] => $this->getApliregant(),
			$keys[16] => $this->getId(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 12:
				...
				break;
			case 13:
				$this->setTrimfis($value);
				break;
			case 14:
				$this->setFecfin108($value);
				break;
			case 15:
				$this->setApliregant($value);
				break;
			case 16:
				$this->setId($value);
				break;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[12], $arr)) $this->setMesini($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setTrimfis($arr[$keys[13]]);
		if (array_key_exists($keys[14], $arr)) $this->setFecfin108($arr[$keys[14]]);
		if (array_key_exists($keys[15], $arr)) $this->setApliregant($arr[$keys[15]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NptipconPeer::MESINI)) $criteria->add(NptipconPeer::MESINI, $this->mesini);
		if ($this->isColumnModified(NptipconPeer::TRIMFIS)) $criteria->add(NptipconPeer::TRIMFIS, $this->trimfis);
		if ($this->isColumnModified(NptipconPeer::FECFIN108)) $criteria->add(NptipconPeer::FECFIN108, $this->fecfin108);
		if ($this->isColumnModified(NptipconPeer::APLIREGANT)) $criteria->add(NptipconPeer::APLIREGANT, $this->apliregant);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setTrimfis($this->trimfis);

		$copyObj->setFecfin108($this->fecfin108);

		$copyObj->setApliregant($this->apliregant);
