...
abstract class BaseNpdefgen extends BaseObject  implements Persistent {
  ...
  public function getForced()
  {
    ...
    return trim($this->forced);

  }
  
  public function getCorayu($val=false)
  {

    if($val) return number_format($this->corayu,2,',','.');
    else return $this->corayu;

  }
  
  public function getSuemin($val=false)
  {

    if($val) return number_format($this->suemin,2,',','.');
    else return $this->suemin;
  ...
  public function getCorpto($val=false)
  {

    if($val) return number_format($this->corpto,2,',','.');
    else return $this->corpto;
  ...
  public function getId()
  {
	...
	public function setForced($v)
	{

    if ($this->forced !== $v) {
        $this->forced = $v;
        $this->modifiedColumns[] = NpdefgenPeer::FORCED;
      }
  
	} 
	
	public function setCorayu($v)
	{

    if ($this->corayu !== $v) {
        $this->corayu = Herramientas::toFloat($v);
        $this->modifiedColumns[] = NpdefgenPeer::CORAYU;
      }
  
	} 
	
	public function setSuemin($v)
	{

    if ($this->suemin !== $v) {
        $this->suemin = Herramientas::toFloat($v);
        $this->modifiedColumns[] = NpdefgenPeer::SUEMIN;
      }
    ...
    if ($this->codsitemp !== $v) {
        $this->codsitemp = $v;
        $this->modifiedColumns[] = NpdefgenPeer::CODSITEMP;
      }
    ...
    if ($this->codestemp !== $v) {
        $this->codestemp = $v;
        $this->modifiedColumns[] = NpdefgenPeer::CODESTEMP;
      }
    ...
    if ($this->codtipgas !== $v) {
        $this->codtipgas = $v;
        $this->modifiedColumns[] = NpdefgenPeer::CODTIPGAS;
      }
  
	} 
	
	public function setCorpto($v)
	{

    if ($this->corpto !== $v) {
        $this->corpto = Herramientas::toFloat($v);
        $this->modifiedColumns[] = NpdefgenPeer::CORPTO;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->forced = $rs->getString($startcol + 35);

      $this->corayu = $rs->getFloat($startcol + 36);

      $this->suemin = $rs->getFloat($startcol + 37);

      $this->codsitemp = $rs->getString($startcol + 38);

      $this->codestemp = $rs->getString($startcol + 39);

      $this->codtipgas = $rs->getString($startcol + 40);

      $this->corpto = $rs->getFloat($startcol + 41);
  ...
  public function get($m, $a)
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 35:
				...
				break;
			case 36:
				return $this->getCorayu();
				break;
			case 37:
				return $this->getSuemin();
				break;
			case 38:
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[35] => $this->getForced(),
			$keys[36] => $this->getCorayu(),
			$keys[37] => $this->getSuemin(),
			$keys[38] => $this->getCodsitemp(),
			$keys[39] => $this->getCodestemp(),
			$keys[40] => $this->getCodtipgas(),
			$keys[41] => $this->getCorpto(),
			$keys[42] => $this->getId(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 34:
				...
				break;
			case 35:
				$this->setForced($value);
				break;
			case 36:
				$this->setCorayu($value);
				break;
			case 37:
				$this->setSuemin($value);
				break;
			case 38:
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[35], $arr)) $this->setForced($arr[$keys[35]]);
		if (array_key_exists($keys[36], $arr)) $this->setCorayu($arr[$keys[36]]);
		if (array_key_exists($keys[37], $arr)) $this->setSuemin($arr[$keys[37]]);
		if (array_key_exists($keys[38], $arr)) $this->setCodsitemp($arr[$keys[38]]);
		if (array_key_exists($keys[39], $arr)) $this->setCodestemp($arr[$keys[39]]);
		if (array_key_exists($keys[40], $arr)) $this->setCodtipgas($arr[$keys[40]]);
		if (array_key_exists($keys[41], $arr)) $this->setCorpto($arr[$keys[41]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NpdefgenPeer::FORCED)) $criteria->add(NpdefgenPeer::FORCED, $this->forced);
		if ($this->isColumnModified(NpdefgenPeer::CORAYU)) $criteria->add(NpdefgenPeer::CORAYU, $this->corayu);
		if ($this->isColumnModified(NpdefgenPeer::SUEMIN)) $criteria->add(NpdefgenPeer::SUEMIN, $this->suemin);
		if ($this->isColumnModified(NpdefgenPeer::CODSITEMP)) $criteria->add(NpdefgenPeer::CODSITEMP, $this->codsitemp);
		if ($this->isColumnModified(NpdefgenPeer::CODESTEMP)) $criteria->add(NpdefgenPeer::CODESTEMP, $this->codestemp);
		if ($this->isColumnModified(NpdefgenPeer::CODTIPGAS)) $criteria->add(NpdefgenPeer::CODTIPGAS, $this->codtipgas);
		if ($this->isColumnModified(NpdefgenPeer::CORPTO)) $criteria->add(NpdefgenPeer::CORPTO, $this->corpto);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setForced($this->forced);

		$copyObj->setCorayu($this->corayu);

		$copyObj->setSuemin($this->suemin);
