...
abstract class BaseNpconsalint extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->codnom !== $v) {
        $this->codnom = $v;
        $this->modifiedColumns[] = NpconsalintPeer::CODNOM;
      }
    ...
    if ($this->codcon !== $v) {
        $this->codcon = $v;
        $this->modifiedColumns[] = NpconsalintPeer::CODCON;
      }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = NpconsalintPeer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      throw new PropelException("Error populating Npconsalint object", $e);
    }
  }
  ...
  public function __call($m, $a)
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			NpconsalintPeer::doDelete($this, $con);
		...
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aNpdefcpt !== null) {
				...
				$this->setNpdefcpt($this->aNpdefcpt);
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = NpconsalintPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += NpconsalintPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = NpconsalintPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = NpconsalintPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = NpconsalintPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NpconsalintPeer::CODNOM)) $criteria->add(NpconsalintPeer::CODNOM, $this->codnom);
		if ($this->isColumnModified(NpconsalintPeer::CODCON)) $criteria->add(NpconsalintPeer::CODCON, $this->codcon);
		if ($this->isColumnModified(NpconsalintPeer::ID)) $criteria->add(NpconsalintPeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(NpconsalintPeer::ID, $this->id);
