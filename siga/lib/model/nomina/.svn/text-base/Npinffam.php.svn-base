 ...
 public function hydrate(ResultSet $rs, $startcol = 1)
   {
      ...
      //Segundo apellido
        ...
        if (self::getSexfam()!='') {
          if (self::getSexfam()=='F')
            $this->nombresex="Femenino";
          else
            $this->nombresex="Masculino";
      }
      $this->nombrepar=H::getX_vacio('TIPPAR','Nptippar','Despar',self::getParfam());
      if (self::getEdociv()!='') {
          if (self::getEdociv()=='S')
            $this->nomedociv="Soltero(a)";
          else if (self::getEdociv()=='C')
            $this->nomedociv="Casado(a)";
          else if (self::getEdociv()=='D')
            $this->nomedociv="Divorciado(a)";
          else if (self::getEdociv()=='V')
            $this->nomedociv="Viudo(a)";
          else
            $this->nomedociv="Concubino(a)";
      }
       if (self::getCarben()!='') {
          if (self::getCarben()=='B')
            $this->nomcarben="Básica";
          else if (self::getCarben()=='A')
            $this->nomcarben="Adicional";
          else
            $this->nomcarben="Segundo Adicional";
      }
      if (self::getDissus()!='') {
          if (self::getDissus()=='S')
            $this->nomdissus="Si";
