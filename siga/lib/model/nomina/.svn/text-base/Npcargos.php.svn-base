...
class Npcargos extends BaseNpcargos
{
	...
	{
		   if (self::getHabCarMix()=='S')
            $sum= self::getCanpmix()-self::getCanmix();
           else
	...
	{
		return Herramientas::getX_vacio('CODNIV','Viadefniv','Desniv',self::getCodniv());
	}
	...
	public function getHabCarMix(){
		return H::getConfApp2('habcarmix', 'nomina', 'nomdefespcar');
	}
