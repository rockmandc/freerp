...
abstract class BaseAtdonaciones extends BaseObject  implements Persistent {
  ...
  public function getAtgrudonId()
  {

    return $this->atgrudon_id;
  ...
  public function getId()
  {
	...
	public function setAtgrudonId($v)
	{

    if ($this->atgrudon_id !== $v) {
        $this->atgrudon_id = $v;
        $this->modifiedColumns[] = AtdonacionesPeer::ATGRUDON_ID;
      }
  
		if ($this->aAtgrudon !== null && $this->aAtgrudon->getId() !== $v) {
			$this->aAtgrudon = null;
		}
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->atgrudon_id = $rs->getInt($startcol + 3);
  ...
  public function get($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aAtgrudon !== null) {
				if ($this->aAtgrudon->isModified()) {
					$affectedRows += $this->aAtgrudon->save($con);
				}
				$this->setAtgrudon($this->aAtgrudon);
			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aAtgrudon !== null) {
				if (!$this->aAtgrudon->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aAtgrudon->getValidationFailures());
				}
			}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setAtgrudonId($arr[$keys[3]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AtdonacionesPeer::ATGRUDON_ID)) $criteria->add(AtdonacionesPeer::ATGRUDON_ID, $this->atgrudon_id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setAtgrudonId($this->atgrudon_id);
	...
	public function setAtgrudon($v)
	{


		if ($v === null) {
			$this->setAtgrudonId(NULL);
		} else {
			$this->setAtgrudonId($v->getId());
		}
	...
	public function getAtgrudon($con = null)
	{
		if ($this->aAtgrudon === null && ($this->atgrudon_id !== null)) {
						include_once 'lib/model/ciudadanos/om/BaseAtgrudonPeer.php';

      $c = new Criteria();
      $c->add(AtgrudonPeer::ID,$this->atgrudon_id);
      
			$this->aAtgrudon = AtgrudonPeer::doSelectOne($c, $con);

			
		}
		return $this->aAtgrudon;
	}
