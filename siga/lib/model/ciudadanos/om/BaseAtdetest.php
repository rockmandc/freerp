...
abstract class BaseAtdetest extends BaseObject  implements Persistent {
  ...
  public function getAtdenunciasId()
  {

    return $this->atdenuncias_id;
  ...
  public function getId()
  {
	...
	public function setAtdenunciasId($v)
	{

    if ($this->atdenuncias_id !== $v) {
        $this->atdenuncias_id = $v;
        $this->modifiedColumns[] = AtdetestPeer::ATDENUNCIAS_ID;
      }
  
		if ($this->aAtdenuncias !== null && $this->aAtdenuncias->getId() !== $v) {
			$this->aAtdenuncias = null;
		}
    ...
    if ($this->descripcion !== $v) {
        $this->descripcion = $v;
        $this->modifiedColumns[] = AtdetestPeer::DESCRIPCION;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->atdenuncias_id = $rs->getInt($startcol + 1);

      $this->atestayu_desde = $rs->getInt($startcol + 2);

      $this->atestayu_hasta = $rs->getInt($startcol + 3);

      $this->usuario = $rs->getString($startcol + 4);
  ...
  public function get($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aAtdenuncias !== null) {
				if ($this->aAtdenuncias->isModified()) {
					$affectedRows += $this->aAtdenuncias->save($con);
				}
				$this->setAtdenuncias($this->aAtdenuncias);
			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aAtdenuncias !== null) {
				if (!$this->aAtdenuncias->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aAtdenuncias->getValidationFailures());
				}
			}
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[1] => $this->getAtdenunciasId(),
			$keys[2] => $this->getAtestayuDesde(),
			$keys[3] => $this->getAtestayuHasta(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setAtdenunciasId($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setAtestayuDesde($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setAtestayuHasta($arr[$keys[3]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AtdetestPeer::ATDENUNCIAS_ID)) $criteria->add(AtdetestPeer::ATDENUNCIAS_ID, $this->atdenuncias_id);
		...
		if ($this->isColumnModified(AtdetestPeer::DESCRIPCION)) $criteria->add(AtdetestPeer::DESCRIPCION, $this->descripcion);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setAtdenunciasId($this->atdenuncias_id);
	...
	public function setAtdenuncias($v)
	{


		if ($v === null) {
			$this->setAtdenunciasId(NULL);
		} else {
			$this->setAtdenunciasId($v->getId());
		}
	...
	public function getAtdenuncias($con = null)
	{
		if ($this->aAtdenuncias === null && ($this->atdenuncias_id !== null)) {
						include_once 'lib/model/ciudadanos/om/BaseAtdenunciasPeer.php';

      $c = new Criteria();
      $c->add(AtdenunciasPeer::ID,$this->atdenuncias_id);
      
			$this->aAtdenuncias = AtdenunciasPeer::doSelectOne($c, $con);

			
		}
		return $this->aAtdenuncias;
	}
