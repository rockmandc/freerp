...
abstract class BaseAtmisiones extends BaseObject  implements Persistent {
  ...
  public function getDesmis()
  {

    return trim($this->desmis);
  ...
  public function getId()
  {
	...
	public function setDesmis($v)
	{

    if ($this->desmis !== $v) {
        $this->desmis = $v;
        $this->modifiedColumns[] = AtmisionesPeer::DESMIS;
      }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = AtmisionesPeer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->desmis = $rs->getString($startcol + 0);
      ...
      throw new PropelException("Error populating Atmisiones object", $e);
    }
  }
  ...
  public function get($m, $a)
	...
	public function delete($con = null)
	{
		...
		if ($con === null) {
			$con = Propel::getConnection(AtmisionesPeer::DATABASE_NAME);
		}

		try {
			$con->begin();
			AtmisionesPeer::doDelete($this, $con);
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = AtmisionesPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += AtmisionesPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = AtmisionesPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = AtmisionesPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = AtmisionesPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getDesmis(),
			$keys[1] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setDesmis($arr[$keys[0]]);
	...
	{
		$criteria = new Criteria(AtmisionesPeer::DATABASE_NAME);

		if ($this->isColumnModified(AtmisionesPeer::DESMIS)) $criteria->add(AtmisionesPeer::DESMIS, $this->desmis);
		if ($this->isColumnModified(AtmisionesPeer::ID)) $criteria->add(AtmisionesPeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(AtmisionesPeer::ID, $this->id);
		...
		if ($this->collAtciudadanos === null) {
			if ($this->isNew()) {
			   $this->collAtciudadanos = array();
				...
				$criteria->add(AtciudadanoPeer::ATMISIONES_ID, $this->getId());
