...
abstract class BaseOptipret extends BaseObject  implements Persistent {
  ...
  public function getMbasmi($val=false)
  {

    if($val) return number_format($this->mbasmi,2,',','.');
    else return $this->mbasmi;

  }
  
  public function getMbasma($val=false)
  {

    if($val) return number_format($this->mbasma,2,',','.');
    else return $this->mbasma;
  ...
  public function getId()
  {
	...
	public function setMbasma($v)
	{

    if ($this->mbasma !== $v) {
        $this->mbasma = Herramientas::toFloat($v);
        $this->modifiedColumns[] = OptipretPeer::MBASMA;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->mbasmi = $rs->getFloat($startcol + 9);

      $this->mbasma = $rs->getFloat($startcol + 10);
  ...
  public function get($m, $a)
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 8:
				...
				break;
			case 9:
				return $this->getMbasmi();
				break;
			case 10:
				return $this->getMbasma();
				break;
			case 11:
				return $this->getId();
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[9] => $this->getMbasmi(),
			$keys[10] => $this->getMbasma(),
			$keys[11] => $this->getId(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 8:
				...
				break;
			case 9:
				$this->setMbasmi($value);
				break;
			case 10:
				$this->setMbasma($value);
				break;
			case 11:
				$this->setId($value);
				break;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[9], $arr)) $this->setMbasmi($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setMbasma($arr[$keys[10]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(OptipretPeer::MBASMI)) $criteria->add(OptipretPeer::MBASMI, $this->mbasmi);
		if ($this->isColumnModified(OptipretPeer::MBASMA)) $criteria->add(OptipretPeer::MBASMA, $this->mbasma);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setMbasmi($this->mbasmi);

		$copyObj->setMbasma($this->mbasma);
