...
abstract class BaseViadetrelvia extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->codart !== $v) {
        $this->codart = $v;
        $this->modifiedColumns[] = ViadetrelviaPeer::CODART;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->fecfac = $rs->getDate($startcol + 3, null);

      $this->montonet = $rs->getFloat($startcol + 4);

      $this->montorec = $rs->getFloat($startcol + 5);

      $this->codcat = $rs->getString($startcol + 6);

      $this->codpar = $rs->getString($startcol + 7);

      $this->refsol = $rs->getString($startcol + 8);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[3] => $this->getFecfac(),
			$keys[4] => $this->getMontonet(),
			$keys[5] => $this->getMontorec(),
			$keys[6] => $this->getCodcat(),
			$keys[7] => $this->getCodpar(),
			$keys[8] => $this->getRefsol(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setFecfac($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setMontonet($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setMontorec($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setCodcat($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setCodpar($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setRefsol($arr[$keys[8]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ViadetrelviaPeer::CODART)) $criteria->add(ViadetrelviaPeer::CODART, $this->codart);
