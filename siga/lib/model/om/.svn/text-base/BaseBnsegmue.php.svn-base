...
abstract class BaseBnsegmue extends BaseObject  implements Persistent {
  ...
  public function getNumdeppol()
  {

    return trim($this->numdeppol);

  }
  
  public function getFecvigdes($format = 'Y-m-d')
  {

    if ($this->fecvigdes === null || $this->fecvigdes === '') {
      return null;
    } elseif (!is_int($this->fecvigdes)) {
            $ts = adodb_strtotime($this->fecvigdes);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [fecvigdes] as date/time value: " . var_export($this->fecvigdes, true));
      }
    } else {
      $ts = $this->fecvigdes;
    }
  ...
  public function getFecvighas($format = 'Y-m-d')
  {

    if ($this->fecvighas === null || $this->fecvighas === '') {
      return null;
    } elseif (!is_int($this->fecvighas)) {
            $ts = adodb_strtotime($this->fecvighas);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [fecvighas] as date/time value: " . var_export($this->fecvighas, true));
      }
    } else {
      $ts = $this->fecvighas;
    }
  ...
  public function getId()
  {
    ...
    if ($this->stasegmue !== $v) {
        $this->stasegmue = strtoupper($v);
        $this->modifiedColumns[] = BnsegmuePeer::STASEGMUE;
      }
  
	} 
	
	public function setNumdeppol($v)
	{

    if ($this->numdeppol !== $v) {
        $this->numdeppol = strtoupper($v);
        $this->modifiedColumns[] = BnsegmuePeer::NUMDEPPOL;
      }
  
	} 
	
	public function setFecvigdes($v)
	{

    if ($v !== null && !is_int($v)) {
      $ts = adodb_strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [fecvigdes] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    if ($this->fecvigdes !== $ts) {
      $this->fecvigdes = $ts;
      $this->modifiedColumns[] = BnsegmuePeer::FECVIGDES;
    }

	} 
	
	public function setFecvighas($v)
	{

    if ($v !== null && !is_int($v)) {
      $ts = adodb_strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [fecvighas] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    if ($this->fecvighas !== $ts) {
      $this->fecvighas = $ts;
      $this->modifiedColumns[] = BnsegmuePeer::FECVIGHAS;
    }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->numdeppol = $rs->getString($startcol + 11);

      $this->fecvigdes = $rs->getDate($startcol + 12, null);

      $this->fecvighas = $rs->getDate($startcol + 13, null);
  ...
  public function get($m, $a)
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 10:
				...
				break;
			case 11:
				return $this->getNumdeppol();
				break;
			case 12:
				return $this->getFecvigdes();
				break;
			case 13:
				return $this->getFecvighas();
				break;
			case 14:
				return $this->getId();
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[11] => $this->getNumdeppol(),
			$keys[12] => $this->getFecvigdes(),
			$keys[13] => $this->getFecvighas(),
			$keys[14] => $this->getId(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 10:
				...
				break;
			case 11:
				$this->setNumdeppol($value);
				break;
			case 12:
				$this->setFecvigdes($value);
				break;
			case 13:
				$this->setFecvighas($value);
				break;
			case 14:
				$this->setId($value);
				break;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[11], $arr)) $this->setNumdeppol($arr[$keys[11]]);
		if (array_key_exists($keys[12], $arr)) $this->setFecvigdes($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setFecvighas($arr[$keys[13]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(BnsegmuePeer::NUMDEPPOL)) $criteria->add(BnsegmuePeer::NUMDEPPOL, $this->numdeppol);
		if ($this->isColumnModified(BnsegmuePeer::FECVIGDES)) $criteria->add(BnsegmuePeer::FECVIGDES, $this->fecvigdes);
		if ($this->isColumnModified(BnsegmuePeer::FECVIGHAS)) $criteria->add(BnsegmuePeer::FECVIGHAS, $this->fecvighas);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setNumdeppol($this->numdeppol);

		$copyObj->setFecvigdes($this->fecvigdes);

		$copyObj->setFecvighas($this->fecvighas);
