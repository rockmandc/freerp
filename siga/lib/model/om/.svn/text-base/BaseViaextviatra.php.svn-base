...
abstract class BaseViaextviatra extends BaseObject  implements Persistent {
  ...
  public function getFecextdes($format = 'Y-m-d')
  {

    if ($this->fecextdes === null || $this->fecextdes === '') {
      return null;
    } elseif (!is_int($this->fecextdes)) {
            $ts = adodb_strtotime($this->fecextdes);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [fecextdes] as date/time value: " . var_export($this->fecextdes, true));
      }
    } else {
      $ts = $this->fecextdes;
    }
  ...
  public function getFecexthas($format = 'Y-m-d')
  {

    if ($this->fecexthas === null || $this->fecexthas === '') {
      return null;
    } elseif (!is_int($this->fecexthas)) {
            $ts = adodb_strtotime($this->fecexthas);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [fecexthas] as date/time value: " . var_export($this->fecexthas, true));
      }
    } else {
      $ts = $this->fecexthas;
    }
  ...
  public function getId()
  {
	...
	public function setFecextdes($v)
	{

    if ($v !== null && !is_int($v)) {
      $ts = adodb_strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [fecextdes] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    if ($this->fecextdes !== $ts) {
      $this->fecextdes = $ts;
      $this->modifiedColumns[] = ViaextviatraPeer::FECEXTDES;
    }

	} 
	
	public function setFecexthas($v)
	{

    if ($v !== null && !is_int($v)) {
      $ts = adodb_strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [fecexthas] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    if ($this->fecexthas !== $ts) {
      $this->fecexthas = $ts;
      $this->modifiedColumns[] = ViaextviatraPeer::FECEXTHAS;
    }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->fecextdes = $rs->getDate($startcol + 9, null);

      $this->fecexthas = $rs->getDate($startcol + 10, null);
  ...
  public function get($m, $a)
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 8:
				...
				break;
			case 9:
				return $this->getFecextdes();
				break;
			case 10:
				return $this->getFecexthas();
				break;
			case 11:
				return $this->getId();
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[9] => $this->getFecextdes(),
			$keys[10] => $this->getFecexthas(),
			$keys[11] => $this->getId(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 8:
				...
				break;
			case 9:
				$this->setFecextdes($value);
				break;
			case 10:
				$this->setFecexthas($value);
				break;
			case 11:
				$this->setId($value);
				break;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[9], $arr)) $this->setFecextdes($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setFecexthas($arr[$keys[10]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ViaextviatraPeer::FECEXTDES)) $criteria->add(ViaextviatraPeer::FECEXTDES, $this->fecextdes);
		if ($this->isColumnModified(ViaextviatraPeer::FECEXTHAS)) $criteria->add(ViaextviatraPeer::FECEXTHAS, $this->fecexthas);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setFecextdes($this->fecextdes);

		$copyObj->setFecexthas($this->fecexthas);
