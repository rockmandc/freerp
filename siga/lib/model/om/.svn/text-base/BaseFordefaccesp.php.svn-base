...
abstract class BaseFordefaccesp extends BaseObject  implements Persistent {
  ...
  public function getFacpes($val=false)
  {

    if($val) return number_format($this->facpes,2,',','.');
    else return $this->facpes;
  ...
  public function getId()
  {
    ...
    if ($this->codcat !== $v) {
        $this->codcat = $v;
        $this->modifiedColumns[] = FordefaccespPeer::CODCAT;
      }
  
	} 
	
	public function setFacpes($v)
	{

    if ($this->facpes !== $v) {
        $this->facpes = Herramientas::toFloat($v);
        $this->modifiedColumns[] = FordefaccespPeer::FACPES;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->codcat = $rs->getString($startcol + 14);

      $this->facpes = $rs->getFloat($startcol + 15);
  ...
  public function __call($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[14] => $this->getCodcat(),
			$keys[15] => $this->getFacpes(),
			$keys[16] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[14], $arr)) $this->setCodcat($arr[$keys[14]]);
		if (array_key_exists($keys[15], $arr)) $this->setFacpes($arr[$keys[15]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FordefaccespPeer::CODCAT)) $criteria->add(FordefaccespPeer::CODCAT, $this->codcat);
		if ($this->isColumnModified(FordefaccespPeer::FACPES)) $criteria->add(FordefaccespPeer::FACPES, $this->facpes);
