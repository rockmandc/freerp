...
abstract class BaseCobdettra extends BaseObject  implements Persistent {
  ...
  public function getMonamo($val=false)
  {

    if($val) return number_format($this->monamo,2,',','.');
    else return $this->monamo;
  ...
  public function getId()
  {
    ...
    if ($this->nroant !== $v) {
        $this->nroant = $v;
        $this->modifiedColumns[] = CobdettraPeer::NROANT;
      }
  
	} 
	
	public function setMonamo($v)
	{

    if ($this->monamo !== $v) {
        $this->monamo = Herramientas::toFloat($v);
        $this->modifiedColumns[] = CobdettraPeer::MONAMO;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->nroant = $rs->getString($startcol + 8);

      $this->monamo = $rs->getFloat($startcol + 9);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[8] => $this->getNroant(),
			$keys[9] => $this->getMonamo(),
			$keys[10] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[8], $arr)) $this->setNroant($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setMonamo($arr[$keys[9]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CobdettraPeer::NROANT)) $criteria->add(CobdettraPeer::NROANT, $this->nroant);
		if ($this->isColumnModified(CobdettraPeer::MONAMO)) $criteria->add(CobdettraPeer::MONAMO, $this->monamo);
