...
abstract class BaseOcunidad extends BaseObject  implements Persistent {
	...
	protected $collOcdefpars;

	
	protected $lastOcdefparCriteria = null;
  ...
  public function __call($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->collOcdefpars !== null) {
				foreach($this->collOcdefpars as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach($this->getOcdefpars() as $relObj) {
				$copyObj->addOcdefpar($relObj->copy($deepCopy));
			}
	...
	public function initOcdefpars()
	{
		if ($this->collOcdefpars === null) {
			$this->collOcdefpars = array();
		}
	}

	
	public function getOcdefpars($criteria = null, $con = null)
	{
				include_once 'lib/model/om/BaseOcdefparPeer.php';
		if ($criteria === null) {
		...
		if ($this->collOcdefpars === null) {
			if ($this->isNew()) {
			   $this->collOcdefpars = array();
				...
				$criteria->add(OcdefparPeer::CODUNI, $this->getCoduni());

				OcdefparPeer::addSelectColumns($criteria);
				$this->collOcdefpars = OcdefparPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				OcdefparPeer::addSelectColumns($criteria);
				if (!isset($this->lastOcdefparCriteria) || !$this->lastOcdefparCriteria->equals($criteria)) {
					$this->collOcdefpars = OcdefparPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastOcdefparCriteria = $criteria;
		return $this->collOcdefpars;
	}

	
	public function countOcdefpars($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/om/BaseOcdefparPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		return OcdefparPeer::doCount($criteria, $distinct, $con);
	}

	
	public function addOcdefpar(Ocdefpar $l)
	{
		$this->collOcdefpars[] = $l;
		$l->setOcunidad($this);
	}


	
	public function getOcdefparsJoinOctippar($criteria = null, $con = null)
	{
				include_once 'lib/model/om/BaseOcdefparPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collOcdefpars === null) {
			if ($this->isNew()) {
				...
				$this->collOcdefpars = OcdefparPeer::doSelectJoinOctippar($criteria, $con);
			}
		} else {
