...
abstract class BaseEmpresa extends BaseObject  implements Persistent {
  ...
  public function getMesislr()
  {

    return trim($this->mesislr);
  ...
  public function getId()
  {
    ...
    if ($this->tipdoc !== $v) {
        $this->tipdoc = $v;
        $this->modifiedColumns[] = EmpresaPeer::TIPDOC;
      }
    ...
    if ($this->rifemp !== $v) {
        $this->rifemp = $v;
        $this->modifiedColumns[] = EmpresaPeer::RIFEMP;
      }
    ...
    if ($this->numtar !== $v) {
        $this->numtar = $v;
        $this->modifiedColumns[] = EmpresaPeer::NUMTAR;
      }
    ...
    if ($this->mes !== $v) {
        $this->mes = $v;
        $this->modifiedColumns[] = EmpresaPeer::MES;
      }
  
	} 
	
	public function setMesislr($v)
	{

    if ($this->mesislr !== $v) {
        $this->mesislr = $v;
        $this->modifiedColumns[] = EmpresaPeer::MESISLR;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->tipdoc = $rs->getString($startcol + 46);

      $this->rifemp = $rs->getString($startcol + 47);

      $this->numtar = $rs->getString($startcol + 48);

      $this->mes = $rs->getString($startcol + 49);

      $this->mesislr = $rs->getString($startcol + 50);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[46] => $this->getTipdoc(),
			$keys[47] => $this->getRifemp(),
			$keys[48] => $this->getNumtar(),
			$keys[49] => $this->getMes(),
			$keys[50] => $this->getMesislr(),
			$keys[51] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[46], $arr)) $this->setTipdoc($arr[$keys[46]]);
		if (array_key_exists($keys[47], $arr)) $this->setRifemp($arr[$keys[47]]);
		if (array_key_exists($keys[48], $arr)) $this->setNumtar($arr[$keys[48]]);
		if (array_key_exists($keys[49], $arr)) $this->setMes($arr[$keys[49]]);
		if (array_key_exists($keys[50], $arr)) $this->setMesislr($arr[$keys[50]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EmpresaPeer::TIPDOC)) $criteria->add(EmpresaPeer::TIPDOC, $this->tipdoc);
		if ($this->isColumnModified(EmpresaPeer::RIFEMP)) $criteria->add(EmpresaPeer::RIFEMP, $this->rifemp);
		if ($this->isColumnModified(EmpresaPeer::NUMTAR)) $criteria->add(EmpresaPeer::NUMTAR, $this->numtar);
		if ($this->isColumnModified(EmpresaPeer::MES)) $criteria->add(EmpresaPeer::MES, $this->mes);
		if ($this->isColumnModified(EmpresaPeer::MESISLR)) $criteria->add(EmpresaPeer::MESISLR, $this->mesislr);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setMesislr($this->mesislr);
