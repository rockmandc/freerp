...
abstract class BaseBnrevact extends BaseObject  implements Persistent {
  ...
  public function getMonmuerev($val=false)
  {

    if($val) return number_format($this->monmuerev,2,',','.');
    else return $this->monmuerev;

  }
  
  public function getMoninmrev($val=false)
  {

    if($val) return number_format($this->moninmrev,2,',','.');
    else return $this->moninmrev;

  }
  
  public function getMonsemrev($val=false)
  {

    if($val) return number_format($this->monsemrev,2,',','.');
    else return $this->monsemrev;

  }
  
  public function getMonimnrev($val=false)
  {

    if($val) return number_format($this->monimnrev,2,',','.');
    else return $this->monimnrev;
  ...
  public function getId()
  {
    ...
    if ($this->monmuerev !== $v) {
        $this->monmuerev = Herramientas::toFloat($v);
    ...
    if ($this->moninmrev !== $v) {
        $this->moninmrev = Herramientas::toFloat($v);
    ...
    if ($this->monsemrev !== $v) {
        $this->monsemrev = Herramientas::toFloat($v);
    ...
    if ($this->monimnrev !== $v) {
        $this->monimnrev = Herramientas::toFloat($v);
