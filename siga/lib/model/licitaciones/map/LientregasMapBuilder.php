...
class LientregasMapBuilder {

	
	const CLASS_NAME = 'lib.model.licitaciones.map.LientregasMapBuilder';
	...
	public function doBuild()
	{
		...
		$tMap = $this->dbMap->addTable('lientregas');
		$tMap->setPhpName('Lientregas');

		$tMap->setUseIdGenerator(true);

		$tMap->setPrimaryKeyMethodInfo('lientregas_SEQ');

		$tMap->addColumn('NUMENT', 'Nument', 'string', CreoleTypes::VARCHAR, false, 8);

		$tMap->addForeignKey('NUMCONT', 'Numcont', 'string', CreoleTypes::VARCHAR, 'licontrat', 'NUMCONT', false, 20);

		$tMap->addColumn('DESENT', 'Desent', 'string', CreoleTypes::VARCHAR, false, 1000);
