...
class LipenalizacionesMapBuilder {

	
	const CLASS_NAME = 'lib.model.licitaciones.map.LipenalizacionesMapBuilder';
	...
	public function doBuild()
	{
		...
		$tMap = $this->dbMap->addTable('lipenalizaciones');
		$tMap->setPhpName('Lipenalizaciones');

		$tMap->setUseIdGenerator(true);

		$tMap->setPrimaryKeyMethodInfo('lipenalizaciones_SEQ');

		$tMap->addColumn('NUMPEN', 'Numpen', 'string', CreoleTypes::VARCHAR, false, 8);

		$tMap->addForeignKey('NUMCONT', 'Numcont', 'string', CreoleTypes::VARCHAR, 'licontrat', 'NUMCONT', false, 20);

		$tMap->addColumn('DETPEN', 'Detpen', 'string', CreoleTypes::VARCHAR, false, 1000);

		$tMap->addForeignKey('CODTIPPEN', 'Codtippen', 'int', CreoleTypes::INTEGER, 'litippen', 'CODTIPPEN', false, null);
