...
class LiaddendumMapBuilder {

	
	const CLASS_NAME = 'lib.model.licitaciones.map.LiaddendumMapBuilder';
	...
	public function doBuild()
	{
		...
		$tMap = $this->dbMap->addTable('liaddendum');
		$tMap->setPhpName('Liaddendum');

		$tMap->setUseIdGenerator(true);

		$tMap->setPrimaryKeyMethodInfo('liaddendum_SEQ');

		$tMap->addColumn('NUMADD', 'Numadd', 'string', CreoleTypes::VARCHAR, false, 8);

		$tMap->addForeignKey('TIPADD', 'Tipadd', 'string', CreoleTypes::VARCHAR, 'litipadd', 'CODTIPADD', false, 8);
