...
class LiactdescontMapBuilder {

	
	const CLASS_NAME = 'lib.model.licitaciones.map.LiactdescontMapBuilder';
	...
	public function doBuild()
	{
		...
		$tMap = $this->dbMap->addTable('liactdescont');
		$tMap->setPhpName('Liactdescont');

		$tMap->setUseIdGenerator(true);

		$tMap->setPrimaryKeyMethodInfo('liactdescont_SEQ');

		$tMap->addColumn('NUMACTDES', 'Numactdes', 'string', CreoleTypes::VARCHAR, false, 8);

		$tMap->addForeignKey('NUMCONT', 'Numcont', 'string', CreoleTypes::VARCHAR, 'licontrat', 'NUMCONT', false, 20);

		$tMap->addColumn('DETACTDES', 'Detactdes', 'string', CreoleTypes::VARCHAR, false, 1000);
