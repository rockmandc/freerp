...
abstract class BaseLisolegrdet extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->codfin !== $v) {
        $this->codfin = $v;
        $this->modifiedColumns[] = LisolegrdetPeer::CODFIN;
      }
    ...
    if ($this->tipconpub !== $v) {
        $this->tipconpub = $v;
        $this->modifiedColumns[] = LisolegrdetPeer::TIPCONPUB;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->codfin = $rs->getString($startcol + 13);

      $this->tipconpub = $rs->getString($startcol + 14);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[13] => $this->getCodfin(),
			$keys[14] => $this->getTipconpub(),
			$keys[15] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[13], $arr)) $this->setCodfin($arr[$keys[13]]);
		if (array_key_exists($keys[14], $arr)) $this->setTipconpub($arr[$keys[14]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(LisolegrdetPeer::CODFIN)) $criteria->add(LisolegrdetPeer::CODFIN, $this->codfin);
		if ($this->isColumnModified(LisolegrdetPeer::TIPCONPUB)) $criteria->add(LisolegrdetPeer::TIPCONPUB, $this->tipconpub);
