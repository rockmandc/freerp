...
abstract class BaseLiprebas extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->fecdecla !== $ts) {
      $this->fecdecla = $ts;
      $this->modifiedColumns[] = LiprebasPeer::FECDECLA;
    }
    ...
    if ($this->monrgo !== $v) {
        ...
        $this->modifiedColumns[] = LiprebasPeer::MONRGO;
      }
    ...
    if ($this->monsub !== $v) {
        ...
        $this->modifiedColumns[] = LiprebasPeer::MONSUB;
      }
    ...
    if ($this->tipconpub !== $v) {
        $this->tipconpub = $v;
        $this->modifiedColumns[] = LiprebasPeer::TIPCONPUB;
      }
    ...
    if ($this->codfin !== $v) {
        $this->codfin = $v;
        $this->modifiedColumns[] = LiprebasPeer::CODFIN;
      }
    ...
    if ($this->codsec !== $v) {
        $this->codsec = $v;
        $this->modifiedColumns[] = LiprebasPeer::CODSEC;
      }
    ...
    if ($this->dirobr !== $v) {
        $this->dirobr = $v;
        $this->modifiedColumns[] = LiprebasPeer::DIROBR;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->fecdecla = $rs->getDate($startcol + 31, null);

      $this->monrgo = $rs->getFloat($startcol + 32);

      $this->monsub = $rs->getFloat($startcol + 33);

      $this->tipconpub = $rs->getString($startcol + 34);

      $this->codfin = $rs->getString($startcol + 35);

      $this->codsec = $rs->getString($startcol + 36);

      $this->dirobr = $rs->getString($startcol + 37);

      $this->codpro = $rs->getString($startcol + 38);

      $this->reqart = $rs->getString($startcol + 39);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[31] => $this->getFecdecla(),
			$keys[32] => $this->getMonrgo(),
			$keys[33] => $this->getMonsub(),
			$keys[34] => $this->getTipconpub(),
			$keys[35] => $this->getCodfin(),
			$keys[36] => $this->getCodsec(),
			$keys[37] => $this->getDirobr(),
			$keys[38] => $this->getCodpro(),
			$keys[39] => $this->getReqart(),
			$keys[40] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[31], $arr)) $this->setFecdecla($arr[$keys[31]]);
		if (array_key_exists($keys[32], $arr)) $this->setMonrgo($arr[$keys[32]]);
		if (array_key_exists($keys[33], $arr)) $this->setMonsub($arr[$keys[33]]);
		if (array_key_exists($keys[34], $arr)) $this->setTipconpub($arr[$keys[34]]);
		if (array_key_exists($keys[35], $arr)) $this->setCodfin($arr[$keys[35]]);
		if (array_key_exists($keys[36], $arr)) $this->setCodsec($arr[$keys[36]]);
		if (array_key_exists($keys[37], $arr)) $this->setDirobr($arr[$keys[37]]);
		if (array_key_exists($keys[38], $arr)) $this->setCodpro($arr[$keys[38]]);
		if (array_key_exists($keys[39], $arr)) $this->setReqart($arr[$keys[39]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(LiprebasPeer::FECDECLA)) $criteria->add(LiprebasPeer::FECDECLA, $this->fecdecla);
		if ($this->isColumnModified(LiprebasPeer::MONRGO)) $criteria->add(LiprebasPeer::MONRGO, $this->monrgo);
		if ($this->isColumnModified(LiprebasPeer::MONSUB)) $criteria->add(LiprebasPeer::MONSUB, $this->monsub);
		if ($this->isColumnModified(LiprebasPeer::TIPCONPUB)) $criteria->add(LiprebasPeer::TIPCONPUB, $this->tipconpub);
		if ($this->isColumnModified(LiprebasPeer::CODFIN)) $criteria->add(LiprebasPeer::CODFIN, $this->codfin);
		if ($this->isColumnModified(LiprebasPeer::CODSEC)) $criteria->add(LiprebasPeer::CODSEC, $this->codsec);
		if ($this->isColumnModified(LiprebasPeer::DIROBR)) $criteria->add(LiprebasPeer::DIROBR, $this->dirobr);
