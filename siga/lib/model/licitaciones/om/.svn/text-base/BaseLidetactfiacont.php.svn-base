...
abstract class BaseLidetactfiacont extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->numcont !== $v) {
        $this->numcont = $v;
        $this->modifiedColumns[] = LidetactfiacontPeer::NUMCONT;
      }
    ...
    if ($this->numact !== $v) {
        $this->numact = $v;
        $this->modifiedColumns[] = LidetactfiacontPeer::NUMACT;
      }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = LidetactfiacontPeer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      throw new PropelException("Error populating Lidetactfiacont object", $e);
    }
  }
  ...
  public function get($m, $a)
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			LidetactfiacontPeer::doDelete($this, $con);
		...
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aLiactas !== null) {
				...
				$this->setLiactas($this->aLiactas);
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = LidetactfiacontPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += LidetactfiacontPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = LidetactfiacontPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = LidetactfiacontPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = LidetactfiacontPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(LidetactfiacontPeer::NUMCONT)) $criteria->add(LidetactfiacontPeer::NUMCONT, $this->numcont);
		if ($this->isColumnModified(LidetactfiacontPeer::NUMACT)) $criteria->add(LidetactfiacontPeer::NUMACT, $this->numact);
		if ($this->isColumnModified(LidetactfiacontPeer::ID)) $criteria->add(LidetactfiacontPeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(LidetactfiacontPeer::ID, $this->id);
	...
	{
		if (self::$peer === null) {
			self::$peer = new LidetactfiacontPeer();
		}
		return self::$peer;
	}
