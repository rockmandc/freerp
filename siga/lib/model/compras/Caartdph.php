...
/**
 * Subclass for representing a row from the 'caartdph'.
 *
 *
 *
 * @package    Roraima
...
class Caartdph extends BaseCaartdph
{
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
   {
      ...
      //$this->cannodes=$canfac - self::getCandph() ;
		if (self::getCandev()>0)
		  $this->cannodes=self::getCandph()-self::getCandev();
        $this->candev2=$this->candev;
		$this->montotdes=self::getMontot() - ($this->cannodes*self::getPreart()) ;
   ...
   /* public function getMontotdes()
    ...
    {
	  return Herramientas::getX_vacio('DPHART','Cadphart','Reqart',$this->getDphart());
    }        
