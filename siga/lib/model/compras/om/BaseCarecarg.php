...
abstract class BaseCarecarg extends BaseObject  implements Persistent {
	...
	protected $collCargosols;

	
	protected $lastCargosolCriteria = null;
  ...
  public function get($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->collCargosols !== null) {
				foreach($this->collCargosols as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach($this->getCargosols() as $relObj) {
				$copyObj->addCargosol($relObj->copy($deepCopy));
			}
	...
	public function initCargosols()
	{
		if ($this->collCargosols === null) {
			$this->collCargosols = array();
		}
	}

	
	public function getCargosols($criteria = null, $con = null)
	{
				include_once 'lib/model/compras/om/BaseCargosolPeer.php';
		if ($criteria === null) {
		...
		if ($this->collCargosols === null) {
			if ($this->isNew()) {
			   $this->collCargosols = array();
				...
				$criteria->add(CargosolPeer::CODRGO, $this->getCodrgo());

				CargosolPeer::addSelectColumns($criteria);
				$this->collCargosols = CargosolPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				CargosolPeer::addSelectColumns($criteria);
				if (!isset($this->lastCargosolCriteria) || !$this->lastCargosolCriteria->equals($criteria)) {
					$this->collCargosols = CargosolPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastCargosolCriteria = $criteria;
		return $this->collCargosols;
	}

	
	public function countCargosols($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/compras/om/BaseCargosolPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		return CargosolPeer::doCount($criteria, $distinct, $con);
	}

	
	public function addCargosol(Cargosol $l)
	{
		$this->collCargosols[] = $l;
		$l->setCarecarg($this);
	}
