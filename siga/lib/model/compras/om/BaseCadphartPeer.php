...
abstract class BaseCadphartPeer {
	...
	const CLASS_DEFAULT = 'lib.model.compras.Cadphart';
	...
	const CODALMUSU = 'cadphart.CODALMUSU';

	
	const CREATED_AT = 'cadphart.CREATED_AT';

	
	const UPDATED_AT = 'cadphart.UPDATED_AT';

	
	const CREATED_BY_USER = 'cadphart.CREATED_BY_USER';

	
	const UPDATED_BY_USER = 'cadphart.UPDATED_BY_USER';

	
	const FECDES = 'cadphart.FECDES';

	
	const FECHAS = 'cadphart.FECHAS';

	
	const CODEDO = 'cadphart.CODEDO';

	
	const CODCIU = 'cadphart.CODCIU';

	
	const CODMUN = 'cadphart.CODMUN';

	
	const CODPAR = 'cadphart.CODPAR';

	
	const CODINST = 'cadphart.CODINST';

	
	const CODPRG = 'cadphart.CODPRG';

	
	const STADEV = 'cadphart.STADEV';

	
	const CODPRO = 'cadphart.CODPRO';

	
	const CODDIREC = 'cadphart.CODDIREC';
	...
	{
		include_once 'lib/model/compras/map/CadphartMapBuilder.php';
		return BasePeer::getMapBuilder('lib.model.compras.map.CadphartMapBuilder');
	}
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(CadphartPeer::CODALMUSU);

		$criteria->addSelectColumn(CadphartPeer::CREATED_AT);

		$criteria->addSelectColumn(CadphartPeer::UPDATED_AT);

		$criteria->addSelectColumn(CadphartPeer::CREATED_BY_USER);

		$criteria->addSelectColumn(CadphartPeer::UPDATED_BY_USER);

		$criteria->addSelectColumn(CadphartPeer::FECDES);

		$criteria->addSelectColumn(CadphartPeer::FECHAS);

		$criteria->addSelectColumn(CadphartPeer::CODEDO);

		$criteria->addSelectColumn(CadphartPeer::CODCIU);

		$criteria->addSelectColumn(CadphartPeer::CODMUN);

		$criteria->addSelectColumn(CadphartPeer::CODPAR);

		$criteria->addSelectColumn(CadphartPeer::CODINST);

		$criteria->addSelectColumn(CadphartPeer::CODPRG);

		$criteria->addSelectColumn(CadphartPeer::STADEV);

		$criteria->addSelectColumn(CadphartPeer::CODPRO);

		$criteria->addSelectColumn(CadphartPeer::CODDIREC);
	...
	public static function doCountJoinCadefalm(Criteria $criteria, $distinct = false, $con = null)
	{
		...
		$criteria->addJoin(CadphartPeer::CODALMUSU, CadefalmPeer::CODALM);
	...
	public static function doSelectJoinCadefalm(Criteria $c, $con = null)
	{
		...
		CadphartPeer::addSelectColumns($c);
		$startcol = (CadphartPeer::NUM_COLUMNS - CadphartPeer::NUM_LAZY_LOAD_COLUMNS) + 1;
		CadefalmPeer::addSelectColumns($c);

		$c->addJoin(CadphartPeer::CODALMUSU, CadefalmPeer::CODALM);
		$rs = BasePeer::doSelect($c, $con);
		$results = array();

		while($rs->next()) {

			$omClass = CadphartPeer::getOMClass();
			...
			foreach($results as $temp_obj1) {
				$temp_obj2 = $temp_obj1->getCadefalm(); 				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
										$temp_obj2->addCadphart($obj1); 					break;
				}
			}
			if ($newObject) {
				$obj2->initCadpharts();
				$obj2->addCadphart($obj1); 			}
			$results[] = $obj1;
		}
		return $results;
	}
	...
	public static function doSelectJoinAll(Criteria $c, $con = null)
	{
		...
		CadphartPeer::addSelectColumns($c);
		$startcol2 = (CadphartPeer::NUM_COLUMNS - CadphartPeer::NUM_LAZY_LOAD_COLUMNS) + 1;
...
if (Propel::isInit()) {
		...
		Propel::log('Could not initialize Peer: ' . $e->getMessage(), Propel::LOG_ERR);
			...
			require_once 'lib/model/compras/map/CadphartMapBuilder.php';
	Propel::registerMapBuilder('lib.model.compras.map.CadphartMapBuilder');
}
