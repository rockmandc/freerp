...
abstract class BaseCainvfis extends BaseObject  implements Persistent {
  ...
  public function getFectras($format = 'Y-m-d')
  {

    if ($this->fectras === null || $this->fectras === '') {
      return null;
    } elseif (!is_int($this->fectras)) {
            $ts = adodb_strtotime($this->fectras);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [fectras] as date/time value: " . var_export($this->fectras, true));
      }
    } else {
      $ts = $this->fectras;
    }
  ...
  public function getId()
  {
	...
	public function setFectras($v)
	{

    if ($v !== null && !is_int($v)) {
      $ts = adodb_strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [fectras] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    if ($this->fectras !== $ts) {
      $this->fectras = $ts;
      $this->modifiedColumns[] = CainvfisPeer::FECTRAS;
    }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->fectras = $rs->getDate($startcol + 6, null);
  ...
  public function get($m, $a)
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[6], $arr)) $this->setFectras($arr[$keys[6]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CainvfisPeer::FECTRAS)) $criteria->add(CainvfisPeer::FECTRAS, $this->fectras);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setFectras($this->fectras);
