...
abstract class BaseCadetent extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->fecela !== $ts) {
      $this->fecela = $ts;
      $this->modifiedColumns[] = CadetentPeer::FECELA;
    }
    ...
    if ($this->codalt !== $v) {
        $this->codalt = $v;
        $this->modifiedColumns[] = CadetentPeer::CODALT;
      }
    ...
    if ($this->unimed !== $v) {
        $this->unimed = $v;
        $this->modifiedColumns[] = CadetentPeer::UNIMED;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->fecela = $rs->getDate($startcol + 11, null);

      $this->codalt = $rs->getString($startcol + 12);

      $this->unimed = $rs->getString($startcol + 13);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[11] => $this->getFecela(),
			$keys[12] => $this->getCodalt(),
			$keys[13] => $this->getUnimed(),
			$keys[14] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[11], $arr)) $this->setFecela($arr[$keys[11]]);
		if (array_key_exists($keys[12], $arr)) $this->setCodalt($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setUnimed($arr[$keys[13]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CadetentPeer::FECELA)) $criteria->add(CadetentPeer::FECELA, $this->fecela);
		if ($this->isColumnModified(CadetentPeer::CODALT)) $criteria->add(CadetentPeer::CODALT, $this->codalt);
		if ($this->isColumnModified(CadetentPeer::UNIMED)) $criteria->add(CadetentPeer::UNIMED, $this->unimed);
