...
abstract class BaseCaregart extends BaseObject  implements Persistent {
  ...
  public function getNacimp()
  {

    return trim($this->nacimp);
  ...
  public function getStaart()
  {

    return trim($this->staart);
  ...
  public function getClaart()
  {

    return trim($this->claart);
  ...
  public function getCosact($val=false)
  {

    if($val) return number_format($this->cosact,2,',','.');
    else return $this->cosact;

  }
  
  public function getFeccac($format = 'Y-m-d')
  {

    if ($this->feccac === null || $this->feccac === '') {
      return null;
    } elseif (!is_int($this->feccac)) {
            $ts = adodb_strtotime($this->feccac);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [feccac] as date/time value: " . var_export($this->feccac, true));
      }
    } else {
      $ts = $this->feccac;
    }
  ...
  public function getClamat()
  {

    return trim($this->clamat);
  ...
  public function getId()
  {
    ...
    if ($this->tippro !== $v) {
        $this->tippro = $v;
        $this->modifiedColumns[] = CaregartPeer::TIPPRO;
      }
    ...
    if ($this->codbar !== $v) {
        $this->codbar = $v;
        $this->modifiedColumns[] = CaregartPeer::CODBAR;
      }
  
	} 
	
	public function setNacimp($v)
	{

    if ($this->nacimp !== $v) {
        $this->nacimp = $v;
        $this->modifiedColumns[] = CaregartPeer::NACIMP;
      }
    ...
    if ($this->codalt !== $v) {
        $this->codalt = $v;
        $this->modifiedColumns[] = CaregartPeer::CODALT;
      }
  
	} 
	
	public function setStaart($v)
	{

    if ($this->staart !== $v) {
        $this->staart = $v;
        $this->modifiedColumns[] = CaregartPeer::STAART;
      }
    ...
    if ($this->codunimed !== $v) {
        $this->codunimed = $v;
        $this->modifiedColumns[] = CaregartPeer::CODUNIMED;
      }
  
	} 
	
	public function setClaart($v)
	{

    if ($this->claart !== $v) {
        $this->claart = $v;
        $this->modifiedColumns[] = CaregartPeer::CLAART;
      }
    ...
    if ($this->nomcom !== $v) {
        $this->nomcom = $v;
        $this->modifiedColumns[] = CaregartPeer::NOMCOM;
      }
  
	} 
	
	public function setCosact($v)
	{

    if ($this->cosact !== $v) {
        $this->cosact = Herramientas::toFloat($v);
        $this->modifiedColumns[] = CaregartPeer::COSACT;
      }
  
	} 
	
	public function setFeccac($v)
	{

    if ($v !== null && !is_int($v)) {
      $ts = adodb_strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [feccac] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    if ($this->feccac !== $ts) {
      $this->feccac = $ts;
      $this->modifiedColumns[] = CaregartPeer::FECCAC;
    }

	} 
	
	public function setConsbue($v)
	{

    if ($this->consbue !== $v) {
        $this->consbue = $v;
        $this->modifiedColumns[] = CaregartPeer::CONSBUE;
      }
    ...
    if ($this->tipmat !== $v) {
        $this->tipmat = $v;
        $this->modifiedColumns[] = CaregartPeer::TIPMAT;
      }
  
	} 
	
	public function setClamat($v)
	{

    if ($this->clamat !== $v) {
        $this->clamat = $v;
        $this->modifiedColumns[] = CaregartPeer::CLAMAT;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->tippro = $rs->getString($startcol + 34);

      $this->codbar = $rs->getString($startcol + 35);

      $this->nacimp = $rs->getString($startcol + 36);

      $this->codalt = $rs->getString($startcol + 37);

      $this->staart = $rs->getString($startcol + 38);

      $this->codunimed = $rs->getString($startcol + 39);

      $this->claart = $rs->getString($startcol + 40);

      $this->nomcom = $rs->getString($startcol + 41);

      $this->cosact = $rs->getFloat($startcol + 42);

      $this->feccac = $rs->getDate($startcol + 43, null);

      $this->consbue = $rs->getBoolean($startcol + 44);

      $this->tipmat = $rs->getString($startcol + 45);

      $this->clamat = $rs->getString($startcol + 46);
  ...
  public function get($m, $a)
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 35:
				...
				break;
			case 36:
				return $this->getNacimp();
				break;
			case 37:
				return $this->getCodalt();
				break;
			case 38:
				return $this->getStaart();
				break;
			case 39:
				return $this->getCodunimed();
				break;
			case 40:
				return $this->getClaart();
				break;
			case 41:
				return $this->getNomcom();
				break;
			case 42:
				return $this->getCosact();
				break;
			case 43:
				return $this->getFeccac();
				break;
			case 44:
				return $this->getConsbue();
				break;
			case 45:
				return $this->getTipmat();
				break;
			case 46:
				return $this->getClamat();
				break;
			case 47:
				return $this->getId();
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[34] => $this->getTippro(),
			$keys[35] => $this->getCodbar(),
			$keys[36] => $this->getNacimp(),
			$keys[37] => $this->getCodalt(),
			$keys[38] => $this->getStaart(),
			$keys[39] => $this->getCodunimed(),
			$keys[40] => $this->getClaart(),
			$keys[41] => $this->getNomcom(),
			$keys[42] => $this->getCosact(),
			$keys[43] => $this->getFeccac(),
			$keys[44] => $this->getConsbue(),
			$keys[45] => $this->getTipmat(),
			$keys[46] => $this->getClamat(),
			$keys[47] => $this->getId(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 35:
				...
				break;
			case 36:
				$this->setNacimp($value);
				break;
			case 37:
				$this->setCodalt($value);
				break;
			case 38:
				$this->setStaart($value);
				break;
			case 39:
				$this->setCodunimed($value);
				break;
			case 40:
				$this->setClaart($value);
				break;
			case 41:
				$this->setNomcom($value);
				break;
			case 42:
				$this->setCosact($value);
				break;
			case 43:
				$this->setFeccac($value);
				break;
			case 44:
				$this->setConsbue($value);
				break;
			case 45:
				$this->setTipmat($value);
				break;
			case 46:
				$this->setClamat($value);
				break;
			case 47:
				$this->setId($value);
				break;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[34], $arr)) $this->setTippro($arr[$keys[34]]);
		if (array_key_exists($keys[35], $arr)) $this->setCodbar($arr[$keys[35]]);
		if (array_key_exists($keys[36], $arr)) $this->setNacimp($arr[$keys[36]]);
		if (array_key_exists($keys[37], $arr)) $this->setCodalt($arr[$keys[37]]);
		if (array_key_exists($keys[38], $arr)) $this->setStaart($arr[$keys[38]]);
		if (array_key_exists($keys[39], $arr)) $this->setCodunimed($arr[$keys[39]]);
		if (array_key_exists($keys[40], $arr)) $this->setClaart($arr[$keys[40]]);
		if (array_key_exists($keys[41], $arr)) $this->setNomcom($arr[$keys[41]]);
		if (array_key_exists($keys[42], $arr)) $this->setCosact($arr[$keys[42]]);
		if (array_key_exists($keys[43], $arr)) $this->setFeccac($arr[$keys[43]]);
		if (array_key_exists($keys[44], $arr)) $this->setConsbue($arr[$keys[44]]);
		if (array_key_exists($keys[45], $arr)) $this->setTipmat($arr[$keys[45]]);
		if (array_key_exists($keys[46], $arr)) $this->setClamat($arr[$keys[46]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CaregartPeer::TIPPRO)) $criteria->add(CaregartPeer::TIPPRO, $this->tippro);
		if ($this->isColumnModified(CaregartPeer::CODBAR)) $criteria->add(CaregartPeer::CODBAR, $this->codbar);
		if ($this->isColumnModified(CaregartPeer::NACIMP)) $criteria->add(CaregartPeer::NACIMP, $this->nacimp);
		if ($this->isColumnModified(CaregartPeer::CODALT)) $criteria->add(CaregartPeer::CODALT, $this->codalt);
		if ($this->isColumnModified(CaregartPeer::STAART)) $criteria->add(CaregartPeer::STAART, $this->staart);
		if ($this->isColumnModified(CaregartPeer::CODUNIMED)) $criteria->add(CaregartPeer::CODUNIMED, $this->codunimed);
		if ($this->isColumnModified(CaregartPeer::CLAART)) $criteria->add(CaregartPeer::CLAART, $this->claart);
		if ($this->isColumnModified(CaregartPeer::NOMCOM)) $criteria->add(CaregartPeer::NOMCOM, $this->nomcom);
		if ($this->isColumnModified(CaregartPeer::COSACT)) $criteria->add(CaregartPeer::COSACT, $this->cosact);
		if ($this->isColumnModified(CaregartPeer::FECCAC)) $criteria->add(CaregartPeer::FECCAC, $this->feccac);
		if ($this->isColumnModified(CaregartPeer::CONSBUE)) $criteria->add(CaregartPeer::CONSBUE, $this->consbue);
		if ($this->isColumnModified(CaregartPeer::TIPMAT)) $criteria->add(CaregartPeer::TIPMAT, $this->tipmat);
		if ($this->isColumnModified(CaregartPeer::CLAMAT)) $criteria->add(CaregartPeer::CLAMAT, $this->clamat);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setNacimp($this->nacimp);

		$copyObj->setCodalt($this->codalt);

		$copyObj->setStaart($this->staart);

		$copyObj->setCodunimed($this->codunimed);

		$copyObj->setClaart($this->claart);

		$copyObj->setNomcom($this->nomcom);

		$copyObj->setCosact($this->cosact);

		$copyObj->setFeccac($this->feccac);

		$copyObj->setConsbue($this->consbue);

		$copyObj->setTipmat($this->tipmat);

		$copyObj->setClamat($this->clamat);
	...
	{
				include_once 'lib/model/compras/om/BaseCarelartsigaPeer.php';
		if ($criteria === null) {
