...
abstract class BaseCamotfal extends BaseObject  implements Persistent {
	...
	protected $collCadettras;

	
	protected $lastCadettraCriteria = null;
  ...
  public function get($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->collCadettras !== null) {
				foreach($this->collCadettras as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach($this->getCadettras() as $relObj) {
				$copyObj->addCadettra($relObj->copy($deepCopy));
			}
	...
	public function initCadettras()
	{
		if ($this->collCadettras === null) {
			$this->collCadettras = array();
		}
	}

	
	public function getCadettras($criteria = null, $con = null)
	{
				include_once 'lib/model/compras/om/BaseCadettraPeer.php';
		if ($criteria === null) {
		...
		if ($this->collCadettras === null) {
			if ($this->isNew()) {
			   $this->collCadettras = array();
				...
				$criteria->add(CadettraPeer::CODFAL, $this->getCodfal());

				CadettraPeer::addSelectColumns($criteria);
				$this->collCadettras = CadettraPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				CadettraPeer::addSelectColumns($criteria);
				if (!isset($this->lastCadettraCriteria) || !$this->lastCadettraCriteria->equals($criteria)) {
					$this->collCadettras = CadettraPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastCadettraCriteria = $criteria;
		return $this->collCadettras;
	}

	
	public function countCadettras($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/compras/om/BaseCadettraPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		return CadettraPeer::doCount($criteria, $distinct, $con);
	}

	
	public function addCadettra(Cadettra $l)
	{
		$this->collCadettras[] = $l;
		$l->setCamotfal($this);
	}
	...
	{
				include_once 'lib/model/compras/om/BaseCaartrcpPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		}
		elseif ($criteria instanceof Criteria)
