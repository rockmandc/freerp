...
class Cainvfis extends BaseCainvfis
{
  ...
  {
     $c = new Criteria();
     $c->add(CainvfisubiPeer::CODART,self::getCodart());
     $c->add(CainvfisubiPeer::CODALM,self::getCodalm());
     $c->add(CainvfisubiPeer::FECINV,self::getFecinv());
     $result=CainvfisubiPeer::doSelect($c);
     if ($result)
     {
          foreach ($result as $datos)
          {
            $exi=number_format($datos->getExiact(),2,',','.');
            $exi2=number_format($datos->getExiact2(),2,',','.');
            $tot='0,00';
            $this->ubicacion=$this->ubicacion . $datos->getCodubi().'_' . $datos->getNomubi().'_' . $exi .'_'. $exi2 .'_'. $tot. '!';
          }
     }
  }
}
