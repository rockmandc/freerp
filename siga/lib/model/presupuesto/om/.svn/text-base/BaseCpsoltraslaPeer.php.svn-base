...
abstract class BaseCpsoltraslaPeer {
	...
	const CLASS_DEFAULT = 'lib.model.presupuesto.Cpsoltrasla';
	...
	const CODDIREC = 'cpsoltrasla.CODDIREC';

	
	const BASCAL = 'cpsoltrasla.BASCAL';

	
	const INIANA = 'cpsoltrasla.INIANA';
	...
	{
		include_once 'lib/model/presupuesto/map/CpsoltraslaMapBuilder.php';
		return BasePeer::getMapBuilder('lib.model.presupuesto.map.CpsoltraslaMapBuilder');
	}
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(CpsoltraslaPeer::CODDIREC);

		$criteria->addSelectColumn(CpsoltraslaPeer::BASCAL);

		$criteria->addSelectColumn(CpsoltraslaPeer::INIANA);
	...
	public static function doCountJoinCpartley(Criteria $criteria, $distinct = false, $con = null)
	{
		...
		$criteria->addJoin(CpsoltraslaPeer::CODART, CpartleyPeer::CODART);
	...
	public static function doSelectJoinCpartley(Criteria $c, $con = null)
	{
		...
		$startcol = (CpsoltraslaPeer::NUM_COLUMNS - CpsoltraslaPeer::NUM_LAZY_LOAD_COLUMNS) + 1;
		CpartleyPeer::addSelectColumns($c);

		$c->addJoin(CpsoltraslaPeer::CODART, CpartleyPeer::CODART);
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				$temp_obj2 = $temp_obj1->getCpartley(); 				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
										$temp_obj2->addCpsoltrasla($obj1); 					break;
				}
			}
			if ($newObject) {
				$obj2->initCpsoltraslas();
				$obj2->addCpsoltrasla($obj1); 			}
			$results[] = $obj1;
		}
		return $results;
	}
	...
	public static function doSelectJoinAll(Criteria $c, $con = null)
	{
		...
		CpsoltraslaPeer::addSelectColumns($c);
		$startcol2 = (CpsoltraslaPeer::NUM_COLUMNS - CpsoltraslaPeer::NUM_LAZY_LOAD_COLUMNS) + 1;
...
if (Propel::isInit()) {
		...
		Propel::log('Could not initialize Peer: ' . $e->getMessage(), Propel::LOG_ERR);
			...
			require_once 'lib/model/presupuesto/map/CpsoltraslaMapBuilder.php';
	Propel::registerMapBuilder('lib.model.presupuesto.map.CpsoltraslaMapBuilder');
}
