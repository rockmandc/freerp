...
abstract class BaseCpdoccau extends BaseObject  implements Persistent {
  ...
  public function getGenlibcom()
  {

    return $this->genlibcom;
  ...
  public function getId()
  {
    ...
    if ($this->codcta !== $v) {
        $this->codcta = $v;
        $this->modifiedColumns[] = CpdoccauPeer::CODCTA;
      }
    ...
    if ($this->codtiptra !== $v) {
        $this->codtiptra = $v;
        $this->modifiedColumns[] = CpdoccauPeer::CODTIPTRA;
      }
  
	} 
	
	public function setGenlibcom($v)
	{

    if ($this->genlibcom !== $v) {
        $this->genlibcom = $v;
        $this->modifiedColumns[] = CpdoccauPeer::GENLIBCOM;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->codcta = $rs->getString($startcol + 8);

      $this->codtiptra = $rs->getString($startcol + 9);

      $this->genlibcom = $rs->getBoolean($startcol + 10);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[8] => $this->getCodcta(),
			$keys[9] => $this->getCodtiptra(),
			$keys[10] => $this->getGenlibcom(),
			$keys[11] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[8], $arr)) $this->setCodcta($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setCodtiptra($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setGenlibcom($arr[$keys[10]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CpdoccauPeer::CODCTA)) $criteria->add(CpdoccauPeer::CODCTA, $this->codcta);
		if ($this->isColumnModified(CpdoccauPeer::CODTIPTRA)) $criteria->add(CpdoccauPeer::CODTIPTRA, $this->codtiptra);
		if ($this->isColumnModified(CpdoccauPeer::GENLIBCOM)) $criteria->add(CpdoccauPeer::GENLIBCOM, $this->genlibcom);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setGenlibcom($this->genlibcom);
