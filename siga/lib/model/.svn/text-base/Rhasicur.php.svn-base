...
class Rhasicur extends BaseRhasicur
{
	...
	{
		$tipoa=H::getX_vacio('Codemp','Rhinscur','Tipper',$this->codemp);
		if ($tipoa=='I')
			return "Interno";
		elseif ($tipoa=='E')
			return "Externo";
	...
	{
		return H::getX_vacio('Codmot','Rhmotini','Desmot',$this->codmot);
	}

	public function getFecins()
	{
		$fecha = H::getX_vacio('Codemp','Rhinscur','Fecins',$this->codemp);
		if($fecha)
			return date("d/m/Y",strtotime($fecha));
		else	
			return '';	
