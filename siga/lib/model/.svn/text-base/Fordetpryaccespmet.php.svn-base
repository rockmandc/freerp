...
/**
 * Subclass for representing a row from the 'fordetpryaccespmet'.
 *
 *
 *
 * @package    Roraima
...
class Fordetpryaccespmet extends BaseFordetpryaccespmet
{
	...
	{
	  return Herramientas::getX('codcat','Fordefcatpre','Nomcat',self::getCodunieje());
	}
	...
	public function getCodparing()
	{
			...
			$c->add(FordisfuefinpryaccmetPeer::ACTFUE,'S');
			...
			if (count($datos) > 1)
			  ...
			  }elseif (count($datos) == 1)
			  {
				...
				$datos = FordisfuefinpryaccmetPeer::doDelete($c);
			  		return '';
			  }
	}
