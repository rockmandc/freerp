...
abstract class BaseContabb extends BaseObject  implements Persistent {
	...
	protected $collContabb1hiss;

	
	protected $lastContabb1hisCriteria = null;
  ...
  public function get($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->collContabb1hiss !== null) {
				foreach($this->collContabb1hiss as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach($this->getContabb1hiss() as $relObj) {
				$copyObj->addContabb1his($relObj->copy($deepCopy));
			}
	...
	{
				include_once 'lib/model/contabilidad/om/BaseContabb1Peer.php';
		if ($criteria === null) {
		...
		if ($this->collContabc1ms === null) {
			if ($this->isNew()) {
			   $this->collContabc1ms = array();
				...
				$criteria->add(Contabc1mPeer::CODCTA, $this->getCodcta());
	...
	public function getContabc1msJoinContabcm($criteria = null, $con = null)
	{
				include_once 'lib/model/contabilidad/om/BaseContabc1mPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collContabc1ms === null) {
			if ($this->isNew()) {
				...
				$this->collContabc1ms = Contabc1mPeer::doSelectJoinContabcm($criteria, $con);
			}
		} else {
	...
	public function initContabb1hiss()
	{
		if ($this->collContabb1hiss === null) {
			$this->collContabb1hiss = array();
		}
	}

	
	public function getContabb1hiss($criteria = null, $con = null)
	{
				include_once 'lib/model/contabilidad/om/BaseContabb1hisPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		}
		elseif ($criteria instanceof Criteria)
		...
		if ($this->collContabb1hiss === null) {
			if ($this->isNew()) {
			   $this->collContabb1hiss = array();
				...
				$criteria->add(Contabb1hisPeer::CODCTA, $this->getCodcta());

				Contabb1hisPeer::addSelectColumns($criteria);
				$this->collContabb1hiss = Contabb1hisPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				Contabb1hisPeer::addSelectColumns($criteria);
				if (!isset($this->lastContabb1hisCriteria) || !$this->lastContabb1hisCriteria->equals($criteria)) {
					$this->collContabb1hiss = Contabb1hisPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastContabb1hisCriteria = $criteria;
		return $this->collContabb1hiss;
	}

	
	public function countContabb1hiss($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/contabilidad/om/BaseContabb1hisPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		return Contabb1hisPeer::doCount($criteria, $distinct, $con);
	}

	
	public function addContabb1his(Contabb1his $l)
	{
		$this->collContabb1hiss[] = $l;
		$l->setContabb($this);
	}
