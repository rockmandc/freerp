...
class Fcsolvencia extends BaseFcsolvencia
{
  ...
  public function getcReferencia(){
      return self::getReferencia()."-".self::getCodcat();
  }
  public function getDestip(){
    $destip ='';
    $c = new Criteria();
    $c->add(FcsollicPeer::CODTIP, self::getCodtip());
    $fsollic = FctipsolPeer::doSelect($c);
     if(count($fsollic)>0){
            foreach($fsollic as $ref){
                $destip = $ref->getDestip();
            }
  }
  return($destip);
}
