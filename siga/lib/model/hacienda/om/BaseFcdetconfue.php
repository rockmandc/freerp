...
abstract class BaseFcdetconfue extends BaseObject  implements Persistent {
  ...
  public function getDescuo()
  {

    return trim($this->descuo);
  ...
  public function getId()
  {
	...
	public function setDescuo($v)
	{

    if ($this->descuo !== $v) {
        $this->descuo = $v;
        $this->modifiedColumns[] = FcdetconfuePeer::DESCUO;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->descuo = $rs->getString($startcol + 6);

      $this->fecven = $rs->getDate($startcol + 7, null);

      $this->fuente = $rs->getString($startcol + 8);
  ...
  public function __call($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[6] => $this->getDescuo(),
			$keys[7] => $this->getFecven(),
			$keys[8] => $this->getFuente(),
			$keys[9] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[6], $arr)) $this->setDescuo($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setFecven($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setFuente($arr[$keys[8]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FcdetconfuePeer::DESCUO)) $criteria->add(FcdetconfuePeer::DESCUO, $this->descuo);
