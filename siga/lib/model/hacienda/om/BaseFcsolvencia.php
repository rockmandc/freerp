...
abstract class BaseFcsolvencia extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->tipo !== $v) {
        $this->tipo = $v;
        $this->modifiedColumns[] = FcsolvenciaPeer::TIPO;
      }
    ...
    if ($this->referencia !== $v) {
        $this->referencia = $v;
        $this->modifiedColumns[] = FcsolvenciaPeer::REFERENCIA;
      }
    ...
    if ($this->funrec !== $v) {
        $this->funrec = $v;
        $this->modifiedColumns[] = FcsolvenciaPeer::FUNREC;
      }
    ...
    if ($this->fecrec !== $ts) {
      $this->fecrec = $ts;
      $this->modifiedColumns[] = FcsolvenciaPeer::FECREC;
    }

	} 
	
	public function setCatastro($v)
	{

    if ($this->catastro !== $v) {
        $this->catastro = $v;
        $this->modifiedColumns[] = FcsolvenciaPeer::CATASTRO;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->tipo = $rs->getString($startcol + 10);

      $this->stasol = $rs->getString($startcol + 11);

      $this->referencia = $rs->getString($startcol + 12);

      $this->motivo = $rs->getString($startcol + 13);

      $this->funrec = $rs->getString($startcol + 14);

      $this->fecrec = $rs->getDate($startcol + 15, null);

      $this->catastro = $rs->getString($startcol + 16);
  ...
  public function __call($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[11] => $this->getStasol(),
			$keys[12] => $this->getReferencia(),
			$keys[13] => $this->getMotivo(),
			$keys[14] => $this->getFunrec(),
			$keys[15] => $this->getFecrec(),
			$keys[16] => $this->getCatastro(),
			$keys[17] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[11], $arr)) $this->setStasol($arr[$keys[11]]);
		if (array_key_exists($keys[12], $arr)) $this->setReferencia($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setMotivo($arr[$keys[13]]);
		if (array_key_exists($keys[14], $arr)) $this->setFunrec($arr[$keys[14]]);
		if (array_key_exists($keys[15], $arr)) $this->setFecrec($arr[$keys[15]]);
		if (array_key_exists($keys[16], $arr)) $this->setCatastro($arr[$keys[16]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FcsolvenciaPeer::TIPO)) $criteria->add(FcsolvenciaPeer::TIPO, $this->tipo);
		if ($this->isColumnModified(FcsolvenciaPeer::STASOL)) $criteria->add(FcsolvenciaPeer::STASOL, $this->stasol);
		if ($this->isColumnModified(FcsolvenciaPeer::REFERENCIA)) $criteria->add(FcsolvenciaPeer::REFERENCIA, $this->referencia);
		if ($this->isColumnModified(FcsolvenciaPeer::MOTIVO)) $criteria->add(FcsolvenciaPeer::MOTIVO, $this->motivo);
		if ($this->isColumnModified(FcsolvenciaPeer::FUNREC)) $criteria->add(FcsolvenciaPeer::FUNREC, $this->funrec);
		if ($this->isColumnModified(FcsolvenciaPeer::FECREC)) $criteria->add(FcsolvenciaPeer::FECREC, $this->fecrec);
		if ($this->isColumnModified(FcsolvenciaPeer::CATASTRO)) $criteria->add(FcsolvenciaPeer::CATASTRO, $this->catastro);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setReferencia($this->referencia);
		...
		$copyObj->setCatastro($this->catastro);
