...
abstract class BaseFcsollic extends BaseObject  implements Persistent {
  ...
  public function getHorini($format = 'H:i:s')
  {
  ...
  public function getHorcie($format = 'H:i:s')
  {

    if ($this->horcie === null || $this->horcie === '') {
      return null;
    } elseif (!is_int($this->horcie)) {
            $ts = strtotime($this->horcie);
  ...
  public function getHorainie($format = 'H:i:s')
  {

    if ($this->horainie === null || $this->horainie === '') {
      return null;
    } elseif (!is_int($this->horainie)) {
            $ts = strtotime($this->horainie);
  ...
  public function getHoraciee($format = 'H:i:s')
  {

    if ($this->horaciee === null || $this->horaciee === '') {
      return null;
    } elseif (!is_int($this->horaciee)) {
            $ts = strtotime($this->horaciee);
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->horini = $rs->getTime($startcol + 13, null);

      $this->horcie = $rs->getTime($startcol + 14, null);
      ...
      $this->horainie = $rs->getTime($startcol + 50, null);

      $this->horaciee = $rs->getTime($startcol + 51, null);
