...
abstract class BaseFcpagos extends BaseObject  implements Persistent {
  ...
  public function getFunval()
  {

    return trim($this->funval);

  }

  public function getFechorval($format = 'Y-m-d H:i:s')
  {

    if ($this->fechorval === null || $this->fechorval === '') {
      return null;
    } elseif (!is_int($this->fechorval)) {
            $ts = strtotime($this->fechorval);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [fechorval] as date/time value: " . var_export($this->fechorval, true));
      }
    } else {
      $ts = $this->fechorval;
    }
  ...
  public function getFechorregpag($format = 'Y-m-d H:i:s')
  {

    if ($this->fechorregpag === null || $this->fechorregpag === '') {
      return null;
    } elseif (!is_int($this->fechorregpag)) {
            $ts = strtotime($this->fechorregpag);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse value of [fechorregpag] as date/time value: " . var_export($this->fechorregpag, true));
      }
    } else {
      $ts = $this->fechorregpag;
    }
    if ($format === null) {
      return $ts;
    } elseif (strpos($format, '%') !== false) {
  ...
  public function getId()
  {
	...
	public function setFunval($v)
	{

    if ($this->funval !== $v) {
        $this->funval = $v;
        $this->modifiedColumns[] = FcpagosPeer::FUNVAL;
      }

	}

	public function setFechorval($v)
	{
    ...
    if ($v !== null && !is_int($v)) {
      $ts = strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [fechorval] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    if ($this->fechorval !== $ts) {
      $this->fechorval = $ts;
      $this->modifiedColumns[] = FcpagosPeer::FECHORVAL;
    }

	}

	public function setFechorregpag($v)
	{
    ...
    if ($v !== null && !is_int($v)) {
      $ts = strtotime($v);
      if ($ts === -1 || $ts === false) {         throw new PropelException("Unable to parse date/time value for [fechorregpag] from input: " . var_export($v, true));
      }
    } else {
      $ts = $v;
    }
    if ($this->fechorregpag !== $ts) {
      $this->fechorregpag = $ts;
      $this->modifiedColumns[] = FcpagosPeer::FECHORREGPAG;
    }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->funval = $rs->getString($startcol + 15);

      $this->fechorval = $rs->getTimestamp($startcol + 16, null);

      $this->fechorregpag = $rs->getTimestamp($startcol + 17, null);
  ...
  public function __call($m, $a)
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 14:
				...
				break;
			case 15:
				return $this->getFunval();
				break;
			case 16:
				return $this->getFechorval();
				break;
			case 17:
				return $this->getFechorregpag();
				break;
			case 18:
				return $this->getId();
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[15] => $this->getFunval(),
			$keys[16] => $this->getFechorval(),
			$keys[17] => $this->getFechorregpag(),
			$keys[18] => $this->getId(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 14:
				...
				break;
			case 15:
				$this->setFunval($value);
				break;
			case 16:
				$this->setFechorval($value);
				break;
			case 17:
				$this->setFechorregpag($value);
				break;
			case 18:
				$this->setId($value);
				break;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[15], $arr)) $this->setFunval($arr[$keys[15]]);
		if (array_key_exists($keys[16], $arr)) $this->setFechorval($arr[$keys[16]]);
		if (array_key_exists($keys[17], $arr)) $this->setFechorregpag($arr[$keys[17]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FcpagosPeer::FUNVAL)) $criteria->add(FcpagosPeer::FUNVAL, $this->funval);
		if ($this->isColumnModified(FcpagosPeer::FECHORVAL)) $criteria->add(FcpagosPeer::FECHORVAL, $this->fechorval);
		if ($this->isColumnModified(FcpagosPeer::FECHORREGPAG)) $criteria->add(FcpagosPeer::FECHORREGPAG, $this->fechorregpag);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setFunval($this->funval);

		$copyObj->setFechorval($this->fechorval);

		$copyObj->setFechorregpag($this->fechorregpag);
		...
		if ($this->collFcdetliqs === null) {
			if ($this->isNew()) {
			   $this->collFcdetliqs = array();
				...
				$criteria->add(FcdetliqPeer::NUMPAG, $this->getNumpag());
