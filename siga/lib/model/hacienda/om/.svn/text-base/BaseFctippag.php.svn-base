...
abstract class BaseFctippag extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->genmov !== $v) {
        $this->genmov = $v;
        $this->modifiedColumns[] = FctippagPeer::GENMOV;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->genmov = $rs->getString($startcol + 1);

      $this->despag = $rs->getString($startcol + 2);
  ...
  public function __call($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[1] => $this->getGenmov(),
			$keys[2] => $this->getDespag(),
			$keys[3] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setGenmov($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setDespag($arr[$keys[2]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FctippagPeer::GENMOV)) $criteria->add(FctippagPeer::GENMOV, $this->genmov);
