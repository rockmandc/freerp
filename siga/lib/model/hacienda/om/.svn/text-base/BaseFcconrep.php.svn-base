...
abstract class BaseFcconrep extends BaseObject  implements Persistent {
	...
	protected $collFcapulicsRelatedByRifcon;

	
	protected $lastFcapulicRelatedByRifconCriteria = null;

	
	protected $collFcapulicsRelatedByRifrep;

	
	protected $lastFcapulicRelatedByRifrepCriteria = null;
  ...
  public function getId()
  {
    ...
    if ($this->funrec !== $v) {
        $this->funrec = $v;
        $this->modifiedColumns[] = FcconrepPeer::FUNREC;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->funrec = $rs->getString($startcol + 21);

      $this->fecdescon = $rs->getDate($startcol + 22, null);

      $this->fecactcon = $rs->getDate($startcol + 23, null);

      $this->stacon = $rs->getString($startcol + 24);

      $this->origen = $rs->getString($startcol + 25);

      $this->nomneg = $rs->getString($startcol + 26);

      $this->reccon = $rs->getString($startcol + 27);

      $this->tem = $rs->getFloat($startcol + 28);
  ...
  public function __call($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->collFcapulicsRelatedByRifcon !== null) {
				foreach($this->collFcapulicsRelatedByRifcon as $referrerFK) {
			...
			if ($this->collFcapulicsRelatedByRifrep !== null) {
				foreach($this->collFcapulicsRelatedByRifrep as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[21] => $this->getFunrec(),
			$keys[22] => $this->getFecdescon(),
			$keys[23] => $this->getFecactcon(),
			$keys[24] => $this->getStacon(),
			$keys[25] => $this->getOrigen(),
			$keys[26] => $this->getNomneg(),
			$keys[27] => $this->getReccon(),
			$keys[28] => $this->getTem(),
			$keys[29] => $this->getId(),
		);
		return $result;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[21], $arr)) $this->setFunrec($arr[$keys[21]]);
		if (array_key_exists($keys[22], $arr)) $this->setFecdescon($arr[$keys[22]]);
		if (array_key_exists($keys[23], $arr)) $this->setFecactcon($arr[$keys[23]]);
		if (array_key_exists($keys[24], $arr)) $this->setStacon($arr[$keys[24]]);
		if (array_key_exists($keys[25], $arr)) $this->setOrigen($arr[$keys[25]]);
		if (array_key_exists($keys[26], $arr)) $this->setNomneg($arr[$keys[26]]);
		if (array_key_exists($keys[27], $arr)) $this->setReccon($arr[$keys[27]]);
		if (array_key_exists($keys[28], $arr)) $this->setTem($arr[$keys[28]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FcconrepPeer::FUNREC)) $criteria->add(FcconrepPeer::FUNREC, $this->funrec);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach($this->getFcapulicsRelatedByRifcon() as $relObj) {
				$copyObj->addFcapulicRelatedByRifcon($relObj->copy($deepCopy));
			}

			foreach($this->getFcapulicsRelatedByRifrep() as $relObj) {
				$copyObj->addFcapulicRelatedByRifrep($relObj->copy($deepCopy));
			}
	...
	public function initFcapulicsRelatedByRifcon()
	{
		if ($this->collFcapulicsRelatedByRifcon === null) {
			$this->collFcapulicsRelatedByRifcon = array();
		}
	}

	
	public function getFcapulicsRelatedByRifcon($criteria = null, $con = null)
	{
				include_once 'lib/model/hacienda/om/BaseFcapulicPeer.php';
		if ($criteria === null) {
		...
		if ($this->collFcapulicsRelatedByRifcon === null) {
			if ($this->isNew()) {
			   $this->collFcapulicsRelatedByRifcon = array();
				...
				$criteria->add(FcapulicPeer::RIFCON, $this->getRifcon());

				FcapulicPeer::addSelectColumns($criteria);
				$this->collFcapulicsRelatedByRifcon = FcapulicPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				FcapulicPeer::addSelectColumns($criteria);
				if (!isset($this->lastFcapulicRelatedByRifconCriteria) || !$this->lastFcapulicRelatedByRifconCriteria->equals($criteria)) {
					$this->collFcapulicsRelatedByRifcon = FcapulicPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastFcapulicRelatedByRifconCriteria = $criteria;
		return $this->collFcapulicsRelatedByRifcon;
	}

	
	public function countFcapulicsRelatedByRifcon($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/hacienda/om/BaseFcapulicPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		return FcapulicPeer::doCount($criteria, $distinct, $con);
	}

	
	public function addFcapulicRelatedByRifcon(Fcapulic $l)
	{
		$this->collFcapulicsRelatedByRifcon[] = $l;
		$l->setFcconrepRelatedByRifcon($this);
	}

	
	public function initFcapulicsRelatedByRifrep()
	{
		if ($this->collFcapulicsRelatedByRifrep === null) {
			$this->collFcapulicsRelatedByRifrep = array();
		}
	}

	
	public function getFcapulicsRelatedByRifrep($criteria = null, $con = null)
	{
				include_once 'lib/model/hacienda/om/BaseFcapulicPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collFcapulicsRelatedByRifrep === null) {
			if ($this->isNew()) {
			   $this->collFcapulicsRelatedByRifrep = array();
				...
				$criteria->add(FcapulicPeer::RIFREP, $this->getRifcon());

				FcapulicPeer::addSelectColumns($criteria);
				$this->collFcapulicsRelatedByRifrep = FcapulicPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				FcapulicPeer::addSelectColumns($criteria);
				if (!isset($this->lastFcapulicRelatedByRifrepCriteria) || !$this->lastFcapulicRelatedByRifrepCriteria->equals($criteria)) {
					$this->collFcapulicsRelatedByRifrep = FcapulicPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastFcapulicRelatedByRifrepCriteria = $criteria;
		return $this->collFcapulicsRelatedByRifrep;
	}

	
	public function countFcapulicsRelatedByRifrep($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/hacienda/om/BaseFcapulicPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
	...
	public function addFcapulicRelatedByRifrep(Fcapulic $l)
	{
		$this->collFcapulicsRelatedByRifrep[] = $l;
		$l->setFcconrepRelatedByRifrep($this);
	}
		...
		if ($this->collFcdetliqs === null) {
			if ($this->isNew()) {
			   $this->collFcdetliqs = array();
				...
				$criteria->add(FcdetliqPeer::RIFCON, $this->getRifcon());
	...
	public function getFcdetliqsJoinFcliqpag($criteria = null, $con = null)
	{
				include_once 'lib/model/hacienda/om/BaseFcdetliqPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collFcdetliqs === null) {
			if ($this->isNew()) {
				...
				$this->collFcdetliqs = FcdetliqPeer::doSelectJoinFcliqpag($criteria, $con);
			}
		} else {
