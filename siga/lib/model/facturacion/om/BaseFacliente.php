...
abstract class BaseFacliente extends BaseObject  implements Persistent {
	...
	protected $collFafacturs;

	
	protected $lastFafacturCriteria = null;

	
	protected $collFapedidos;

	
	protected $lastFapedidoCriteria = null;
	...
	protected $collFacarordsRelatedByCodpro;

	
	protected $lastFacarordRelatedByCodproCriteria = null;

	
	protected $collFacarordsRelatedByCodban;

	
	protected $lastFacarordRelatedByCodbanCriteria = null;

	
	protected $collFadonacions;

	
	protected $lastFadonacionCriteria = null;
  ...
  public function getEsprospec()
  {

    return $this->esprospec;

  }
  
  public function getValdoc($val=false)
  {

    if($val) return number_format($this->valdoc,2,',','.');
    else return $this->valdoc;

  }
  
  public function getCodctaant()
  {

    return trim($this->codctaant);
  ...
  public function getId()
  {
	...
	public function setEsprospec($v)
	{

    if ($this->esprospec !== $v) {
        $this->esprospec = $v;
        $this->modifiedColumns[] = FaclientePeer::ESPROSPEC;
      }
  
	} 
	
	public function setValdoc($v)
	{

    if ($this->valdoc !== $v) {
        $this->valdoc = Herramientas::toFloat($v);
        $this->modifiedColumns[] = FaclientePeer::VALDOC;
      }
  
	} 
	
	public function setCodctaant($v)
	{

    if ($this->codctaant !== $v) {
        $this->codctaant = $v;
        $this->modifiedColumns[] = FaclientePeer::CODCTAANT;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->esprospec = $rs->getBoolean($startcol + 53);

      $this->valdoc = $rs->getFloat($startcol + 54);

      $this->codctaant = $rs->getString($startcol + 55);
  ...
  public function get($m, $a)
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->collFafacturs !== null) {
				foreach($this->collFafacturs as $referrerFK) {
			...
			if ($this->collFapedidos !== null) {
				foreach($this->collFapedidos as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collFacarordsRelatedByCodpro !== null) {
				foreach($this->collFacarordsRelatedByCodpro as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collFacarordsRelatedByCodban !== null) {
				foreach($this->collFacarordsRelatedByCodban as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collFadonacions !== null) {
				foreach($this->collFadonacions as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 52:
				...
				break;
			case 53:
				return $this->getEsprospec();
				break;
			case 54:
				return $this->getValdoc();
				break;
			case 55:
				return $this->getCodctaant();
				break;
			case 56:
				return $this->getId();
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[53] => $this->getEsprospec(),
			$keys[54] => $this->getValdoc(),
			$keys[55] => $this->getCodctaant(),
			$keys[56] => $this->getId(),
		);
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 52:
				...
				break;
			case 53:
				$this->setEsprospec($value);
				break;
			case 54:
				$this->setValdoc($value);
				break;
			case 55:
				$this->setCodctaant($value);
				break;
			case 56:
				$this->setId($value);
				break;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[53], $arr)) $this->setEsprospec($arr[$keys[53]]);
		if (array_key_exists($keys[54], $arr)) $this->setValdoc($arr[$keys[54]]);
		if (array_key_exists($keys[55], $arr)) $this->setCodctaant($arr[$keys[55]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FaclientePeer::ESPROSPEC)) $criteria->add(FaclientePeer::ESPROSPEC, $this->esprospec);
		if ($this->isColumnModified(FaclientePeer::VALDOC)) $criteria->add(FaclientePeer::VALDOC, $this->valdoc);
		if ($this->isColumnModified(FaclientePeer::CODCTAANT)) $criteria->add(FaclientePeer::CODCTAANT, $this->codctaant);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setEsprospec($this->esprospec);

		$copyObj->setValdoc($this->valdoc);

		$copyObj->setCodctaant($this->codctaant);


		if ($deepCopy) {
									$copyObj->setNew(false);

			foreach($this->getFafacturs() as $relObj) {
				$copyObj->addFafactur($relObj->copy($deepCopy));
			}

			foreach($this->getFapedidos() as $relObj) {
				$copyObj->addFapedido($relObj->copy($deepCopy));
			}
			...
			foreach($this->getFacarordsRelatedByCodpro() as $relObj) {
				$copyObj->addFacarordRelatedByCodpro($relObj->copy($deepCopy));
			}

			foreach($this->getFacarordsRelatedByCodban() as $relObj) {
				$copyObj->addFacarordRelatedByCodban($relObj->copy($deepCopy));
			}

			foreach($this->getFadonacions() as $relObj) {
				$copyObj->addFadonacion($relObj->copy($deepCopy));
			}
	...
	public function initFafacturs()
	{
		if ($this->collFafacturs === null) {
			$this->collFafacturs = array();
		}
	}

	
	public function getFafacturs($criteria = null, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFafacturPeer.php';
		if ($criteria === null) {
		...
		if ($this->collFafacturs === null) {
			if ($this->isNew()) {
			   $this->collFafacturs = array();
				...
				$criteria->add(FafacturPeer::CODCLI, $this->getCodpro());

				FafacturPeer::addSelectColumns($criteria);
				$this->collFafacturs = FafacturPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				FafacturPeer::addSelectColumns($criteria);
				if (!isset($this->lastFafacturCriteria) || !$this->lastFafacturCriteria->equals($criteria)) {
					$this->collFafacturs = FafacturPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastFafacturCriteria = $criteria;
		return $this->collFafacturs;
	}

	
	public function countFafacturs($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFafacturPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		return FafacturPeer::doCount($criteria, $distinct, $con);
	}

	
	public function addFafactur(Fafactur $l)
	{
		$this->collFafacturs[] = $l;
		$l->setFacliente($this);
	}

	
	public function initFapedidos()
	{
		if ($this->collFapedidos === null) {
			$this->collFapedidos = array();
		}
	}

	
	public function getFapedidos($criteria = null, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFapedidoPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		}
		elseif ($criteria instanceof Criteria)
		...
		if ($this->collFapedidos === null) {
			if ($this->isNew()) {
			   $this->collFapedidos = array();
				...
				$criteria->add(FapedidoPeer::CODCLI, $this->getCodpro());

				FapedidoPeer::addSelectColumns($criteria);
				$this->collFapedidos = FapedidoPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				FapedidoPeer::addSelectColumns($criteria);
				if (!isset($this->lastFapedidoCriteria) || !$this->lastFapedidoCriteria->equals($criteria)) {
					$this->collFapedidos = FapedidoPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastFapedidoCriteria = $criteria;
		return $this->collFapedidos;
	}

	
	public function countFapedidos($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFapedidoPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		return FapedidoPeer::doCount($criteria, $distinct, $con);
	}

	
	public function addFapedido(Fapedido $l)
	{
		$this->collFapedidos[] = $l;
		$l->setFacliente($this);
	}
	...
	{
				include_once 'lib/model/facturacion/om/BaseFarecproPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		}
		elseif ($criteria instanceof Criteria)
	...
	public function initFacarordsRelatedByCodpro()
	{
		if ($this->collFacarordsRelatedByCodpro === null) {
			$this->collFacarordsRelatedByCodpro = array();
		}
	}

	
	public function getFacarordsRelatedByCodpro($criteria = null, $con = null)
	{
		...
		if ($this->collFacarordsRelatedByCodpro === null) {
			if ($this->isNew()) {
			   $this->collFacarordsRelatedByCodpro = array();
				...
				$criteria->add(FacarordPeer::CODPRO, $this->getCodpro());

				FacarordPeer::addSelectColumns($criteria);
				$this->collFacarordsRelatedByCodpro = FacarordPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				FacarordPeer::addSelectColumns($criteria);
				if (!isset($this->lastFacarordRelatedByCodproCriteria) || !$this->lastFacarordRelatedByCodproCriteria->equals($criteria)) {
					$this->collFacarordsRelatedByCodpro = FacarordPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastFacarordRelatedByCodproCriteria = $criteria;
		return $this->collFacarordsRelatedByCodpro;
	}

	
	public function countFacarordsRelatedByCodpro($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFacarordPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
	...
	public function addFacarordRelatedByCodpro(Facarord $l)
	{
		$this->collFacarordsRelatedByCodpro[] = $l;
		$l->setFaclienteRelatedByCodpro($this);
	}


	
	public function getFacarordsRelatedByCodproJoinFaentcre($criteria = null, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFacarordPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collFacarordsRelatedByCodpro === null) {
			if ($this->isNew()) {
				...
				$this->collFacarordsRelatedByCodpro = FacarordPeer::doSelectJoinFaentcre($criteria, $con);
			}
		} else {
	...
	public function initFacarordsRelatedByCodban()
	{
		if ($this->collFacarordsRelatedByCodban === null) {
			$this->collFacarordsRelatedByCodban = array();
		}
	}

	
	public function getFacarordsRelatedByCodban($criteria = null, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFacarordPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collFacarordsRelatedByCodban === null) {
			if ($this->isNew()) {
			   $this->collFacarordsRelatedByCodban = array();
				...
				$criteria->add(FacarordPeer::CODBAN, $this->getCodpro());

				FacarordPeer::addSelectColumns($criteria);
				$this->collFacarordsRelatedByCodban = FacarordPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				FacarordPeer::addSelectColumns($criteria);
				if (!isset($this->lastFacarordRelatedByCodbanCriteria) || !$this->lastFacarordRelatedByCodbanCriteria->equals($criteria)) {
					$this->collFacarordsRelatedByCodban = FacarordPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastFacarordRelatedByCodbanCriteria = $criteria;
		return $this->collFacarordsRelatedByCodban;
	}

	
	public function countFacarordsRelatedByCodban($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFacarordPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
	...
	public function addFacarordRelatedByCodban(Facarord $l)
	{
		$this->collFacarordsRelatedByCodban[] = $l;
		$l->setFaclienteRelatedByCodban($this);
	}


	
	public function getFacarordsRelatedByCodbanJoinFaentcre($criteria = null, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFacarordPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collFacarordsRelatedByCodban === null) {
			if ($this->isNew()) {
				...
				$this->collFacarordsRelatedByCodban = FacarordPeer::doSelectJoinFaentcre($criteria, $con);
			}
		} else {
	...
	public function initFadonacions()
	{
		if ($this->collFadonacions === null) {
			$this->collFadonacions = array();
		}
	}

	
	public function getFadonacions($criteria = null, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFadonacionPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		}
		elseif ($criteria instanceof Criteria)
		...
		if ($this->collFadonacions === null) {
			if ($this->isNew()) {
			   $this->collFadonacions = array();
				...
				$criteria->add(FadonacionPeer::CODPRO, $this->getCodpro());

				FadonacionPeer::addSelectColumns($criteria);
				$this->collFadonacions = FadonacionPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				FadonacionPeer::addSelectColumns($criteria);
				if (!isset($this->lastFadonacionCriteria) || !$this->lastFadonacionCriteria->equals($criteria)) {
					$this->collFadonacions = FadonacionPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastFadonacionCriteria = $criteria;
		return $this->collFadonacions;
	}

	
	public function countFadonacions($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/facturacion/om/BaseFadonacionPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		return FadonacionPeer::doCount($criteria, $distinct, $con);
	}

	
	public function addFadonacion(Fadonacion $l)
	{
		$this->collFadonacions[] = $l;
		$l->setFacliente($this);
	}
