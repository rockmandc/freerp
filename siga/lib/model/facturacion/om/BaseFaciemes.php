...
abstract class BaseFaciemes extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->fecini !== $ts) {
      $this->fecini = $ts;
      $this->modifiedColumns[] = FaciemesPeer::FECINI;
    }
    ...
    if ($this->feccie !== $ts) {
      $this->feccie = $ts;
      $this->modifiedColumns[] = FaciemesPeer::FECCIE;
    }
    ...
    if ($this->pereje !== $v) {
        $this->pereje = $v;
        $this->modifiedColumns[] = FaciemesPeer::PEREJE;
      }
    ...
    if ($this->fecdes !== $ts) {
      $this->fecdes = $ts;
      $this->modifiedColumns[] = FaciemesPeer::FECDES;
    }
    ...
    if ($this->fechas !== $ts) {
      $this->fechas = $ts;
      $this->modifiedColumns[] = FaciemesPeer::FECHAS;
    }
    ...
    if ($this->status !== $v || $v === 'A') {
        $this->status = $v;
        $this->modifiedColumns[] = FaciemesPeer::STATUS;
      }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = FaciemesPeer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      throw new PropelException("Error populating Faciemes object", $e);
    }
  }
  ...
  public function get($m, $a)
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			FaciemesPeer::doDelete($this, $con);
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = FaciemesPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += FaciemesPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = FaciemesPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = FaciemesPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = FaciemesPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FaciemesPeer::FECINI)) $criteria->add(FaciemesPeer::FECINI, $this->fecini);
		if ($this->isColumnModified(FaciemesPeer::FECCIE)) $criteria->add(FaciemesPeer::FECCIE, $this->feccie);
		if ($this->isColumnModified(FaciemesPeer::PEREJE)) $criteria->add(FaciemesPeer::PEREJE, $this->pereje);
		if ($this->isColumnModified(FaciemesPeer::FECDES)) $criteria->add(FaciemesPeer::FECDES, $this->fecdes);
		if ($this->isColumnModified(FaciemesPeer::FECHAS)) $criteria->add(FaciemesPeer::FECHAS, $this->fechas);
		if ($this->isColumnModified(FaciemesPeer::STATUS)) $criteria->add(FaciemesPeer::STATUS, $this->status);
		if ($this->isColumnModified(FaciemesPeer::ID)) $criteria->add(FaciemesPeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(FaciemesPeer::ID, $this->id);
