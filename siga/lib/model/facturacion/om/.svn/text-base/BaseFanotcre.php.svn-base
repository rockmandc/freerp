...
abstract class BaseFanotcre extends BaseObject  implements Persistent {
  ...
  public function getStanot()
  {

    return trim($this->stanot);
  ...
  public function getId()
  {
    ...
    if ($this->desnot !== $v) {
        $this->desnot = $v;
        $this->modifiedColumns[] = FanotcrePeer::DESNOT;
      }
  
	} 
	
	public function setStanot($v)
	{

    if ($this->stanot !== $v) {
        $this->stanot = $v;
        $this->modifiedColumns[] = FanotcrePeer::STANOT;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->desnot = $rs->getString($startcol + 4);

      $this->stanot = $rs->getString($startcol + 5);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[4] => $this->getDesnot(),
			$keys[5] => $this->getStanot(),
			$keys[6] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setDesnot($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setStanot($arr[$keys[5]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FanotcrePeer::DESNOT)) $criteria->add(FanotcrePeer::DESNOT, $this->desnot);
		if ($this->isColumnModified(FanotcrePeer::STANOT)) $criteria->add(FanotcrePeer::STANOT, $this->stanot);
