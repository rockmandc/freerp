...
abstract class BaseFaartfacpro extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->facafi !== $v) {
        $this->facafi = $v;
        $this->modifiedColumns[] = FaartfacproPeer::FACAFI;
      }
    ...
    if ($this->codpro !== $v) {
        $this->codpro = $v;
        $this->modifiedColumns[] = FaartfacproPeer::CODPRO;
      }
    ...
    if ($this->fecdev !== $ts) {
      $this->fecdev = $ts;
      $this->modifiedColumns[] = FaartfacproPeer::FECDEV;
    }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->facafi = $rs->getString($startcol + 43);

      $this->codpro = $rs->getString($startcol + 44);

      $this->fecdev = $rs->getDate($startcol + 45, null);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[43] => $this->getFacafi(),
			$keys[44] => $this->getCodpro(),
			$keys[45] => $this->getFecdev(),
			$keys[46] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[43], $arr)) $this->setFacafi($arr[$keys[43]]);
		if (array_key_exists($keys[44], $arr)) $this->setCodpro($arr[$keys[44]]);
		if (array_key_exists($keys[45], $arr)) $this->setFecdev($arr[$keys[45]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FaartfacproPeer::FACAFI)) $criteria->add(FaartfacproPeer::FACAFI, $this->facafi);
		if ($this->isColumnModified(FaartfacproPeer::CODPRO)) $criteria->add(FaartfacproPeer::CODPRO, $this->codpro);
		if ($this->isColumnModified(FaartfacproPeer::FECDEV)) $criteria->add(FaartfacproPeer::FECDEV, $this->fecdev);
