 ...
 * @version SVN: $Id: GridHelper.php 44637 2011-06-02 22:27:59Z amelendez $
 *
       ...
       if (!$oculta[$j]) {
         ...
         switch ($tiposobj[$j]){
           ...
           case 'cc':
             ...
             if(is_object($datos[$i])) $getcombo = $datos[$i]->$metodocombo();
             else $getcombo = array();
             $tagw = '     <td class="grid_fila" >';
    ...
    '.$functioneliminar.' = function(i,c)
      ...
      {' .
        ...
        for (col=1;col<=c;col++)
        {
          ...
          if (document.getElementById(y).type=="checkbox"){
            document.getElementById(x).checked=document.getElementById(y).checked;
        ...
        'yid = "'.$name.'"+"x"+fila2+"id";' .
        '
        if (document.getElementById(yid).type=="checkbox"){
          document.getElementById(xid).checked=document.getElementById(yid).checked;
          document.getElementById(yid).checked="";
        }else {
      ...
      if (i=='.$filas.'-1)
      {
        for (col=1;col<=c;col++)
        {
          ...
          if (document.getElementById(x).type=="checkbox")
...
function grid_tag_v2($obj,$objelim = array())
{
  ...
  $comofecha=$obj["comofecha"];
  $tabindex= $obj["tabindex"];
  $tabindexstart=$obj["tabindexstart"];
  $tabindexstart--;
  $camposcomoindices=$obj["camposcomoindices"];
  $ajaxupdate= $obj["ajaxupdate"];
  $jsfilas = array();
  $jsobjs = array();
  ...
  '.javascript_include_tag('tools', 'dFilter', 'grid', 'numberformat', 'event.simulate.js');
  /////////////////
  // Inicio Grid //
  /////////////////
  ...
  while ($i<$contdatos){
    ...
    if ($eliminar) {
      ...
      $tagf .= $jseliminar.'" ></td>';      
      #$tagf .= '" '.$jseliminar.' ></td>'; MAL CONCATENADO
    }
    ...
    while ($j<count($campos)) {
      ...
      if ($ajax[$j]!="") {
        ...
        $jsajax = " accionRemotaGrid('".url_for($modulo.'/ajaxfila')."','".$name."','fila','".$i."',new Array(".$ajaxadic."),this,'".$ajaxupdate[$j]."'); ";
      }elseif($ajaxcolumna[$j]) {
        $jsajax=" accionRemotaGrid('".url_for($modulo.'/ajaxcolumna')."','".$name."','columna','".($j+1)."',new Array(".$ajaxadic."),this,'".$ajaxupdate[$j]."'); ";
      }elseif($ajaxgrid[$j]) {
         $jsajax=" accionRemotaGrid('".url_for($modulo.'/ajaxgrid')."','".$name."','grid','0',new Array(".$ajaxadic."),this,'".$ajaxupdate[$j]."'); ";
      }else $jsajax="";
      ...
      if ($catalogos[$j][0]!="") {
       ...
       if(count($catalogos[$j][4])>0)
       {
         ...
         //       print '$url='.$url.'
         //';
       if($version=='v1') $catobj=button_to_popup('...',$url,'','','imagencatgrid ','','','popup_'.$name.'_'.$i.'_'.$jmasuno);
       else $catobj=button_to_modal('...',$url,'','','imagencatgrid ','','','popup_'.$name.'_'.$i.'_'.$jmasuno);
        ...
        if(!is_array($datos[$i])){
          ...
          switch ($tiposobj[$j]){
            ...
            case 'm':
              ...
              break;
            default:
              if($comofecha[$j]) $get = $datos[$i]->$metodo('d/m/Y');
              else $get = $datos[$i]->$metodo();
          } // switch
          $getid = $datos[$i]->getId();
        }else{
       ...
       if (!$oculta[$j]) {
         ...
         switch ($tiposobj[$j]){
           case 'f':   //Fecha
             $tagw ='     <td class="grid_fila g_'.$name.'_f '.$name.'c'.$j.'" align="left" height="10">';
               ...
               $tagw .= input_date_tag($name.'x_'.$i.'_'.$jmasuno, $valor2, array (
                'name' => 'grid'.$name.'['.$i.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']',
                'rich' => true,
                'maxlength' => 10,
                'calendar_button_img' => '/sf/sf_admin/images/date.png',
                'onkeyup' => "javascript: mascara(this,'/',patron,true)",
           ...
           case 't':   //texto
             ...
             $tagw .= input_tag($name.'x_'.$i.'_'.$jmasuno,$get, 'name="grid'.$name.'['.$i.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" style="border:none" class="grid_txtleft '.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" tabindex="'.$tabindexopc.'" onChange="'.$change.'" onBlur=" '.$blur.' '.$js[$j].' " '.$taghtml);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             break;
           case 'a':   //texto
             $tagw = '     <td class="grid_fila g_'.$name.'_t '.$name.'c'.$j.'" align="left" height="15">';
             $tagw .= textarea_tag($name.'x_'.$i.'_'.$jmasuno,$get, 'name="grid'.$name.'['.$i.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" style="border:none" class="'.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" tabindex="'.$tabindexopc.'" onChange="'.$change.'"  onBlur=" '.$blur.' '.$js[$j].' " '.$taghtml);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             break;
           case 'm':   //Monto
             $tagw = '     <td class="grid_fila g_'.$name.'_m '.$name.'c'.$j.'" align="right" height="25">';
             $tagw .= input_tag($name.'x_'.$i.'_'.$jmasuno,$get, 'name="grid'.$name.'['.$i.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" style="border:none" class="grid_txtright '.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" tabindex="'.$tabindexopc.'" onChange="'.$change.'" onBlur=" javascript: obj=this; ValidarMontoGridv2(obj,'.$decimal[$j].'); '.$blur.' '.$js[$j].' " '.$taghtml);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             $jsobjs[] = $name.'x_'.$i.'_'.$jmasuno;
             break;
           case 'e':   //Monto Entero
             $tagw = '     <td class="grid_fila g_'.$name.'_m '.$name.'c'.$j.'" align="right" height="25">';
             $tagw .= input_tag($name.'x_'.$i.'_'.$jmasuno,$get, 'name="grid'.$name.'['.$i.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" style="border:none" class="grid_txtright '.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" tabindex="'.$tabindexopc.'" onChange="'.$change.'" onKeyPress="javascript:return validaEntero(event)" onBlur=" '.$blur.' '.$js[$j].' " '.$taghtml);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             $jsobjs[] = $name.'x_'.$i.'_'.$jmasuno;
             break;             
           case 'c':  //Combo
             $arr_for_select = (is_array($combo[$j]) && $combo[$j]!='') ? $combo[$j] : call_user_func(array(ucfirst($tabla).'Peer', $combo[$j]));
             if(count($arr_for_select)==0 && $get!='') $arr_for_select = array((string)$get => $get);
             $tagw = '     <td class="grid_fila g_'.$name.'_c '.$name.'c'.$j.'" >';
             $tagw .= select_tag($name.'x_'.$i.'_'.$jmasuno, options_for_select( $arr_for_select ,$get,'include_custom=Seleccione...'), 'name="grid'.$name.'['.$i.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" class="'.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" style="border:none" tabindex="'.$tabindexopc.'" onChange="'.$blur.' '.$change.' '.$js[$j].' " '.$taghtml.' '.$js[$j]);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             break;
           case 'cc': //Otro Tipo de Combo
             ...
             $tagw = '     <td class="grid_fila g_'.$name.'_c '.$name.'c'.$j.'" >';
             $tagw .= select_tag($name.'x_'.$i.'_'.$jmasuno, options_for_select($getcombo,$get,'include_custom=Seleccione...'), 'name="grid'.$name.'['.$i.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" style="border:none" class="'.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" '.$taghtml.' '.$js[$j].' tabindex="'.$tabindexopc.'" onChange="'.$change.'" onBlur="'.$blur.'"');
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             break;
           case 'k':   //check
             ...
             $tagw .= checkbox_tag($name.'x_'.$i.'_'.$jmasuno,1,$checked, 'name="grid'.$name.'['.$i.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" style="border:none" class="grid_txtcenter '.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" tabindex="'.$tabindexopc.'" onChange=" '.$blur.' '.$change.' '.$js[$j].' " '.$taghtml);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
         }
       }else {
         $tagw ='<input type="hidden" class="'.$name.'_ids '.$name.'c'.$j.'" name="grid'.$name.'['.$i.']['.($camposcomoindices ? 'id' : $j).']" id="'.$name.'x_'.$i.'_'.$jmasuno.'" '.$js[$j].' value="'.$get.'" >';
       }
      ...
      //print 'TipoObj='.$tiposobj[$j].'--$j='.$j.'>>';
       if (!$oculta[$j]) {
         ...
         switch ($tiposobj[$j]){
           ...
           case 't':
             $tagw = '     <td class="grid_fila g_'.$name.'_t '.$name.'c'.$j.'" align="left" height="15">';
             $tagw .= input_tag($name.'x_'.$i.'?'.'_'.$jmasuno,$default[$j], 'name="grid'.$name.'['.$i.'?'.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" style="border:none" class="grid_txtleft '.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" tabindex="'.$tabindexopc.'" onChange="'.$change.'" onBlur=" '.$blur.' '.$js[$j].' " '.$taghtml);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             break;
           case 'a':
             $tagw = '     <td class="grid_fila g_'.$name.'_t '.$name.'c'.$j.'" align="left" height="15">';
             $tagw .= textarea_tag($name.'x_'.$i.'?'.'_'.$jmasuno,$default[$j], 'name="grid'.$name.'['.$i.'?'.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" style="border:none" class="'.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" tabindex="'.$tabindexopc.'"  onChange="'.$change.'" onBlur=" '.$blur.' '.$js[$j].' " '.$taghtml);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             break;
           case 'm':
             $tagw = '     <td class="grid_fila g_'.$name.'_m '.$name.'c'.$j.'" align="right" height="25">';
             $tagw .= input_tag($name.'x_'.$i.'?'.'_'.$jmasuno,$default[$j], 'name="grid'.$name.'['.$i.'?'.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" style="border:none" class="grid_txtright '.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" tabindex="'.$tabindexopc.'" onChange="'.$change.'" onBlur=" javascript: obj=this; ValidarMontoGridv2(obj,'.$decimal[$j].'); '.$blur.' '.$js[$j].' " '.$taghtml);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             $jsobjs[] = $name.'x_'.$i.'_'.$jmasuno;
             break;
           case 'e':   //Monto Entero
             $tagw = '     <td class="grid_fila g_'.$name.'_m '.$name.'c'.$j.'" align="right" height="25">';
             $tagw .= input_tag($name.'x_'.$i.'?'.'_'.$jmasuno,$default[$j], 'name="grid'.$name.'['.$i.'?'.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" style="border:none" class="grid_txtright '.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" tabindex="'.$tabindexopc.'" onChange="'.$change.'" onKeyPress="javascript:return validaEntero(event)" onBlur='.$blur.' '.$js[$j].' " '.$taghtml);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             $jsobjs[] = $name.'x_'.$i.'_'.$jmasuno;
             break;                          
           case 'c':
             $arr_for_select = (is_array($combo[$j]) && $combo[$j]!='') ? $combo[$j] : call_user_func(array(ucfirst($tabla).'Peer', $combo[$j]));
             if(count($arr_for_select)==0 && $default[$j]!='') $arr_for_select = array((string)$default[$j] => $default[$j]);
             $tagw = '     <td class="grid_fila g_'.$name.'_c '.$name.'c'.$j.'" >';
             $tagw .= select_tag($name.'x_'.$i.'?'.'_'.$jmasuno, options_for_select($arr_for_select,$default[$j],'include_custom=Seleccione...'), 'name="grid'.$name.'['.$i.'?'.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" class="'.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" style="border:none" tabindex="'.$tabindexopc.'" onChange="'.$blur.' '.$change.' '.$js[$j].' " '.$taghtml.' '.$js[$j]);
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             break;
           case 'cc':
             $tagw = '     <td class="grid_fila g_'.$name.'_c '.$name.'c'.$j.'" >';
             $tagw .= select_tag($name.'x_'.$i.'_'.$jmasuno, options_for_select(array(),'','include_custom=Seleccione...'), 'name="grid'.$name.'['.$i.'?'.']['.($camposcomoindices ? strtolower($campos[$j]) : $j).']" class="'.$name.'fil'.$i.' '.$name.'col'.$jmasuno.'" style="border:none" '.$taghtml.' '.$js[$j].' tabindex="'.$tabindexopc.'" onChange="'.$change.'" onBlur="'.$blur.'"');
             $tagw .= $catobj.$btnobj.$tagId.'</td>';
             break;
           case 'k':
              if ($default[$j]=="" || $default[$j]=="0"){
               $checked=false;
             }
             else{
               $checked=true;
         ...
         $tagw ='<input type="hidden" class="'.$name.'_ids '.$name.'c'.$j.'" name="grid'.$name.'['.$i.'?'.']['.($camposcomoindices ? 'id' : $j).']" id="'.$name.'x_'.$i.'?'.'_'.$jmasuno.'" '.$js[$j].' value="'.$default[$j].'" >';
       }
       $jsfilas[$i][] = $name.'x_'.$i.'_'.$jmasuno;
       if($tagId!='') $jsfilas[$i][] = $name.'x'.$i.'id';
...
//  $tagciclo3 = str_replace("*","?",$tagciclo3);
  ...
  $scriptActSal = '
      <script type="text/javascript">
       ...
       updateVarsGrid'.$name.' = function() {
          window.objs_montos_'.$name.' = '.$strarrayobjs.';
          window.objs_filas_'.$name.' = null;
          window.inputs_filas_'.$name.' = '.$strarrayfilas.';
          window.inputs_cols_'.$name.' = null;
          window.inputs_grid_'.$name.' = null;
          window.ArrTotales_'.$name.' = new Array('.$campostotales.');
          window.grid_acum_'.$name.'='.$acum.';
          window.tabindexstart_'.$name.'='.$tabindexstart.';
          window.tabindex_'.$name.'='.($tabindex ? 'true' : 'false').';
