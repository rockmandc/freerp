...
/**
 * button_to_modal: Función para generar la ventana emergente con el formulario de catalogos
 * que permite hacer busquedas de un catalogo de datos
 *
 * @package    Roraima
 * @subpackage helper
 ...
 * @license    http://opensource.org/licenses/gpl-2.0.php GPLv2
 */
  function button_to_modal($name, $internal_uri, $sql='', $space='', $class = '', $width ='', $height = '', $id='')
  {
    //if(stripos($internal_uri,'?') || stripos($internal_uri,'='))
    //  $internal_uri = Herramientas::genUrl($internal_uri);
    if($sql!='')
    ...
    $url_options = H::url_to_array($internal_uri);
    if (empty($width)){ $width='490';}
      if (empty($height)){ $height='490';}

    return button_to_function('...', "catalogo('".$url_options['metodo']."','" . $internal_uri. "','" . $url_options['obj1'] . "','" . $url_options['campo1'] . "',true);", array(
        'class'  => $class,
        'id' => $id,
      )).Javascript_tag("Event.observe(window, 'load',function() { catalogo('".$url_options['metodo']."','" . $internal_uri. "','" . $url_options['obj1'] . "','" . $url_options['campo1'] . "',false); });");
  }
