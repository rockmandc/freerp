...
class Columna
{
   ...
   * @return void
   */
  public function setCatalogov2($clase,$form,$objs,$metodo = '',$param = array()){
  ...
  /**
   * Establece si Se imprime los datos de la celda como si fuera un tipo fecha (para que salga con el formato)
   *
   * @param $val string con el valor por defecto
   * @return void
   */
  public function setComofecha($val){

    $this->comofecha = $val;

  }
  /**
   * Establece el div que se va a actualizar en el ajaxgrid, ajaxfila y ajaxcolumna
   *
   * @param $val string con el valor por defecto
   * @return void
   */
...
/**
  ...
  public function getAjaxonchange(){ // $campos
    ...
    /**
   * Obtiene el valor que identifica si el valor de la celda se imprimo como si fuera una fecha
   *
   * @return bool
   */
  public function getComofecha(){ // $campos
    return $this->comofecha;
  }

    /**
   * Obtiene el valor del div que se va a actualizar en el ajaxgrid, ajaxfila y ajaxcolumna
   *
   * @return bool
   */
  public function getAjaxupdate(){ // $campos
