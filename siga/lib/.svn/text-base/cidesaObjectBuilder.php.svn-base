...
";
  ...
  */
  protected function addGet(&$script)
  {
    $script .= "
  /**
  * Función para agilizar la generación de funciones Get y Set de clases relacionadas
  * Generado por Luis Hernández.
  */
...
";
  ...
  */
  ...
  ";
    }elseif($col->getType() == PropelTypes::VARCHAR  && substr($this->getColumnConstant($col),0,2)=='Bn'){
      $script .= ") {
        \$this->$clo = strtoupper(\$v);
        \$this->modifiedColumns[] = ".$this->getColumnConstant($col).";
      }
  ";
    }else{
...
//              Código Agregado - Luis Hernández - 24/06/07
    ...
    if(\$val) return number_format(\$this->".$clo.",2,',','.');
      ...
      if($clo=='created_by_user'){
      $script .= "
    if(\$this->".$clo."=='') \$this->set".$cfc."(sfContext::getInstance()->getUser()->getAttribute('loguse'));
    return \$this->".$clo.";

  }";}elseif($clo=='updated_by_user'){
      $script .= "
    \$this->set".$cfc."(sfContext::getInstance()->getUser()->getAttribute('loguse'));
    return \$this->".$clo.";
