...
/**********************************Clasificador Presupuestario(Recursos)*******************************************/
  ...
  public static function Actualiza_Fuentes_de_Financiamiento_Real($forencpryaccespmet,$grid)
  {
	...
	try
	{
		...
		$per = FordisfuefinpryaccmetPeer::doSelectOne($c);
  ...
  public static function GrabarDisFueFinPryAccMet($forencpryaccespmet,$grid)
  {
    ...
    while ($j<count($x))
    {
      ...
      if ($reg)
      {
       ...
       if (count($monto)>1 && count($codparing)>1) {
	    ...
	    while ($j<count($x))
	    {
	      //echo $j." ".$x[$j]->getCodpre()."<br>";
	      	...
	      	$per = FordismonprepryaccmetueaPeer::doDelete($c);
	       	  ...
	       	  $monto_asig = split("_",$x[$j]->getGrid_());
...
public static function salvarforpoa_uae($forencpryaccespmet,$grid)
  ...
  public static function salvarObjetivosUnidades($clasemodelo,$grid)
  {
      $codunidad= $clasemodelo->getCoduni();
      $x=$grid[0];
      $j=0;
      while ($j<count($x))
      {
 ...
 public static function cargarPeriodosCosCat($categoria,$articulo,$cadena,&$acum)
 {
   $periodos=array();
   $acum=0;
   ...
   if ($cadena==''){
       $c = new Criteria();
       $c->add(ForestpercatPeer::CODCAT,$categoria);
       $c->add(ForestpercatPeer::CODART,$articulo);
       $reg= ForestpercatPeer::doSelect($c);
       if ($reg)
       {
           $i=0;
           foreach ($reg as $obj)
...
public static function cargarFuentesCosCat($categoria, $articulo, $cadena)
{
   $fuentes=array();
   $result=array();
   ...
   if ($cadena==''){
       $c = new Criteria();
       $c->add(ForestfuecatPeer::CODCAT,$categoria);
       $c->add(ForestfuecatPeer::CODART,$articulo);
       $reg= ForestfuecatPeer::doSelect($c);
       if ($reg)
       {
           $i=0;
           foreach ($reg as $obj)
 ...
 public static function grabarEstructuraCostosCat($clasemodelo,$grid)
 {
    $t= new Criteria();
    $t->add(ForestcoscatPeer::CODCAT,$clasemodelo->getCodcat());
    $reg=ForestcoscatPeer::doSelect($t);
    if (!$reg) {
       ...
       while ($j<count($x))
       {
           if ($x[$j]->getCodart()!='')
           {
               $forestcoscat= new Forestcoscat();
               $forestcoscat->setCodcat($clasemodelo->getCodcat());
               $forestcoscat->setCodart($x[$j]->getCodart());
               $forestcoscat->setCodpar($x[$j]->getCodpar());
               $forestcoscat->setCanart($x[$j]->getCanart());
               if ($x[$j]->getCadenaper()!='')
               {
                  $f= new Criteria();
                  $f->add(ForestpercatPeer::CODCAT,$clasemodelo->getCodcat());
                  $f->add(ForestpercatPeer::CODART,$x[$j]->getCodart());
                  ForestpercatPeer::doDelete($f);
                  ...
                  while ($r<(count($cadenaper)-1))
                  {
                    ...
                    $forestpercat= new Forestpercat();
                    $forestpercat->setCodcat($clasemodelo->getCodcat());
                    $forestpercat->setCodart($x[$j]->getCodart());
                    $forestpercat->setPerpre($aux2[0]);
                    $forestpercat->setCanper(H::toFloat($aux2[1]));
                    $forestpercat->save();
                    $r++;
                  }
               }
               $forestcoscat->setMonart($x[$j]->getMonart());
               $forestcoscat->setTotpre($x[$j]->getTotpre());
               $forestcoscat->setCodfin($x[$j]->getCodfin());
               if ($x[$j]->getCadenafin()!='')
               {
                  $f= new Criteria();
                  $f->add(ForestfuecatPeer::CODCAT,$clasemodelo->getCodcat());
                  $f->add(ForestfuecatPeer::CODART,$x[$j]->getCodart());
                  ForestfuecatPeer::doDelete($f);
                  ...
                  while ($r<(count($cadenafin)-1))
                  {
                    ...
                    $forestfuecat= new Forestfuecat();
                    $forestfuecat->setCodcat($clasemodelo->getCodcat());
                    $forestfuecat->setCodart($x[$j]->getCodart());
                    $forestfuecat->setCodparing($aux2[0]);
                    $forestfuecat->setMonfin(H::toFloat($aux2[2]));
                    $forestfuecat->save();

                    $r++;
                  }
               }
               $forestcoscat->setCodtip($x[$j]->getCodtip());
               $forestcoscat->setObserv($x[$j]->getObserv());
               $forestcoscat->save();
           }
