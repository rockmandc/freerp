<?php
/**
 * Obras: Clase estática para manejar el módulo de obras
 *
 * @package    Roraima
 * @subpackage obras
...
public static function salvarLicitacion($ocreglic, $grid)
{
  ...
  public static function eliminarOycregpro($caprovee)
 {
  ...
  public static function chequearDisponibilidadPresupuesto($codpre,$anno,$mes,$grid,$num,$monto,&$mondis)
   {
   	 $mitotal=0;
     if ($num=='1')
     {
  ...
  public static function salvarOycadjobr($ocadjobr,&$error)
  {
    ...
    if ($ocadjobr->getStatus()== 1 && $ocadjobr->getCodprogan()!='')
    {
      ...
      if ($dat)
      {
        ...
        $mes=substr($dat->getFecini(),5,2);
      }

      $nopuedeaumentar = false;
      if (!self :: chequearDisponibilidad($codpre,$anno,$mes,$gridnuevo))
      {
   ...
   public static function chequearDisponibilidad($codpre,$anno,$mes,$grid)
   {
   	 $mitotal=0;
   	 $i = 0;
     while ($i < count($grid))
 ...
 //Guardar
  ...
  public static function generarCorrelativoObra($obra,&$tienecorrelativo,&$r,$corraut)
  {
    ...
    elseif (Herramientas::getVerCorrelativo('numini','ocdefemp',$r))
    {
      ...
      {        
      	$newcodobr="OB".substr($obra->getCodobr(),2,8);
      }
    }
    else
      {
  ...
  public static function grabarProyectos($ocbanpro,$grid)
  {
    if (!$ocbanpro->getId())
       $ocbanpro->save();
    ...
    while ($j<count($x))
    {
      if ($x[$j]->getCodpar()!='')
      {
      	$x[$j]->setCodbanpro($ocbanpro->getCodbanpro());
        $x[$j]->save();
      }
      $j++;
    }
