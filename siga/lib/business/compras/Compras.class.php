...
class Compras {
  ...
  public static function Grabar_DetallesCotizacion($datos, $grid, $tipo) {
    try {
      ...
      if ($datos->getRefsol()=="") $moneda=$datos->getTipmon();
      else $moneda=Herramientas::getX_vacio('reqart','Casolart','Tipmon',$datos->getRefsol());
      $moneda2=H::getX_vacio('CODEMP', 'Opdefemp', 'Codmon', '001');
      if ($moneda2!=$moneda) {
        if ($datos->getRefsol()=="") $valor=$datos->getValmon();
        else $valor=Herramientas::getX('reqart','Casolart','Valmon',$datos->getRefsol());
    }else
      ...
      while ($j < count($x)) {
        ...
        if ($x[$j]['codart'] != "" && $cantord > 0) {
          ...
          $detalle->setUnimed(trim($x[$j]['unimed']));
          $detalle->setCanord($cantord);
          $detalle->setCosto($x[$j]['costo']*$valor);
          $detalle->setMondes($x[$j]['mondes']*$valor);
          $detalle->setTotdet($x[$j]['totdet']*$valor);
          $detalle->setCodcat($x[$j]['codcat']);
          if ($x[$j]['fecent'] != "") {
            $detalle->setFecent($x[$j]['fecent']);
          }
              ...
              if ($x[$j]['datosrecargo'] != '') {
                ...
                while ($r < (count($cadenarec) - 1)) {
                  ...
                  if ($aux2[0] != "") {
                    ...
                    if ($claartdes == 'S')
                      $c->add(CacotdisrgoPeer::DESART, trim($x[$j]['desart']));
                    ...
                    if ($tipo == "M") { //Modificacion
                      $c = new Criteria();
                      $c->add(CadisrgoPeer::REQART, $refcot);
                      $c->add(CadisrgoPeer::CODART, $x[$j]['codart']);
                      if ($claartdes == 'S')
                        $c->add(CadisrgoPeer::DESART, trim($x[$j]['desart']));
                      $c->add(CadisrgoPeer::CODCAT, $x[$j]['codcat']);
  ...
  public static function salvarPrioridadCotizaciones($grid, $reqart, $actsolegr, $casolart, & $error, $grid1) {
    ...
    if ($casolart->getPorcostart() == '1') {
      ...
      while ($j < count($x)) {
        if ($x[$j]->getPriori() != '') {
            ...
            $t = new criteria();
            $t->add(CadetcotPeer::REFCOT, $x[$j]->getRefcot());
            $t->add(CadetcotPeer::CODART, $x[$j]->getCodart());
            $resul = CadetcotPeer::doSelectOne($t);
            if ($resul) {
                $resul->setPriori($x[$j]->getPriori());
                $resul->setJustifica($x[$j]->getJustifica());
                if ($x[$j]->getPriori() == 1 && $casolart->getObservaciones() != "")
                  $resul->setObservaciones($casolart->getObservaciones());
                else
                  $resul->setObservaciones($x[$j]->getObservaciones());
                $resul->save();
              }
    ...
    $claartdes = H::getConfApp2('claartdes', 'compras', 'almsolegr');
    $aplrecar = H::getConfApp2('aplrecar', 'compras', 'almcotiza');
    $actdesuni=H::getConfApp2('actdesuni', 'compras', 'almpriori');
    if ($actsolegr == '1') {
      $c = new Criteria();
      ...
      if ($result) {
        foreach ($result as $datos) {
          ...
          $c->add(CaartsolPeer :: CODCAT, $datos->getCodcat());
          if ($claartdes == 'S')
            $c->add(CaartsolPeer :: DESART, trim($datos->getDesart()));
          $resul = CaartsolPeer :: doSelect($c);
          if ($resul) {
            foreach ($resul as $resul2) {
              $indice = count($gridnuevo) + 1;
              $gridnuevo[$indice - 1][0] = $datos->getCodart();
              $cantidadnew = $datos->getCanord();
              if ($cantidadnew != $resul2->getCanreq()) {                
                $gridnuevo[$indice - 1][1] = $cantidadnew; //Cantidad Nueva
              } else {
                $gridnuevo[$indice - 1][1] = $resul2->getCanreq(); 
              }
              $costonew = $datos->getCosto();
              if ($costonew != $resul2->getCosto()) {
                $gridnuevo[$indice - 1][2] = H::toFloat($costonew,6); //Costo nuevo
              } else {
                $gridnuevo[$indice - 1][2] = H::toFloat($resul2->getCosto(),6); 
              }
              ...
              if ($tippro != 'P') {
                if ($aplrecar == 'S') {
                  ...
                  if ($claartdes == 'S')
                    $c->add(CacotdisrgoPeer :: DESART, trim($datos->getDesart()));
                  ...
                  if ($claartdes == 'S')
                    $c->add(CadisrgoPeer :: DESART, trim($datos->getDesart()));
              ...
              if ($claartdes == 'S')
                $gridnuevo[$indice - 1][12] = $datos->getDesart();

              if ($actdesuni=='S'){
                if ($resul2->getDesart()!=$datos->getDesart())
                  $gridnuevo[$indice - 1][13] = $datos->getDesart();
                if ($resul2->getUnimed()!=$datos->getUnimed())
                  $gridnuevo[$indice - 1][14] = $datos->getUnimed();
              }
            }
          }
        }
      }
      //H::PrintR($gridnuevo); exit;
      ...
      if ($aplrecar == 'S') {
          ...
          if ($dat) {
            foreach ($dat as $resultado) {
              ...
              if ($claartdes == 'S')
                $gridnuevorec[$indice2 - 1][7] = $resultado->getDesart();
      ...
      //H::PrintR($gridnuevorec); exit;
      $nopuedeaumentar = false;
      $datosGrid = array();             
      $tiporec = Herramientas :: getX('CODEMP', 'Cadefart', 'Asiparrec', '001'); 
      $w= new Criteria();
      $w->add(CasolartPeer::REQART,$reqart);
      $datos= CasolartPeer::doSelectOne($w);
      if ($datos)
      {
          $moneda=$datos->getTipmon();
          $moneda2=H::getX_vacio('CODEMP', 'Opdefemp', 'Codmon', '001');
          if ($moneda2!=$moneda)
          {
              $valor=$datos->getValmon();
      ...
      while ($z < count($gridnuevo)) {
          ...
          if ($gridnuevo[$z][2] < 0) {
            ...
            if ($gridnuevo[$z][1] != "") { //&& ($gridnuevo[$z][2]>$gridnuevo[$z][11])) {
              ...
              if ($gridnuevo[$z][5] > $gridnuevo[$z][7]) {                               
                $codpre=H::getCodPreDis($gridnuevo[$z][8]);
                $cantidad=$gridnuevo[$z][1];
                /*if ($moneda2!=$moneda) $costo=$gridnuevo[$z][2]*$valor;
                else  $costo=$gridnuevo[$z][2];*/
                $costo=$gridnuevo[$z][2];
                $recargo=$gridnuevo[$z][4];
                if ($tiporec!='C')        
                ...
                if ($pos==0)
                {
                 ...
                 $datosGrid[$i-1]["monsol"]=($gridnuevo[$z][11]*$gridnuevo[$z][1]);
                }
                else
                {
                 $datosGrid[$pos-1]["monimp"]=$datosGrid[$pos-1]["monimp"]+$monto;
                 $datosGrid[$pos-1]["monsol"]+=($gridnuevo[$z][11]*$gridnuevo[$z][1]);
                }
        ...
        $monreqori=H::getX_vacio('REQART','Casolart','Monreq',$reqart);  
        $actcomanacot = H::getConfApp2('actcomanacot', 'compras', 'almpriori');  
        $c = new Criteria();
        $resul = CadefartPeer :: doSelectOne($c);
        if ($resul) {
            if (($resul->getPrcasopre() == 'S' && $resul->getPrcreqapr() != 'S') || $actcomanacot=='S') {          
              $p=0;
              while ($p<count($datosGrid) && H::toFloat($montototal1)>H::toFloat($monreqori))
              {
                $disponible = H::Monto_disponible($datosGrid[$p]["codpre"],H::getX_vacio('REQART','Casolart','Fecreq',$reqart));
                $disponible = $datosGrid[$p]["monsol"] + $disponible;
       ...
       if($maxcomut=='S' && $nopuedeaumentar != true){
          $valunitri=H::toFloat(H::getX_vacio('CODEMP','Opdefemp','unitri','001'));
          $canunitri=0;
          $coddirec=H::getX_vacio('REQART','Casolart','Coddirec',$reqart);
          $q= new Criteria();
          $q->add(CaunitridirPeer::CODDIREC,$coddirec);
          ...
          self :: montoTotal($gridnuevo, $montototal1, $montototal2);
          if (H::toFloat($montototal1)>H::toFloat($monto))
          {
            $nopuedeaumentar = true; 
            $error = 3008; 
            return true;
      ...
      if ($nopuedeaumentar != true) {
        self :: actualizarSolicitud($reqart, $gridnuevo, $gridnuevo2, $gridnuevorec, $tippro,$casolart->getFecana());
      } else {
        $c = new Criteria();
        $c->addJoin(CacotizaPeer :: REFCOT, CadetcotPeer :: REFCOT);
        $c->add(CacotizaPeer :: REFSOL, $reqart);
  ...
  public static function actualizarSolicitud($reqart, $gridnuevo, $gridnuevo2, &$gridnuevorec, $tippro,$fecana) {
    ...
    if ($resul) {
      ...
      $resul->setFecana($fecana);
      
      $valmon=$resul->getValmon();
    ...
    while ($j < count($gridnuevo)) { //Detalle
      ...
      if ($claartdes == 'S')
        $c->add(CaartsolPeer :: DESART, trim($gridnuevo[$j][12]));
      $c->add(CaartsolPeer :: CODCAT, $gridnuevo[$j][10]);
      $dato = CaartsolPeer :: doSelect($c);
      if ($dato) {
        foreach ($dato as $dato2) {
          $dato2->setCanreq($gridnuevo[$j][1]);
          $dato2->setCosto($gridnuevo[$j][2]*$valmon);
          $dato2->setMondes($gridnuevo[$j][3]*$valmon);
          $dato2->setMonrgo($gridnuevo[$j][4]*$valmon);
          $dato2->setMontot($gridnuevo[$j][5]*$valmon);
          if ($actdesuni=='S'){
            if ($dato2->getDesart()!=$gridnuevo[$j][13])
              $dato2->setDesart($gridnuevo[$j][13]);
            if ($dato2->getUnimed()!=$gridnuevo[$j][14])
              $dato2->setUnimed($gridnuevo[$j][14]);
          }
          $dato2->save();
        }
      }
    ...
    while ($l < count($gridnuevorec)) { //Distribucion de Recargos
      if ($aplrecar == 'S') {
          ...
          $newdisrecargo->setMonrgo($gridnuevorec[$l][1]*$valmon);
        ...
        if ($claartdes == 'S')
          $c->add(CadisrgoPeer :: DESART, trim($gridnuevorec[$l][7]));
        ...
        if ($dato2) {
          foreach ($dato2 as $dato3) {
            $dato3->setMonrgo($gridnuevorec[$l][1]*$valmon);
            $dato3->save();
          }
        }
      }
  ...
  public static function generaPrecompromiso($referencia) {
    ...
    {
      $c = new Criteria();
      $c->add(CpdisevePeer :: REFDOC, $referencia);
      $c->add(CpdisevePeer :: TIPMOV, 'PRC');
      CpdisevePeer :: doDelete($c);
    }
    ...
    if (!$existe) {
      ...
      if ($data) {
        ...
        $cpprecom->setMonprc(H::toFloat($data->getMonreq())*H::toFloat($data->getValmon(),6));
        ...
        if ($filsoldir=='S'){
          $cpprecom->setCoddirec($data->getCoddirec());
        }
        $cpprecom->save();
      }
    }
        ...
        if ($claartdes == 'S') {
          if ($codart == $gridnuevorec[$t][5] && $codcat == $gridnuevorec[$t][6] && $desart == $gridnuevorec[$t][7]) {
            ...
            if ($dato) {
              ...
              if ($dato->getTiprgo() == 'M') {
                $gridnuevorec[$t][1] = $gridnuevorec[$t][4];//$dato->getMonrgo();
              } else
              if ($dato->getTiprgo() == 'P') {
      ...
      if ($gridnuevorec[0][0] != "") {
        ...
        while ($l < count($gridnuevo)) {
          if ($gridnuevo[$l][6] == 1) {
            ...
            if ($claartdes == 'S')
              self::calcularecargosxart($gridnuevo[$l][0], $gridnuevo[$l][10], $monuni-$mondes, $monrgotot, $gridnuevorec, $gridnuevo[$l][12]);
            else
              self::calcularecargosxart($gridnuevo[$l][0], $gridnuevo[$l][10], $monuni-$mondes, $monrgotot, $gridnuevorec);
            if ($sumaresta == 'S') {
 ...
 /*   $cont = -1;
    while ($h < count($gridnuevorec)) {
      ...
      if (self::BuscarCodrgoenArreglo($gridnuevo2, $codrgo, &$j)) {
        ...
        if ($claartdes == 'S')
          $gridnuevo2[$cont][5] = $gridnuevorec[$h][7]; //desart
  ...
  public static function recalcularRecargos(&$gridnuevo2, &$gridnuevo, &$nopuedeaumentar, $reqart, &$gridnuevorec) {
    ...
    $aplrecar = H::getConfApp2('aplrecar', 'compras', 'almcotiza');
    $fecreq=H::getX_vacio('REQART','Casolart','Fecreq',$reqart);
       ...
       $datosGrid = array();
        ...
        if ($dato) {
          ...
          if ($dato->getAsiparrec() == 'C') {
            ...
            $j = 0;
            $procede = true;
            while ($j < count($gridnuevo)) {
              if ($gridnuevo[$j][5] > $gridnuevo[$j][7]) {
                $codpre=H::getCodPreDis($gridnuevo[$j][8]);
                $cantidad=$gridnuevo[$j][1];
                if ($moneda2!=$moneda) $costo=$gridnuevo[$j][2]*$valor;
                else  $costo=$gridnuevo[$j][2];
                $recargo=$gridnuevo[$j][4];
                if ($tiporec!='C')        
                  $monto= $cantidad*$costo;        
                else
                  $monto= $recargo;
            ...
            $p=0;
            while ($p<count($datosGrid) && $procede)
            {
              $disponible = H::Monto_disponible($datosGrid[$p]["codpre"],$fecreq);
              if($datosGrid[$p]["monimp"] > $disponible){
                 $nopuedeaumentar = true;
                 $procede = false;
                 $a = 0;
            ...
            while ($j < count($gridnuevo2) && $procede) {
              if ($gridnuevo2[$j][1] > $gridnuevo2[$j][4]) {
                 ...
                 $c= new Criteria();
                 $c->add(CarecargPeer::CODRGO,$gridnuevo2[$j][0]);
                 ...
                 if ($data)
                 {
                    if ($tiporec=='P') {          
                       ...
                       $t= new Criteria();
                       $t->add(CpdeftitPeer::CODPRE,$gridnuevo2[$j][6].'-'.$data->getCodpre());
                       $trajo= CpdeftitPeer::doSelectOne($t);
                       if ($trajo) {
                            $a= new Criteria();
                            $a->add(CpasiiniPeer::PERPRE,'00');
                            $a->add(CpasiiniPeer::CODPRE,$gridnuevo2[$j][6].'-'.$data->getCodpre());
                            $data2= CpasiiniPeer::doSelectOne($a);
                            if (!$data2)
                            {
                                  $nopuedeaumentar = true;
                       ...
                       $codpre=H::getCodPreDis($gridnuevo2[$j][6].'-'.$data->getCodpre());
                    }

                    if ($moneda2!=$moneda) $monrecar=$elmonto*$valor;
                    else  $monrecar=$elmonto;
             ...
             $g=0;
             while ($g<count($datosGridRec) && $procede)
             {
              $disponible = H::Monto_disponible($datosGridRec[$g]["codpre"],$fecreq);
  ...
  public static function chequearDisponibilidad($elgrid, $fila, $tiporec, $reqart) {
    ...
    if ($elgrid[$fila][8] != "") {
      ...
      $fech=Herramientas :: getX('REQART', 'Casolart', 'Fecreq', $reqart);
      $anno = substr($fech, 0, 4);
      $mes = substr($fech, 5, 2);
      if (Herramientas::Monto_disponible_ejecucion($anno, $codigopresupuestario, $mes, $mondis)) {
        if ($mitotal > $mondis) {
  ...
  public static function salvarAlmaujoc($reg, $grid) {
    ...
    if ($reg->getAjuoc()=='########')
    {
      ...
      while (!$encontrado)
      {
        ...
        $r = $cacorrel->getSecajuoc();
        $numero=str_pad($r, 8, '0', STR_PAD_LEFT);

        $sql="select ajuoc from caajuoc where ajuoc='".$numero."'";
        if (Herramientas::BuscarDatos($sql,$result))
        {
    ...
    {
      $r = null;
      $reg->setAjuoc(str_replace('#','0',$reg->getAjuoc()));
    }
    ...
    $result = self :: Grabar_Ajuste($reg, $grid,$totalaju);
    if ($result > -1)
      return $result;

    if ($nuevo == 'S') { 
      if ($Hay_Presupuesto) {
        self::Grabar_AjusteCompromiso($reg, $grid,$totalaju);
      }
  ...
  public static function validarAlmajuoc($reg, $grid) {
    ...
    if ($resultado) {
      $sql="select refcom from cpimpcom where refcom='".$reg->getOrdcom()."' group by refcom having sum(moncau)=sum(monimp-monaju)";
    ...
    foreach ($ajus as $regajus) {
      if (H::toFloat($regajus->getCanajuord())!=0){     
        $totalaju++;
      }
    }
    if ($totalaju==0)
  ...
  public static function Actualizar_ArticulosOrden($reg, $grid) {
    ...
    foreach ($ajus as $regaju) {
      if (H::toFloat($regaju->getCanajuord())>0) {
      $caartord = CaartordPeer :: retrieveByPK($regaju->getId());
      $ordencom=$caartord->getOrdcom();      
      $caartord->setCanaju($caartord->getCanaju() + H::toFloat($regaju->getCanajuord()));
      ...
      if ($caartord->getCanord() <> 0) {
        ...
        $caartord->setRgoart(H::toFloat($regaju->getMonrecaju()));//round($caartord->getCantot() * ($caartord->getRgoart() / $caartord->getCanord()), 2));
        $caartord->setTotart(H::toFloat($regaju->getMontotaju()));//round($caartord->getCantot() * ($caartord->getTotart() / $caartord->getCanord()), 2));
      } else {
      ...
      $caartord->save();      
      if ($caartord->getCanaju() <> 0) {
        $c = new Criteria();
        $c->add(CadisrgoPeer :: REQART, $caartord->getOrdcom());
        $c->add(CadisrgoPeer :: CODART, $caartord->getCodart());
        if ($claartdes=='S') $c->add(CadisrgoPeer :: DESART, $caartord->getDesart());
        $c->add(CadisrgoPeer :: CODCAT, $caartord->getCodcat());
        $dato2 = CadisrgoPeer :: doSelect($c);
        if ($dato2) {
            $montorecargo =  (H::toFloat($caartord->getCantot())*$caartord->getPreart());
            foreach ($dato2 as $dato3)
            {
              $ca = new Criteria();
              $ca->add(CarecargPeer :: CODRGO, $dato3->getCodrgo());
              $datoa = CarecargPeer :: doselectOne($ca);
              if ($datoa) {
                if ($datoa->getTiprgo() == 'M') {
                  $dato3->setMonrgo($montorecargo);
                } else
                if ($datoa->getTiprgo() == 'P') {
                  $dato3->setMonrgo(($montorecargo * $datoa->getMonrgo())/100);
                } else {
                  $dato3->setMonrgo(0);
                }
              } 
      ...
      $c->add(CaresordcomPeer :: CODARTPRO, $caartord->getCodcat());
      if ($claartdes=='S') $c->add(CaresordcomPeer :: DESRES, $caartord->getDesart());
      $caresordcom = CaresordcomPeer :: doSelectOne($c);
      if ($caresordcom) {
        $caresordcom->setCanaju($caresordcom->getCanaju() + H::toFloat($regaju->getCanajuord()));
        $caresordcom->setCantot($caresordcom->getCanord() - $caresordcom->getCanaju());
        if ($caresordcom->getCanord() <> 0) {
          $caresordcom->setRgoart(H::toFloat($regaju->getMonrecaju()));//round($caresordcom->getCantot() * ($caresordcom->getRgoart() / $caresordcom->getCanord()), 2));
          $caresordcom->setTotart(H::toFloat($regaju->getMontotaju()));//round($caresordcom->getCantot() * ($caresordcom->getTotart() / $caresordcom->getCanord()), 2));
        } else {
    ...
    $q = new Criteria();
    $q->add(CargosolPeer::REQART, $ordencom);
    CargosolPeer::doDelete($q);
    $sql = "select reqart,codrgo,sum(coalesce(monrgo,0)) as monrgo from cadisrgo where reqart='" . $ordencom . "'  group by reqart,codrgo";
    $result = array();
    if (Herramientas::BuscarDatos($sql,$result)) {
      $i = 0;
      while ($i < count($result)) {
        $cargosol = new Cargosol();
        $cargosol->setReqart($ordencom);
        $cargosol->setCodrgo($result[$i]['codrgo']);
        $cargosol->setMonrgo($result[$i]['monrgo']);
        $cargosol->setTipdoc(H::getX_vacio('ORDCOM','Caordcom','Doccom',$ordencom));
        $cargosol->save();
        $i++;
      }// while ($i<count($result))
    }
  ...
  public static function Grabar_Ajuste($reg, $grid, &$totalaju) {
  ...
  public static function Grabar_DetalleAjuste($reg, $grid, & $totalaju) {
    ...
    foreach ($ajus as $regajus) {
      if (H::toFloat($regajus->getCanajuord())!=0){        
        ...
        if ($claartdes=='S') $c->add(CaartordPeer::DESART,$regajus->getDesart());
        $c->add(CaartordPeer::CODCAT, $regajus->getCodcat());
        $caartord = CaartordPeer :: doSelectOne($c);
        if ($caartord) {
          $preciounitario = H::toFloat($regajus->getCosart()); //$caartord->getPreciounitario();
          $caartaoc = new Caartaoc();
          $caartaoc->setAjuoc($reg->getAjuoc());
          $caartaoc->setCodart($caartord->getCodart());
          $caartaoc->setDesart($caartord->getDesart());
          $caartaoc->setCodcat($caartord->getCodcat());
          $caartaoc->setCanord($caartord->getCanord());
          $caartaoc->setCanaju(H::toFloat($regajus->getCanajuord()));
          $caartaoc->setMonaju($preciounitario * H::toFloat($regajus->getCanajuord()));
          $monrec = self :: Calcular_RecargoAju($reg->getOrdcom(), H::toFloat($regajus->getCanajuord()), $regajus->getCodart(), $regajus->getCodcat(),$preciounitario,$regajus->getDesart());
  ...
  private static function Devolver_ArticulosOrden($reg, $grid) {
    ...
    foreach ($grid[0] as $artajuoc) {
      ...
      if ($claartdes=='S') $c->add(CaartordPeer :: DESART, $artajuoc->getDesart());
      ...
      if ($caartord) {
        ...
        $acum+=$caartord->getTotart();
        $caartord->save();
        
      $c = new Criteria();
      $c->add(CadisrgoPeer :: REQART,$reg->getOrdcom());
      $c->add(CadisrgoPeer :: CODART, $artajuoc->getCodart());
      if ($claartdes=='S') $c->add(CadisrgoPeer :: DESART, $artajuoc->getDesart());
      $c->add(CadisrgoPeer :: CODCAT, $artajuoc->getCodcat());
      $dato2 = CadisrgoPeer :: doSelect($c);
      if ($dato2) {
          $montorecargo =  (H::toFloat($caartord->getCantot())*$caartord->getPreart());
          foreach ($dato2 as $dato3)
      ...
      $c = new Criteria();
      $c->add(CaresordcomPeer :: ORDCOM, $reg->getOrdcom());
      $c->add(CaresordcomPeer :: CODART, $artajuoc->getCodart());
      if ($claartdes=='S') $c->add(CaresordcomPeer :: DESRES, $artajuoc->getDesart());
      $caresordcom = CaresordcomPeer :: doSelectOne($c);
      if ($caresordcom) {        
          $caresordcom->setRgoart($caartord->getRgoart() + $artajuoc->getMonrec());
          $caresordcom->setTotart($caartord->getTotart() + $artajuoc->getMontot());        
          $caresordcom->setCanaju($caartord->getCanaju() - $artajuoc->getCanaju());
          $caresordcom->setCantot($caartord->getCanord() + $caartord->getCanaju());
    ...
    $qa = new Criteria();
    $qa->add(CaordcomPeer::ORDCOM, $reg->getOrdcom());
    $rew=CaordcomPeer::doSelectOne($qa);    
    if ($rew){
        $rew->setMonord($acum);
        $rew->save();
    }
    ...
    $q = new Criteria();
    $q->add(CargosolPeer::REQART, $reg->getOrdcom());
    CargosolPeer::doDelete($q);


    $sql = "select reqart,codrgo,sum(coalesce(monrgo,0)) as monrgo from cadisrgo where reqart='" . $reg->getOrdcom() . "'  group by reqart,codrgo";
    $result = array();
    if (Herramientas::BuscarDatos($sql, $result)) {
      $i = 0;
      while ($i < count($result)) {
        $cargosol = new Cargosol();
        $cargosol->setReqart($reg->getOrdcom());
        $cargosol->setCodrgo($result[$i]['codrgo']);
        $cargosol->setMonrgo($result[$i]['monrgo']);
        $cargosol->setTipdoc(H::getX_vacio('ORDCOM','Caordcom','Doccom',$reg->getOrdcom()));
        $cargosol->save();
        $i++;
      }// while ($i<count($result))
    }
  ...
  public static function Obtener_codigo_OC($ordcom,$codart,$codcat,$desart){
    $claartdes=H::getConfApp2('claartdes', 'compras', 'almsolegr');
     $c = new Criteria();
     $c->add(CaartordPeer :: ORDCOM, $ordcom);
     $c->add(CaartordPeer :: CODART, $codart);
     if ($claartdes=='S') $c->add(CaartordPeer :: DESART, $desart);
     $c->add(CaartordPeer :: CODCAT, $codcat);
     $ordcomreg = CaartordPeer :: doSelectOne($c);
     if ($ordcomreg){
        return $codcat.'-'.$ordcomreg->getCodpar();
     }else return '';
  }
  ...
  private static function Hay_CompromisoCausado($reg, $grid) {
    ...
    foreach ($artAju as $art) {
      if (H::toFloat($art->getCanajuord())>0) {
      $codpre = self::Obtener_codigo_OC($reg->getOrdcom(),$art->getCodart(), $art->getCodcat(),$art->getDesart());
      $c = new Criteria();
  ...
  private static function Agrupar_por_Codigos($codpre, $arts, $tipo, $numordcom) {
    ...
    foreach ($arts as $artAju) {
      if (H::toFloat($artAju->getCanajuord())>0) {
      $codpreart = self::Obtener_codigo_OC($numordcom,$artAju->getCodart(), $artAju->getCodcat(),$artAju->getDesart());
      if ($codpreart == $codpre) {
        if ($tipo == 'A')
          //$cantidadgrupo = $cantidadgrupo + ($artAju->getPreciounitario() * $artAju->getCanaju());
          $cantidadgrupo = $cantidadgrupo + (H::toFloat($artAju->getCosart()) * H::toFloat($artAju->getCanajuord()));
        else {
          $monrec = self :: Calcular_RecargoAju($numordcom, H::toFloat($artAju->getCanajuord()), $artAju->getCodart(), $artAju->getCodcat(),H::toFloat($artAju->getCosart()), $artAju->getDesart());
  ...
  private static function Grabar_AjusteCompromiso($reg, $grid,$totalaju) {
    ...
    $cpimpcom = CpimpcomPeer::doSelectOne($c);
    if ($cpimpcom)
      $refereprecom = $cpimpcom->getRefere();
    else
      $refereprecom = 'NULO';
    ...
    if ($cpcompro) {
      ...
      if (Herramientas::getVerCorrelativo('coraju','cpdefniv',$r))
      {
        ...
        while (!$encontrado)
        {
          $numero='AC'. str_pad($r, 6, '0', STR_PAD_LEFT);
      ...
      $cpajuste = CpajustePeer::doSelectOne($c);
      if (!$cpajuste) {
        $cpajus = new Cpajuste();
        ...
        foreach ($detaju as $art) {
          ...
          if ($art->getCodart() != '' && (H::toFloat($art->getCosart()) * H::toFloat($art->getCanajuord())) > 0) {
            $codigoarticulo = $art->getCodart();
            $codigopresupuestario = self::Obtener_codigo_OC($reg->getOrdcom(),$art->getCodart(), $art->getCodcat(),$art->getDesart());
            ...
            if (!$movaju) {
              ...
              if ($arr) {
                foreach ($arr as $cadisrgo) {
                  if ($cadisrgo['codpre'] <> '') {
                    if (self::BuscarDatosenArreglo($arrmovajurgo, $cadisrgo['codpre'], $referencia, $indenc)) {//ya existe esa referencia y ese codpre
                      $indact = $indenc;
  ...
  private static function Eliminar_AjusteCompromiso($reg, $grid) {
    $nroajuste = $reg->getRefaju(); //'AC' . substr($reg->getOrdcom(), 2);
...
//**************** Programar Compra de Artículos-- AlmProComArt ***********************************
  ...
  public static function asignarPrioridadMonCot($reqart, $casolart) {
    $c = new Criteria();
    $c->add(CacotizaPeer :: REFSOL, $reqart);
    $c->add(CacotizaPeer::MONCOT,0,Criteria::GREATER_THAN);
  ...
  public static function asignarPrioridadTimEnt($reqart, $casolart) {
    ...
    if ($claartdes == 'S')
      $c->addAscendingOrderByColumn(CadetcotPeer::DESART);
  ...
  public static function MigrarVentas($clase, &$cadena, &$caderr=array(), $urlauto='') {
    ...
    if ($urlauto)
      ...
      $url = sfConfig::get('sf_upload_dir') . "//" . $clase->getArchivo();

    $file = fopen($url, "r");
    if ($file) {
      $md5arch = md5_file($url);
      
      $sql = "Select * from Camigtxtven where md5arch='$md5arch'";
      ...
      if (!H::BuscarDatos($sql, $output)) {
        ...
        while (!feof($file)) {
          ...
          if ($cont >= 4) {
            ...
            $y = new Criteria();
            $y->add(CaregartPeer::CODBAR, $codqpr);
            $resul = CaregartPeer::doSelectOne($y);
            if ($resul) {
              ...
              $c = new Criteria();
              $c->add(CaregartPeer::CODART, $resul->getCodart());
              $regart = CaregartPeer::doSelectOne($c);

              if ($regart) {
                $arreglo[$rr]["coding"] = $regart->getCoding();
                $arreglo[$rr]["ctavta"] = $regart->getCtavta();
                $arreglo[$rr]["desctavta"] = H::getX_vacio('codcta', 'contabb', 'descta', $regart->getCtavta());
                $arreglo[$rr]["ctacos"] = $regart->getCtacos();
                $arreglo[$rr]["desctacos"] = H::getX_vacio('codcta', 'contabb', 'descta', $regart->getCtacos());
                $arreglo[$rr]["codcta"] = $regart->getCodcta();
                $arreglo[$rr]["descodcta"] = H::getX_vacio('codcta', 'contabb', 'descta', $regart->getCodcta());
              } else {
                $arreglo[$rr]["coding"] = '';
                $arreglo[$rr]["ctavta"] = '';
                $arreglo[$rr]["desctavta"] = '';
                $arreglo[$rr]["ctacos"] = '';
                $arreglo[$rr]["desctacos"] = '';
                $arreglo[$rr]["codcta"] = '';
                $arreglo[$rr]["descodcta"] = '';
              }
              $rr++;
            } else {
              if (strlen($codqpr) > 4) {
        ...
        while ($i < count($arreglo)) {
          if ($arreglo[$i]["codart"] != "") {
            if ($arreglo[$i]["coding"] != "" && $arreglo[$i]["ctavta"] != "" && $arreglo[$i]["ctacos"] != "" && $arreglo[$i]["codcta"] != "") {
              // Ingreso
              self::guardarAsientos($arreglo[$i]["coding"], $arreglo[$i]["subtot"], $arreglo[$i]["iva"], $arrasientos, $pos);
              // Contabilidad
              self::guardarAsientosContables($arreglo[$i]["ctavta"], $arreglo[$i]["subtot"], $arreglo[$i]["iva"], $arrasientoscontables_ctavta, $pos);
              self::guardarAsientosContables($arreglo[$i]["codcta"], $arreglo[$i]["subtot"], $arreglo[$i]["iva"], $arrasientoscontables_codcta, $pos);
              self::guardarAsientosContables($arreglo[$i]["ctacos"], $arreglo[$i]["subtot"], $arreglo[$i]["iva"], $arrasientoscontables_ctacos, $pos);
            } else {
              return 557;
            }
          }
        ...
        if (count($arreglo) > 0 && count($arrasientoscontables_ctavta) > 0 && count($arrasientoscontables_ctacos) > 0 && count($arrasientoscontables_codcta) > 0) {
          $recart = CarecargPeer::doSelectOne(new Criteria());

          self::grabarMigracion($arreglo, $md5arch);
          $numcom_ctavta = self::grabarComprobanteMigracion($arrasientoscontables_ctavta, $arrasientoscontables_ctacos, $arrasientoscontables_codcta, $codalm, $recart);
          self::grabarIngreso($codalm, $arreglo, $cireging, $numcom_ctavta);
  ...
  public static function grabarComprobanteMigracion($arrasientoscontables_ctavta, $arrasientoscontables_ctacos, $arrasientoscontables_codcta, $codalm, $recart) {
    $numcom = '########';
    $reftra = '';
    $feccom = date('Y-m-d');
    $descom_venta = 'COMPROBANTE GENERADO POR MIGRACIÓN DE PUNTO DE VENTA DE LA LOCALIDAD ' . $codalm;
    $descom_inventario = 'COMPROBANTE GENERADO POR MIGRACIÓN DE INVENTARIO DE VENTA DE LA LOCALIDAD ' . $codalm;
    $total = 0;
    $recargo = 0;
    $debito = 0;
    $credito = 0;
    $grid = array();
    $grid[] = array();
    $cuentarecargo = ''; // cuenta de recargo
    $cuentadebito = '';  // Cuenta por cobrar
    $desctadebito = '';
    $tipcom = 'VEN';
    $total_ctavta = 0.0;
    $recargo_ctavta = 0.0;
    ...
    $cadefalm = CadefalmPeer::doSelectOne($c);
    if ($cadefalm) {
      $cuentadebito = $cadefalm->getCodcta();
      $desctadebito = $cadefalm->getDescta();
      if ($cuentadebito) {

        // Cuentas de Venta
        foreach ($arrasientoscontables_ctavta as $asiento) {
          $total_ctavta += $asiento['1'];
          $recargo_ctavta += $asiento['2'];
        }

        // asientos de articulos (Cuentas de Venta)
        foreach ($arrasientoscontables_ctavta as $conta) {
          $contabc1 = new Contabc1();
          $contabc1->setCodcta($conta['0']);
          $contabc1->setDescta($conta['3']);
          $contabc1->setMondebito(0);
          $contabc1->setMoncredito($conta['1']);

          $grid[0][] = $contabc1;
        }

        // Asiento de Recargo (Cuenta de Venta)
        if ($recargo_ctavta > 0.0) {
          $contabc1 = new Contabc1();
          $contabc1->setCodcta($recart->getCodcta());
          $contabc1->setDescta($recart->getDescta());
          $contabc1->setMondebito(0);
          $contabc1->setMoncredito($recargo_ctavta);
          $grid[0][] = $contabc1;
        }

        $debito = $total_ctavta + $recargo_ctavta;
        $credito = $debito;

        // Asiento de Credito
        $contabc1 = new Contabc1();
        $contabc1->setCodcta($cuentadebito);
        $contabc1->setDescta($desctadebito);
        $contabc1->setMondebito($debito);
        $contabc1->setMoncredito(0);
        $grid[0][] = $contabc1;


        $numcom_ctavta = Comprobante::SalvarComprobante($numcom, $reftra, $feccom, $descom_venta, $debito, $credito, $grid, true, $tipcom);
        ...
        foreach ($arrasientoscontables_ctacos as $asiento) {
          $total_ctacos += $asiento['1'];
          $recargo_ctacos += $asiento['2'];
        }

        // asientos de inventario de articulos
        foreach ($arrasientoscontables_ctacos as $conta) {
          $contabc1 = new Contabc1();
          $contabc1->setCodcta($conta['0']);
          $contabc1->setDescta($conta['3']);
          $contabc1->setMondebito(0);
        ...
        // Comprobante de inventario
        foreach ($arrasientoscontables_codcta as $asiento) {
          $total_codcta += $asiento['1'];
          $recargo_codcta += $asiento['2'];
        }

        // asientos de inventario de articulos
        foreach ($arrasientoscontables_codcta as $conta) {
          $contabc1 = new Contabc1();
          $contabc1->setCodcta($conta['0']);
          $contabc1->setDescta($conta['3']);
          $contabc1->setMondebito($conta['1']);
          $contabc1->setMoncredito(0);

          $grid[0][] = $contabc1;
        }

        $numcom_ctainv = Comprobante::SalvarComprobante($numcom, $reftra, $feccom, $descom_inventario, $debito, $credito, $grid, true, $tipcom);

        return $numcom_ctavta;
      }
    }
  }

  public static function grabarMigracion($arreglo, $md5arch) {
    $i = 0;
    while ($i < count($arreglo)) {
      ...
      if ($resg) {
        ...
        if ($tipoart == 'A') {
          ...
          $c->addOr(CaartalmubiPeer::CODUBI,'001-01');
          $c->addOr(CaartalmubiPeer::CODUBI,'001-02');
          $c->addOr(CaartalmubiPeer::CODUBI,'001-03');
          $c->addOr(CaartalmubiPeer::CODUBI,'001-04');
          $c->addAscendingOrderByColumn(CaartalmubiPeer::CODUBI);
          $c->addDescendingOrderByColumn(CaartalmubiPeer::EXIACT);
          $alms = CaartalmubiPeer::doSelect($c);
          if ($alms) {
            $canarttot = H::toFloat($arreglo[$i]["cantidad"]);
            foreach($alms as $index => $alm){
              if($canarttot>0){
                if ($alm->getExiact() >= $canarttot) {
                  $act2 = $alm->getExiact() - $canarttot;
                  $alm->setExiact($act2);
                  $alm->save();
                  $canarttot=0;
                }else{
                  if($alm->getExiact()==0 && count($alms)==($index+1)){
                    $act2 = $alm->getExiact() - $canarttot;
                    $alm->setExiact($act2);
                    $canarttot=0;
                  }else if ($alm->getExiact()!=0){
                    $exiact=abs($alm->getExiact());
                    $alm->setExiact($exiact-abs($canarttot));
                    $canarttot = $canarttot - abs($exiact);
                  }
                  $alm->save();
                }
              }
      ...
      $camigtxtven->setMd5arch($md5arch);
  ...
  public static function guardarAsientosContables($coding, $subtot, $iva, &$arrasientos, &$pos) {
    $i = 0;
    while ($i <= ($pos - 1)) {
      if (isset($arrasientos[$i]["0"])) {
        if ($arrasientos[$i]["0"] == $coding) {
          $arrasientos[$i]["1"] = ($arrasientos[$i]["1"] + $subtot);
          $arrasientos[$i]["2"] = ($arrasientos[$i]["2"] + $iva);
          return true;
    ...
    $arrasientos[$pos]["1"] = $subtot;
    $arrasientos[$pos]["2"] = $iva;
    $arrasientos[$pos]["3"] = H::getX('codcta', 'Contabb', 'descta', $coding);
    $pos = $pos + 1;
    return true;
  }

  public static function grabarIngreso($codalm, $arreglo, &$cireging, $numcom='') {
    ...
    if (Herramientas::BuscarDatos($sql, $result2)) {
      ...
      $cireging->setMontot($moning + $acumrec);
  ...
  public static function grabarImpIng($cireging, $arrasientos) {
    ...
    while ($i < count($arrasientos)) {
      ...
      $ciimping->setMontot(($arrasientos[$i]["1"]) + ($arrasientos[$i]["2"]));
  ...
  public static function GenerarCompromisov2($clasemodelo, $monto, &$refcom) {
   // try {
      $refcom = '';
      Presupuesto::correlativo('corcom','cpdefniv','########','refcom','Cpcompro',$refcom);

        $tipcom=H::getX_vacio('CODEMP','Cadefart','Tipdoccon','001');
        ...
        $reqaut=H::getX('TIPCOM','Cpdoccom','Reqaut',$tipcom);
        if ($reqaut=='S')              
        ...
        if ($per) {
          foreach ($per as $r) {
            ...
            $cpimpcom_new->setMonimp($r->getCancon()*$r->getMoncon());
  ...
  public static function salvardatosEnvioOC($clasemodelo, $grid) {
    $x = $grid[0];
    $j = 0;
    while ($j < count($x)) {
      if ($x[$j]->getMarque() == '1' && $clasemodelo->getEnviad() != '') {
        $x[$j]->setEnviad($clasemodelo->getEnviad());
        if ($clasemodelo->getEnviad() == 'R')
          $x[$j]->setFecrec($clasemodelo->getFecrec());
        else
          $x[$j]->setFecenv($clasemodelo->getFecrec());
        $x[$j]->save();
      }
      $j++;
    }
  }
   public static function salvardatosAprTras($clasemodelo,$grid)
   {
    $x=$grid[0];
    $i=0;
    while ($i<count($x))
    {   $c= new Criteria();
        $c->add(CatraalmPeer::CODTRA, $x[$i]->getCodtra());
        $catraalm= CatraalmPeer::doSelectOne($c);
        if($catraalm){
            $catraalm->setObstra($x[$i]->getObstra());
            $catraalm->setStatra('P');
            $catraalm->save();
        }
        $i++;
    }
   ...
   public static function salvarPlanAbastecimiento($capdaoc,$grid)
   {
       $capdaoc->save();
       self::grabarDetallePDA($capdaoc,$grid);
       self::grabarDistribucionAlm($capdaoc,$grid);
}
   
   public static function grabarDetallePDA($capdaoc,$grid)
   {
      $x=$grid[0];
      $j=0;
      while ($j<count($x))
      {      
         $x[$j]->setRefpda($capdaoc->getRefpda());
         $x[$j]->save();
         $j++;
      }
  ...
  public static function grabarDistribucionAlm($capdaoc,$grid)
  {
      $x=$grid[0];
      $j=0;
      $refpda=$capdaoc->getRefpda();
      while ($j<count($x))
      {
         if ($x[$j]->getDistri()!='')
         {
            $cadenarec=split('!',$x[$j]->getDistri());
            $r=0;
            while ($r<(count($cadenarec)-1))
            {
              $aux=$cadenarec[$r];
              ...
              {
                $c= new Criteria();
                $c->add(CaalmpdaPeer::REFPDA,$refpda);
                $c->add(CaalmpdaPeer::CODART,$x[$j]->getCodart());
                $c->add(CaalmpdaPeer::ID_REG,$x[$j]->getId());
                $c->add(CaalmpdaPeer::CODALM,$aux2[0]);
                $c->add(CaalmpdaPeer::CODUBI,$aux2[2]);
                CaalmpdaPeer::doDelete($c);

                $distribucion = new Caalmpda();
                $distribucion->setRefpda($refpda);
                $distribucion->setCodart($x[$j]->getCodart());
                $distribucion->setIdReg($x[$j]->getId());
                $distribucion->setCodalm($aux2[0]);
                $distribucion->setCodubi($aux2[2]);
                $cantidad= Herramientas::toFloat($aux2[4]);
                $distribucion->setCandis($cantidad);
                $distribucion->save();
              }
              $r++;
            }//while
  ...
  public static function salvarAsigCenUni($clasemodelo,$grid)
  {
      $x=$grid[0];
      $j=0;
      while ($j<count($x))
  ...
  public static function salvarSolCot($clasemodelo,$grid2)
  {
    $tienecorrelativo = false;
    if (Herramientas::getVerCorrelativo('corsolcot', 'cacorrel', $r)) {
      if ($clasemodelo->getNumsolcot() == '########') {
        $tienecorrelativo = true;
        $encontrado = false;
        while (!$encontrado) {
          $numero = str_pad($r, 8, '0', STR_PAD_LEFT);

          $sql = "select numsolcot from casolcot where numsolcot='" . $numero . "'";
          if (Herramientas::BuscarDatos($sql, $result)) {
            $r = $r + 1;
          } else {
            $encontrado = true;
        ...
        $solcot = str_pad($r, 8, '0', STR_PAD_LEFT);
        $clasemodelo->setNumsolcot($solcot);
      }
    }

    if ($tienecorrelativo) {
      Herramientas::getSalvarCorrelativo('corsolcot', 'cacorrel', 'Referencia', $r, $msg);
    }
    $clasemodelo->save();
    self::grabarDetalleProveedores($clasemodelo, $grid2);
  }

  public static function grabarDetalleProveedores($clasemodelo, $grid) {
    $x = $grid[0];
    $j = 0;
    while ($j < count($x)) {
      if ($x[$j]->getCodpro() != '') {
        $x[$j]->setNumsolcot($clasemodelo->getNumsolcot());
        $x[$j]->save();
      }
      $j++;
    }
  ...
  public static function grabarFirmasDocumentosPreimpresos($clasemodelo, $grid) {
    ...
    while ($j < count($x)) {
      if ($x[$j]->getNomfir() != '') {
        $x[$j]->setCoddirec($clasemodelo->getCoddirec());
        $x[$j]->setTipdoc($clasemodelo->getTipdoc());
        $x[$j]->setNumfir($j+1);
        $x[$j]->save();
      }
      $j++;
    ...
    return -1;
     ...
     public static function Calcular_RecargoAju($numordcom, $cantidad, $codart, $codcat,$precio,$desart) {
    ...
    if ($caartord) {      
      if ($cantidad > 0) {
        $monrgotot=0;
        $monuni=H::toFloat($cantidad)*H::toFloat($precio);
        $c1= new Criteria();
        $c1->add(CadisrgoPeer::REQART,$numordcom);
        $c1->add(CadisrgoPeer::CODART,$codart);
        $c1->add(CadisrgoPeer::CODCAT,$codcat);
        $c1->add(CadisrgoPeer::TIPDOC,H::getX_vacio('ORDCOM', 'Caordcom', 'Doccom', $numordcom));
        $result1=CadisrgoPeer::doSelect($c1);
        if ($result1){
          foreach ($result1 as $obj1) {
            $c2 = new Criteria();
            $c2->add(CarecargPeer :: CODRGO, $obj1->getCodrgo());
            $dato = CarecargPeer :: doselectOne($c2);
            if ($dato) {
              if ($dato->getTiprgo() == 'M') {
                $montorec = $obj1>getMonrgo();
              } else
              if ($dato->getTiprgo() == 'P') {
                $montorec = ($monuni * $dato->getMonrgo()) / 100;
              } 
              $monrgotot = $monrgotot + $montorec;
            }
          }
        }
        return $monrgotot;
...
public static function salvarRangosRotacion($grid){
    $x = $grid[0];
    $j = 0;
    while ($j < count($x)) {
        $x[$j]->save();      
