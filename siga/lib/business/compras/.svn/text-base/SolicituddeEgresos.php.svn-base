...
class SolicituddeEgresos
{
  public static function salvarAlmsolegr($solegreso,$grid,$grid2,$grid3,$generar,&$msj)
  {
  	if ($solegreso->getId()) //Validacion que permite guardar solamente la descripcion
  	{
        ...
        $reg1->setDesreq($solegreso->getObsreq());
			$reg1->save();
			//$msj=-1;
			$msj=-11;
			//return -1;
  ...
  public static function grabarSolicitudEgreso($solegreso,$grid,$grid2,$grid3,$generar,&$msj)
  {
    ...
    if ($solegreso->getId()!="")
      ...
      $solegrcordis=H::getConfApp2('solegrcordis', 'compras', 'almsolegr');
      if ($solegrcordis=='S') {
        if ($solegreso->getTipreq()=='A')
              $tipreq='corsolman';
        else if ($solegreso->getTipreq()=='S')
            $tipreq='corsolser';        
        else
            $tipreq='corsol';
      }else $tipreq='corsol';

    if (Herramientas::getVerCorrelativo($tipreq,'cacorrel',$r))
    {
      if ($solegreso->getReqart()=='########')
      {
        if ($fornumuni!="S") {
            ...
            if ($solegrcordis=='S') {
              ...
              if($solegreso->getTipreq()=='C') {
                 $numsol='SC'.(substr($numero,2,strlen($numero)));
              }else if($solegreso->getTipreq()=='A') {
                 $numsol='SM'.(substr($numero,2,strlen($numero)));
              }else if($solegreso->getTipreq()=='S') {
              ...
              $sql="select reqart from casolart where reqart='".$numsol."' and tipreq='".$solegreso->getTipreq()."'";
            }else {
  	          $numero=str_pad($r, 8, '0', STR_PAD_LEFT);
  	          $numsol='SC'.(substr($numero,2,strlen($numero)));
    ...
    $tieprc = Herramientas::getX_vacio('refprc','cpprecom','refprc',$numsol);
    if ($tieprc!='')
    {
      $msj=3003;
      return false;
    ...
    $c= new Criteria();
     ...
     if ($msj==-1)
     {
      ...
      {
      	 if ($fornumuni!="S") {
           Herramientas::getSalvarCorrelativo($tipreq,'cacorrel','Solicitud',$r,$msg);
      	 }else {
            $t= new Criteria();
        	$t->add(CauniforPeer::CODCAT,$solegreso->getUnires());
        	$reg= CauniforPeer::doSelectOne($t);
      ...
      if ($nuevoregistro=='S')
        $solegreso->setLoguse(sfContext::getInstance()->getUser()->getAttribute('loguse'));
     ...
     // Actualizar liprebas si aplica
     if($solegreso->getPrebas()!=''){
      $c = new Criteria();
      $c->add(LiprebasPeer::NUMPRE, $solegreso->getPrebas());
      $prebas = LiprebasPeer::doSelectOne($c);
      if($prebas){
        $prebas->setReqart($solegreso->getReqart());
        $prebas->save();
      }
     }
      if ($generar!="")
  ...
  public static function grabarSolicitudDetalle($solegreso,$grid)
  {
    ...
    $moneda=$solegreso->getTipmon();
    $moneda2=H::getX_vacio('CODEMP', 'Opdefemp', 'Codmon', '001');
    if ($moneda2!=$moneda)
            $valor=$solegreso->getValmon();
    else
    ...
    if (($x[$j]->getCodart()!="") and ($cantidad>0))
      {
        ...
        $x[$j]->setDesart(trim(strtoupper($x[$j]->getDesartsol())));
        $x[$j]->setCodpar($x[$j]->getCodpre());
        if ($x[$j]->getId()=='') $x[$j]->setCanord(0);
        if ($moneda2!=$moneda)
        {
          if ($x[$j]->getId()==''){
            $x[$j]->setCosto($x[$j]->getCosto()*$valor);
            $x[$j]->setMontot($x[$j]->getMontot()*$valor);
            $x[$j]->setMonrgo($x[$j]->getMonrgo()*$valor);
          }else {
            $x[$j]->setCosto($x[$j]->getCosto()*$valor*$valor);
            $x[$j]->setMontot($x[$j]->getMontot()*$valor*$valor);
            $x[$j]->setMonrgo($x[$j]->getMonrgo()*$valor*$valor);
          }
        }
        $nomcom=H::getX_vacio('CODART','Caregart','Nomcom',$x[$j]->getCodart());
        if ($nomcom!='')
          if ($x[$j]->getObserv()=='')
             $x[$j]->setObserv($nomcom);
           else
            $x[$j]->setObserv($x[$j]->getObserv().'-'.$nomcom);
        $x[$j]->save();
      }
    $j++;
   }
  ...
  public static function generaPrecompromiso($solegreso,$referencia,&$msj)
  {
    ...
    if (!$existe)
    {
      ...
      if ($moneda2!=$moneda) $cpprecom->setMonprc(H::toFloat($solegreso->getMonreq())*H::toFloat($valor,6));
      else $cpprecom->setMonprc($solegreso->getMonreq());
      ...
      if ($filsoldir=='S'){
        $cpprecom->setCoddirec($solegreso->getCoddirec());
      }
      $cpprecom->save();
      return true;
    }
  ...
  public static function generarImputacionesPrecompromiso($referencia)
  {
    ...
    if ($prenoiva=='S')
    {
       ...
       $cr->add(CadisrgoPeer::TIPDOC,$tipdoc);
       ...
       if ($regrec)
       {
        ...
        $cm = new Criteria();
        $cm->add(CpprecomPeer::REFPRC,$referencia);
        $maestro=CpprecomPeer::doSelectOne($cm);
        if ($maestro)
        {
          $maestro->setMonprc($maestro->getMonprc()-$acumrec);
          $maestro->save();
        }
    }
    ...
    { $result=array();
      if ($definicion->getAsiparrec()!='C')
      {
        ...
        $sql="select (A.Codcat||'-'||A.Codpar) as codpre, SUM(A.Canreq*A.Costo) as montot From Caartsol A, Caregart B WHERE A.codart=B.codart and A.reqart='".$referencia."' GROUP BY (A.codcat||'-'||A.codpar)";
      }
      else
      {
      ...
      if ($definicion->getAsiparrec()!='C' && $prenoiva!='S')
      {
        $result1=array();
      ...
      if ($manevento=='S')
      {
         $tipdoc=H::getX_vacio('REFPRC','Cpprecom','Tipprc',$referencia);
         $codeve=H::getX_vacio('REQART','Casolart','Codeve',$referencia);
         $e= new Criteria();
         $e->add(CpimpprcPeer::REFPRC,$referencia);
         $trajo= CpimpprcPeer::doSelect($e);
         if ($trajo)
         {
           foreach ($trajo as $objprc) {
              $cpdiseve= new Cpdiseve();
              $cpdiseve->setRefdoc($objprc->getRefprc());
              $cpdiseve->setCodpre($objprc->getCodpre());
              $cpdiseve->setCodeve($codeve);
              $cpdiseve->setMoneve($objprc->getMonimp());
              $cpdiseve->setTipdoc($tipdoc);
              $cpdiseve->setTipmov('PRC');
              $cpdiseve->save();
           }
         }
      }
  ...
  public static function eliminarPrecompromiso($code)
  {
    ...
    $t = new Criteria();
    $t->add(CpdisevePeer::REFDOC,$code);
    $t->add(CpdisevePeer::TIPMOV ,'PRC');
   ...
   public static function chequearDisponibilidadPresupuesto($solegreso,$grid,$j,$id,$tiporec,&$sobregiro)
   {
     ...
     while ($z<count($l))
     {
       ...
       if ($l[$j]->getCodigopre()==$codigopresupuestario)
       {
         ...
         if ($moneda2!=$moneda) $costo=$l[$z]->getCosto()*$valor;
         else  $costo=$l[$z]->getCosto();
    ...
    if ($id!="")
    {
     ...
     while ($b<count($l))
     {
       ...
       if ($l[$j]->getCodigopre()==$codigopresupuestario)
       {
         if ($moneda2!=$moneda) $total=$l[$b]->getMontot2()*$valor;
         else $total=$l[$b]->getMontot2();
    ...
    if ($l[$j]->getCodigopre()!="")
    {
     ...
     $mes=substr($solegreso->getFecreq(),5,2);

      if (Herramientas::Monto_disponible_ejecucion($anno,$l[$j]->getCodigopre(),$mes,$mondis))
      {
        if ($mitotal > $mondis)
        {
   ...
   public static function ultimoChequeo($solegreso,$grid,$grid2,$id,$tiporec,&$msjuno,&$codi1,&$msjdos,&$codi2)
   {
    ...
    //Chequear disponibilidad por articulo
    $j=0;       
    while ($j<count($x))
    {
        $a= new Criteria();
        $a->add(CpasiiniPeer::PERPRE,'00');
        $a->add(CpasiiniPeer::CODPRE,$x[$j]->getCodigopre());
        $data2= CpasiiniPeer::doSelectOne($a);
        if (!$data2)
        {
            $msjuno=512; $codi1=$x[$j]->getCodart();
            return false;
        }
        
        $codpre=H::getCodPreDis($x[$j]->getCodigopre());
        $cantidad=$x[$j]->getCanreq();
        if ($moneda2!=$moneda) $costo=$x[$j]->getCosto()*$valor;
        else  $costo=$x[$j]->getCosto();
        $recargo=$x[$j]->getMonrgo();
        if ($tiporec!='C')        
    ...
    {
      $disponible = H::Monto_disponible($datosGrid[$p]["codpre"],$solegreso->getFecreq());
      if($datosGrid[$p]["monimp"] > $disponible){
        $mdis=$disponible;
        $mimp=$datosGrid[$p]["monimp"];
        $resta=$datosGrid[$p]["monimp"]-$disponible;
        $msjuno=113; $codi1=$datosGrid[$p]["codart"].", el monto a imputar es de: ".H::FormatoMonto($mimp) ." y ". " su Código presupuestario asociado ".$datosGrid[$p]["codpre"]." tiene monto disponible es: ".H::FormatoMonto($mdis).", necesita una disponibilidad adicional de: ".H::FormatoMonto($resta);
        return false;
      }
      $p++;
    }
  ...
  if ($prenoiva!='S') {
    ...
    while ($h < count($arr_recargo))
    {
         ...
         if ($data)
         {
            if ($tiporec=='P') {          
               ...
               if ($trajo) {
                ...
                if (!$data2)
                {
                    $msjdos=2114; $codi2=$arr_recargo[$h]['codrgo'];
                    return false;
                }
               }else {
                   $msjdos=2113; $codi2=$arr_recargo[$h]['codrgo'];
                    return false;
               }
               ...
               if ($trajo) {
                    ...
                    if (!$data2)
                    {
                        $msjdos=2114; $codi2=$arr_recargo[$h]['codcat'].'-'.$data->getCodpre(); //$arr_recargo[$h]['codrgo'];
                        return false;
                    }
               }else {
                   $msjdos=2113; $codi2=$arr_recargo[$h]['codcat'].'-'.$data->getCodpre();//$arr_recargo[$h]['codrgo'];
                    return false;
               }
     ...
     {
      $disponible = H::Monto_disponible($datosGridRec[$g]["codpre"],$solegreso->getFecreq());
      if($datosGridRec[$g]["monimp"] > $disponible){
        $mdis=$disponible;
        $mimp=$datosGridRec[$g]["monimp"];
        $resta=$datosGridRec[$g]["monimp"]-$disponible;
        $msjdos=114; $codi2=$datosGridRec[$g]["codrgo"].", el monto a imputar es de: ".H::FormatoMonto($mimp) ." y ". " su Código presupuestario asociado ".$datosGridRec[$g]["codpre"]." tiene monto disponible es: ".H::FormatoMonto($mdis).", necesita una disponibilidad adicional de: ".H::FormatoMonto($resta);
        return false;
      }
      $g++;
     }
  ...
  public static function montoDisponible($codigopre,$perhas='12')
  {
    $c= new Criteria();
    ...
    if (count($resul)>0)
    {
      ...
      if (Herramientas::BuscarDatos($a,$result))
      {
        ...
        if (Herramientas::Monto_disponible_ejecucion($anno[0],$var,$perhas,$mondis))
        {
  ...
  while ($j<count($x))
  {
   ...
   if ($marcado=="1")
   {
    ...
    $cadenarec=split('!',$x[$j]->getDatosrecargo());
        ...
        CadisrgoPeer::doDelete($c);
          ...
          if ($moneda2!=$moneda) $distribucion->setMonrgo($montorecargo*H::toFloat($valor,6));
          else $distribucion->setMonrgo($montorecargo);
          $distribucion->setTipdoc($tipdoc);
          $distribucion->save();
          }
          $r++;
   ...
   if (!empty($z[$l]))
   {
    while ($l<count($z))
    {
      $marcado=$z[$l]->getCheck();
      if ($marcado=="1")
      {
        if ($z[$l]->getDatosrecargo()!='')
        {
          $cadenarec=split('!',$z[$l]->getDatosrecargo());
          $r=0;
          while ($r<(count($cadenarec)-1))
          {
            $aux=$cadenarec[$r];
            ...
            if ($aux2[0]!="" )
            {
              ...
              $c= new Criteria();
              $c->add(CadisrgoPeer::REQART,$requi);
              $c->add(CadisrgoPeer::CODART,$z[$l]->getCodart());
              if ($claartdes=='S') $c->add(CadisrgoPeer::DESART,trim($z[$l]->getDesart()));
              $c->add(CadisrgoPeer::CODCAT,$z[$l]->getCodcat());
              $c->add(CadisrgoPeer::CODRGO,$aux2[0]);
              $c->add(CadisrgoPeer::TIPDOC,$tipdoc);
              CadisrgoPeer::doDelete($c);
            }
 ...
 */
  public static function salvarAlmaprsolegr($clasemodelo,$grid,$login,$aprobpresu)
  {
    ...
    while ($j < count($x)) {
      ...
      if ($x[$j]->getRechazar()=='1')
      {
        if ($aprobpresu=='S' && $aprob=='S')
        {
          if ($aprcompre=='S'){
            if ($modulo=='presupuesto'){
              self::eliminarPrecompromiso($x[$j]->getReqart());
            }else {
              $x[$j]->setUsuapr(null);
              $x[$j]->setFecapr(null);
              $x[$j]->setAprreq(null);
  ...
  public static function totalImputacion($reqart)
  {
    ...
    if ($prenoiva=='S')
    {
       $tipdoc=Compras::ObtenerTipoDocumentoPrecompromiso();
       $cr = new Criteria();
       $cr->add(CadisrgoPeer::REQART,$reqart);
       $cr->add(CadisrgoPeer::TIPDOC,$tipdoc);
       $regrec = CadisrgoPeer::doSelect($cr);
       if ($regrec)
       {
      ...
      public static function chequearDisponibilidadPresupuesto2($solegreso,$grid,$j,$id,$tiporec,&$sobregiro)
   {
    ...
    if ($l[$j]->getCodigopre()!="")
    {
      ...
      if (Herramientas::BuscarDatos($sql,$result))
      {
        $mondis=self::montoDisponible($l[$j]->getCodigopre());
        if ($mitotal > $mondis)
        {
          $chequeardisponibilidadpresupuesto=false;
          $sobregiro=true;
     ...
     public static function montoDisponible2($codigopre)
  {
    ...
    if (count($resul)>0)
    {
      ...
      if (Herramientas::BuscarDatos($a,$result))
      {
        ...
        $anno=split('-',$resul->getFeccie());
        $var = substr($codigopre, 0, $longitud);
        if (Herramientas::Monto_disponible_ejecucion($anno[0],$var,$anno[1],$mondis))
        {
           $montodisponible=$mondis;
        }
        else{ $montodisponible=0;}
   ...
   public static function verificarDispGenComp($solegreso,&$msjuno,&$codi1,&$msjdos,&$codi2,&$codi3,&$msjtres,&$mdis,&$mimp,&$resta)
   {
    $msjuno=-1; $codi1="";
    $msjdos=-1; $codi2="";
    $msjtres=-1;
    ...
    if ($objetos)
    {
       foreach ($objetos as $obj)
       {
          ...
          $t= new Criteria();
          $t->add(CpdeftitPeer::CODPRE,$obj->getCodcat().'-'.$obj->getCodpre());
          $trajo= CpdeftitPeer::doSelectOne($t);
          if ($trajo) {
            $a= new Criteria();
            $a->add(CpasiiniPeer::PERPRE,'00');
            $a->add(CpasiiniPeer::CODPRE,$obj->getCodcat().'-'.$obj->getCodpre());
            $data2= CpasiiniPeer::doSelectOne($a);
            if (!$data2)
            {
                $msjuno=512; $codi1=$obj->getCodart(); $codi3=$obj->getCodcat().'-'.$obj->getCodpre();
                return false;
            }
          }else {            
             $msjuno=2103; $codi1=$obj->getCodart(); $codi3=$obj->getCodcat().'-'.$obj->getCodpre();
             return false;
          }

            $codpre=H::getCodPreDis($obj->getCodcat().'-'.$obj->getCodpre());
            $cantidad=$obj->getCanreq();
            if ($moneda2!=$moneda) $costo=$obj->getCosto()*$valor;
            else  $costo=$obj->getCosto();
            ...
            if ($pos==0)
            {
             ...
             $datosGrid[$i-1]["codpre"]=$obj->getCodcat().'-'.$obj->getCodpre();
            }
            else
            {
             $datosGrid[$pos-1]["monimp"]=$datosGrid[$pos-1]["monimp"]+$monto;
   ...
   public static function chequearDisponibilidadPresupuesto3($solegreso,$objetos,$codpre,$tiporec,&$sobregiro)
   {
    ...
    if ($codpre!="")
    {
     ...
     $mes=substr($solegreso->getFecreq(),5,2);
      ...
      if (Herramientas::Monto_disponible_ejecucion($anno,$codpre,$mes,$mondis))
      {
        if ($mitotal > $mondis)
        {
          $chequeardisponibilidadpresupuesto=false;
   ...
   public static function generarDetalleSoli($grid, &$arreglodet){
      $x=$grid[0];
      $j=0;
      $i=0;
      $arreglodet=array();
      ...
      {
        if ($x[$j]->getDatossol()!=''){
          $cadenarec=split('!',$x[$j]->getDatossol());
          $r=0;
          while ($r<(count($cadenarec)-1))
          {
            $aux=$cadenarec[$r];
            ...
            if ($aux2[0]!="" )
            {
              ...
              if ($pos==0)
              {
               $i=count($arreglodet)+1;               
               $arreglodet[$i-1]["codart"]=$aux2[1];
               $arreglodet[$i-1]["codcat"]=$aux2[2];
               $arreglodet[$i-1]["canreq"]='0';
               $arreglodet[$i-1]["canord"]="0,00";
               $arreglodet[$i-1]["costo"]="0,00";
               $arreglodet[$i-1]["monrgo"]="0,00";
               $arreglodet[$i-1]["montot"]="0,00";
               $arreglodet[$i-1]["codigopre"]="0,00";
               $arreglodet[$i-1]["numreqori"]=$aux2[0];
               $arreglodet[$i-1]["id"]="9";
              }
              else
              {
  ...
  public static function CalcularRecargos2($tiprgo,$monrgo,$monbasart,$mondes,$monrgoori)
  {
    if ($tiprgo=='M')
    {
     $recargo= $monrgo;
    ...
    else if ($tiprgo=='P')
    {
      //echo " monbasart: ".$monbasart;
      //echo " monrgo: ".$monrgo." ";
      $totconrec= $monbasart-$mondes;

      $recargo = (($monbasart*$monrgo)/100);
      $recargocondes = (($totconrec*$monrgo)/100);
      if (H::toFloat($recargocondes,4)==H::toFloat($monrgoori,4))
        $recargo=$recargocondes;
      //echo "calculo > ".$recargo;
     //exit();
    }
    else
