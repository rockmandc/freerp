...
class Proveedor
{
   ...
   */
    public static function salvarAlmregpro($caprovee,$grid,$gridC,$gridR,$gridRE,$manprocor,$mascararif)
    {
      ...
      {
        $trajo=self::BuscarCorrelativoActiv($caprovee,$r);
      }else $trajo=-1;

      if ($trajo == -1){
      	self::Grabar_Proveedor($caprovee,$manprocor,$mascararif,$r);
...
/**
   ...
   * @return void
   */
    public static function Grabar_Proveedor($caprovee,$manprocor,$mascararif,$r){

    if ($mascararif=='S')
        {
           $otralet=H::getConfApp2('otrlet', 'compras', 'almregpro');
           if ($otralet!='S')
   ...
   */
    public static function Grabar_Beneficiario($caprovee)
    {
        ...
        if (isset($opbenefi))
        {
          ...
          $opbenefi->setCodban($caprovee->getCodban());
          $opbenefi->setNumcueb($caprovee->getNumcue());
          $opbenefi->setTipper($caprovee->getNitpro());
          ...
          $opbenefi->setCodtipban($caprovee->getCodtip());
          $opbenefi->save();
        ...
        else
        {
          ...
          $codti= OptipbenPeer::doSelectOne(new Criteria());
          if ($codti)
            $codtipben=$codti->getCodtipben();
          else
            $codtipben=null;
...
/**
  ...
  public static function validarCodart($articulo)
  {
   ...
   public static function BuscarCorrelativoActiv(&$caprovee,&$r)
   {
     $y= new Criteria();
    ...
    while (!$encontrado)
    {
      ...
      $sql="select codpro from caprovee where codpro='".$codpro."'";
      if (Herramientas::BuscarDatos($sql,$result))
      {
        $r=$r+1;
      }
