<?php
/**
 * Cierre de Nominas Especiales: Clase estática para procesar el cierre de pre-nóminas especiales
 *
 * @package    Roraima
 * @subpackage nomina
 * @author     $Author: dmartinez $ <desarrollo@cidesa.com.ve>
 * @version SVN: $Id: CierredeNominaEspecial.class.php 61684 2015-04-23 20:13:30Z dmartinez $
 *
...
class CierredeNominaEspecial
{
 ...
 public static function procesoCierre($codnomina,$ultfec,$profec,&$msj,$codnomesp,$numsem, $intpre)
 {
   $msj = '';
   $profecha = Herramientas::getX('CODNOM','Npnomina','Profec',$codnomina);

   if (self::generarNompag($codnomina,$ultfec,$profec,$codnomesp,$sobregiro,$intpre))
   {
	   	if ($intpre=='S')
	   	{
 ...
 public static function generarNompag($codnomina,$ultfec,$profec,$codnomesp,&$sobregiro,$intpre)
 {
   self::eliminarCierre($codnomina,$ultfec,$profec,$codnomesp);
   if(self::generar($codnomina,$ultfec,$profec,$codnomesp,$sobregiro,$intpre))
   { return true;}
   else {return false;}
 }
 ...
 public static function cierre($codnomina,$ultfec,$profec,$codnomesp,$numsem)
 {
 	try{
		...
		$sql1 = "INSERT INTO NPHISCON (codnom,codemp, codcar, codcon, fecnom, monto, codcat, codpar, codescuela, codniv, codtipgas, nomcon, numrec, cantidad, fecnomdes, especial, fecnomespdes, fecnomesphas,
			     codnomesp, nomnomesp, codban, nomban, cuenta_banco, nomemp, cedemp, nomcar, desniv, nomcat, asided, numsem, codtippag, impcpt, ordpag, afepre, codtipemp,nomnom, moncar)
			     (SELECT A.CodNom,A.CodEmp,A.CodCar,A.CodCon,A.FecNom,A.Saldo,categoriaemp(A.CodNom,A.CodEmp,A.CodCar,A.CodCon),partidaconcepto(A.CodCon,A.CodNom,A.CodCar),' ',D.CodNiv,B.CODTIPGAS,C.NOMCON,A.NUMREC,
					...
					(CASE when D.CodTipPag='01' then D.Codban else D.Codemp end),
                                        (select nomban from npbancos where codban=D.codban) as nomban,G.CUENTA_BANCO,D.NOMEMP,D.CEDEMP,F.NOMCAR,H.DESNIV,B.NOMCAT, A.asided,'".$numsem."', D.codtippag, C.impcpt, C.ordpag, C.afepre, B.codtipemp, A.nomnom, F.Suecar
					...
					,NPASICAREMP B,NPDEFCPT C,NPCARGOS F
					WHERE
...
//exit($sql1);
  Herramientas::insertarRegistros($sql1);
		...
		$sql2 = "INSERT INTO NPHISCON (codnom,codemp, codcar, codcon, fecnom, monto, codcat, codpar, codescuela, codniv, codtipgas, nomcon, numrec, cantidad, fecnomdes, especial, fecnomespdes, fecnomesphas,
			     codnomesp, nomnomesp, codban, nomban, cuenta_banco, nomemp, cedemp, nomcar, desniv, nomcat, asided, numsem, codtippag, impcpt, ordpag, afepre, codtipemp,nomnom,moncar)
				 (SELECT A.CodNom,A.CodEmp,A.CodCar,A.CodCon,A.FecNom,A.Saldo,categoriaemp(A.CodNom,A.CodEmp,A.CodCar,A.CodCon),partidaconcepto(A.CodCon,A.CodNom,A.CodCar),' ',D.CodNiv,B.CODTIPGAS,C.NOMCON,A.NUMREC,
					A.CANTIDAD, A.FECNOMDES, A.ESPECIAL,A.FECNOMESPDES,A.FECNOMESPHAS,A.CODNOMESP,A.NOMNOMESP,D.CODBAN,(select nomban from npbancos where codban=D.codban) as nomban,H.CUENTA_BANCO,
					D.NOMEMP,D.CEDEMP,G.NOMCAR,I.DESNIV,
					B.NOMCAT, A.asided, '".$numsem."', D.codtippag, C.impcpt, C.ordpag, C.afepre, B.codtipemp, A.nomnom, G.Suecar
					FROM NPNOMCAL A,NPASICAREMP B,NPDEFCPT C,NPHOJINT D left outer join NPEMPLEADOS_BANCO H on D.CODEMP = H.CODEMP AND D.CODBAN = H.CODBAN and H.codnom='".$codnomina."' left outer join NPESTORG I on D.CODNIV=I.CODNIV ,NPCONCEPTOSCATEGORIA E,NPCARGOS G
					WHERE  A.CODNOMESP='".$codnomesp."' AND ESPECIAL='S' AND A.CODNOM='".$codnomina."' AND  B.STATUS='V' AND
...
//Las Nóminas Especiales No actualizan fechas
 ...
 public static function Validarcodprenomina($codnomina,$codnomesp,$fecha,&$sobregiro=false, &$codpred='', $intpre = 'N')
 {
...
//    unset($dato1);
  ...
  public static function Validarcodprenominav2($codnomina,$codnomesp, $fecha,&$sobregiro=false, $intpre = 'N'){
     $anopresu = substr(Herramientas :: getX('codemp', 'cpdefniv', 'fecper', '001'), 0, 4);
     $sql="select a.*
            from
            ...
            WHERE a.CODNOM='".$codnomina."' and a.CODNOMESP='".$codnomesp."' 
            AND a.SALDO <> 0 AND 
            a.ESPECIAL='S' and
