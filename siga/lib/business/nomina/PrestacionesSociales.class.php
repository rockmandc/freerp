 ...
 * @package    Roraima
 * @subpackage nomina
 * @author     $Author: ogutierrez $ <desarrollo@cidesa.com.ve>
 * @version SVN: $Id: PrestacionesSociales.class.php 59813 2014-12-05 03:27:47Z ogutierrez $
 *
...
End Function*/
  ...
  public static function saveNppresoc($nppresoc,$grid, $detalle)
  {
  	if($grid[4][0]=='V')
  	{
    ...
    else if($grid[4][0]=='142')
    {
     // if ((self::GrabarEncabezadoNuevoRegimen142($nppresoc,$grid)) !=-1){ return 0;}
      if ($detalle=="S"){
          if ((self::GrabarDetallesNuevoRegimen142($nppresoc,$grid)) !=-1){ return 0;}
      }
    }
    else
  	{
  ...
  public static function GrabarEncabezadoNuevoRegimen($nppresoc,$grid)
  {
      ...
      $nppresoc->setCodcon($x3[0]["codtipcon"]);   //!CodCon = strCodCon
      //$nppresoc->setCodcon($nppresoc->getCodtipcon());   //!CodCon = strCodCon
 ...
 * */
      ...
      //totales
      $nppresoc->setIntacu( H::formatonum($x2[0])+H::formatonum($x2[5]));  //getTotintacu   --> !IntAcu = dblTotIntAcu
      $nppresoc->setAdeint( H::formatonum($x2[1])+H::formatonum($x2[6]));  //getTotmonadeint --> !adeint = dblTotAdeInt
      $nppresoc->setAntacu( H::formatonum($x2[4])+H::formatonum($x2[9]));  //getTotmonant  --> !antacu = dblTotAntAcu
      $nppresoc->setAdepre( H::formatonum($x2[3])+H::formatonum($x2[8]));  //getTotcapitalact   --> !AdePre = dblTotAdePre
	$nppresoc->setMonpre(H::formatonum($x2[4])+H::Formatonum($x2[0])+H::Formatonum($x2[5])+H::Formatonum($x2[9]));  //getTotintacu + getTotmonant
      $nppresoc->save();
    ...
    while ($j<count($x))
    {
     ...
     // $x[$j]->settipo('');
      ...
      if ($x[$j]->getTipo()=='AJUSTE DIAS ADICIONALES NO DEPOSITADOS')
      {
      	...
      	$x[$j]->setTipo('A');
      }else if ($x[$j]->getTipo()=='DEPOSITADOS ART 142'){
        $x[$j]->setTipo('N');
      }else
      {
      	$x[$j]->setTipo('');
...
//!AdePre = dblTotAdePre (capitalact del ultimo registro)
  ...
  public static function GrabarEncabezadoNuevoRegimen142($nppresoc,$grid)
  {
    try
    {
    $c = new Criteria();
 ...
 * */
  ...
  public static function GrabarDetallesNuevoRegimen142($nppresoc,$grid)
  {
    try{
        $c = new Criteria();
        $c->add(npimppresocPeer::CODEMP,$nppresoc->getCodemp());
        $c->add(npimppresocPeer::TIPO,"N");
        npimppresocPeer::doDelete($c);
