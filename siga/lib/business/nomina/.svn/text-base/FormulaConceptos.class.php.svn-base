...
class FormulaConceptos {
    ...
    public static function CargarListasVariables($codcon,$codnom,$tipo) {
			...
			{
                                /* ESTO SOLO MUESTRA EN LA LISTA DE HISTÓRICOS LOS CONCEPTOS QUE ESTÉN DEFINIDOS COMO MONTO Y/O CANTIDAD
                                 * 
				$codigo=$codnom;
				$c2 = new Criteria();                                                               
				...
				if ($obj2)
				{
					...
					{                                         
						...
						$c3->add(NpdefmovPeer::CODCON,$res->getCodcon());
                                                $c3->add(NpdefmovPeer::CODNOM,$codigo);
						$c3->setDistinct();
						...
						}else
						{
							...
							//$r++;
                                ...
                                //MUESTRA TODOS LOS CONCEPTOS DEFINIDOS PARA TODAS LAS NOMINAS DEFINIDAS
                                $criterio = "select a.codcon,a.nomcon,b.codnom from npdefcpt a,npnomina b order by b.codnom,a.codcon";
                                if (Herramientas :: BuscarDatos($criterio, $datosper)) {                                  
                                   foreach ($datosper as $dat) {
                                       $v = "H" . $dat["codnom"] . $dat["codcon"];
				       $objlist2[$v] = "H" . $dat["codnom"] . $dat["codcon"] . " " . $dat["nomcon"];
