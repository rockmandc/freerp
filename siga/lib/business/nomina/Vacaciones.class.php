...
*/
  ...
  public static function actualizar_vacsalidas($npvacsalidas,$gridd)
  {
  	$grid=$gridd[0];
	foreach($grid as $r)
	{
		if($r['diasvac']>0 && $r['diasajust']>0)
		{
			#Actualizamos npvacdsifrute
		    $c4= new Criteria();
		    $c4->add(NpvacdisfrutePeer::CODEMP,$npvacsalidas->getCodemp());
		    ...
		    if ($objNpvacdisfrute)
		    {
			$objNpvacdisfrute->setDiasdisfrutados(($r['diasdisfrutados']));
			$objNpvacdisfrute->setDiasajust($r['diasajust']);
			$objNpvacdisfrute->save();
                    }

                    $c5= new Criteria();
		    $c5->add(NpvacsalidasDetPeer::CODEMP,$npvacsalidas->getCodemp());
		    $c5->add(NpvacsalidasDetPeer::PERINI,$r['perini']);
		    $c5->add(NpvacsalidasDetPeer::PERFIN,$r['perfin']);
		    $objNpvacsalidasdet = NpvacsalidasDetPeer::doSelectOne($c5);
		    if ($objNpvacsalidasdet)
		    {
			$objNpvacsalidasdet->setDiasdisfrutados(($r['diasdisfrutados']));
			$objNpvacsalidasdet->setDiasajust($r['diasajust']);
			$objNpvacsalidasdet->save();
                    }
		}
	}
