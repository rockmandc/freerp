<?php
/**
 * Empleados Bancos: Clase estática para el manejo de empleados y bancos
 *
 * @package    Roraima
 * @subpackage nomina
...
public static function Grabar_grid_Vacdiafer($npvacdiafer_new,$grid)
	{
		$val=-1;
    $codniv=$npvacdiafer_new->getCodniv();
		  $x=$grid[0];
      if ($codniv!='')
		    $sql="delete from npvacdiafer where codniv='".$codniv."'";
      else
               	...
               	if ($v->getMes()!='')
               	   {
                        ...
                        if ($codniv!='')
                          $npvacdiafer_new->setCodniv($codniv);
