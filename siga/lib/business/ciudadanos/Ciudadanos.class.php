...
class Ciudadanos {
    ...
    public static function actualizarAciayudas($Atayudas,$griditems,$gridrecaudos)
    {
      $result=array();
        ...
        if($Atayudas->getId()=='') $Atayudas->setUsucre(substr(sfContext::getInstance()->getUser()->getAttribute('loguse','Sin Autenticar'),0,50));
      else $Atayudas->setUsumod(substr(sfContext::getInstance()->getUser()->getAttribute('loguse','Sin Autenticar'),0,50));
        ...
        if($Atayudas->getNroexp()=='XXXXXX' || $Atayudas->getNroexp()=='XXXXX' || $Atayudas->getNroexp()=='XXXX' || $Atayudas->getNroexp()=='XXX' || $Atayudas->getNroexp()=='XX' || $Atayudas->getNroexp()=='X'){
          ...
          if($empresa){
            if($empresa->getPrecordon()!=""){
              $Atayudas->setNroexp($empresa->getPrecordon().$Atayudas->getNroexp());
            }
          }
        }
