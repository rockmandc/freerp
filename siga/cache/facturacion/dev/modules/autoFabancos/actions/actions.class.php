...
class autoFabancosActions extends sfActions
{
  ...
  {
    return $this->forward('fabancos', 'list');
  }
  ...
  public function executeList()
  {
    ...
    $this->filters = $this->getUser()->getAttributeHolder()->getAll('sf_admin/fabancos/filters');


     // 15    // pager
    $this->pager = new sfPropelPager('Fabancos', 15);
    $c = new Criteria();
    $this->c ? $c=$this->c : '';
  ...
  {
    return $this->forward('fabancos', 'edit');
  }
  ...
  {
    $this->params=array();
    $this->fabancos = $this->getFabancosOrCreate();

    $this->editing();

    if ($this->getRequest()->getMethod() == sfRequest::POST)
    {
      $this->updateFabancosFromRequest();

      if($this->saveFabancos($this->fabancos) ==-1){
        {$this->setFlash('notice', 'Your modifications have been saved');

         $id= $this->fabancos->getId();
         $this->SalvarBitacora($id ,'Guardo');}

        if ($this->getRequestParameter('save_and_add'))
        {
          return $this->redirect('fabancos/create');
        }
        else if ($this->getRequestParameter('save_and_list'))
        {
          return $this->redirect('fabancos/list');
        }
        else
        {
            return $this->redirect('fabancos/edit?id='.$this->fabancos->getId());
        }
  ...
  {
    $this->fabancos = FabancosPeer::retrieveByPk($this->getRequestParameter('id'));
    $this->forward404Unless($this->fabancos);

    try
    {
      $this->deleteFabancos($this->fabancos);
      $id= $this->fabancos->getId();
   ...
   *
   */
  protected function saveFabancos($fabancos)
  {
    ...
    try {
      ...
      $this->coderr = $this->saving($fabancos);
   ...
   *
   */
  protected function saving($fabancos)
  {
    $fabancos->save();
    return -1;
   ...
   *
   */
  protected function deleting($fabancos)
  {
  	$fabancos->delete();
    return -1;
   ...
   *
   */
  protected function deleteFabancos($fabancos)
  {
    try {
      ...
      // OJO ----> Eliminar esta linea al modificar este método
      $this->coderr = $this->deleting($fabancos);
   ...
   *
   */
  protected function updateFabancosFromRequest()
  {
    $fabancos = $this->getRequestParameter('fabancos');

    $fields = FabancosPeer::getFieldNames();

    if(array_search('Codalmusu', $fields))
    {
      if (isset($fabancos['codalmusu']))
      {
        $this->fabancos->setCodalmusu($fabancos['codalmusu']);
      }
    }
    if (isset($fabancos['codban']))
    {
      $this->fabancos->setCodban($fabancos['codban']);
    }
    if (isset($fabancos['nomban']))
    {
      $this->fabancos->setNomban($fabancos['nomban']);
    }
    if (isset($fabancos['grid']))
    {
      $this->fabancos->setGrid($fabancos['grid']);
    }
   ...
   *
   */
  protected function getFabancosOrCreate($id = 'id')
  {
    if (!$this->getRequestParameter($id))
    {
      $fabancos = new Fabancos();
    }
    else
    {
      $fabancos = FabancosPeer::retrieveByPk($this->getRequestParameter($id));

      $this->forward404Unless($fabancos);
    }
  ...
  protected function processFilters()
  {
    if ($this->getRequest()->hasParameter('filter'))
    {
      ...
      $this->getUser()->getAttributeHolder()->removeNamespace('sf_admin/fabancos/filters');
      $this->getUser()->getAttributeHolder()->add($filters, 'sf_admin/fabancos/filters');
    }
  }
  ...
  protected function processSort()
  {
    ...
    {
      $this->getUser()->setAttribute('sort', $this->getRequestParameter('sort'), 'sf_admin/fabancos/sort');
      $this->getUser()->setAttribute('type', $this->getRequestParameter('type', 'asc'), 'sf_admin/fabancos/sort');
    }

    if (!$this->getUser()->getAttribute('sort', null, 'sf_admin/fabancos/sort'))
    {
      $this->getUser()->setAttribute('sort', 'nomban', 'sf_admin/fabancos/sort');
      $this->getUser()->setAttribute('type', 'asc', 'sf_admin/fabancos/sort');
    }
  }
  ...
  protected function addFiltersCriteria($c)
  {
    ...
    {
      $criterion = $c->getNewCriterion(FabancosPeer::NOMBAN, '');
      $criterion->addOr($c->getNewCriterion(FabancosPeer::NOMBAN, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['nomban']) && $this->filters['nomban'] !== '')
    {
      $c->add(FabancosPeer::NOMBAN, strtr("%".$this->filters['nomban']."%", '*', '%'), Criteria::LIKE);
  ...
  {
    if ($sort_column = $this->getUser()->getAttribute('sort', null, 'sf_admin/fabancos/sort'))
    {
      $sort_column = FabancosPeer::translateFieldName($sort_column, BasePeer::TYPE_FIELDNAME, BasePeer::TYPE_COLNAME);
      if ($this->getUser()->getAttribute('type', null, 'sf_admin/fabancos/sort') == 'asc')
      {
  ...
  {
    $arreglo=array(
                  'fabancos{codban}' => 'Nro Cuenta:',
              'fabancos{nomban}' => 'Nombre:',
                      'fabancos{grid}' => '.:',
                          'fabancos{codban}' => 'Nro Cuenta:',
              'fabancos{nomban}' => 'Nombre:',
              'fabancos{id}' => 'Id:',
 ...
 public function SalvarBitacora($id, $acc)
  {
    try{
      ...
      $segbitaco->setCodmod('fabancos');
      $segbitaco->setValcla('Fabancos');
