...
class autoFatipcteActions extends sfActions
{
  ...
  {
    return $this->forward('fatipcte', 'list');
  }
  ...
  public function executeList()
  {
    ...
    $this->filters = $this->getUser()->getAttributeHolder()->getAll('sf_admin/fatipcte/filters');


     // 15    // pager
    $this->pager = new sfPropelPager('Fatipcte', 15);
    $c = new Criteria();
    $this->c ? $c=$this->c : '';
  ...
  {
    return $this->forward('fatipcte', 'edit');
  }
  ...
  {
    $this->params=array();
    $this->fatipcte = $this->getFatipcteOrCreate();

    $this->editing();

    if ($this->getRequest()->getMethod() == sfRequest::POST)
    {
      $this->updateFatipcteFromRequest();

      if($this->saveFatipcte($this->fatipcte) ==-1){
        {$this->setFlash('notice', 'Your modifications have been saved');

         $id= $this->fatipcte->getId();
         $this->SalvarBitacora($id ,'Guardo');}

        if ($this->getRequestParameter('save_and_add'))
        {
          return $this->redirect('fatipcte/create');
        }
        else if ($this->getRequestParameter('save_and_list'))
        {
          return $this->redirect('fatipcte/list');
        }
        else
        {
            return $this->redirect('fatipcte/edit?id='.$this->fatipcte->getId());
        }
  ...
  {
    $this->fatipcte = FatipctePeer::retrieveByPk($this->getRequestParameter('id'));
    $this->forward404Unless($this->fatipcte);

    try
    {
      $this->deleteFatipcte($this->fatipcte);
      $id= $this->fatipcte->getId();
   ...
   *
   */
  protected function saveFatipcte($fatipcte)
  {
    ...
    try {
      ...
      $this->coderr = $this->saving($fatipcte);
   ...
   *
   */
  protected function saving($fatipcte)
  {
    $fatipcte->save();
    return -1;
   ...
   *
   */
  protected function deleting($fatipcte)
  {
  	$fatipcte->delete();
    return -1;
   ...
   *
   */
  protected function deleteFatipcte($fatipcte)
  {
    try {
      ...
      // OJO ----> Eliminar esta linea al modificar este método
      $this->coderr = $this->deleting($fatipcte);
   ...
   *
   */
  protected function updateFatipcteFromRequest()
  {
    $fatipcte = $this->getRequestParameter('fatipcte');

    $fields = FatipctePeer::getFieldNames();

    if(array_search('Codalmusu', $fields))
    {
      if (isset($fatipcte['codalmusu']))
      {
        $this->fatipcte->setCodalmusu($fatipcte['codalmusu']);
      }
    }
    if (isset($fatipcte['nomtipcte']))
    {
      $this->fatipcte->setNomtipcte($fatipcte['nomtipcte']);
    }
   ...
   *
   */
  protected function getFatipcteOrCreate($id = 'id')
  {
    if (!$this->getRequestParameter($id))
    {
      $fatipcte = new Fatipcte();
    }
    else
    {
      $fatipcte = FatipctePeer::retrieveByPk($this->getRequestParameter($id));

      $this->forward404Unless($fatipcte);
    }
  ...
  protected function processFilters()
  {
    if ($this->getRequest()->hasParameter('filter'))
    {
      ...
      $this->getUser()->getAttributeHolder()->removeNamespace('sf_admin/fatipcte/filters');
      $this->getUser()->getAttributeHolder()->add($filters, 'sf_admin/fatipcte/filters');
    }
  }
  ...
  protected function processSort()
  {
    ...
    {
      $this->getUser()->setAttribute('sort', $this->getRequestParameter('sort'), 'sf_admin/fatipcte/sort');
      $this->getUser()->setAttribute('type', $this->getRequestParameter('type', 'asc'), 'sf_admin/fatipcte/sort');
    }

    if (!$this->getUser()->getAttribute('sort', null, 'sf_admin/fatipcte/sort'))
    {
      $this->getUser()->setAttribute('sort', 'nomtipcte', 'sf_admin/fatipcte/sort');
      $this->getUser()->setAttribute('type', 'asc', 'sf_admin/fatipcte/sort');
    }
  }
  ...
  {
    if (isset($this->filters['nomtipcte_is_empty']))
    {
      $criterion = $c->getNewCriterion(FatipctePeer::NOMTIPCTE, '');
      $criterion->addOr($c->getNewCriterion(FatipctePeer::NOMTIPCTE, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['nomtipcte']) && $this->filters['nomtipcte'] !== '')
    {
      $c->add(FatipctePeer::NOMTIPCTE, strtr("%".$this->filters['nomtipcte']."%", '*', '%'), Criteria::LIKE);
  ...
  {
    if ($sort_column = $this->getUser()->getAttribute('sort', null, 'sf_admin/fatipcte/sort'))
    {
      $sort_column = FatipctePeer::translateFieldName($sort_column, BasePeer::TYPE_FIELDNAME, BasePeer::TYPE_COLNAME);
      if ($this->getUser()->getAttribute('type', null, 'sf_admin/fatipcte/sort') == 'asc')
      {
 ...
 public function SalvarBitacora($id, $acc)
  {
    try{
      ...
      $segbitaco->setCodmod('fatipcte');
      $segbitaco->setValcla('Fatipcte');
