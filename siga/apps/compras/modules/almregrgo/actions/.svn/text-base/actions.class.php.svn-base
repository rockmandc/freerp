...
class almregrgoActions extends autoalmregrgoActions
{
     ...
     $cajtexcom=$this->getRequestParameter('cajtexcom');
	  if ($this->getRequestParameter('ajax')=='1')
	    ...
	    else  if ($this->getRequestParameter('ajax')=='2')
	    {
	  		$js="";
        $p= new Criteria();
        $p->add(ContabbPeer::CODCTA,$this->getRequestParameter('codigo'));
  ...
  {
    $this->carecarg = CarecargPeer::retrieveByPk($this->getRequestParameter('id'));
    $this->forward404Unless($this->carecarg);

    try
    {
       $tiene = H::getX_vacio('Codrgo','Cargosol','Codrgo',$this->carecarg->getCodrgo());
      if ($tiene=='') {
        $this->deleteCarecarg($this->carecarg);
        $this->Bitacora('Elimino');
      }else {
        $this->getRequest()->setError('delete', 'No se pudo borrar la registro seleccionado. Asegúrese de que no tiene ningún tipo de registros asociados.');
        return $this->forward('almregrgo', 'list');
      }
    }
