 ...
 *
 * @package    siga
 * @subpackage almapradq
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class almapradqActions extends autoalmapradqActions
{
  public function executeIndex()
  {
    return $this->forward('almapradq', 'edit');
  }
   ...
   public function configGrid($fec1='', $fec2='')
  {
    ...
    if ($filsoldir=='S')
     ...
     $c->add(CasolartPeer::APRGESADM,'S');     
     ...
     $this->columnas = Herramientas::getConfigGrid(sfConfig::get('sf_app_module_dir').'/almapradq/'.sfConfig::get('sf_app_module_config_dir_name').'/grid');
  ...
  public function saving($clasemodelo)
  {
    ...
    foreach($grid[0] as  $dat)
    {
        if($dat->getAprobar2()=='1')
        {
            ...
            {
               $per->setAprdiradq('S');
               $per->setUsuaprdad($loguse);
               $per->setFecaprdad(date('Y-m-d'));
               $per->save();
            }
        }
        if ($dat->getRechazar3()=='1')
       {
          $c = new Criteria();
          $c->add(CasolartPeer::REQART,$dat->getReqart());
          $per = CasolartPeer::doSelectOne($c);            
          ...
          {
             $per->setAprdiradq('N');
             $per->setUsuaprdad(null);
             $per->setFecaprdad(null);
             $per->save();
          }
       } 
    }
