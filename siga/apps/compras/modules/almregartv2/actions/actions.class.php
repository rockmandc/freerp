 ...
 */
class almregartv2Actions extends autoalmregartv2Actions
{
  ...
  public function validateEdit()
    {
      if($this->getRequest()->getMethod() == sfRequest::POST)
      {
        ...
        if($descodbar==true || $descodbar=='S'){
           if($this->caregart->getCodbar()=='' && $this->caregart->UltimoNivel()) self::$coderror= 2105;
        }
        if (self::$coderror<>-1){
          return false;
        }else return true;
  ...
  public function executeEdit()
    {
      ...
      $this->configGridAlmUbi('','',$xx);
      $this->configGridCodigosAlt();
...
$this->Bitacora('Guardo');
        ...
        {
          return $this->redirect('almregartv2/create');
        }
        else if ($this->getRequestParameter('save_and_list'))
        {
          return $this->redirect('almregartv2/list');
        }
        else
        {
          return $this->redirect('almregartv2/edit?id='.$this->caregart->getId());
        }
      }
      else
      {
  ...
  protected function updateCaregartFromRequest()
  {
    ...
    if (isset($caregart['tippro']))
    {
      $this->caregart->setTippro($caregart['tippro']);
    }
    if (isset($caregart['codbar']))
    {
      $this->caregart->setCodbar($caregart['codbar']);
    }
    if (isset($caregart['nacimp']))
    {
      $this->caregart->setNacimp($caregart['nacimp']);
    }
    if (isset($caregart['staart']))
    {
      $this->caregart->setStaart($caregart['staart']);
    }
    if (isset($caregart['codunimed']))
    {
      $this->caregart->setCodunimed($caregart['codunimed']);
    }
    if (isset($caregart['claart']))
    {
      $this->caregart->setClaart($caregart['claart']);
    }
    }
   ...
   *
   */
  public function configGrid($codart = '')
    {
      $this->mensaler="";
//      $c = new Criteria();
...
//      $per = CaartalmPeer::doSelect($c);
      ...
      if(!$codart) if(isset($this->caregart)) $codart = $this->caregart->getCodart(); else $codart='';      
          ...
          if ($per)
          {
            ...
            {
               if ($datalm['exiact'] <= $datalm['ptoreo'])
               {
                 $this->mensaler="La Existencia Actual del Artículo es menor o igual al Punto de Reorden para el Almacén  ".$datalm['codalm'];
                 break;
               }
            }
          }      
      ...
      if(!isset ($this->mascaraubicacion)) $this->setVars();
...
*/
      ...
      $col7->setHTML('type="text" size="1" style="border:none" class="imagenalmacen" readonly=true onClick="mostrar(this.id)"');
  ...
  public function configGridAlmUbi($codart='',$codalm='',&$filas)
   {
      ...
      $params = array('param1'=>$codalm,'param2'=>'val2');
      ...
      $col1->setAjax('almregartv2',3,2);
      $col1->setHTML('type="text" size="20" maxlength='.chr(39).$longmas.chr(39).'  onKeyDown="javascript:return dFilter (event.keyCode, this,'.chr(39).$mascaraubicacion.chr(39).')" onKeyPress="javascript:cadena=rayaenter(event,this.value);if (event.keyCode==13 || event.keyCode==9){document.getElementById(this.id).value=cadena;}"  onBlur="javascript:event.keyCode=13; ajax(event,this.id);"');
      ...
      if ($manartlot=='S')
      {
          ...
          $col4->setHTML('type="text" size="15" maxlength="100" onBlur="javascript: Verifica_ubinumlote_repetido(this.id);"');
  ...
  /*  if ($caregart->getId())
    ...
    {*/
     ...
     $gridcod=Herramientas::CargarDatosGrid($this,$this->obj3);
      /*try
      {*/
       Articulos::salvarAlmregart($caregart,$grid,$gridunid,$gridpar,$gridcod);
     /* }
       catch (Exception $ex)
      {*/
        $coderr = 0;
   ...
   $cajtexmos=$this->getRequestParameter('cajtexmos');
  ...
  else  if ($this->getRequestParameter('ajax')=='7')
    {
    ...
    else  if ($this->getRequestParameter('ajax')=='8')
      {
            ...
            if (!$datos)
            {
             $javascript="alert('El Código de la Cuenta Contable no existe');$('caregart_codcta').value='';";
              ...
              if ($datos->getCargab()!='C')
                $javascript="alert('La Cuenta Contable no es Cargable');$('caregart_codcta').value='';";
            }
      ...
      else  if ($this->getRequestParameter('ajax')=='9')
      {
            ...
            $codubi=$this->getRequestParameter('ubicacion');
            ...
            $manartlot=H::getConfApp2('manartlot', 'compras', 'almregart');
            if ($manartlot=='S')
                $ubica=$codubi.'_'.H::getX_vacio('CODUBI', 'Cadefubi', 'Nomubi', $codubi).'_'.$c.'_'.$c1.'__'.'!';
            else
                $ubica=$codubi.'_'.H::getX_vacio('CODUBI', 'Cadefubi', 'Nomubi', $codubi).'_'.$c.'!';
            
            if ($nuevo=="") {
                $t= new Criteria();
                $t->add(CaalmubiPeer::CODUBI,$codubi);
                $regis= CaalmubiPeer::doSelect($t);
                if ($regis)
                {                   
                    foreach ($regis as $dat) {                       
                       $des1="";
                       $cadena=$cadena.$dat->getCodalm()."*".$des1."*".$c."*".$c."*".$c."*".$c."*".$ubica."@";
                    }
                    $javascript="colocarAlmacen('$cadena');";
                }
            }else {
                $t= new Criteria();
                $t->add(CaalmubiPeer::CODUBI,$codubi);
                $this->sql="caalmubi.codalm not in (select codalm from caartalmubi where codart='".$codart."')";
                $t->add(CaalmubiPeer::CODALM,$this->sql,Criteria::CUSTOM);
                $regis= CaalmubiPeer::doSelect($t);
                if ($regis)
                {                 
                    foreach ($regis as $dat) {   
      ...
      else  if ($this->getRequestParameter('ajax')=='10')
      {
        $c=new Criteria();
        $c->add(CadefubiPeer::CODUBI,$this->getRequestParameter('codigo'));
        $datos=CadefubiPeer::doSelectOne($c);
        if ($datos)
        {
           $nombre=$datos->getNomubi();
           $output = '[["'.$cajtexmos.'","'.$nombre.'",""]]';
        }
        else
        {
         $javascript="alert('El Código de Ubicación no existe');";
         $output = '[["javascript","'.$javascript.'",""],["'.$cajtexmos.'","",""]]';
        }
      ...
      else  if ($this->getRequestParameter('ajax')=='11')
      {
            ...
            $t= new Criteria();
            $t->add(CaartalmubiPeer::CODART,$codart);
            $reg= CaartalmubiPeer::doSelectOne($t);
            if (!$reg) {            
                $cadena="";
                $c=0;
                $c1='01';
      ...
      {
        $c=new Criteria();
        $c->add(CadefunimedPeer::CODUNIMED,$this->getRequestParameter('codigo'));
        $datos=CadefunimedPeer::doSelectOne($c);
        if ($datos)
        {
           $nombre=$datos->getDesunimed();
           $output = '[["'.$cajtexmos.'","'.$nombre.'",""]]';
        }
        else
        {
         $javascript="alert('El Código de Unidad de Medida no existe');";
         $output = '[["javascript","'.$javascript.'",""],["'.$cajtexmos.'","",""]]';
        }
      ...
      else  if ($this->getRequestParameter('ajax')=='13')
      {
            ...
            if (!$datos)
            {
             $javascript="alert('El Código de la Cuenta Contable no existe');$('$cajtexcom').value='';";
            }else {
              if ($datos->getCargab()!='C')
                $javascript="alert('La Cuenta Contable no es Cargable');$('$cajtexcom').value='';";
            }
            $output = '[["javascript","'.$javascript.'",""]]';
            $this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
            return sfView::HEADER_ONLY;
    ...
    if (Articulos::Buscar_CodigoHijo($this->caregart->getCodart())) //no se puede eliminar, tiene hijos
     {
      ...
      return $this->forward('almregartv2', 'list');
  ...
  public function configGridUnidades()
  {
     ...
     $this->columnas = Herramientas :: getConfigGrid(sfConfig :: get('sf_app_module_dir') . '/almregartv2/' . sfConfig :: get('sf_app_module_config_dir_name') . '/grid_unidades');

     $this->obj5 = $this->columnas[0]->getConfig($per);
  }

  public function executeAjaxdetalle()
  {
    $codart=$this->getRequestParameter('codart');

    $this->configGrid($codart);
  }
  ...
  public function executeAjaxfila()
  {
    ...
    switch ($col) {
         case ('5'):   //Fecha Elaboración
            if ($grid[$fila][4]!="")
            {
                $fec_aux = split("/", $grid[$fila][4]);
                if (checkdate(intval($fec_aux[1]), intval($fec_aux[0]), intval($fec_aux[2])) )
                {
                   if ($grid[$fila][5]!="")
                   {
                       $fec_aux2 = split("/", $grid[$fila][5]);
                       if (checkdate(intval($fec_aux2[1]), intval($fec_aux2[0]), intval($fec_aux2[2])) )
                       {
                          $dateFormat = new sfDateFormat('es_VE');
                          $fecven = $dateFormat->format($grid[$fila][5], 'i', $dateFormat->getInputPattern('d'));

                          $dateFormat = new sfDateFormat('es_VE');
                          $fecela = $dateFormat->format($grid[$fila][4], 'i', $dateFormat->getInputPattern('d'));
                          
                          if ($fecela>$fecven)
                          {
                              $javascript = "alert_('La Fecha de Elaboraci&oacute;n no puede ser mayor a la Fecha de Vencimiento');";
                              $grid[$fila][4]="";
                          }                         
                          
                       }else {
                            $javascript = "alert_('La Fecha de Vencimiento es inv&aacute;lida');";
                            $grid[$fila][4]="";
                        }
                   }
                }else {
                    $javascript = "alert_('La Fecha de Elaboraci&oacute;n es inv&aacute;lida');";
                    $grid[$fila][4]="";
                }               
            }
            $jsonextra = ',["javascript","' . $javascript . '",""]';
            break;        
         case ('6'):   //Fecha Vencimiento
            if ($grid[$fila][5]!="")
            {
                $fec_aux = split("/", $grid[$fila][5]);
                if (checkdate(intval($fec_aux[1]), intval($fec_aux[0]), intval($fec_aux[2])) )
                {
                   if ($grid[$fila][4]!="")
                   {
                       $fec_aux2 = split("/", $grid[$fila][4]);
                       if (checkdate(intval($fec_aux2[1]), intval($fec_aux2[0]), intval($fec_aux2[2])) )
                       {
                          $dateFormat = new sfDateFormat('es_VE');
                          $fecela = $dateFormat->format($grid[$fila][4], 'i', $dateFormat->getInputPattern('d'));
                          ...
                          if ($fecven<$fecela)
                          {
                              $javascript = "alert_('La Fecha de Vencimiento no puede ser menor a la Fecha de Elaboraci&oacute;n');";
                              $grid[$fila][5]="";
                          }                         
                    ...
                    $javascript = "alert_('Debe introducir la Fecha de Elaboraci&oacute;n');";
                    $grid[$fila][5]=="";
                } 
                }else {
                    $javascript = "alert_('La Fecha de Vencimiento es inv&aacute;lida');";
 ...
 /**
    ...
    switch ($col) {
          case '1':
            ...
            if ($reg) {
              ...
              {
                  $grid[$fila][1]=$reg->getDesunimed();
              }else {
                 $grid[$fila][$col-1]="";
                 $grid[$fila][1]="";
                 $javascript="alert('La Unidad de Medida esta Repetida');";
              }
            }else {
                $grid[$fila][$col-1]="";
                $grid[$fila][1]="";
                $javascript="alert('La Unidad de Medida no Existe ...');";
            }
   ...
   *
   */
  public function configGridCodigosAlt()
  {
      $c = new Criteria();
      $c->add(CaartaltPeer::CODART,$this->caregart->getCodart());
      $per = CaartaltPeer::doSelect($c);

     $this->columnas = Herramientas :: getConfigGrid(sfConfig :: get('sf_app_module_dir') . '/almregartv2/' . sfConfig :: get('sf_app_module_config_dir_name') . '/grid_codigos');

     $this->obj3 = $this->columnas[0]->getConfig($per);
  }  
