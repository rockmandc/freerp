 ...
 * @version SVN: $Id: actions.class.php 61952 2015-05-08 13:28:51Z dmartinez $
 *
...
private static $coderror=-1;
  ...
  public function validateEdit()
    ...
    $resp2 = -1;
      ...
      $valforcodpro=H::getConfApp2('valforcodpro', 'compras', 'almregpro');
      if($valforcodpro=="S"){
        $expreg = '/^[a-zA-Z0-9]+$/i';    
        if(preg_match($expreg, $valor)==0){
          $this->coderror = 2110;
          return false;          
        }
...
$this->Bitacora('Guardo');
  ...
  protected function updateCaproveeFromRequest()
  {
   ...
   if (isset($caprovee['codtip']))
    ...
    if (isset($caprovee['estapo']))
    {
      $this->caprovee->setEstapo($caprovee['estapo']);
    }
     if (isset($caprovee['clapro']))
    {
      $this->caprovee->setClapro($caprovee['clapro']);
    }
    if (isset($caprovee['fecact']))
    {
      if ($caprovee['fecact'])
      {
        try
        {
          $dateFormat = new sfDateFormat($this->getUser()->getCulture());
                              if (!is_array($caprovee['fecact']))
          {
            $value = $dateFormat->format($caprovee['fecact'], 'i', $dateFormat->getInputPattern('d'));
          }
          else
          {
            $value_array = $caprovee['fecact'];
            $value = $value_array['year'].'-'.$value_array['month'].'-'.$value_array['day'].(isset($value_array['hour']) ? ' '.$value_array['hour'].':'.$value_array['minute'].(isset($value_array['second']) ? ':'.$value_array['second'] : '') : '');
          }
          $this->caprovee->setFecact($value);
        }
        catch (sfException $e)
        {
          // not a date
      ...
      {
        $this->caprovee->setFecact(null);
      }
    }
    if (isset($caprovee['codniv']))
    {
      $this->caprovee->setCodniv($caprovee['codniv']);
    }
  }
...
/**
  ...
  public function executeAjax()
  {
    ...
    if ($this->getRequestParameter('ajax')=='1')
      ...
      {
        $l= new Criteria();
        $l->add(ContabbPeer::CODCTA,$this->getRequestParameter('codigo'));
        $regl= ContabbPeer::doSelectOne($l);
        if ($regl){
          if ($regl->getCargab()=='C')
            $dato=$regl->getDescta();
          else
            $js="alert('La Cuenta Contable no es Cargable'); $('$cajtexcom').value=''; $('$cajtexmos').value=''; $('$cajtexcom').focus();";
        }else {
          $js="alert('La Cuenta Contable no existe'); $('$cajtexcom').value=''; $('$cajtexmos').value=''; $('$cajtexcom').focus();";
        }
  ...
  {
    Herramientas::EliminarRegistro('Carecpro','CODPRO',$caprovee->getCodpro()); //Recaudos
    Herramientas::EliminarRegistro('Caconpro','CODPRO',$caprovee->getCodpro()); //Contactos
    Herramientas::EliminarRegistro('Carampro','CODPRO',$caprovee->getCodpro()); //Ramos
    Herramientas::EliminarRegistro('Caproret','CODPRO',$caprovee->getCodpro()); //Retenciones
    Herramientas::EliminarRegistro('Opbenefi','CEDRIF',$caprovee->getRifpro()); //Beneficiarios
  ...
  public function executeDelete()
  {
    ...
    $esta=H::getX_vacio('CODPRO','Cacotiza','Codpro',$this->caprovee->getCodpro());
    $esta2=H::getX_vacio('CODPRO','Caordcom','Codpro',$this->caprovee->getCodpro());
    ...
    try
    {
      ...
      $this->Bitacora('Elimino');
        ...
        $this->getRequest()->setError('delete', 'No se pudo borrar la registro seleccionado. Asegúrese de que no esta asociado a una Cotización o Orden de Compra.');
