  ...
  protected function updateCasolartFromRequest()
  {
    ...
    if (isset($casolart['fecana']))
    {
      if ($casolart['fecana'])
      {
        ...
        {
          $dateFormat = new sfDateFormat($this->getUser()->getCulture());
                              if (!is_array($casolart['fecana']))
          {
            $value = $dateFormat->format($casolart['fecana'], 'i', $dateFormat->getInputPattern('d'));
          }
          else
          {
            $value_array = $casolart['fecana'];
            $value = $value_array['year'].'-'.$value_array['month'].'-'.$value_array['day'].(isset($value_array['hour']) ? ' '.$value_array['hour'].':'.$value_array['minute'].(isset($value_array['second']) ? ':'.$value_array['second'] : '') : '');
          }
          $this->casolart->setFecana($value);
        }
        catch (sfException $e)
        {
          // not a date
 ...
 */
  ...
  $per=array();
      if ($desart!='') {
        $comple=" and a.desart='".$desart."'";
        $comple2=", a.desart";
      }else {  $comple=""; $comple2="";}
    $sql="select a.refcot, a.codart, a.costo from cadetcot a, cacotiza b where a.refcot=b.refcot and b.refsol='".$refsol."' 
     and a.codart='".$codart."' ".$comple." group by a.refcot, a.codart ".$comple2.", a.costo order by a.costo";
     ...
     if (count($result)>0)
     {
         ...
         while ($i<count($result))
         {
             $cadetcot= new Cadetcot();
             $cadetcot->setRefcot($result[$i]["refcot"]);
             $cadetcot->setCodart($result[$i]["codart"]);
             $cadetcot->setCosto($result[$i]["costo"]);             
             $per[]=$cadetcot;             
             $i++;
         }
     }
	...
	public function executeGrid()
	{
	  ...
	  if ($this->getRequestParameter('ajax')=='1')
	  {
            $this->ajax="1";
            $claartdes=H::getConfApp2('claartdes', 'compras', 'almsolegr');
            $codart=str_replace('*','%',$this->getRequestParameter('codart'));
            $valores= split('!',$codart);
            if ($claartdes=='S')
  ...
  public function configGrid2($refsol=' ')
   {
	...
	$col2 = new Columna('Monto Cotización');
    $col2->setTipo(Columna::MONTO);
    $col2->setEsGrabable(true);
