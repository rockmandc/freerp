...
<div id="divCombo">
 <?php echo select_tag('casolart[articulo]', options_for_select($articulos,$casolart->getArticulo(),'include_custom=Seleccione un Artículo'),array('onChange'=> remote_function(array(
	...
	'with'   => "'reqart='+document.getElementById('casolart_reqart').value+'&codart='+this.value.replace(/%/gi,'*')"
  )))); ?></div></div>
   </th>
   <th>
     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<br>
<?php echo label_for('casolart[fecana]', __($labels['casolart{fecana}']), 'class="required" ') ?>
  <div class="content<?php if ($sf_request->hasError('casolart{fecana}')): ?> form-error<?php endif; ?>">
  <?php if ($sf_request->hasError('casolart{fecana}')): ?>
    <?php echo form_error('casolart{fecana}', array('class' => 'form-error-msg')) ?>
  <?php endif; ?>

  <?php $value = object_input_date_tag($casolart, 'getFecana', array (
  'rich' => true,
  'calendar_button_img' => '/sf/sf_admin/images/date.png',
  'size' => '10',
  'control_name' => 'casolart[fecana]',
