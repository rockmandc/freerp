<?php
/**
 * almordcomcor actions.
 *
 * @package    Roraima
 * @subpackage almordcomcor
...
class almordcomcorActions extends autoalmordcomcorActions
{
  ...
  $this->caordcom = $this->getCaordcomOrCreate();
    ...
    if($this->getRequest()->getMethod() == sfRequest::POST)
    {
      if ($this->getRequestParameter('id')=="")
      {
        ...
        $prefijomixto=H::getConfApp('prefijomixto', 'compras', 'almordcomcor');
        ...
        if(H::getConfApp2('valresp', 'compras', 'almordcomcor')=='S'){
        ...
        if(H::getConfApp2('valunidad', 'compras', 'almordcomcor')=='S'){
        ...
        if ($moneda2!=$moneda) {
                if ($refe==0) $valor=H::toFloat($this->getRequestParameter('caordcom[valmon]'));
          ...
          $p=0;
            while ($p<count($datosGrid))
            {
              ...
              if($datosGrid[$p]["monimp"] > $disponible){
                  $hay_disponibilidad=false;
                  $this->codigo_presupuestario=$datosGrid[$p]["codpre2"];
                  $this->coderror1=118;
                  return false;
              }
              $p++;
              ...
              $g=0;
                 while ($g<count($datosGridRec))
                 {
                  ...
                  if($datosGridRec[$g]["monimp"] > $disponible){
                    $this->codigo_presupuestario=$datosGridRec[$g]["codpre"];
                    $this->coderror1=119;
                    return false;
                  }
                  $g++;
    ...
    if ($this->getRequest()->getMethod() == sfRequest::POST)
    {
      ...
      if ($coderr ==-1)
      {
        ...
        if ($this->getRequestParameter('save_and_add'))
          return $this->redirect('almordcomcor/create');
        else if ($this->getRequestParameter('save_and_list'))
          return $this->redirect('almordcomcor/list');
        else
          return $this->redirect('almordcomcor/edit?imp=S&id='.$this->caordcom->getId());

      }//if ($this->saveCaordcom($this->caordcom)==-1)
      else if ($coderr==-11){

	        $this->setFlash('notice', 'Se ha guardado solamente la DescripciÃ³n');
	        ...
	        {
	          return $this->redirect('almordcomcor/edit?id='.$this->caordcom->getId());
	        }
      }
      else
      {
  ...
  public function executeAjaxcompromiso()
  {
	...
	}else $msj="No hay disponibilidad para el siguiente CÃ³digo presupuestario: ".$cod1;
  ...
  public function executeAjaxcomprobante()
  {
   ...
   if ($reg)
   {
      ...
      if ($msjuno=="")
      {
	      $form="sf_admin/almordcomcor/confincomgen";
	      $i=0;
	      $f[$i]=$form.$i;
        ...
        if ($recargosreg)
        {
          if ($recargosreg->getCodpre()!="")
          {
	        ...
	        if ($tiprgo=='M')//Tipo recargo puntual (monto)
          	...
          	$javascript="alert('Debe asociarle al Recargo el CÃ³digo Presupuestario'); $('$cajtexcom').value='';";
          }
        }
        else {
        	$desrgo=""; $monrgo="0,00"; $tiprgo=""; $reccalformat="0,00"; $codpar="";
  ...
  public function configGrid($ordcom='',$referencia='',$tipo='')
  {
    ...
    #Agregado requerimiento de leobardo y pablo
    ...
    $novalgriddis=H::getConfApp2('novalgriddis', 'compras', 'almordcomcor');
    if ($novalgriddis=='S'){
    ...
    if ($referencia==0) $col15->setAjax('almordcomcor',5,15);
  ...
  $this->getUser()->setAttribute('referencia', '1');
    ...
    $opciones->setTitulo('Detalle de la Orden de RequisiciÃ³n');
    $opciones->setHTMLTotalFilas(' ');
  ...
  public function configGrid_Recargos($ordcom='',$referencia_recargo='')
  {
    $c = new Criteria();
      ...
      $col1->setAjax('almordcomcor',4,2,4);
  ...
  public function configGrid_FormasEntrega($ordcom='')
  {
    ...
    $col1->setAjax('almordcomcor',22,2);
    ...
    $col3->setAjax('almordcomcor',21,4);
	...
	if(array_key_exists('aplicacion',$varemp))
	 if(array_key_exists('compras',$varemp['aplicacion']))
	   if(array_key_exists('modulos',$varemp['aplicacion']['compras']))
	     if(array_key_exists('almordcomcor',$varemp['aplicacion']['compras']['modulos'])){
	       if(array_key_exists('deshabilmonrec',$varemp['aplicacion']['compras']['modulos']['almordcomcor']))
	       {
	       	$this->deshabmonrec=$varemp['aplicacion']['compras']['modulos']['almordcomcor']['deshabilmonrec'];
	       }
               if(array_key_exists('ordcomdesh',$varemp['aplicacion']['compras']['modulos']['almordcomcor']))
	       {
	       	$this->ordcomdesh=$varemp['aplicacion']['compras']['modulos']['almordcomcor']['ordcomdesh'];
	       }
           if(array_key_exists('mansolocor',$varemp['aplicacion']['compras']['modulos']['almordcomcor']))
	       {
	       	$this->mansolocor=$varemp['aplicacion']['compras']['modulos']['almordcomcor']['mansolocor'];
	       }
	       if(array_key_exists('bloqfec',$varemp['aplicacion']['compras']['modulos']['almordcomcor']))
	       {
	       	$this->bloqfec=$varemp['aplicacion']['compras']['modulos']['almordcomcor']['bloqfec'];
	       }
	       if(array_key_exists('oculeli',$varemp['aplicacion']['compras']['modulos']['almordcomcor']))
	       {
	       	$this->oculeli=$varemp['aplicacion']['compras']['modulos']['almordcomcor']['oculeli'];
	       }
	       if(array_key_exists('oculsave',$varemp['aplicacion']['compras']['modulos']['almordcomcor']))
	       {
	       	$this->oculsave=$varemp['aplicacion']['compras']['modulos']['almordcomcor']['oculsave'];
	       }
	     	 if(array_key_exists('fechaanuserv',$varemp['aplicacion']['compras']['modulos']['almordcomcor']))
	       {
	       	$this->fechaanuserv=$varemp['aplicacion']['compras']['modulos']['almordcomcor']['fechaanuserv'];
	       }
                if(array_key_exists('etiqtipord',$varemp['aplicacion']['compras']['modulos']['almordcomcor']))
	       {
	       	$this->etiqtipord=$varemp['aplicacion']['compras']['modulos']['almordcomcor']['etiqtipord'];
	       }
         }
    ...
    if (!Orden_compra::Eliminar($caordcom,&$coderror))
      {
          if($coderror!=-1)
        {
          ...
          return $this->forward('almordcomcor', 'list');
        }
      }
  }
...
//<--------------------------------------------------------------------------------------------------------------------------------------------------->
  /**
   * FunciÃ³n para manejar el salvado del formulario.
   * cabe destacar que en las versiones nuevas del formulario (cidesaPropel)
   * llama internamente a la funciÃ³n $this->saving
   * Esta funciÃ³n saving siempre debe retornar un valor >=-1.
   * En esta funcciÃ³n se debe realizar el proceso de guardado de informacion
...
//<--------------------------------------------------------------------------------------------------------------------------------------------------->
  ...
  if ($this->getRequestParameter('ajax')=='1')
  {
    ...
    if ($numero_filas > 0)
    {
      ...
      {
        //print 'Hola';
        // Verifico si el ganador es un sÃ³lo proveedor
...
/*     if (($refsol!='') and ($this->getRequestParameter('codigo')!='') and ($parcial=='S') and ($seguir))
  ...
  public function configGridRecargo($refere="",$codart="",$coduni="")
   {
       ...
       $col1 = new Columna('CÃ³digo Recargo');
       $col1->setTipo(Columna::TEXTO);
       $col1->setEsGrabable(true);
       $col1->setAlineacionObjeto(Columna::CENTRO);
       $col1->setAlineacionContenido(Columna::CENTRO);
  ...
  public function executeCreate()
  {
    ...
    if (!$conpag)
    {
      $this->getRequest()->setError('valida', 'Debe definir al menos una CondiciÃ³n de Pago.');
      return $this->forward('almordcomcor', 'list');
    }
    ...
    return $this->forward('almordcomcor', 'edit');
  }
}