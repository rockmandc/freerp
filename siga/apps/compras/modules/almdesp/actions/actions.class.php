 ...
 * @version SVN: $Id: actions.class.php 41775 2010-12-20 22:17:33Z cramirez $
 * 
...
class almdespActions extends autoalmdespActions
{
 private $coderror =-1;
  ...
  public function executeList()
  {
    ...
    $filiscen=H::getConfApp2('filiscen', 'compras', 'almdesp');
    ...
    if ($filiscen=='S' && $filsoldir=='S') {
      $this->sql='cadphart.codcen in (select codcen from "SIMA_USER".segcenusu where loguse=\''.$loguse.'\') and cadphart.coddirec in (select coddirec from "SIMA_USER".segdirusu where loguse=\''.$loguse.'\')';
      $c->add(CadphartPeer::CODCEN,$this->sql,Criteria::CUSTOM);      
    }elseif ($filiscen=='S') {
     $this->sql='cadphart.codcen in (select codcen from "SIMA_USER".segcenusu where loguse=\''.$loguse.'\')';
      $c->add(CadphartPeer::CODCEN,$this->sql,Criteria::CUSTOM);       
    }else if ($filsoldir=='S'){
  ...
  public function validateEdit()
  {
     $resp=-1;
      if($this->getRequest()->getMethod() == sfRequest::POST)
      {
         ...
         try{  $this->updateCadphartFromRequest(); } catch(Exception $ex){}
      ...
      while ($j<count($x))
      {
              ...
              if ($arti)
              {
                ...
                {
                   $r= new Criteria();
                   $r->add(CaartreqPeer::REQART,$this->cadphart->getReqart());
                   $r->add(CaartreqPeer::CODART,$x[$j]->getCodart());
              ...
              if ($manartlot=='S')
              {
                 ...
                 $encontro=true;                 
                 if (!Despachos::verificaexisydisp($x[$j]->getCodart(),$x[$j]->getCodalm(),$x[$j]->getCodubi(),$cantd,$msg,$x[$j]->getNrolot()))
                 {
                    ...
                    return false;
				      	 ...
				      	 $encontro=true;
				         if (!Despachos::verificaexisydisp($x[$j]->getCodart(),$x[$j]->getCodalm(),$x[$j]->getCodubi(),$cantd,$msg))
                 {
                 	$msg=$msg.". Coloque la cantidad a despachar disponible en cero (0) a este articulo si desea continuar con el despacho del resto de los articulos...";
    							$this->getRequest()->setError('',$msg);
    			 				return false;
  ...
  if ($codalm!="")
  {
   ...
   if($per)
   {
      $cedemp = H::GetX('Loguse','Usuarios','Cedemp',sfContext::getInstance()->getUser()->getAttribute('loguse'));

      $c=new Criteria();
      $c->add(CausualmPeer::CEDEMP,$cedemp);
      $c->add(CadefalmPeer::CODALM,$codalm);
      $c->addJoin(CadefalmPeer::CODALM,CausualmPeer::CODALM);      
      ...
      {
        $nomalm="";
        $javascript="alert('Codigo del Almacen no esta registra o no esta asociado a su usuario...')";
  ...
  public function executeGrid()
	{
       ...
       $filsoldir=H::getConfApp2('filsoldir', 'compras', 'almsolegr'); 
	     if ($this->getRequestParameter('codigo')!="")
	     {
            ...
            $c->add(CareqartPeer::STAREQ,'N',Criteria::NOT_EQUAL);
            if ($filsoldir=='S'){
              ...
              if ($manreqcie=='S')
              {
                if ($datos->getStadesp()=='C'){
                   //la requisicion ya está Despachada
                   $this->configGrid();
                   $this->mensaje="La Requisición ya está Despachada.";
                }else {
                   if ($datos->getAprreq()=='S') {
                    ...
                    $output = '[["'.$cajtexmos.'","'.$desreq.'",""],["cadphart_codori","'.$uniori.'",""],["cadphart_nomcat","'.$desuniori.'",""],["cadphart_codcen","'.$codcen.'",""],["cadphart_descen","'.$descen.'",""],["cadphart_coddirec","'.$coddirec.'",""],["cadphart_desdirec","'.$desdirec.'",""],["'.$cajtexcom.'","8","c"]]';
                    $this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
  ...
  public function setVars()
  {
   ...
   $this->orddesveh = H::getConfApp('orddesveh','compras','almdesp');
   $this->getUser()->setAttribute('orddesveh',$this->orddesveh,'almdesp');
     ...
     protected function addFiltersCriteria($c)
  {
    ...
    {
      $criterion = $c->getNewCriterion(CadphartPeer::CODCEN, '');
      $criterion->addOr($c->getNewCriterion(CadphartPeer::CODCEN, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['codcen']) && $this->filters['codcen'] !== '')
    {
      ...
      if ($filiscen=='S') {
        ...
        $this->sql='cadphart.CODCEN ILIKE \''.$codigocen.'\'  AND cadphart.codcen in (select codcen from "SIMA_USER".segcenusu where loguse=\''.$loguse.'\')';
        $c->add(CadphartPeer::CODCEN,$this->sql,Criteria::CUSTOM);      
      }else $c->add(CadphartPeer::CODCEN, '%'.strtr($this->filters['codcen'], '*', '%').'%', Criteria::LIKE);
      $c->setIgnoreCase(true);
    }
    if (isset($this->filters['descen_is_empty']))
    {
      $criterion = $c->getNewCriterion(CadphartPeer::DESCEN, '');
      $criterion->addOr($c->getNewCriterion(CadphartPeer::DESCEN, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['descen']) && $this->filters['descen'] !== '')
    {
      ...
      if ($filiscen=='S') {
        ...
        $this->sql='cadefcen.DESCEN ILIKE \''.$descrcen.'\'  AND cadphart.codcen in (select codcen from "SIMA_USER".segcenusu where loguse=\''.$loguse.'\')';
        $c->add(CadphartPeer::CODCEN,$this->sql,Criteria::CUSTOM);      
        $c->addJoin(CadphartPeer::CODCEN, CadefcenPeer::CODCEN);
      }else {
    ...
    if (isset($this->filters['codori_is_empty']))
    {
      $criterion = $c->getNewCriterion(CadphartPeer::CODORI, '');
      $criterion->addOr($c->getNewCriterion(CadphartPeer::CODORI, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['codori']) && $this->filters['codori'] !== '')
    {
      $c->add(CadphartPeer::CODORI, '%'.strtr($this->filters['codori'], '*', '%').'%', Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
    if (isset($this->filters['nomcat_is_empty']))
    {
      $criterion = $c->getNewCriterion(CadphartPeer::NOMCAT, '');
      $criterion->addOr($c->getNewCriterion(CadphartPeer::NOMCAT, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['nomcat']) && $this->filters['nomcat'] !== '')
    {
      $c->add(BnubibiePeer::NOMUBI, '%'.strtr($this->filters['nomcat'], '*', '%').'%', Criteria::LIKE);
      $c->addJoin(CadphartPeer::CODORI,BnubibiePeer::CODUBI);
      $c->setIgnoreCase(true);
    }
    if (isset($this->filters['desdph_is_empty']))
    {
      $criterion = $c->getNewCriterion(CadphartPeer::DESDPH, '');
      $criterion->addOr($c->getNewCriterion(CadphartPeer::DESDPH, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['desdph']) && $this->filters['desdph'] !== '')
    {
      $c->add(CadphartPeer::DESDPH, '%'.strtr($this->filters['desdph'], '*', '%').'%', Criteria::LIKE);
      $c->setIgnoreCase(true);
    }
    ...
    {
      $c->add(CaartdphPeer::CODALM, strtr("%".$this->filters['codalm']."%", '*', '%'), Criteria::LIKE);
      $c->addJoin(CaartdphPeer::DPHART, CadphartPeer::DPHART);
      $c->setIgnoreCase(true);
    }
    if (isset($this->filters['nomalm_is_empty']))
    {
      $criterion = $c->getNewCriterion(CadphartPeer::NOMALM, '');
      $criterion->addOr($c->getNewCriterion(CadphartPeer::NOMALM, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['nomalm']) && $this->filters['nomalm'] !== '')
    {
      $c->add(CadefalmPeer::NOMALM, '%'.strtr($this->filters['nomalm'], '*', '%').'%', Criteria::LIKE);
      $c->addJoin(CaartdphPeer::CODALM,CadefalmPeer::CODALM);
      $c->addJoin(CadphartPeer::DPHART,CaartdphPeer::DPHART);
      $c->setIgnoreCase(true);
    }
