  ...
  public function executeEdit()
    {
      ...
      $this->configGridParFab();
      $this->configGridRefcru();
      $this->configGridAplMon();
  ...
  public function configGrid($numsol='')
    {
      ...
      if ($numsol!=''){
        $per=array();
        $t= new Criteria();
        $t->add(ManexicatPeer::NUMSOL,$numsol);
        $regt= ManexicatPeer::doSelect($t);
        if ($regt){
          foreach ($regt as $objt) {
            $new_caartalm= new Caartalm();
            $new_caartalm->setCodalm($objt->getCodalm());
            $new_caartalm->setEximin($objt->getEximin());
            $new_caartalm->setEximax($objt->getEximax());
            $new_caartalm->setPtoreo($objt->getPtoreo());
            $per[]=$new_caartalm;
          }
        }
      }else {
      ...
      $col1->setCatalogo('cadefalm','sf_admin_edit_form', array('codalm'=> 1, 'nomalm'=> 2),'Cadelfalm_Almordrec');
...
*/
  ...
  /*  if ($caregart->getId())
    ...
    {*/
     ...
     $gridparfab=Herramientas::CargarDatosGridv2($this,$this->obj4);
     $gridapl=Herramientas::CargarDatosGridv2($this,$this->obj7);
     $gridref=Herramientas::CargarDatosGridv2($this,$this->obj6);
      /*try
      {*/

       Articulos::salvarAlmregart($caregart,$grid,$gridunid,$gridpar,$gridcod,$gridparfab,$gridapl,$gridref);
     /* }
       catch (Exception $ex)
      {*/
        $coderr = 0;
  ...
  public function executeAjax()
  {
   ...
   $this->ajaxs='';
    if ($this->getRequestParameter('ajax')=='1')
      ...
      {
        $c=new Criteria();
        $c->add(CadefalmPeer::CODALM,$this->getRequestParameter('codigo'));
        ...
        else
        {
		        ...
		        if ($this->getRequestParameter('tipart')=='true')
		          $tipo="A";
		             ...
		             $tipo="S";
            else if ($this->getRequestParameter('tipman')=='true')
                 $tipo="M";
  ...
  else  if ($this->getRequestParameter('ajax')=='7')
    {
      ...
      else  if ($this->getRequestParameter('ajax')=='10')
      {
            ...
            $c=new Criteria();
            $c->add(MancatmatPeer::NUMSOL,$codigo);
            $datos=MancatmatPeer::doSelectOne($c);
            if (!$datos)
            {
             $javascript="alert('El N&uacute;mero de Solicitud no existe');$('caregart_numsol').value='';";
            }else {
                $com=',["caregart_desart","'.$datos->getDesite().'",""],["caregart_unimed","'.$datos->getUnimed().'",""]';
                $javascript="CargarExistencia('$codigo'); CargarPartes('$codigo'); CargarApls('$codigo'); CargarReferencias('$codigo'); ";
            }
  ...
  public function setVars()
  {
    ...
    $this->mascaraci =Herramientas::getX('Codemp','Cidefniv','Forpre','001');
    $this->longci=strlen($this->mascaraci);
  }
   ...
   *
   */
  public function configGridParFab($numsol='')
  {
    if ($numsol!=''){
        $per=array();
        $t= new Criteria();
        $t->add(ManparfabcatPeer::NUMSOL,$numsol);
        $regt= ManparfabcatPeer::doSelect($t);
        if ($regt){
          foreach ($regt as $objt) {
            $new_caparfabart= new Caparfabart();
            $new_caparfabart->setCodfab($objt->getCodfab());
            $new_caparfabart->setNumpar($objt->getNumpar());
            $per[]=$new_caparfabart;
          }
        }
      }else {
        $c = new Criteria();
        $c->add(CaparfabartPeer::CODART,$this->caregart->getCodart());
        $per = CaparfabartPeer::doSelect($c);
      }

     $this->columnas = Herramientas :: getConfigGrid(sfConfig :: get('sf_app_module_dir') . '/almregart/' . sfConfig :: get('sf_app_module_config_dir_name') . '/par_fab');

     $this->obj4 = $this->columnas[0]->getConfig($per);
  }
   ...
   *
   */
  public function configGridRefcru($numsol='')
  {
      if ($numsol!=''){
        $per=array();
        $t= new Criteria();
        $t->add(ManrefcruPeer::NUMSOL,$numsol);
        $regt= ManrefcruPeer::doSelect($t);
        if ($regt){
          foreach ($regt as $objt) {
            $new_carefcruart= new Carefcruart();
            $new_carefcruart->setStccod($objt->getStccod());
            $new_carefcruart->setRefere($objt->getRefere());
            $per[]=$new_carefcruart;
          }
        }
      }else {
        $c = new Criteria();
        $c->add(CarefcruartPeer::CODART,$this->caregart->getCodart());
        $per = CarefcruartPeer::doSelect($c);
      }

     $this->columnas = Herramientas :: getConfigGrid(sfConfig :: get('sf_app_module_dir') . '/almregart/' . sfConfig :: get('sf_app_module_config_dir_name') . '/grid_refcru');
     $this->columnas[1][1]->setCombo(array('E' => 'Ensamblaje', 'R' => 'Reemplazo', 'S' => 'Sustituto', 'C' => 'En Conjunto'));
     
     $this->obj6 = $this->columnas[0]->getConfig($per);
  }  
   ...
   *
   */
  public function configGridAplMon($numsol='')
  {
      if ($numsol!=''){
        $per=array();
        $t= new Criteria();
        $t->add(ManaplmonPeer::NUMSOL,$numsol);
        $regt= ManaplmonPeer::doSelect($t);
        if ($regt){
          foreach ($regt as $objt) {
            $new_caaplmonart= new Caaplmonart();
            $new_caaplmonart->setNumequ($objt->getNumequ());
            $new_caaplmonart->setCanins($objt->getCanins());
            $per[]=$new_caaplmonart;
          }
        }
      }else {
        $c = new Criteria();
        $c->add(CaaplmonartPeer::CODART,$this->caregart->getCodart());
        $per = CaaplmonartPeer::doSelect($c);
      }

    $this->columnas = Herramientas :: getConfigGrid(sfConfig :: get('sf_app_module_dir') . '/almregart/' . sfConfig :: get('sf_app_module_config_dir_name') . '/grid_apl');

    $this->obj7 = $this->columnas[0]->getConfig($per);
  }   
  ...
  public function configGridCodigosAlt()
  {
     ...
     $this->columnas = Herramientas :: getConfigGrid(sfConfig :: get('sf_app_module_dir') . '/almregart/' . sfConfig :: get('sf_app_module_config_dir_name') . '/grid_codigos');

     $mancodbar = H::getConfApp2('mancodbar', 'compras', 'almregart');
     
     if ($mancodbar=='S'){
       $this->columnas[0]->setTitulo('Códigos de Barra');
       $this->columnas[1][1]->setCombo(array('S'=>'Si','N'=>'No'));
 ...
 */
  public function executeAjaxgrid() {
    ...
    $javascript="";  $jsonextra="";
      switch ($name) {
        case ('g'):   //grid Partes - Fabricantes
            switch ($col) { 
              case ('2'):  //Código del Fabricante
                $c= new Criteria();
