...
protected function addFiltersCriteria($c)
  {
    ...
    {
      $criterion = $c->getNewCriterion(CasalalmPeer::CODPRO, '');
      $criterion->addOr($c->getNewCriterion(CasalalmPeer::CODPRO, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['codpro']) && $this->filters['codpro'] !== '')
    {
      $c->add(CasalalmPeer::CODPRO, strtr("%".$this->filters['codpro']."%", '*', '%'), Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
    ...
    if (isset($this->filters['observ_is_empty']))
    {
      $criterion = $c->getNewCriterion(CasalalmPeer::OBSERV, '');
      $criterion->addOr($c->getNewCriterion(CasalalmPeer::OBSERV, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['observ']) && $this->filters['observ'] !== '')
    {
      $c->add(CasalalmPeer::OBSERV, strtr("%".$this->filters['observ']."%", '*', '%'), Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
    if (isset($this->filters['reqart_is_empty']))
    {
      $criterion = $c->getNewCriterion(CasalalmPeer::REQART, '');
      $criterion->addOr($c->getNewCriterion(CasalalmPeer::REQART, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['reqart']) && $this->filters['reqart'] !== '')
    {
      $c->add(CasalalmPeer::REQART, strtr("%".$this->filters['reqart']."%", '*', '%'), Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
