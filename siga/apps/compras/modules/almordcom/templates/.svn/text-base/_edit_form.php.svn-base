...
<?php $mascararif2=H::getX_vacio('CODEMP', 'Cadefart', 'forpro', '001'); ?>    
  ...
  'onKeyDown' => " if ($('id').value=='' && '$mascararif2'!='') {javascript: cadena=this.value;cadena=cadena.toUpperCase();document.getElementById('caordcom_rifpro').value=cadena; dFilter (event.keyCode, this,'$mascararif2')}",
  'onBlur'=> remote_function(array(
...
if ($ocullic!='S')
  echo __(" $etiqtipord ").radiobutton_tag('caordcom[tipo]', 'L', $v2) ?>&nbsp;
...
<?php echo label_for('caordcom[numcon]', __($labels['caordcom{numcon}']), 'class="required"') ?>
<div  class="content<?php if ($sf_request->hasError('caordcom{numcon}')): ?> form-error<?php endif; ?>">
<?php if ($sf_request->hasError('caordcom{numcon}')): ?> <?php echo form_error('caordcom{numcon}', array('class' => 'form-error-msg')) ?>
<?php endif; ?>
  <?php $value = object_input_tag($caordcom, 'getNumcon', array (
  'size' => 20,
  'maxlength' => 20,
  'control_name' => 'caordcom[numcon]',
)); echo $value ? $value : '&nbsp;' ?>
</div>
 ...
 function Mostrar_orden_preimpresa()
  {
      ...
      if(confirm("¿Desea imprimir la orden Pre-Impresa?"))
      {
        ...
        if (moneord!=monofi)
        {
           pagina=ruta+"/<?php echo $sf_user->getAttribute('reportes_web');?>/jasper.php?m=compras&r=carordpreext.php&s=<?php echo $sf_user->getAttribute('schema');?>&ordcomdes="+ordcomdes+"&ordcomhas="+ordcomhas+"&codprodes="+codprodes+"&codprohas="+codprohas+"&codartdes="+codartdes+"&codarthas="+codarthas+"&fecorddes="+fecorddes+"&fecordhas="+fecordhas+"&tipcom="+tipcom+"&doccom="+tipcom;
        }else {
        if (nombrerep!=''){         
          if (mostrarjasper=='S') 
              pagina=ruta+"/<?php echo $sf_user->getAttribute('reportes_web');?>/jasper.php?m=compras&r="+nombrerep+".php&s=<?php echo $sf_user->getAttribute('schema');?>&ordcomdes="+ordcomdes+"&ordcomhas="+ordcomhas+"&codprodes="+codprodes+"&codprohas="+codprohas+"&codartdes="+codartdes+"&codarthas="+codarthas+"&fecorddes="+fecorddes+"&fecordhas="+fecordhas+"&tipcom="+tipcom+"&doccom="+tipcom;
          else
            pagina=ruta+"/reportes/reportes/compras/r.php?r="+nombrerep+".php&ordcomdes="+ordcomdes+"&ordcomhas="+ordcomhas+"&codprodes="+codprodes+"&codprohas="+codprohas+"&codartdes="+codartdes+"&codarthas="+codarthas+"&fecorddes="+fecorddes+"&fecordhas="+fecordhas+"&tipcom="+tipcom+"&doccom="+tipcom;
        }else {

          if (mostrarjasper=='S') 
              pagina=ruta+"/<?php echo $sf_user->getAttribute('reportes_web');?>/jasper.php?m=compras&r=<?php echo $caordcom->getReptipcom();?>&s=<?php echo $sf_user->getAttribute('schema');?>&ordcomdes="+ordcomdes+"&ordcomhas="+ordcomhas+"&codprodes="+codprodes+"&codprohas="+codprohas+"&codartdes="+codartdes+"&codarthas="+codarthas+"&fecorddes="+fecorddes+"&fecordhas="+fecordhas+"&tipcom="+tipcom+"&doccom="+tipcom;
          else
