...
class almordcomActions extends autoalmordcomActions
{
  ...
  $this->caordcom = $this->getCaordcomOrCreate();
    ...
    if($this->getRequest()->getMethod() == sfRequest::POST)
    {
      ...
      if ($valfecultreg=='S')
      {
       ...
       if(H::getConfApp2('valcencos', 'compras', 'almordcom')=='S'){
        ...
        if ($moneda2!=$moneda) {
          ...
          while ($i<count($grid_detalle_orden_arreglos))
          {
            ...
            if ($tiporec=='C') {       
              if ($moneda2!=$moneda) $elmonto=H::toFloat(str_replace("'","",$grid_detalle_orden_arreglos[$i][$campo13]))*H::toFloat($valor);
  ...
  else  if ($this->getRequestParameter('ajax')=='3')
  {
    ...
    if ($tipord=='A') $tipord='M';
    if($tipord=='S' || $this->getRequestParameter('tipord')=='T') $c->add(CaregartPeer::TIPO,$tipord);
...
//    $dato4=NppartidasPeer::getNompar($dato3);
  ...
  else  if ($this->getRequestParameter('ajax')=='13')
  {
    ...
    if (Herramientas::BuscarDatos($sql,$result))
    {
      ...
      $mondis=SolicituddeEgresos::montoDisponible($codpre);
      if ($monto > $mondis)
      {
  ...
  public function configGrid($ordcom='',$referencia='',$tipo='')
  {
        ...
        foreach ($per as $objw) {
          ...
          $datosrecargo="";
           ...
           if ($result)
           {
              ...
              {                 
                 $reccal=SolicituddeEgresos::CalcularRecargos($datos->getTiprgo(),$datos->getMonrgoc(),H::toFloat($objw->getCancost()));
                 $monrgoc=number_format($datos->getMonrgoc(),2,',','.');
                 if ($datos->getTiprgo()=='M')
                   $reccal=number_format($datos->getMonrgo(),2,',','.');
          ...
          $objw->setMonrgo(H::toFloat($acum));
          $objw->setDatosrecargo($datosrecargo);
          }
          $objw->setMontot((H::toFloat($objw->getCancost())+(H::toFloat($objw->getMonrgo())*$valor)-($objw->getMondes()*$valor))*$valor);
          $per1[]=$objw;
        }
        $per=$per1;
      }
    ...
    if ($referencia==0 and $filas_arreglo>0) $col4->setJScript('onKeyDown="javascript:return dFilter (event.keyCode, this,'.chr(39).$formatocategoria.chr(39).')" onKeyPress="javascript:cadena=rayaenter(event,this.value);if (event.keyCode==13 || event.keyCode==9){document.getElementById(this.id).value=cadena;}" onBlur="javascript:event.keyCode=13; actualizo_cod_presupuestario(this.id);ajax_detalle_codigo_pre(event,this.id); actualizar_grid_dependientes();"');
    ...
    $params = array("'+$(this.id).up().previous(15).descendants()[0].value+'",'val2');
    ...
    if ($filcencos=='S')
        $col20->setCatalogo('Cadefcen','sf_admin_edit_form', array('codcen' => 20,'descen' => 21),'Cadefcen_Almsolegr2',$params);
    else
    ...
    $col22->setHTML('type="text" size="7" maxlength="6"');
    $col22->setCatalogo('Cadefunimed','sf_admin_edit_form', array('codunimed' => 22,'desunimed' => 23),'Cadefunimed_Almregart');
    $col22->setAjax('almordcom',24,23);
    ...
    $col23->setHTML('type="text" size="25" readonly="true"');    
...
//<--------------------------------------------------------------------------------------------------------------------------------------------------->
  ...
  {
  //<!-----------------------------guardo comprobante---------------------->
      if ($this->getUser()->getAttribute('grabo',null,$this->getUser()->getAttribute('formulario'))=='S' && $this->getRequestParameter('caordcom[genctaord]')=='S')
      {
...
//<--------------------------------------------------------------------------------------------------------------------------------------------------->
  ...
  protected function updateCaordcomFromRequest()
  {
    ...
    if (isset($caordcom['numcon']))
    {
      $this->caordcom->setNumcon($caordcom['numcon']);
    }
  ...
  if ($this->getRequestParameter('ajax')=='1')
  {
    ...
    if ($numero_filas > 0)
    {
      ...
      else
      {
        ...
        if (Herramientas::BuscarDatos($sql,$result)){
          ...
          $numproc = H::getX_vacio('reqart','Casolart','numproc',trim($this->getRequestParameter('ordcom')));
          $motreq = eregi_replace("[\n|\r|\n\r]", "", H::getX_vacio('reqart','Casolart','motreq',trim($this->getRequestParameter('ordcom'))));
          ...
          $js="$('caordcom_valmon').readOnly=true; $('moneref').value='".$tipmon."';";

          $output = '[["'.$cajtexmos.'","'.$dato.'",""],["'.$filas_orden.'","'.$numero_filas.'",""],["'.$cajita.'","'.$validacion_fec_egresos.'",""],["caordcom_desord","'.$desreq.'",""],["caordcom_tipfin","'.$filas[0]->getTipfin().'",""],["caordcom_nomfin","'.$desfin.'",""],["caordcom_codcen","'.$codcen.'",""],["caordcom_descen","'.$descen.'",""],["caordcom_numproc","'.$numproc.'",""],["caordcom_justif","'.$motreq.'",""],["caordcom_tipmon","'.$tipmon.'",""],["caordcom_valmon","'.$valmon.'",""],["javascript","'.$js.'",""]'.$provee.']';
          $this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
          if ($codigopro!=""){
    ...
    $javascript="alert('La Referencia : ".$this->getRequestParameter('refsol').", no existe o no esta aprobado...'); $('caordcom_refsol').value=''; $('caordcom_refsol').focus();";
...
/*     if (($refsol!='') and ($this->getRequestParameter('codigo')!='') and ($parcial=='S') and ($seguir))
  ...
  public function executeCreate()
  {
    ...
    return $this->forward('almordcom', 'edit');
  }
