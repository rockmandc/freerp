...
<?php echo input_hidden_tag('casalalm[manunialt]', $casalalm->getManunialt()) ?>
...
if ($modulo=='facturacion')
    echo  button_to_popup('...',cross_app_link_to('herramientas','catalogo').'/metodo/Rifcli_Fapedido/clase/Facliente/frame/sf_admin_edit_form/obj1/casalalm_codpro/obj2/casalalm_nompro/campo1/codpro/campo2/nompro','','','botoncat');
else 
    echo  button_to_popup('...',cross_app_link_to('herramientas','catalogo').'/metodo/Caprovee_Almentalm/clase/Caprovee/frame/sf_admin_edit_form/obj1/casalalm_codpro/obj2/casalalm_nompro/campo1/codpro/campo2/nompro','','','botoncat');    
?>
    <?php } else { ?>
...
<br>
 <?php echo label_for('casalalm[coddirec]', __($labels['casalalm{coddirec}']), 'class="required" ') ?>
  <div class="content<?php if ($sf_request->hasError('casalalm{coddirec}')): ?> form-error<?php endif; ?>">
  <?php if ($sf_request->hasError('casalalm{coddirec}')): ?>
    <?php echo form_error('casalalm{coddirec}', array('class' => 'form-error-msg')) ?>
  <?php endif; ?>

  <?php $value = object_input_tag($casalalm, 'getCoddirec', array (
    'size' => 20,
    'control_name' => 'casalalm[coddirec]',
    'maxlength' => 4,
    'onBlur'=> remote_function(array(
      'url'      => 'almreq/ajax',
      'condition' => "$('casalalm_coddirec').value != '' && $('id').value == ''",
      'complete' => 'AjaxJSON(request, json)',
      'with' => "'ajax=9&cajtexmos=casalalm_desdirec&cajtexcom=casalalm_coddirec&codigo='+this.value"
          ))
  )); echo $value ? $value : '&nbsp;' ?>

  <?php echo  button_to_popup('...',cross_app_link_to('herramientas','catalogo').'/metodo/Cadefdirec_Almdefdiv/clase/Cadefdirec/frame/sf_admin_edit_form/obj1/casalalm_coddirec/obj2/casalalm_desdirec/campo1/coddirec/campo2/desdirec','','','botoncat')?>

   <?php $value = object_input_tag($casalalm, 'getDesdirec', array (
  'disabled' => true,
  'size' => 60,
  'control_name' => 'casalalm[desdirec]',
)); echo $value ? $value : '&nbsp;' ?>
</div>
...
<script type="text/javascript">
  ...
  {
      var  codsal='<? echo $casalalm->getCodsal();?>';

      var  ruta='http://'+'<?echo $this->getContext()->getRequest()->getHost();?>';
      
      pagina=ruta+"/<?php echo $sf_user->getAttribute('reportes_web');?>/compras/r.php?r=carsalalm.php&codsaldes="+codsal+"&codsalhas="+codsal;

      window.open(pagina,codsal,"menubar=yes,toolbar=yes,scrollbars=yes,width=1200,height=800,resizable=yes,left=1000,top=80");      
  }

  function combounidad(id){
   var aux = id.split("_");
   var name=aux[0];
   var fil=parseInt(aux[1]);
   var col=parseInt(aux[2]);
  ...
  if ($('casalalm_manunialt').value=='S')
  {
      new Ajax.Updater(unidad, getUrlModuloAjax(), {asynchronous:true, evalScripts:true, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=10&id='+$('id').value+'&codigo='+cod});
  }
 }
</script>