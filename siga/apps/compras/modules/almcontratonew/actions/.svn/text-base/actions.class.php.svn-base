...
class almcontratonewActions extends autoalmcontratonewActions
{
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      ...
      case '4':
        ...
        break;
      case '5':
          $anno=substr($this->getRequestParameter('fecha'),6,4);
          $mes=substr($this->getRequestParameter('fecha'),3,2);
          $idtot=$this->getRequestParameter('idtot');
          if (Herramientas::Monto_disponible_ejecucion($anno,$this->getRequestParameter('codpre'),$mes,$mondis))
          {
  ...
  {
    Herramientas::EliminarRegistro("Cadetordc", "Ordcon", $clasemodelo->getOrdcon());
    Herramientas::EliminarRegistro("Cacrocon", "Ordcon", $clasemodelo->getOrdcon());
    Herramientas::EliminarRegistro("Caconcla", "Ordcon", $clasemodelo->getOrdcon());
    Herramientas::EliminarRegistro("Cafiacon", "Ordcon", $clasemodelo->getOrdcon());
  ...
  {
    $o= new Criteria();
    $o->add(CadefartPeer::CODEMP,'001');
    $defmon=CadefartPeer::doSelectOne($o); 
    if ($defmon) {
      if ($defmon->getTipdoccon()=='') {
        $this->getRequest()->setError('valida', 'Debe definir en el Modulo de Compras -> Definición de Artículos el Tipo de Documento del Contrato.');
        return $this->forward('almcontratonew', 'list');
      }
    }
    return $this->forward('almcontratonew', 'edit');
  }
}
