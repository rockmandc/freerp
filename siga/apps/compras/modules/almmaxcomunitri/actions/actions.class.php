...
/**
 * almmaxcomunitri actions.
 *
 * @package    siga
 * @subpackage almmaxcomunitri
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class almmaxcomunitriActions extends autoalmmaxcomunitriActions
{
      ...
      case '1':
        ...
        if ($reg)
        {
           $dato=$reg->getDesdirec();                     
            ...
            $js="alert_('El Estado no existe o no esta asociada al usuario'); $('caunitridir_coddirec').value=''; $('caunitridir_coddirec').focus();";
        }
  ...
  public function validateEdit()
  {
    ...
    if($this->getRequest()->getMethod() == sfRequest::POST){
      ...
      $this->caunitridir = $this->getCaunitridirOrCreate();
      try{ $this->updateCaunitridirFromRequest();}
      catch (Exception $ex){}      

      if (!$this->caunitridir->getId()){
        $r= new Criteria();
        $r->add(CaunitridirPeer::CODDIREC,$this->caunitridir->getCoddirec());
        $r->add(CaunitridirPeer::FECVIG,$this->caunitridir->getFecvig());
        $regr= CaunitridirPeer::doSelectOne($r);
        if ($regr)
          $this->coderr=3007;
      }
