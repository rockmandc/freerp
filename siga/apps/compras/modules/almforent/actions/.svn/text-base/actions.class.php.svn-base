...
class almforentActions extends autoalmforentActions
{
  ...
  protected function updateCaforentFromRequest()
	{
		...
		{
      $c = new Criteria();
      $c->addDescendingOrderByColumn(CaforentPeer::CODFORENT);
      $max = CaforentPeer::doSelectOne($c);
      if($max){
        $max_codforent = intval($max->getCodforent());
        $codforent = str_pad($max_codforent + 1, 4, '0', STR_PAD_LEFT);
      }else{
        $codforent = '0001';
      }
  	  $this->caforent->setCodforent($codforent);
    }
  ...
  {
    $this->caforent = CaforentPeer::retrieveByPk($this->getRequestParameter('id'));
    $this->forward404Unless($this->caforent);

    try
    {
      $tiene = H::getX_vacio('Forent','Cacotiza','Forent',$this->caforent->getCodforent());
      $tiene2 = H::getX_vacio('Forent','Caordcom','Forent',$this->caforent->getCodforent());
      if ($tiene=='' && $tiene2=='') {
        $this->deleteCaforent($this->caforent);
        $this->Bitacora('Elimino');
      }else {
        $this->getRequest()->setError('delete', 'No se pudo borrar la registro seleccionado. Asegúrese de que no tiene ningún tipo de registros asociados.');
        return $this->forward('almforent', 'list');
      }
    }
    ...
    return $this->redirect('almforent/list');
  }
