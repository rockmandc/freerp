...
public function executeList()
  {
    ...
    $this->filters = $this->getUser()->getAttributeHolder()->getAll('sf_admin/carcpart/filters');

    $loguse= $this->getUser()->getAttribute('loguse');    
    $filsoldir=H::getConfApp2('filsoldir', 'compras', 'almsolegr'); 

     // 15    // pager
    $this->pager = new sfPropelPager('Carcpart', 15);
    $c = new Criteria();
    if ($filsoldir=='S'){
      $this->sql='carcpart.coddirec in (select coddirec from "SIMA_USER".segdirusu where loguse=\''.$loguse.'\')';
      $c->add(CarcpartPeer::CODDIREC,$this->sql,Criteria::CUSTOM); 
    }   
     ...
     $cajtexcom=$this->getRequestParameter('cajtexcom');
	  ...
	  if ($codalm!="")
	  {
          ...
          else  if ($this->getRequestParameter('ajax')=='7')
	    {
            ...
            $cangrireal=$this->getRequestParameter('cangrireal');
            ...
            $output = '[["'.$cangri.'","'.$canrec.'",""],["'.$cangrireal.'","'.$canrec.'",""],["'.$tot.'","'.$total.'",""],["javascript","'.$javascript.'",""]]';
	    }
            else  if ($this->getRequestParameter('ajax')=='8')
          {
            ...
            if ($reg)
            {
               $dato=$reg->getDesunimed(); $javascript="";
                ...
                $dato="";
                $javascript="alert_('La Unidad de Medida no existe'); $('$cajtexcom').value=''; $('$cajtexcom').focus();";
            }
            $output = '[["'.$cajtexmos.'","'.$dato.'",""],["javascript","'.$javascript.'",""]]';
          }
	   		...
	   		if ($datos)
	   		{
	             ...
	             if ($datos->getFecord()<=$fecreccom)
	             {
	                ...
	                if (Herramientas::BuscarDatos($sql,$result))
	                {
                        ...
                        $refpda=H::getX_vacio('Ordcom','Cadetpda','Refpda',$datos->getOrdcom());
                        if ($refpda!="")
                        {
                            Recepcion::buscarDetalleOC($this->getRequestParameter('codigo'),$refpda,$arreglo);
                            $this->configGrid($this->getRequestParameter('codigo'),$this->getRequestParameter('codalm'),$arreglo);
                        }else {
                            $this->configGrid($this->getRequestParameter('codigo'),$this->getRequestParameter('codalm'));
                        }
                                        
			            $output = '[["carcpart_fecord","'.$fecha.'",""],["carcpart_codpro","'.$codpro.'",""],["carcpart_nompro","'.$nompro.'",""],["carcpart_desconpag","'.$conpag.'",""],["carcpart_desforent","'.$forent.'",""],["carcpart_desrcp","'.$des.'",""],["carcpart_monrcp","'.$monord.'",""],["carcpart_codcen","'.$codcen.'",""],["carcpart_descen","'.$descen.'",""],["carcpart_coddirec","'.$coddirec.'",""],["carcpart_desdirec","'.$desdirec.'",""],["'.$cajtexcom.'","8","c"]]';
			            $this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
			            ////
 ...
 protected function getCarcpartOrCreate($id = 'id')
  {
    if (!$this->getRequestParameter($id))
    {
      ...
      $refpda=H::getX_vacio('Ordcom','Cadetpda','Refpda',$this->getRequestParameter('carcpart[ordcom]'));
      if ($refpda!="")
      {
         Recepcion::buscarDetalleOC($this->getRequestParameter('carcpart[ordcom]'),$refpda,$arreglo);
         $this->configGrid($this->getRequestParameter('carcpart[ordcom]'),$this->getRequestParameter('carcpart[codalm]'),$arreglo);
      }else {
   ...
   *
   */
  public function configGrid($codigo='',$codalm='', $arreglo=array())
	 {
	//////////////////////
	//GRID
    ...
    if (count($arreglo)>0)
    {
        ...
        {
            $caartordnew= new Caartord();
            $caartordnew->setCodart($arreglo[$j]["codart"]);
            $caartordnew->setDesart($arreglo[$j]["desart"]);
            $caartordnew->setCanrecgri($arreglo[$j]["canrecgri"]);
            $caartordnew->setPreart($arreglo[$j]["preart"]);
            $caartordnew->setDtoart($arreglo[$j]["dtoart"]);
            $caartordnew->setRgoart($arreglo[$j]["rgoart"]);
            $caartordnew->setMontot($arreglo[$j]["montot"]);
            $caartordnew->setCodcat($arreglo[$j]["codcat"]);
            $caartordnew->setFecest($arreglo[$j]["fecest"]);
            $caartordnew->setCanrecgrireal($arreglo[$j]["canrecgri"]);
            $caartordnew->setCanord($arreglo[$j]["canord"]);
            $caartordnew->setCodalm($arreglo[$j]["codalm"]);
            $caartordnew->setNomalm($arreglo[$j]["nomalm"]);
            $caartordnew->setCodubi($arreglo[$j]["codubi"]);
            $caartordnew->setNomubi($arreglo[$j]["nomubi"]);
            $caartordnew->setCodunimed($arreglo[$j]["codunimed"]);
            $per[]=$caartordnew;
            $j++;
        }
    }
   ...
   $manartlot=H::getConfApp2('manartlot', 'compras', 'almregart');
    ...
    $col27 = new Columna('Unidad de Medida');
    $col27->setTipo(Columna::TEXTO);
    $col27->setEsGrabable(true);
    $col27->setAlineacionObjeto(Columna::CENTRO);
    $col27->setAlineacionContenido(Columna::CENTRO);
    $col27->setNombreCampo('codunimed');
    $col27->setHTML('type="text" size="10" maxlength="6"');
    $col27->setCatalogo('Cadefunimed','sf_admin_edit_form', array('codunimed' => 27,'desunimed' => 28),'Cadefunimed_Almregart');
    $col27->setAjax('almordrec',8,28);

    $col28 = new Columna('Descripción');
    $col28->setTipo(Columna::TEXTO);
    $col28->setEsGrabable(true);
    $col28->setAlineacionObjeto(Columna::IZQUIERDA);
    $col28->setAlineacionContenido(Columna::IZQUIERDA);
    $col28->setNombreCampo('desunimed');
    $col28->setHTML('type="text" size="25" readonly="true"');      
  ...
  public function configGridConsulta($rcpart)
    ...
    $col17->setHTML('type="text" size="30x1" readonly=true');
     ...
     $manartlot=H::getConfApp2('manartlot', 'compras', 'almregart');
      ...
      $col22 = new Columna('Unidad de Medida');
      $col22->setTipo(Columna::TEXTO);
      $col22->setAlineacionObjeto(Columna::CENTRO);
      $col22->setAlineacionContenido(Columna::CENTRO);
      $col22->setEsGrabable(true);
      $col22->setNombreCampo('codunimed');
      $col22->setHTML('type="text" size="10" readonly=true');

      $col23 = new Columna('Descripción');
      $col23->setTipo(Columna::TEXTAREA);
      $col23->setEsGrabable(true);
      $col23->setNombreCampo('desunimed');
      $col23->setAlineacionObjeto(Columna::CENTRO);
      $col23->setAlineacionContenido(Columna::CENTRO);
      $col23->setHTML('type="text" size="30x1" readonly=true');      
...
$this->Bitacora('Guardo');
 ...
 /**
  ...
  protected function updateCarcpartFromRequest()
  {
    ...
    if (isset($carcpart['coddirec']))
    {
      $this->carcpart->setCoddirec($carcpart['coddirec']);
    }
  }
...
protected function addFiltersCriteria($c)
  {
    ...
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::RCPART, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::RCPART, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['rcpart']) && $this->filters['rcpart'] !== '')
    {
      $c->add(CarcpartPeer::RCPART, '%'.strtr($this->filters['rcpart'], '*', '%').'%', Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
    if (isset($this->filters['fecrcp_is_empty']))
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::FECRCP, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::FECRCP, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['fecrcp']))
    {
      ...
      {
        $criterion = $c->getNewCriterion(CarcpartPeer::FECRCP, date('Y-m-d', $this->filters['fecrcp']['from']), Criteria::GREATER_EQUAL);
      }
      if (isset($this->filters['fecrcp']['to']) && $this->filters['fecrcp']['to'] !== '')
      {
        if (isset($criterion))
        {
          $criterion->addAnd($c->getNewCriterion(CarcpartPeer::FECRCP, date('Y-m-d', $this->filters['fecrcp']['to']), Criteria::LESS_EQUAL));
        }
        else
        {
          $criterion = $c->getNewCriterion(CarcpartPeer::FECRCP, date('Y-m-d', $this->filters['fecrcp']['to']), Criteria::LESS_EQUAL);
        }
      }
    ...
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::ORDCOM, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::ORDCOM, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['ordcom']) && $this->filters['ordcom'] !== '')
    {
      $c->add(CarcpartPeer::ORDCOM, '%'.strtr($this->filters['ordcom'], '*', '%').'%', Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
    if (isset($this->filters['codcen_is_empty']))
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::CODCEN, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::CODCEN, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['codcen']) && $this->filters['codcen'] !== '')
    {
      $c->add(CarcpartPeer::CODCEN, '%'.strtr($this->filters['codcen'], '*', '%').'%', Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
    if (isset($this->filters['descen_is_empty']))
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::DESCEN, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::DESCEN, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['descen']) && $this->filters['descen'] !== '')
    {
      $c->add(CadefcenPeer::DESCEN, '%'.strtr($this->filters['descen'], '*', '%').'%', Criteria::LIKE);
      $c->addJoin(CarcpartPeer::CODCEN, CadefcenPeer::CODCEN);
      $c->setIgnoreCase(true);
    }
    if (isset($this->filters['codalm_is_empty']))
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::CODALM, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::CODALM, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['codalm']) && $this->filters['codalm'] !== '')
    {
      $c->add(CaartrcpPeer::CODALM, '%'.strtr($this->filters['codalm'], '*', '%').'%', Criteria::LIKE);
      $c->addJoin(CarcpartPeer::RCPART,CaartrcpPeer::RCPART);
      $c->setIgnoreCase(true);
    }
    if (isset($this->filters['nomalm_is_empty']))
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::NOMALM, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::NOMALM, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['nomalm']) && $this->filters['nomalm'] !== '')
    {
      $c->add(CadefalmPeer::NOMALM, '%'.strtr($this->filters['nomalm'], '*', '%').'%', Criteria::LIKE);
      $c->addJoin(CaartrcpPeer::CODALM,CadefalmPeer::CODALM);
      $c->addJoin(CarcpartPeer::RCPART,CaartrcpPeer::RCPART);
    }
    if (isset($this->filters['desrcp_is_empty']))
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::DESRCP, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::DESRCP, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['desrcp']) && $this->filters['desrcp'] !== '')
    {
      $c->add(CarcpartPeer::DESRCP, '%'.strtr($this->filters['desrcp'], '*', '%').'%', Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
    if (isset($this->filters['codedo_is_empty']))
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::CODEDO, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::CODEDO, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    ...
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::NOMEDO, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::NOMEDO, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    ...
    if (isset($this->filters['numfac_is_empty']))
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::NUMFAC, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::NUMFAC, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['numfac']) && $this->filters['numfac'] !== '')
    {
      $c->add(CarcpartPeer::NUMFAC, '%'.strtr($this->filters['numfac'], '*', '%').'%', Criteria::LIKE);
      $c->setIgnoreCase(true);
    }
    if (isset($this->filters['nroent_is_empty']))
    {
      $criterion = $c->getNewCriterion(CarcpartPeer::NROENT, '');
      $criterion->addOr($c->getNewCriterion(CarcpartPeer::NROENT, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['nroent']) && $this->filters['nroent'] !== '')
    {
      $c->add(CarcpartPeer::NROENT, '%'.strtr($this->filters['nroent'], '*', '%').'%', Criteria::LIKE);
      $c->setIgnoreCase(true);
    }
  }
   ...
   public function getLabels()
  {
    ...
    if ($cambiareti=='S')
      $labels['carcpart{coddirec}'] = 'Estado';
    return $labels;
  }
}
