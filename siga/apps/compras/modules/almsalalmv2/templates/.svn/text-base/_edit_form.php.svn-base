...
?>
<?php echo form_tag('almsalalmv2/edit', array(
...
<?php if(H::getConfApp2('usualms','autenticacion','login')=='S') : ?>
  ...
  <?php $fields = CasalalmPeer::getFieldNames(); ?>
  ...
  <?php if(array_search('Codalmusu', $fields)) :  ?>
    ...
    <div class="form-row" id="divusualms">
      <?php echo label_for('casalalm[codalmusu]', __('Almacenes Por Usuario: '), 'class="required" Style="text-align:left; width:150px"') ?>
      <div class="content">
      <?php
        ...
        if($codalm == '') echo label_for('casalalm[codalmusu]', __('Usuario sin Almacen Asociado'), 'class="required" Style="text-align:left; width:150px"').'<br><br><br>';
        else echo select_tag('casalalm[codalmusu]',options_for_select($almacenes,$casalalm->getCodalmusu()!='' ? $casalalm->getCodalmusu() : $codalm), ( ($casalalm->getId()) ? 'disabled=true' : '').' style=width:500px');
      ?>
      </div>
    </div>
    </fieldset>
...
<?php if ($casalalm->getId()=="") { ?>
  <?php echo input_auto_complete_tag('casalalm[codpro]', $casalalm->getCodpro(),
    'almsalalmv2/autocomplete?ajax=2',  array('autocomplete' => 'off','maxlength' => 18, 'onBlur'=> remote_function(array(
			  'url'      => 'almsalalmv2/ajax',
...
if ($modulo=='facturacion' || $modulo=='facturacionv2')
    echo  button_to_popup('...',cross_app_link_to('herramientas','catalogo').'/metodo/Rifcli_Fapedido/clase/Facliente/frame/sf_admin_edit_form/obj1/casalalm_codpro/obj2/casalalm_nompro/campo1/rifpro/campo2/nompro','','','botoncat');
else 
...
<br>
  ...
  <?php echo input_auto_complete_tag('casalalm[tipmov]', $casalalm->getTipmov(),
    'almsalalmv2/autocomplete?ajax=3',  array('autocomplete' => 'off','maxlength' => 3,   'readonly'  =>  $casalalm->getId()!='' ? true : false, 'onBlur'=> remote_function(array(
			  'url'      => 'almsalalmv2/ajax',
			  'complete' => 'AjaxJSON(request, json)',
...
<?php echo button_to(__('delete'), 'almsalalmv2/delete?id='.$casalalm->getId(), array (
  'post' => true,
