  ...
  public function autenticate_native($nombre,$passwd,$codemp,$db='')
  {
     ...
     if(array_key_exists('autenticacion',$varemp['aplicacion']))
       if(array_key_exists('modulos',$varemp['aplicacion']['autenticacion']))
         if(array_key_exists('login',$varemp['aplicacion']['autenticacion']['modulos'])){
           ...
           if(array_key_exists('valintdia',$varemp['aplicacion']['autenticacion']['modulos']['login']))
           {
            $valintdia=$varemp['aplicacion']['autenticacion']['modulos']['login']['valintdia'];
           }
         ...
         if(array_key_exists('pagemiord',$varemp['aplicacion']['tesoreria']['modulos'])){
           if(array_key_exists('filubiadm',$varemp['aplicacion']['tesoreria']['modulos']['pagemiord']))
           {
            $filubiadm=$varemp['aplicacion']['tesoreria']['modulos']['pagemiord']['filubiadm'];
           }
           if(array_key_exists('filcatpre',$varemp['aplicacion']['tesoreria']['modulos']['pagemiord']))
           {
            $filcatpre=$varemp['aplicacion']['tesoreria']['modulos']['pagemiord']['filcatpre'];
           }
         }
    }
    ...
    if($objUsuario)
    {
        if(trim($objUsuario[0]->getLoguse())==strtoupper($nombre)){
          ...
          if ($valintdia=='S'){
            ...
            sfContext::getInstance()->getLogger()->info(Constantes::Autenticacion.' Credenciales = '.$credenciales, 'info');
             ...
             $cad="";
             if ($usucat=='S' || $filcatpre=='S')
             {
              $h = new Criteria();
              $h->add(SegcatusuPeer::LOGUSE,strtoupper($nombre).'%',Criteria::LIKE);
            ...
            //llenar variable de sesión con Ubicaciones Administrativas asociadas al usuario
             $cad2="";
             if ($filubiadm=='S')
             {
              $b = new Criteria();
              $b->add(SegubausuPeer::LOGUSE,strtoupper($nombre).'%',Criteria::LIKE);
              $resultb = SegubausuPeer::doSelect($b);
              if($resultb)
              {
                $f=0;
                foreach ($resultb as $objrb) {
                  if ($f==0)
                    $cad2=$objrb->getCodubi();
                  else
                    $cad2=$cad2."*".$objrb->getCodubi();
                  $f++;
                }
              }
             }
             $this->setAttribute('ubiadmusu', $cad2);
            ...
            $dirrepconfig_cp = sfConfig::get('app_reportes').'reportes/config.yml';
            $configyml_sigacp = sfYaml::load($dirrepconfig_cp);
            $_SESSION["reportes_siga_cp"]=$configyml_sigacp;
            $_SESSION["reportes_web"]=sfConfig::get('app_reportes_web');
        ...
        if ($valint=='S')
        {
           ...
           $regdat = UsuariosPeer::doSelectOne($a);
           if ($regdat) {
              if ($regdat->getFecint()=='' or is_null($regdat->getFecint()))
              { 
                $regdat->setFecint(date('Y-m-d'));
                $regdat->setConint(1);
                $regdat->save();
                $credenciales = '8';
              }else {
                if ($regdat->getFecint()==date('Y-m-d'))
                {
                  if ($regdat->getConint()<=$numintentos) {
                    $regdat->setConint($regdat->getConint()+1);
                    $regdat->save();
                    $credenciales = '8';
                  }else {
                     $credenciales = '6';
