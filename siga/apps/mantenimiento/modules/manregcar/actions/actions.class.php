 ...
 *
 * @package    siga
 * @subpackage manregcar
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class manregcarActions extends automanregcarActions
{
    ...
    switch ($ajax){
      case '1':
          ...
          if ($reg)
          {
             ...
             $output = '[["'.$cajtexmos.'","'.$dato.'",""],["mancarlub_centro","'.$centro.'",""],
             ["mancarlub_estatus","'.$estatus.'",""],["mancarlub_tipequ","'.$tipequ.'",""],
             ["javascript","'.$js.'",""]]';
          }else {
            $js="alert_('El C&oacute;digo del Equipo no esta registrado'); $('mancarlub_numequ').value=''; $('mancarlub_nomequ').value=''; $('mancarlub_centro').value=''; $('mancarlub_estatus').value=''; $('mancarlub_tipequ').value=''; $('mancarlub_numequ').focus();";
            $output = '[["javascript","'.$js.'",""]]';        
          }
        break;
      case '2':
          $q= new Criteria();
          $q->add(MantiplubPeer::CODTLU,$codigo);
          $reg= MantiplubPeer::doSelectOne($q);
          if ($reg)
          {
             $dato=$reg->getDestlu();             
             $lubric=$reg->getTipo();
             $unidad=$reg->getCodume().' - '.$reg->getDesume();

             $output = '[["'.$cajtexmos.'","'.$dato.'",""],["mancarlub_lubric","'.$lubric.'",""],["mancarlub_unidad","'.$unidad.'",""],
             ["javascript","'.$js.'",""]]';
          }else {
            $js="alert_('El C&oacute;digo del Tipo de Lubricante no esta registrado'); $('mancarlub_codtlu').value=''; $('mancarlub_destlu').value=''; $('mancarlub_lubric').value=''; $('mancarlub_codtlu').focus();";
            $output = '[["javascript","'.$js.'",""]]';        
          }
        break;
      default:
