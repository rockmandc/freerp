...
/*    if (isset($bnsegmue['cobsegmue']))
   ...
   if (isset($bnsegmue['monsegmue']))
    ...
    if (isset($bnsegmue['fecvigdes']))
    {
      if ($bnsegmue['fecvigdes'])
      {
        try
        {
          $dateFormat = new sfDateFormat($this->getUser()->getCulture());
                              if (!is_array($bnsegmue['fecvigdes']))
          {
            $value = $dateFormat->format($bnsegmue['fecvigdes'], 'i', $dateFormat->getInputPattern('d'));
          }
          else
          {
            $value_array = $bnsegmue['fecvigdes'];
            $value = $value_array['year'].'-'.$value_array['month'].'-'.$value_array['day'].(isset($value_array['hour']) ? ' '.$value_array['hour'].':'.$value_array['minute'].(isset($value_array['second']) ? ':'.$value_array['second'] : '') : '');
          }
          $this->bnsegmue->setFecvigdes($value);
        }
        catch (sfException $e)
        {
          // not a date
      ...
      {
        $this->bnsegmue->setFecvigdes(null);
      }
    }
    if (isset($bnsegmue['fecvighas']))
    {
      if ($bnsegmue['fecvighas'])
      {
        try
        {
          $dateFormat = new sfDateFormat($this->getUser()->getCulture());
                              if (!is_array($bnsegmue['fecvighas']))
          {
            $value = $dateFormat->format($bnsegmue['fecvighas'], 'i', $dateFormat->getInputPattern('d'));
          }
          else
          {
            $value_array = $bnsegmue['fecvighas'];
            $value = $value_array['year'].'-'.$value_array['month'].'-'.$value_array['day'].(isset($value_array['hour']) ? ' '.$value_array['hour'].':'.$value_array['minute'].(isset($value_array['second']) ? ':'.$value_array['second'] : '') : '');
          }
          $this->bnsegmue->setFecvighas($value);
        }
        catch (sfException $e)
        {
          // not a date
      ...
      {
        $this->bnsegmue->setFecvighas(null);
      }
    }
    if (isset($bnsegmue['numdeppol']))
    {
      $this->bnsegmue->setNumdeppol($bnsegmue['numdeppol']);
    }
  }
 ...
 /**
  ...
  protected function addFiltersCriteria($c)
  {
    ...
    {
      $criterion = $c->getNewCriterion(BnsegmuePeer::CODACT, '');
      $criterion->addOr($c->getNewCriterion(BnsegmuePeer::CODACT, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['codact']) && $this->filters['codact'] !== '')
    {
      $c->add(BnsegmuePeer::CODACT, '%'.strtr($this->filters['codact'], '*', '%').'%', Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
    if (isset($this->filters['codmue_is_empty']))
    {
      $criterion = $c->getNewCriterion(BnsegmuePeer::CODMUE, '');
      $criterion->addOr($c->getNewCriterion(BnsegmuePeer::CODMUE, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['codmue']) && $this->filters['codmue'] !== '')
    {
      $c->add(BnsegmuePeer::CODMUE, '%'.strtr($this->filters['codmue'], '*', '%').'%', Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
    if (isset($this->filters['codalt_is_empty']))
    {
      $criterion = $c->getNewCriterion(BnsegmuePeer::CODALT, '');
      $criterion->addOr($c->getNewCriterion(BnsegmuePeer::CODALT, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['codalt']) && $this->filters['codalt'] !== '')
    {
      $c->add(BnregmuePeer::CODALT, strtr("%".$this->filters['codalt']."%", '*', '%'), Criteria::LIKE);
      $c->addJoin(BnsegmuePeer::CODMUE, BnregmuePeer::CODMUE);
      $c->addJoin(BnsegmuePeer::CODACT, BnregmuePeer::CODACT);
      $c->setIgnoreCase(true);
    }
  }  
}
