...
class biedefubiActions extends autobiedefubiActions
{
  ...
  public function executeAjax()
  {
   ...
   {    
    ...
    if ($result)
    {
      $forma=H::ObtenerFormato('Opdefemp','Forubi');
      if (strlen($codigo)==strlen($forma)) {
        $dato=$result->getDesubi();
        $output = '[["'.$cajtexmos.'","'.$dato.'",""]]';
      }else {
        $javascript="alert('El código no es de ultimo nivel...'); $('bnubibie_codubiadm').value='';";
        $dato="";
   ...
   else if ($this->getRequestParameter('ajax')=='2')
   {
       ...
       { 
        ...
        $formato=H::ObtenerFormato('Bndefins','forubi');
        if (strlen($codigo)!=strlen($formato))
           $javascript="$('bnubibie_codubiadm').readOnly=true; $$('.botoncat')[0].disabled=true";
         else $javascript="";
       }
