...
<?php $modulo=sfContext::getInstance()->getUser()->getAttribute('menu','','autenticacion'); ?>
  ...
  'size' => 20,
  'maxlength' => $modulo=='mantenimiento' ? 4: 6,
  'onBlur'  => "enter(this.id)",
...
<fieldset id="sf_fieldset_none" class="">
<legend><b><?echo __('Afecta La Vida Util Del Bien') ?></b></legend>
<div class="form-row">
...
<fieldset id="sf_fieldset_none" class="">
<legend><b><?echo __('Faltante') ?></b></legend>
<div class="form-row">
  <?php echo label_for('bndisbie[faltan]', __($labels['bndisbie{faltan}']), 'class="required" ') ?>
  <div class="content<?php if ($sf_request->hasError('bndisbie{faltan}')): ?> form-error<?php endif; ?>">
  <?php if ($sf_request->hasError('bndisbie{faltan}')): ?>
    <?php echo form_error('bndisbie{faltan}', array('class' => 'form-error-msg')) ?>
  <?php endif; ?>

  <?php if($bndisbie->getFaltan()=='S') $val = true; else $val=false; ?>
  <?php echo "Si ".radiobutton_tag('bndisbie[faltan]', 'S', $val) ?>&nbsp;
  <?php echo "No ".radiobutton_tag('bndisbie[faltan]', 'N', !$val) ?>
    </div>
    <br>
</div>
</fieldset>
...
function enter(id){
  ...
  document.getElementById('bndisbie_coddis').value=valor;
  document.getElementById('bndisbie_coddis').disabled=false;
}
</script>