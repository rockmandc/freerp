 ...
 /* public function executeList()
  {
    ...
    $c = new Criteria();
    if ($filconusu == 'S') {
      $sql = 'npasiconemp.codcon in (select codcon from "SIMA_USER".segconusu where loguse=\'' . $loguse . '\')';
      $c->add(NpasiconempPeer::CODCON, $sql, Criteria::CUSTOM);
      $c->addJoin(NpasicarempPeer::CODCAR,NpasiconempPeer::CODCAR);
  ...
  {
    return $this->redirect('nomnommovnomconcar/edit');
  }
  ...
  public function executeAjax()
  {
	 ...
	 else if ($this->getRequestParameter('ajax')=='4')
	 {
       ...
       $loguse = sfContext::getInstance()->getUser()->getAttribute('loguse');
       $js="";
       $cod=$this->getRequestParameter('codcon');

       $c = new Criteria();
       $c->add(NpdefmovPeer::CODNOM,$this->getRequestParameter('codnom'));
       if ($filconusu == 'S') {
          $sql = 'npdefcpt.codcon=\'' .$cod. '\' and npdefcpt.codcon in (select codcon from "SIMA_USER".segconusu where loguse=\'' . $loguse . '\')';
          $c->add(NpdefcptPeer::CODCON, $sql, Criteria::CUSTOM);
       }else $c->add(NpdefcptPeer::CODCON,$cod);             
       $reg=NpdefcptPeer::doSelectOne($c);
       if ($reg){
          $dato=$reg->getNomcon(); 
       }else {
           $dato="";
           $js="alert('El Concepto no existe'); $('$cajtexmos').value=''; $('$cajtexmos').focus();";
       }
