 ...
 public function configGridPersonal($codcur='',$numcla='',$id='')
  {
    ...
    $col7 = new Columna('Motivo de Inasistencia');
    $col7->setTipo(Columna::TEXTO);
    $col7->setEsGrabable(true);
    $col7->setAlineacionObjeto(Columna::CENTRO);
    $col7->setAlineacionContenido(Columna::CENTRO);
    $col7->setNombreCampo('codmot');
    $col7->setHTML('type="text" size="10" maxlength="4"');
    $col7->setCatalogo('Rhmotini','sf_admin_edit_form', array('codmot' =>8 ,'desmot' => 9),'Rhmotini_nomasicur');
    $col7->setAjax('nomasicur',2,9);
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      ...
      $dato="";
        ...
        $q= new Criteria();
        $q->add(RhmotiniPeer::CODMOT,$codigo);
        $regq=RhmotiniPeer::doSelectOne($q);
        if ($regq){
          $dato=$regq->getDesmot();
          $js="";
        }else $js="alert('No esta registrado ese Motivo'); $('$cajtexcom').value=''";
  ...
  public function saving($clasemodelo)
  {
	...
	foreach($grid[0] as $dat)
	{
      ...
      if($dat->getAsicla()=='0')
        $rhasicur->setCodmot($dat->getCodmot());
			$rhasicur->save();
		//}		
	}
    return '-1';
