...
//print $this->coderror1; exit;
  ...
  protected function updateNptipconFromRequest(&$alic = '',&$a146 = '', &$fid = '')
  {
    ...
    if (isset($nptipcon['fecfin108']))
    {
      if ($nptipcon['fecfin108'])
      {
        try
        {
          $dateFormat = new sfDateFormat($this->getUser()->getCulture());
                              if (!is_array($nptipcon['fecfin108']))
          {
            $value = $dateFormat->format($nptipcon['fecfin108'], 'i', $dateFormat->getInputPattern('d'));
          }
          else
          {
            $value_array = $nptipcon['fecfin108'];
            $value = $value_array['year'].'-'.$value_array['month'].'-'.$value_array['day'].(isset($value_array['hour']) ? ' '.$value_array['hour'].':'.$value_array['minute'].(isset($value_array['second']) ? ':'.$value_array['second'] : '') : '');
          }
          $this->nptipcon->setFecfin108($value);
        }
        catch (sfException $e)
        {
          // not a date
      ...
      {
        $this->nptipcon->setFecfin108(null);
      }
    }
    ...
    if (isset($nptipcon['mesini']))
    {
      $this->nptipcon->setMesini($nptipcon['mesini']);
    }
    if (isset($nptipcon['trimfis']))
    {
      $this->nptipcon->setTrimfis($nptipcon['trimfis']);
    }
        if (isset($nptipcon['apliregant']))
    {
      $this->nptipcon->setApliregant($nptipcon['apliregant']);
    }
  }
