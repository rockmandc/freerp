 ...
 * @version SVN: $Id: actions.class.php 59813 2014-12-05 03:27:47Z ogutierrez $
 *
...
private static $coderror=-1;
 ...
 /**
  ...
  public function executeAjax()
  {
    ...
    if ($this->getRequestParameter('ajax')=='2')
    {
	   ...
	   //$output = '[["javascript","'.$javascript.'",""]]';
         $output = '[["nphojint_suedia","'.$suedia.'",""],["nphojint_suediario","'.$suediario.'",""],["javascript","'.$javascript.'",""] ]';
  ...
  public function configGrid($codemp="", $nuevo="",&$ultimosueldo=0,&$ultsue=0,&$suenor=0,$sal='')
  {
	...
	########SUELDOS OTROS##########
	#SueldoUltimo
	$sql = "select distinct salemp as sueult from npimppresoc a where a.codemp='$codemp' and a.tipo IN ('','N')
	...
	if (H::BuscarDatos($sql,$rs))
		...
		$sueult= 0;
      #SueldoIntegral
      $sql = "select distinct (salemp+aliuti+alibono+aliadi) as sueult from npimppresoc a where a.codemp='$codemp' and a.tipo IN ('','N')
          and a.fecini=(select max(fecini) from npimppresoc where codemp=a.codemp and tipo IN ('','N')) ";
      if (H::BuscarDatos($sql,$rs))
        $sueldointegral=$rs[0]["sueult"];
      else
        $sueldointegral= 0;

	#Sueldo Promedio
	$sql =  "select coalesce(avg(salemp),0) as suepro from (
				select distinct salemp,fecini from npimppresoc where codemp='$codemp' and tipo IN ('','N')
	...
	if($suenor==0)
             $ultimosueldo=$sueldointegral+$porcion;
        else
        	$ultimosueldo=$suenor;
		#}
