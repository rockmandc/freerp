...
class nominscurActions extends autonominscurActions
{
  ...
  public function configGridPersonal($codcur='',$codplacur='')
  {
    $per=array();
    if (!$this->rhinscur->getId() && $codplacur!='')
    {
      $q= new Criteria();
      $q->add(RhdetplacurPeer::CODPLACUR,$codplacur);
      $resulq= RhdetplacurPeer::doSelect($q);
      if ($resulq){
        foreach ($resulq as $objq) {
          $rnicur= new Rhinscur();
          $rnicur->setCodemp($objq->getCodemp());
          $rnicur->setCodcar($objq->getCodcar());
          $rnicur->setTipper($objq->getTipper());
          $rnicur->setFecins(date('Y-m-d'));
          $per[]=$rnicur;
        }
      }
    }else {
  ...
  public function executeAjax()
  {
    ...
    case '3':
      ...
      $c = new Criteria();
      $c->add(RhplacurPeer::CODPLACUR,$codigo);
      $c->add(RhplacurPeer::CODCUR,$curso);
      $per = RhplacurPeer::doSelectOne($c);
      if($per)
      {
        $dato=$per->getDesplacur();
      }         
      else { 
        $js="alert('La planificación no existe o no esta asociada al Curso'); $('rhinscur_codplacur').value=''; $('rhinscur_desplacur').value=''; $('rhinscur_codplacur').focus();";
        $codigo='';
        $curso='';
      } 
      ...
      $this->configGridPersonal($curso,$codigo);
  ...
  {
  	$grid = Herramientas::CargarDatosGridv2($this,$this->obj);
	$arreglo=array('Codcur', 'Codplacur');
