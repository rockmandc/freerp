...
class nomcalconActions extends autonomcalconActions
{
  ...
  $c->addSelectColumn(NpcalconPeer::CODNOM);
   ...
   // $c->addSelectColumn(NpconfonPeer::CODNOM." AS ID");
    ...
    if ($filconusu=='S') {
        $sql = 'npcalcon.codcon in (select codcon from "SIMA_USER".segconusu where loguse=\'' . $loguse . '\')';
        $c->add(NpcalconPeer::CODCON, $sql, Criteria :: CUSTOM); 
    }
...
$this->Bitacora('Guardo');
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
        ...
        }else $c->add(NpdefcptPeer::CODCON,$codigo);
  ...
  public function configGrid($codcon='',$codnom='')
  {
    ...
    $col5 = new Columna('Condición/Fórmula');
      ...
      $col5->setHTML('type="text" size="100" maxlength="5000" readonly="true"');
 ...
 protected function getNpcalconOrCreate($id = 'id')
  ...
  protected function addFiltersCriteria($c)
  {
    ...
    {
      $criterion = $c->getNewCriterion(NpcalconPeer::CODCON, '');
      $criterion->addOr($c->getNewCriterion(NpcalconPeer::CODCON, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['codcon']) && $this->filters['codcon'] !== '')
    {      
      if ($filconusu == 'S') {
        $filtro=$this->filters['codcon'];
        $sql = 'npcalcon.codcon like \'%' . $filtro . '%\' and npcalcon.codcon in (select codcon from "SIMA_USER".segconusu where loguse=\'' . $loguse . '\')';
        $c->add(NpcalconPeer::CODCON, $sql, Criteria::CUSTOM);
      }else $c->add(NpcalconPeer::CODCON, '%'.strtr($this->filters['codcon'], '*', '%').'%', Criteria::LIKE);
      $c->setIgnoreCase(true);
    }
    if (isset($this->filters['codnom_is_empty']))
    {
      $criterion = $c->getNewCriterion(NpcalconPeer::CODNOM, '');
      $criterion->addOr($c->getNewCriterion(NpcalconPeer::CODNOM, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['codnom']) && $this->filters['codnom'] !== '')
    {
      $c->add(NpcalconPeer::CODNOM, '%'.strtr($this->filters['codnom'], '*', '%').'%', Criteria::LIKE);
    $c->setIgnoreCase(true);
    }
  }  
