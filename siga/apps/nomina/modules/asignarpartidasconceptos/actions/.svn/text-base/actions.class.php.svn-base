...
class asignarpartidasconceptosActions extends autoasignarpartidasconceptosActions
{
  ...
  public function executeList()
  {
    ...
    $c = new Criteria();
    $c->addJoin(NpdefcptPeer::CODCON,NpasiparconPeer::CODCON);
  $c->setDistinct();
  ...
  public function configGrid($codcon='',$codcar='',$codpar='')
   {
    ...
    if (H::BuscarDatos($sql,$registro))
    {
      ...
      foreach ($registro as $reg)
      {
          ...
          {
            $n= new Criteria();
            $n->add(NpasiconparcarPeer::CODCAR,$new_array[$i]['codcar']);
            $n->add(NpasiconparcarPeer::CODCON,$new_array[$i]['codcon']);
            $trajoreg= NpasiconparcarPeer::doSelectOne($n);
            if ($trajoreg)
              $new_array[$i]['codpar']=$trajoreg->getCodpar();
            else
              $new_array[$i]['codpar']='';
          }else $new_array[$i]['codpar']=$reg['codpar'];
          $new_array[$i]['nompar']=Herramientas::getX('codpar', 'nppartidas', 'nompar',$new_array[$i]['codpar']);
