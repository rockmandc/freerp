...
class vacsalidasActions extends autovacsalidasActions
{
  ...
  public function configGrid($codemp='',$nomina='',$fecing='',$diavac=0,$nuevo='',$fecdesde='')
  {
    ...
    $col8->setHTML('type="text" size="8" onBlur="validardiasbono(this.id)"');
    ...
    switch ($ajax){
      case '1':
        ...
        $output = '[["npvacsalidas_fecing","'.$fecing.'",""],["npvacsalidas_nomemp","'.$nomemp.'",""],
					...
					["npvacsalidas_fechas","'.date('d/m/Y',mktime(0, 0, 0, date("m")  , date("d")+1, date("Y"))).'",""],
                                        ["npvacsalidas_fecfin","'.date('d/m/Y',mktime(0, 0, 0, date("m")  , date("d"), date("Y"))).'",""],    
	  ...
	  case '2':
		...
		if($dia_c>0)
		{
			...
			$sqlfecfinal = "select to_char(fecharetorno(to_date('$fecret','dd/mm/yyyy'),'$codnom',$dia_c,'$jor')-1,'dd/mm/yyyy') as fecharetorno;";
			if (H::BuscarDatos($sqlfecfinal,$arrfin))
				$fecfinal=$arrfin[0]['fecharetorno'];  			
		...
		{
		    $jor='H';
			$sqlfecfinal = "select to_char(fecharetorno(to_date('$fecret','dd/mm/yyyy'),'$codnom',$dia_h,'$jor')-1,'dd/mm/yyyy') as fecharetorno;";
			if (H::BuscarDatos($sqlfecfinal,$arrfin))
				$fecfinal=$arrfin[0]['fecharetorno'];                      
			...
			if (H::BuscarDatos($sqlfecretorno,$arrret))
                ...
                if($frenom=='Q')
                {
                    $sqlfecnomina = "select to_char(case when to_char(to_date('$fecdes','dd/mm/yyyy')-1,'dd')::numeric<=15
		...
		$output = '[["npvacsalidas_diaspend","'.$this->totdia.'",""],["npvacsalidas_fechas","'.$fecret.'",""],["npvacsalidas_fecfin","'.$fecfinal.'",""],["npvacsalidas_diasdisfrutar","'.$diavac.'",""],
                            ["npvacsalidas_fecsalnom","'.$fecsalnom.'",""],["npvacsalidas_fecreinom","'.$fecreinom.'",""]]';
		$this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');

        break;
      case '3':
		$fecsalnom = $this->getRequestParameter('fecsalnom','');
                $fecdes = $this->getRequestParameter('fecdes','');
                /*$sqlfec = "select
                            case when to_char(to_date('$fecret','dd/mm/yyyy'),'dd')>'15' then
                            to_char(cast(to_char(to_date(to_char(to_date('$fecret','dd/mm/yyyy'),'dd')
                            ||lpad((to_char(to_date('$fecret','dd/mm/yyyy'),'mm')::integer+1)::text,'2','0')
                            ||to_char(to_date('$fecret','dd/mm/yyyy'),'yyyy')
                            ,'ddmmyyyy'),'yyyy-mm')||'-01' as date)-1,'dd/mm/yyyy') else
                            '15'||'/'||to_char(to_date('$fecret','dd/mm/yyyy'),'mm/yyyy') end  as fecha
                            ";
                if (H::BuscarDatos($sqlfec,$arrfecnom))
                {*/
                    //$fecreinom=$arrfecnom[0]['fecha'];
               // }
		//$output = '[["npvacsalidas_fecreinom","'.$fecreinom.'",""]]';
                	$output = '[["","",""]]';
		$this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
  ...
  public function saving($npvacsalidas)
  {
     ...
     if ($npvacsalidas->getId())
     {
        Vacaciones::actualizar_vacsalidas($npvacsalidas,$grid); 
     }else {
  ...
  protected function getLabels()
  {
    ...
    $arreglo=array(
                  'npvacsalidas{codemp}' => 'Empleado:',
              'npvacsalidas{fecing}' => 'Fecha de Ingreso:',
                      'npvacsalidas{fecvac}' => 'Fecha de la Solicitud:',
              'npvacsalidas{fecsalreg}' => '.:',
              'npvacsalidas{dias}' => '.:',
              'npvacsalidas{fecnom}' => '.:',
              'npvacsalidas{nomsup}' => 'Nombre del Suplente:',
              'npvacsalidas{monsup}' => 'Monto de la Suplencia '.$denominacion.':',
              'npvacsalidas{observa}' => 'Observaciones:',
                      'npvacsalidas{gridvacaciones}' => '.:',
                          'npvacsalidas{codemp}' => 'Empleado:',
              'npvacsalidas{fecing}' => 'Fecha de Ingreso:',
              'npvacsalidas{fecvac}' => 'Fecha de la Solicitud:',
              'npvacsalidas{fecdes}' => 'Fecha de Salida:',
              'npvacsalidas{fechas}' => 'Fecha de Incorporaci&oacute;n:',
              'npvacsalidas{diasdisfrutar}' => 'Días  de Vacaciones:',
              'npvacsalidas{observa}' => 'Observaciones:',
              'npvacsalidas{diaspend}' => 'Días Pendientes:',
              'npvacsalidas{fecpagbonvac}' => 'Fecha del Pago de Vacaciones:',
              'npvacsalidas{fecsalnom}' => 'Fecha Nómina de Salida:',
              'npvacsalidas{fecreinom}' => 'Fecha Nómina de Reingreso:',
              'npvacsalidas{fecfin}' => 'Fecha Final Vac.:',
            );
   return $arreglo;
}
