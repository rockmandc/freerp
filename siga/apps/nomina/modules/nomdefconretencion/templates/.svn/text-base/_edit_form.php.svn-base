...
<?php echo input_auto_complete_tag('npcontipaporet[codtipapo]', $npcontipaporet->getCodtipapo(),
    'nomdefconretencion/autocomplete?ajax=3',  array('autocomplete' => 'off','maxlength' => 4,
	'readonly'  =>  $npcontipaporet->getId()!='' ? true : false ,
    'onBlur'=> remote_function(array(
			  'url'      => 'nomdefconretencion/ajax',
...
<?php echo button_to(__('delete'), 'nomdefconretencion/delete?id='.$npcontipaporet->getId().'&codigo='.$npcontipaporet->getCodtipapo(), array (
  'post' => true,
...
<script language="JavaScript" type="text/javascript">
  ...
  function ajax(e,id)
 {
   var aux = id.split("_");
    ...
    if ($(id).value!='')
    {
      new Ajax.Request('/nomina'+getEnv()+'.php/nomdefconretencion/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json), validargrid(id); }, parameters:'ajax=2&cajtexmos='+descripcion+'&nomina='+nom+'&cajtexcom='+id+'&codigo='+cod})
    }
  }
 }
</script>