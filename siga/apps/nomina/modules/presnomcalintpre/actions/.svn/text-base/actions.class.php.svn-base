...
class presnomcalintpreActions extends autopresnomcalintpreActions {
  ...
  public function executeAjax() {
    ...
    if ($this->getRequestParameter('ajax') == '1') {
      ...
      if ($this->getRequestParameter('codemp') != "") {
        ...
        $this->configGridRN142($this->getRequestParameter('codemp'));
	  ...
	  //$js="toAjaxUpdater('id2',7,getUrlModulo()+'ajax','7');";
        $this->getUser()->setAttribute('obj142', $this->obj142);
        $js="toAjaxUpdater('id2',7,getUrlModulo()+'ajax','7'); toAjaxUpdater('id142',8,getUrlModulo()+'ajax','8');";
        $output = '[["nppresoc_nomemp","' . $nomemp . '",""],["nppresoc_cedemp","' . $cedemp . '",""],["nppresoc_fecing","' . $fecing . '",""],                    ["nppresoc_feccalpres","' . $feccalpres . '",""],["nppresoc_destipcon","' . $destipcon . '",""],
        ["nppresoc_feccor","' . $feccor . '",""],
        ...
        ["codigotipocon","' . $this->codigotipocon . '",""],
        ["totfil","' . $this->tfil . '",""],["totfil2","' . $this->tfil2 . '",""],["totfil3","' . $this->tfil3 . '",""],
        ["totcapitalact","' . H::FormatoMonto($this->capitalact) . '",""],
        ["nppresoc_codniv","'.$codniv.'",""],
        ["nppresoc_codtipcon","'.$this->codigotipocon.'",""],
        ["nppresoc_desniv","'.$desniv.'",""],
        ...
        ["totcapitalact142","'.H::Formatomonto($this->capitalact142).'",""],
        ["totintacu142","'.H::Formatomonto($this->intacu142).'",""],
        ["javascript","'.$js.'",""]]';
          ...
          $this->obj2 = $this->getUser()->getAttribute('obj2', null, 'presnomcalintpre');
            ...
            $this->obj3 = $this->getUser()->getAttribute('obj3', null, 'presnomcalintpre');
              ...
              if ($this->getRequestParameter('codemp') != "" and $this->getRequestParameter('feccor') != "" and $this->getRequestParameter('capita') != "")
              {
                ...
                $this->configGrid142($this->getRequestParameter('codemp'), $this->getRequestParameter('feccor'), $this->getRequestParameter('capita'), $salario,$anno);
		    $this->configGrid_bueno($this->getRequestParameter('codemp'), $this->getRequestParameter('feccor'), $this->getRequestParameter('capita'), $salario,$anno);
			    $this->getUser()->setAttribute('obj2', $this->obj2);
                      ...
                      $this->getUser()->setAttribute('obj142', $this->obj142);
			    $js="toAjaxUpdater('id2',7,getUrlModulo()+'ajax','7'); toAjaxUpdater('id142',8,getUrlModulo()+'ajax','8');";
                      $output = '[["diaserrn","'.$this->antdiasrn.'",""],["messerrn","'.$this->antmesesrn.'",""],["anoserrn","'.$this->antannosrn.'",""],["diaserra","'.$this->antdias2.'",""],["messerra","'.$this->antmeses2.'",""],["anoserra","'.$this->antannos2.'",""],["totcapitalact","'.H::Formatomonto($this->capitalact).'",""],["totintacu","'.H::Formatomonto($this->intacu).'",""],["totcapitalact2","'.H::Formatomonto($this->capitalact2).'",""],["totintacu2","'.H::Formatomonto($this->intacu2).'",""],["nppresoc_diatra","' . $this->antdias . '",""],["nppresoc_mestra","' . $this->antmeses . '",""],["nppresoc_anotra","' . $this->antannos. '",""],["totcapitalact142","'.H::Formatomonto($this->capitalact142).'",""],["totintacu142","'.H::Formatomonto($this->intacu142).'",""],["javascript","'.$js.'",""]]';
      ...
      {
        $this->varg='c';
        $this->obj142= $this->getUser()->getAttribute('obj142');
      }
  ...
  {
    $sql1 = "Select codtipcon from NPASIEMPCONT Where CodEmp = '" . $codemp. "'";
      if (H :: BuscarDatos($sql1, $rsnpasiempcont)) {
        $tipocontrato = $rsnpasiempcont[0]["codtipcon"];
      } else {
        $tipocontrato = "";
      }
    $result=array();
      $sql = "select '' as id,a.codtipcon,antdias,antmeses,antannos,
              ...
              from calculopresant('$codemp','$fecha','$capital',$anno) a, nptipcon b where b.codtipcon='$tipocontrato' and apliregant='S' order by fecinior";
...
if ($codemp=='')
  $result=array();
    ...
    Herramientas :: BuscarDatos($sql, $result);
      ...
      $this->intacu=0;
	  $this->capitalact=0;
    $fecing = NphojintPeer :: getFecing($codemp,'Y-m-d');

    $sql = "select b.fecinireg from npasiempcont a inner join nptipcon b on a.codtipcon=b.codtipcon where codemp='$codemp' limit 1";

    Herramientas :: BuscarDatos($sql, $resultinireg);
      ...
      if ($result)
      {
        ...
        if($resultinireg){
          $inireg = $resultinireg[0]['fecinireg'];
          if(strtotime($fecing) >  strtotime($inireg)){
            $this->antannosrn  = $result[$this->tfil-1]['antannos'];
            $this->antmesesrn  = $result[$this->tfil-1]['antmeses'];
            $this->antdiasrn   = $result[$this->tfil-1]['antdias'];
          }else{
   ...
   *
   */
  public function configGrid142($codemp = '', $fecha= '', $capital= '', $salario= '',$anno= '')
  {
    if($this->salario=="true")
      $cadena="round(monpres,2)";
    else
     ...
     $sql = "select ' ' as id, TO_CHAR(fecfin,'MM') as mesactual, antmeses, antdias, antannos, codtipcon, tipo,
      ...
      round(case when tipo='DEPOSITADOS' then (dias-5)*mondiapro  else 0 end,2) as montoadicionales,
      round(case when tipo='DEPOSITADOS' then 5*mondia else 0 end,2) as monto5dias
              from calculopreslottt('$codemp','$fecha','$capital','$salario'),npdefgen b order by fecini1";
if ($codemp=='')
  $result=array();
else
    Herramientas :: BuscarDatos($sql, $result);
    ...
    Herramientas :: BuscarDatos($sql, $resultinireg);
      ...
      if ($result)
      {
        ...
        $this->antmeses  = $result[$this->tfil-1]['antmeses'];
        $this->antdias   = $result[$this->tfil-1]['antdias'];
        $this->capitalact142= $result[$this->tfil-1]['capitalact'];
    ...
    if($this->tfil>1)
      ...
      $col31 = new Columna('Alícuota Utilidad');
      $col31->setTipo(Columna :: MONTO);
      $col31->setAlineacionObjeto(Columna :: CENTRO);
      $col31->setAlineacionContenido(Columna :: CENTRO);
      $col31->setNombreCampo('aliuti');
      ...
      $col32 = new Columna('Alícuota Vacación');
      $col32->setTipo(Columna :: MONTO);
      $col32->setAlineacionObjeto(Columna :: CENTRO);
      $col32->setAlineacionContenido(Columna :: CENTRO);
      $col32->setNombreCampo('alibono');
      ...
      $col33 = new Columna('Alícuota Adicional');
      $col33->setTipo(Columna :: MONTO);
      $col33->setAlineacionObjeto(Columna :: CENTRO);
      $col33->setAlineacionContenido(Columna :: CENTRO);
      $col33->setNombreCampo('aliadi');
      ...
      $col4->setHTML('type="text" size="5" maxlength="5"  readonly=true');
      $col4->setEsGrabable('true');
      $col4->setEsTotal(true, 'totmonpres142');

      $col5 = new Columna('Salario Día Adicional');
      $col5->setTipo(Columna :: MONTO);
      $col5->setAlineacionObjeto(Columna :: CENTRO);
      $col5->setAlineacionContenido(Columna :: CENTRO);
      $col5->setNombreCampo('mondiapro');
      ...
      $col8 = new Columna('Valor Art.142');
      $col8->setTipo(Columna :: MONTO);
      $col8->setAlineacionObjeto(Columna :: CENTRO);
      $col8->setAlineacionContenido(Columna :: CENTRO);
      $col8->setNombreCampo('monpres');
      ...
      $col22->setNombreCampo('monantbf');
      $col22->setOculta(true);
      $col22->setEsTotal(true, 'totmonant142');
      $col22->setEsGrabable('false');
      ...
      $col23->setNombreCampo('monadeintbf');
      $col23->setOculta(true);
      $col23->setEsTotal(true, 'totmonadeint142');
      $col23->setEsGrabable('false');

      $col24 = new Columna('Dias Adicionales');
      $col24->setTipo(Columna :: MONTO);
      $col24->setAlineacionObjeto(Columna :: CENTRO);
      $col24->setAlineacionContenido(Columna :: CENTRO);
      $col24->setNombreCampo('adicionales');
      ...
      $col25 = new Columna('Monto Dias Adicionales');
      $col25->setTipo(Columna :: MONTO);
      $col25->setAlineacionObjeto(Columna :: CENTRO);
      $col25->setAlineacionContenido(Columna :: CENTRO);
      $col25->setNombreCampo('montoadicionales');
      ...
      $this->obj142 = $opciones->getConfig($per);
...
////$this->getUser()->getAttributeHolder()->remove('calculado');
      if ($calculado=='SI'){
        ...
        $this->configGrid142($this->nppresoc->getCodemp(),@adodb_date("d/m/Y",adodb_strtotime($this->nppresoc->getFeccor())), $this->nppresoc->getCapitalizacion(),$this->nppresoc->getSalario());
...
$this->Bitacora('Guardo');
  ...
  public function configGridRN($codigo='') {
    ...
    $c->add(NpimppresocPeer :: TIPO, "(npimppresoc.TIPO in ('A','P') or  trim(coalesce(npimppresoc.TIPO,''))=''  )", Criteria :: CUSTOM);
...
/**
   ...
   *
   */
  public function configGridRN142($codigo='') {
    /////PARA LA CONSULTA//////
    $c = new Criteria();
    $c->add(NpimppresocPeer :: CODEMP, $codigo);
    $c->add(NpimppresocPeer :: TIPO, "(npimppresoc.TIPO<>'T' and npimppresoc.TIPO='N' )", Criteria :: CUSTOM);
    $c->addAscendingOrderByColumn(NpimppresocPeer :: FECINI);
    $c->addAscendingOrderByColumn(NpimppresocPeer :: TIPO);
    $per = NpimppresocPeer :: doSelect($c);
    ...
    $this->capitalact142 = $per[$this->tfil-1]->getAntacum();
    foreach($per as $r)
    {
      if($r->getIntacum()!=0)
      $this->intacu142 = $r->getIntacum();
    }
        // Copio los dos ultimo registro
        $this->grid_1 = $per[$this->tfil-1];
        $this->grid_2 = $per[$this->tfil-2];
    ...
    $col9 = new Columna('Dias Art. 142');
    $col9->setTipo(Columna :: MONTO);
    $col9->setAlineacionObjeto(Columna :: CENTRO);
    $col9->setAlineacionContenido(Columna :: CENTRO);
    $col9->setNombreCampo('diaart108');
    $col9->setEsTotal(true, 'totmonpres142');
    $col9->setHTML('type="text" size="5" maxlength="5"  readonly=true');
    ...
    $col13 = new Columna('Valor Art. 142');
    $col13->setTipo(Columna :: MONTO);
    $col13->setAlineacionObjeto(Columna :: CENTRO);
    $col13->setAlineacionContenido(Columna :: CENTRO);
    $col13->setNombreCampo('valart108');
    ...
    $col18->setAlineacionContenido(Columna :: CENTRO);
    $col18->setNombreCampo('adeant');
    $col18->setEsTotal(true, 'totmonant142');
    $col18->setHTML('type="text" size="10" maxlength="10"  readonly=true');
    ...
    $col19->setAlineacionContenido(Columna :: CENTRO);
    $col19->setNombreCampo('adepre');
    $col19->setEsTotal(true, 'totmonadeint142');
    $col19->setHTML('type="text" size="10" maxlength="10"  readonly=true');
  ...
  protected function saveNppresoc($nppresoc) {
    ...
    $gridrn = Herramientas :: CargarDatosGrid($this, $this->obj);
	$gridra = Herramientas :: CargarDatosGrid($this, $this->obj2);
  $grid142 = Herramientas :: CargarDatosGrid($this, $this->obj142);
    ...
    //try {
      ...
      if(count($gridra[0])>0)
      {
        ...
        $coderr[] = PrestacionesSociales :: saveNppresoc($nppresoc, $grid,"S");
      ...
      if(count($gridrn[0])>0)
       {
        ...
        $detalle="S";
          ...
          $detalle="N";
          $grid[0]["codtipcon"]=$this->codigotipocon;
        }
        $grid[2] = array ($this->getRequestParameter('totintacu'), $this->getRequestParameter('totmonadeint'), $this->getRequestParameter('totmonpres'), $this->getRequestParameter('totmonant'), $this->getRequestParameter('totcapitalact'),$this->getRequestParameter('totintacu142'), $this->getRequestParameter('totmonadeint142'), $this->getRequestParameter('totmonpres142'), $this->getRequestParameter('totmonant142'), $this->getRequestParameter('totcapitalact142'));
        $grid[3][] = $this->grid_1;
        $grid[3][] = $this->grid_2;
        $grid[3][] = array($this->antdiasrn,$this->antmesesrn,$this->antannosrn);
        $grid[4][] = 'N';

        $coderr[] = PrestacionesSociales :: saveNppresoc($nppresoc, $grid, $detalle);
      //}
      if(count($grid142[0])>0)
      {
        $grid=$grid142;
        $grid[2] = array ($this->getRequestParameter('totintacu142'), $this->getRequestParameter('totmonadeint142'), $this->getRequestParameter('totmonpres142'), $this->getRequestParameter('totmonant142'), $this->getRequestParameter('totcapitalact142'));
        $grid[3][] = $this->grid_1;
        $grid[3][] = $this->grid_2;
        $grid[3][] = array($this->antdiasrn,$this->antmesesrn,$this->antannosrn);
        $grid[4][] = '142';
...
//    }
  ...
  protected function updateNppresocFromRequest()
  {
    ...
    if (isset($nppresoc['codtipcon']))
    {
      $this->nppresoc->setCodtipcon($nppresoc['codtipcon']);
    }
