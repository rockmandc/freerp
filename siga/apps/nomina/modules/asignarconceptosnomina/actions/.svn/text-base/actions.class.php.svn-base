...
$this->Bitacora('Guardo');
  ...
  {
			$sql = "select 1 as check,'N' as asilot, a.codcon as codcon, a.nomcon as nomcon, (case when a.conact='S' then 'SI' else 'NO' end) as conact, b.frecon as frecon,a.id as id  
            ...
            $col6 = new Columna('Asignar en Lote');
            $col6->setTipo(Columna::COMBO);
            $col6->setEsGrabable(true);
            $col6->setNombreCampo('asilot');
            $col6->setCombo(Constantes::PagoDoble());
            $col6->setHTML(' ');
  ...
  public function configGridno($nomina='',$fre)
  {
     ...
     if ($filconusu=='S')
       $concat='and a.codcon in (select codcon from "SIMA_USER".segconusu where loguse=\''.$loguse.'\')';
     else $concat='';
      ...
      $sql="select 0 as check, a.codcon as codcon, b.codnom, a.nomcon as nomcon, (case when a.conact='S' then 'SI' else 'NO' end) as conact, a.id as id
			...
			where trim(b.codnom) is null ".$concat." order by a.codcon";
  ...
  public function saveNpnomina($Npnomina)
  {
    ...
    Nomina::salvarAsignarconceptosLote($Npnomina->getCodnom(),$gridsi);
  }
  ...
  public function validateEdit()
  {
    ...
    if($this->getRequest()->getMethod() == sfRequest::POST){
        ...
        $gridsi=Herramientas::CargarDatosGrid($this,$this->obj,true);//1
        $gridno=Herramientas::CargarDatosGrid($this,$this->obj2,true);//2
  	    $resp=Nomina::validarAsignacion($gridsi,$gridno,$codcon);
  	    

      if($resp!=-1){
        $this->codigocon=$codcon;
        $this->coderror = $resp; 
        return false;
      } else return true;
    ...
    if(!$this->validateEdit())
     {
      if($this->coderror!=-1)
      {
       ...
       $this->getRequest()->setError('','El Concepto '.$this->codigocon.' '.$err);
      }
     }	
