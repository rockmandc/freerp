 ...
 * @version SVN: $Id: actions.class.php 61497 2015-04-14 15:57:36Z dmartinez $
 *
...
class preaprcomActions extends autopreaprcomActions
{
   ...
   *
   */
  public function configGrid($refcomd='', $refcomh='', $reg = array(),$regelim = array())
  {
      ...
      if ($refcomd!='' && $refcomh!='') {
        ...
        $c->add(CpdoccomPeer::REQAUT,'S');
        if ($filnoaprpre=='S'){
          $sql = "((cpcompro.APRCOM<>'S' and cpcompro.APRCOM<>'R') or cpcompro.APRCOM isnull) and  (cpcompro.refcom>='".$refcomd."' and cpcompro.refcom<='".$refcomh."')";
        }else {
            $sql = "cpcompro.REFCOM not in (select distinct refere from cpimpcau  where refere is not null and staimp!='N') and  (cpcompro.refcom>='".$refcomd."' and cpcompro.refcom<='".$refcomh."')";
      	}
        $c->add(CpcomproPeer::REFCOM, $sql, Criteria :: CUSTOM);
