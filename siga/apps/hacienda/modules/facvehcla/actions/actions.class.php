...
class FacvehclaActions extends autoFacvehclaActions
{
  ...
  public function editing()
  {
    ...
    $this->fcusoveh->setPorveh(Herramientas::getX('Codemp','fcdefins','porveh','001'));
  }
  ...
  public function validateEdit()
  {
    ...
    $this->updateFcusovehFromRequest();
        ...
        if (!checkdate(intval('01'), intval('01'), intval($this->getRequestParameter('fcusoveh[anovig]'))) )
    {
          $this->coderr=726;
          return false;
        }
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
         $formato=Herramientas::getX('Codemp','fcdefins','forveh','001');
         $posrup1=Herramientas::instr($formato,'-',0,1);
         $posrup1=$posrup1-1;
         if (strlen(trim($codigo))<$posrup1)
          {
           $js="alert_('El C&oacute;digo no puede estar en Blanco ó longitud del código invalida'); $('fcusoveh_coduso').value='';";
         }else  {
           Herramientas::FormarCodigoPadre($codigo,$nivelcodigo,$ultimo,$formato);
            $c= new Criteria();
            $c->add(FcusovehPeer::CODUSO,$ultimo);
            $fcusoveh= FcusovehPeer::doSelectOne($c);
            if (!$fcusoveh)
            {
               if ($nivelcodigo == 0)
                $js="alert_('El Nivel Anterior No Existe'); $('fcusoveh_coduso').value='';";
          }
        }
  ...
  {
    if (H::Buscar_CodigoHijoGeneral($clasemodelo->getCoduso(),'Fcusoveh','coduso')) //no se puede eliminar, tiene hijos
    {
       return 725;
    }else {
     $clasemodelo->delete();
