 ...
 *
 * @package    siga
 * @subpackage facactcat
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class facactcatActions extends autofacactcatActions
{

  public function executeIndex()
  {
    return $this->forward('facactcat', 'edit');
  }      
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
          $fc= new Criteria();
          $fc->add(FcreginmPeer::NROINM,$codigo);
          $reg= FcreginmPeer::doSelectOne($fc);
          if ($reg)
          {
              $dato1=$reg->getRifcon();
              $dato2=$reg->getNomcon();
          }else {
              $js="alert_('El N&uacute;mero del Inmueble no esta registrada'); $('".$cajtexcom."').value=''; $('".$cajtexcom."').focus();";
          }
        $output = '[["'.$cajtexmos.'","'.$dato1.'",""],["fcreginm_nomcon","'.$dato2.'",""],["javascript","'.$js.'",""]]';
        break;
      default:
        $output = '[["","",""],["","",""],["","",""]]';
    }
  ...
  public function validateEdit()
  {
    ...
    if($this->getRequest()->getMethod() == sfRequest::POST){
       ...
       if ($this->getRequestParameter('fcreginm[nroinm]')=="" || $this->getRequestParameter('fcreginm[nroinmnew]')=="")
        {
          $this->coderr=770;
        }else {
            $c= new Criteria();
            $c->add(FcreginmPeer::NROINM,$this->getRequestParameter('fcreginm[nroinmnew]'));
  ...
  public function saving($clasemodelo)
  {
     ...
     $result=array();
     $nueva=$clasemodelo->getNroinmnew();
     $vieja=$clasemodelo->getNroinm();
     
     $sql='Select A.TABLE_NAME as nomtab,B.COLUMN_NAME as columna  
            FROM "information_schema".TABLES A,"information_schema".COLUMNS B 
            WHERE A.TABLE_NAME =B.TABLE_NAME AND A.TABLE_NAME LIKE \'%fc%\' AND A.TABLE_NAME NOT IN (\'fcpaging\')
            AND A.TABLE_SCHEMA =B.TABLE_SCHEMA 
            AND A.TABLE_SCHEMA=\''.$schema.'\'
            AND A.TABLE_TYPE=\'BASE TABLE\' 
            AND B.COLUMN_NAME IN (\'nroinm\',\'numref\')';
     if (Herramientas::BuscarDatos($sql,$result))
     {
         //H::PrintR($result); exit;
