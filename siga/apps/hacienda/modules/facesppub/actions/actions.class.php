...
class facesppubActions extends autofacesppubActions
{
  ...
  public function configGrid($codigo='') {
  	...
  	if ($this->fcesplic->getId()){
		...
		$fctipesp = FctipespdetPeer :: doSelect($c);
                if($fctipesp){
                    foreach ($fctipesp as $reg) {
                    $fcesplicdet= new Fcesplicdet();
                    $fcesplicdet->setValor($reg->getValor());
                    $fcesplicdet->setCampo($reg->getTipvar());
  ...
  public function executeAjax()
  {
	...
	case '3';
			...
			$exoesp = Herramientas :: getX_vacio('tipesp', 'fctipesp', 'exoesp', $codigo);
                 $paresp = Herramientas :: getX_vacio('tipesp', 'fctipesp', 'paresp', $codigo);
                 $frepar = Herramientas :: getX_vacio('tipesp', 'fctipesp', 'frepar', $codigo);
                 $label="";$labelsemana="";
	          if ($exoesp=='S')
	          {
	          	$javascript = $javascript ."$('fcesplic_exoesp').checked=true; $('fcesplic_exoesp').readOnly=true; $('fcesplic_texexo').readOnly=false;";
	          }
	          else
	          {
	          	$javascript = $javascript ."$('fcesplic_exoesp').checked=false; $('fcesplic_exoesp').readOnly=true; $('fcesplic_texexo').readOnly=true;";
	          }
                 switch($frepar){
                    case '3':
                        $javascript = $javascript ."$('label41').innerHTML = 'Semanas:'; $('fcesplic_semdia').readOnly= false;";
                        break;
                    case '4':
                        $javascript = $javascript ."$('label41').innerHTML = 'Dias:'; $('fcesplic_semdia').readOnly= false;";
                        break;
                    default:
