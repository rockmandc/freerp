...
class FaccatfinfisActions extends autoFaccatfinfisActions
{
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
         $formato=Hacienda::Cargar_mascara();
         $posrup1=Herramientas::instr($formato,'-',0,1);
         $posrup1=$posrup1-1;
         if (strlen(trim($codigo))<$posrup1)
         {
           $js="alert_('El C&oacute;digo no puede estar en Blanco ó longitud del código invalida'); $('fccatfis_codcatfis').value='';";
         }else  {
           Herramientas::FormarCodigoPadre($codigo,$nivelcodigo,$ultimo,$formato);
            $c= new Criteria();
            $c->add(FccatfisPeer::CODCATFIS,$ultimo);
            $fccatfis = FccatfisPeer::doSelectOne($c);
            if (!$fccatfis)
            {
               if ($nivelcodigo == 0)
                $js="alert_('El Nivel Anterior No Existe'); $('fccatfis_codcatfis').value='';";
            }
         }
  ...
  {
    if (Hacienda::Buscar_CodigoHijo($clasemodelo->getCodcatfis())) //no se puede eliminar, tiene hijos
    {
       return 723;
    }else {
     $clasemodelo->delete();
