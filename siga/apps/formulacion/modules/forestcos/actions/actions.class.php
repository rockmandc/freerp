...
class forestcosActions extends autoforestcosActions
{
  ...
  public function configGridCostos($codmet='',$codpro='')
  {
    ...
    $obj0=array('codact' => 1, 'Desact' => 2);
    $params0= array('param1'=>"'+$('forestcos_codmet').value+'", 'param2'=>"'+$('forestcos_codpro').value+'");   
    $this->columnas[1][0]->setCatalogo('fordefact','sf_admin_edit_form',$obj0,'Fordefact_Forasoactmet',$params0);
    ...
    switch ($ajax){
      ...
      case '8':
         ...
         if ($result)
         {
           ...
           $p= new Criteria();
           $p->add(ForasoactproPeer::CODMET,$accion);
           $p->add(ForasoactproPeer::CODPRO,$unimed);
           $p->add(ForasoactproPeer::CODACT,$codigo);
           $resul= ForasoactproPeer::doSelectOne($p);
           if ($resul) {
               $dato=$result->getDesact();
           }else {
             $javascript="alert_('La Meta no esta asociado a la acci&oacute;n y Unidad de Medidad seleccionada'); $('$cajtexcom').value=''; $('$cajtexcom').focus();";
           }             
