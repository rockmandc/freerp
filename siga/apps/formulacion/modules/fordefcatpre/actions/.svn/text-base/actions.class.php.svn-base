...
$this->Bitacora('Guardo');
  ...
  {
    $this->fordefcatpre = FordefcatprePeer::retrieveByPk($this->getRequestParameter('id'));
    $this->forward404Unless($this->fordefcatpre);

    try
    {
      $trajo=H::getX_vacio('CODUNIEJE', 'Fordetpryaccespmet', 'Codunieje', $this->fordefcatpre->getCodcat());
      if ($trajo==''){
          $this->deleteFordefcatpre($this->fordefcatpre);
          ...
          return $this->forward('fordefcatpre', 'list');
      }
    }
