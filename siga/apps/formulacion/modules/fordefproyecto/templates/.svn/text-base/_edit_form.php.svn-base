...
<div class="form-row">
 ...
 <?php echo select_tag('fordefpry[proacc]', options_for_select($pacc,$fordefpry->getProacc(),'include_custom=Seleccione Uno'), array(
     'onChange' => 'bloquearcampos()'
 )); ?>
    </div></th>
   </tr>
  </table>
...
<br>
<div id="nvaeta" style="display: none">
<table>
 <tr>
  <th>
...
&nbsp;&nbsp;&nbsp;
  <?php echo input_auto_complete_tag('fordefpry[codobjnvaeta]', $fordefpry->getCodobjnvaeta(),
    'fordefproyecto/autocomplete?ajax=4',  array('autocomplete' => 'off','size' => 3,'maxlength' => 3, 'onBlur'=> remote_function(array(
			  'url'      => 'fordefproyecto/ajax',
			  'complete' => 'AjaxJSON(request, json)',
			  'condition' => "$('fordefpry_codobjnvaeta').value != '' ",
  			  'with' => "'ajax=4&cajtexmos=fordefpry_desobjnva&cajtexcom=fordefpry_codobjnvaeta&codigo='+this.value"
			  ))),
     array('use_style' => 'true')
  )
?>
...
<div id="pndes" style="display: none" >
...
?></div></th>
    <th>&nbsp;&nbsp;&nbsp;
    <?php echo  button_to_popup('...',cross_app_link_to('herramientas','catalogo').'/metodo/Fordefpry_fordefproyecto/clase/Nphojint/frame/sf_admin_edit_form/obj1/fordefpry_codemp/obj2/fordefpry_nomemp2/campo1/codemp/campo2/nomemp/param1/6')?></th>
...
<?php echo button_to(__('delete'), 'fordefproyecto/delete?id='.$fordefpry->getId(), array (
 ...
 function ajax(id)
 {
   var aux = id.split("_");
    ...
    new Ajax.Request('/formulacion'+getEnv()+'.php/fordefproyecto/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json), validargrid(id); }, parameters:'ajax=7&cajtexmos='+descripcion+'&cajtexcom='+id+'&codigo='+cod})
 }
