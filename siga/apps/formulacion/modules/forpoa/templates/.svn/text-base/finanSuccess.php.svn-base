...
<script language="JavaScript">
	function validarmonto(e,id)
	{
	  if ((e.keyCode==13) && (document.getElementById(id).value!=''))
	  {
		...
		var col  = parseInt(aux[2]);
                var arreglof='<?php echo $arrfue; ?>';

		var montodisp = name+"_"+fil+"_"+'3';
                var fuente = name+"_"+fil+"_"+'1';
                var totalfuente=buscartotalfuente($(fuente).value,arreglof);
		var montodisp = toFloat(montodisp); // - (totalfuente);
    ...
    function buscartotalfuente(fuente,arreglo)
    {
       var acum=0;
       var z=0;
       var aux2=arreglo.split(",");
       while (z<((aux2.length)))
       {
         reg=aux2[z];
         aux3=reg.split("_");
	  ...
	  //  alert(monfin);
		 ...
		 //alert(id);
		  ...
		  var datos=cuantos(numero_reg);
	  	  ...
	  	  //var proyec=$('forencpryaccespmet_codpro').value;
	  	  //var accion=$('forencpryaccespmet_codaccesp').value;
	  	  //var meta=$('forencpryaccespmet_codmet').value;
	  	  //var unidad=name+"_"+fil+"_"+'1';
	  	  var codpre=window.opener.document.getElementById(name+"_"+fil+"_"+'1').value+'-'+window.opener.document.getElementById(name+"_"+fil+"_"+'3').value;
	  	  var proyec=window.opener.document.getElementById('forencpryaccespmet_codpro').value;
	  	  var accion=window.opener.document.getElementById('forencpryaccespmet_codaccesp').value;
	  	  var meta=window.opener.document.getElementById('forencpryaccespmet_codmet').value;
	  	  var unidad=window.opener.document.getElementById(name+"_"+fil+"_"+'1').value;

	  	  new Ajax.Request('/formulacion'+getEnv()+'.php/forpoa/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=6&proyec='+proyec+'&accion='+accion+'&meta='+meta+'&unidad='+unidad+'&codpre='+codpre+'&cod='+cod+'&variable='+variable})
