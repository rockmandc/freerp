...
class forpoaActions extends autoforpoaActions {
  ...
  public function executeEdit()
  {
    ...
    if ($this->getRequest()->getMethod() == sfRequest::POST)
    {
     ...
     if ($this->saveForencpryaccespmet($this->forencpryaccespmet)==-1){
...
$this->Bitacora('Guardo');
	      ...
	      {
	        return $this->redirect('forpoa/create');
	      }
	      else if ($this->getRequestParameter('save_and_list'))
	      {
	        return $this->redirect('forpoa/list');
	      }
	      else
	      {
	        return $this->redirect('forpoa/edit?id='.$this->forencpryaccespmet->getId());
	      }
	    }
	    else
	    {
...
//       $this->configGrid($forencpryaccespmet->getCodpro(),$forencpryaccespmet->getCodaccesp(),$forencpryaccespmet->getCodmet());
	...
	public function executeFinan()
	{
	  ...
	  $this->monfin=$this->getRequestParameter('monfin');
          $this->arrfue=$this->getRequestParameter('arreglo');
		...
		if ($this->getRequestParameter('ajax') == '1') {
			...
			if ($this->getRequestParameter('ajax') == '2') {
				...
				if ($this->getRequestParameter('ajax') == '3') {
					...
					if (!empty($dato2))
					{
            $t= new Criteria(); //Borramos el detalle que cumpla con estas condiciones
            $t->add(FordisfuefinpryaccmetPeer::CODPRO,$this->getRequestParameter('codigo2'));
            $t->add(FordisfuefinpryaccmetPeer::CODACCESP,$this->getRequestParameter('codigo3'));
            $t->add(FordisfuefinpryaccmetPeer::CODMET,$this->getRequestParameter('codigo'));
            $t->add(FordisfuefinpryaccmetPeer::ACTFUE,'N');
            $resul= FordisfuefinpryaccmetPeer::doSelect($t);
            if ($resul)
            {
          ...
          $this->getResponse()->setHttpHeader("X-JSON", '(' . $output . ')');
            ...
            if ($this->getRequestParameter('ajax') == '6') {
             $codpro=$this->getRequestParameter('proyec');
             $accion=$this->getRequestParameter('accion');
             $meta=$this->getRequestParameter('meta');
             $unidad=$this->getRequestParameter('unidad');
             $codpre=$this->getRequestParameter('codpre');
             $cod=$this->getRequestParameter('cod');
             $variable=$this->getRequestParameter('variable');             

             $t= new Criteria(); //Borramos el detalle que cumpla con estas condiciones
             $t->add(FordisfuefinpryaccmetPeer::CODPRO,$codpro);
             $t->add(FordisfuefinpryaccmetPeer::CODACCESP,$accion);
             $t->add(FordisfuefinpryaccmetPeer::CODMET,$meta);
             $t->add(FordisfuefinpryaccmetPeer::CODPRE,$codpre);
             $resul= FordisfuefinpryaccmetPeer::doSelect($t);
             if ($resul)
             {
               foreach ($resul as $obj) {
              ...
              $codparing = split("_",$cod);
              $monto = split("_",$variable);
              $i=0;
              while($i<count($codparing))
              {
                if ($monto[$i]>0){
                  $fordisfuefinpryaccmet= new Fordisfuefinpryaccmet();
                  $fordisfuefinpryaccmet->setCodpro($codpro);
                  $fordisfuefinpryaccmet->setCodaccesp($accion);
                  $fordisfuefinpryaccmet->setCodmet($meta);
                  $fordisfuefinpryaccmet->setCodpre($codpre);
                  $fordisfuefinpryaccmet->setCodparing($codparing[$i]);
                  $fordisfuefinpryaccmet->setMonfin($monto[$i]);
                  $fordisfuefinpryaccmet->setActfue('N');
  ...
  public function configGrid($codpro=' ',$codaccesp=' ',$codmet=' ')
  {
    ...
    */
    ...
    $col1->setCatalogo('Fordefcatpre','sf_admin_edit_form', array('codcat' => 1,'nomcat' => 2), 'Fordefcatpre_Forpoa', array('param1' =>$formatouae, 'param2' => $codaccesp));
   ...
   // $col3->setAjax('forpoa',5,4);
    ...
    $col15->setTitulo('grid Asig Presup');
   ...
   // $col17->setBoton(false);
    ...
    $col17->setNombreCampo('cajaoculta2');
    ...
    $col18->setNombreCampo('cajaoculta');
  ...
  public function configGrid_financiamiento($codpro=' ',$codaccesp=' ',$codmet=' ', $codpre=' ', $monfin=' ')
	{
		 ...
		 **                  Jesus Lobaton Graba en Clase                        **
		 **************************************************************************/
		// Se actualiza el monto auxiliar en la tabla fortipfin
     $sql="update fortipfin set montodis = obtenermontofin(CODFIN,'G'), montodisaux = obtenermontofin(CODFIN,'G')";
     Herramientas::insertarRegistros($sql);
		...
		"obtenermontofin(fortipfin.CODFIN,'G') as MONTODISAUX, " .
	...
	"GROUP BY " .
		...
		"fortipfin.ID order by fortipfin.CODFIN";
    ...
    $resp = Herramientas::BuscarDatos($sql,$per);
		...
		$col3->setHTML('type="text" size="10" disabled=true');
  ...
  public function saveForencpryaccespmet($Forencpryaccespmet) {
		...
		try {
			...
			// correcto en la clase del negocio, ej:
			 $coderr = Formulacion::salvarForpoa($Forencpryaccespmet,$grid);
	...
	public function deleteForencpryaccespmet($Forencpryaccespmet) {
		...
		try {
			...
			// Modificar la siguiente línea para llamar al método
			// correcto en la clase del negocio, ej:
			 $coderr = Formulacion::EliminarForpoa($Forencpryaccespmet,$grid);
  ...
  public function validateEdit() {
		...
		if ($this->getRequest()->getMethod() == sfRequest :: POST) {
			...
			// Los resultados de cada llamado deben ser analizados por ejemplo:
			 ...
			 $resp = Formulacion::validarForpoa($this->forencpryaccespmet,$grid);
			// o
