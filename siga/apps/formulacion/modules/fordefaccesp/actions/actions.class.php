  ...
  public function configGrid()
	{
		...
		$col2->setHTML('type="text" size="60"');
                ...
                $col2a = new Columna('Factor Peso');
                $col2a->setTipo(Columna::MONTO);
                $col2a->setEsGrabable(true);
                $col2a->setAlineacionContenido(Columna::DERECHA);
                $col2a->setAlineacionObjeto(Columna::DERECHA);
                $col2a->setNombreCampo('facpes');
                $col2a->setEsNumerico(true);
                $col2a->setHTML('type="text" size="15" maxlength="21"');
                $col2a->setJScript('onKeyPress="entermonto(event,this.id);" onBlur="validartotal(this.id);"');
                $col2a->setEsTotal(true,'totpes');

		$col3 = new Columna('Ejecutor'); //En este campo codcat se grabara el codigo de organismo pub solitado por JPereira y PSilva
