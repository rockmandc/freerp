...
class ingasiiniActions extends autoingasiiniActions
{
  ...
  {
    $mascarapre=Herramientas::ObtenerFormato('Cidefniv','Forpre');
    $this->ciasiini->setMascarapre($mascarapre);
    $this->ciasiini->setLonpre(strlen($mascarapre));
    $this->configGrid();
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      ...
      case '2':
        $longitud=strlen(Herramientas::ObtenerFormato('Cidefniv','Forpre'));
        $dato="";  $javascript="";
        if ($longitud==strlen($codigo))
        {
          ...
          {
              $cq= new Criteria();
              $cq->add(CiasiiniPeer::CODPRE,$codigo);
              $regq= CiasiiniPeer::doSelectOne($cq);
              if (!$regq)
              {
                $dato=$reg->getNompre();
              }else {
