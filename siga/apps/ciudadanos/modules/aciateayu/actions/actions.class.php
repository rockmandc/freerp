    ...
    switch ($ajax){
     ...
     case '5':
      ...
      case '6':
        ...
        $atprovee = CaproveePeer::doSelectOne($c);
        if($atprovee){
          $output = '[["'.$inputtag.'","'.$atprovee->getNompro().'",""],["atprovee_id","'.$atprovee->getId().'",""],["","",""]]';
        }else {
          $js="alert('El Proveedor no esta registrado');";
          $output = '[["'.$inputtag.'","",""],["atayudas_rifpro","",""],["atprovee_id","",""],["javascript","'.$js.'",""]]';
        }
        $this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
        return sfView::HEADER_ONLY;
        break;
      case '7':
        ...
        $atprovee = CaordcomPeer::doSelectOne($c);
        if($atprovee){
          $output = '[["'.$inputtag.'","'.$atprovee->getDesord().'",""],["","",""],["","",""]]';
        }else $output = '[["","",""],["","",""],["","",""]]';
        $this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
        return sfView::HEADER_ONLY;
        break;
