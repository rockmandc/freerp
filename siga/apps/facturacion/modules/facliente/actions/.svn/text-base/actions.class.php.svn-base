...
$this->Bitacora('Guardo');
  ...
  {
   $c = new Criteria();
   $c->add(FacliperPeer::CODPRO,$this->facliente->getCodpro());
   $per = FacliperPeer::doSelect($c);

   $this->columnas = Herramientas::getConfigGrid(sfConfig::get('sf_app_module_dir').'/facliente/'.sfConfig::get('sf_app_module_config_dir_name').'/grid_contactos');

   $this->obj = $this->columnas[0]->getConfig($per);

   $this->facliente->setObj($this->obj);
  ...
  {
    $grid = Herramientas::CargarDatosGridv2($this,$this->obj);
    Cliente::salvarFacliente($facliente,$grid);
  }
