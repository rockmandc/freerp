...
/**
 * facaprordtra actions.
 *
 * @package    siga
 * @subpackage facaprordtra
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class facaprordtraActions extends autofacaprordtraActions
{

  public function executeList()
  {
    return $this->forward('facaprordtra', 'edit');
  }
  ...
  public function configGrid($fecdes="", $fechas="", $reg = array())
  {
     if ($fecdes!="" && $fechas!="") {
         $c = new Criteria();     
         $this->sql="faordtra.fecord>='".$fecdes."' and faordtra.fecord<='".$fechas."' and (faordtra.aprordtra<>'S' or faordtra.aprordtra is null)";
         $c->add(FaordtraPeer::FECORD,$this->sql, Criteria::CUSTOM);         
         $reg = FaordtraPeer::doSelect($c);
     }
     
     $this->obj = Herramientas::getConfigGrid(sfConfig::get('sf_app_module_dir').'/facaprordtra/'.sfConfig::get('sf_app_module_config_dir_name').'/grid');
  ...
  public function saving($clasemodelo)
  {
    ...
    foreach($grid[0] as  $dat)
    {
        if($dat->getCheck()=='1')
        {
            ...
            {
               $per->setAprordtra('S');
               $per->setUsuaprord($loguse);
               if ($dat->getFecaprord()!='')
                 $per->setFecaprord($dat->getFecaprord());
               else
                 $per->setFecaprord(date('Y-m-d'));
               $per->setObsaprord($dat->getObsaprord());
               $per->save();
            }
        }
    }
