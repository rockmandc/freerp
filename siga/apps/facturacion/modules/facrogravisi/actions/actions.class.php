...
/**
 * facrogravisi actions.
 *
 * @package    siga
 * @subpackage facrogravisi
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class facrogravisiActions extends autofacrogravisiActions
{
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
        $q= new Criteria();
        $q->add(FaregvendPeer::RIFVEN,$codigo);
        $result= FaregvendPeer::doSelectOne($q);
        if ($result)
        {
           $dato=$result->getNomven();
        }else {
           $js="alert_('El C&oacute;digo del Vendedor no est&aacute; Registrado'); $('facrovis_rifven').value=''; $('$cajtexmos').value=''; $('facrovis_rifven').focus(); ";
        }
        $output = '[["javascript","'.$js.'",""],["'.$cajtexmos.'","'.$dato.'",""],["","",""]]';
        break;
      case '2':
        $q= new Criteria();
        $q->add(FaclientePeer::RIFPRO,$codigo);
        $result= FaclientePeer::doSelectOne($q);
        if ($result)
        {
           $dato=$result->getNompro();
        }else {
           $js="alert_('El C&oacute;digo del Cliente no est&aacute; Registrado'); $('facrovis_rifpro').value=''; $('$cajtexmos').value=''; $('facrovis_rifpro').focus(); ";
        }
        $output = '[["javascript","'.$js.'",""],["'.$cajtexmos.'","'.$dato.'",""],["","",""]]';
        break;        
      default:
