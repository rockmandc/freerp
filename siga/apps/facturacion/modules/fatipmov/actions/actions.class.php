 ...
 */
class fatipmovActions extends autofatipmovActions
{
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
        $q= new Criteria();
        $q->add(ContabbPeer::CODCTA,$codigo);
        $result= ContabbPeer::doSelectOne($q);
        if ($result) {
          if ($result->getCargab()=='C')
            $dato=$result->getDescta();
          else
            $js="alert_('La Cuenta Contable no es Cargable'); $('fatipmov_codcta').value=''; $('fatipmov_codcta').focus();";
        }else
          $js="alert_('La Cuenta Contable no existe'); $('fatipmov_codcta').value=''; $('fatipmov_codcta').focus();";
        
        $output = '[["'.$cajtexmos.'","'.$dato.'",""],["javascript","'.$js.'",""]]';
        break;
      case '2':
        $q= new Criteria();
        $q->add(CotiptraPeer::CODTIPTRA,$codigo);
        $result= CotiptraPeer::doSelectOne($q);
        if ($result)
          $dato=$result->getDestiptra();
        else
          $js="alert_('El Tipo de Transaci&oacute;n no existe'); $('fatipmov_codtiptra').value=''; $('fatipmov_codtiptra').focus();";
