...
<table width="100%">
  <tr>
    <th><strong><font color="#CC0000" size="2" face="Verdana, Arial, Helvetica, sans-serif"> <? print $fadevolu->getEstatus();?></font></strong></th>
  </tr>
</table>
...
<br/>
<div id="divcoddirec">
  <?php if($labels['fadevolu{coddirec}']!='.:') { ?>
  <?php echo label_for('fadevolu[coddirec]', __($labels['fadevolu{coddirec}' ]), 'class="required" ') ?>
  <div class="content<?php if ($sf_request->hasError('fadevolu{coddirec}')): ?> form-error<?php endif; ?>">
  <?php if ($sf_request->hasError('fadevolu{coddirec}')): ?>
    <?php echo form_error('fadevolu{coddirec}', array('class' => 'form-error-msg')) ?>
  <?php endif; }?>  
  
    
  <?php echo Catalogo($fadevolu, 3, array (
...
<?php if ($fadevolu->getId() && $fadevolu->getStadph()!='N') {?>
  <input type="button" name="Submit" value="Anular" class="sf_admin_action_delete" onclick="javascript:anular();" />
<?php }?>
</li>
      <li class="float-left"><?php if ($fadevolu->getId() &&  $fadevolu->getStadph()!='N'): ?>
...
var deshab='<?php echo H::getConfApp2('bloqfec', 'facturacion', 'fadevolu'); ?>';
  if (deshab=='S')
  {
    $('trigger_fadevolu_fecdev').hide();
    $('fadevolu_fecdev').readOnly=true;
  }
  ...
  function anular()
  {
    var nrodev=$('fadevolu_nrodev').value;
    var fecdev=$('fadevolu_fecdev').value;

    window.open(getUrlModulo()+'anular?nrodev='+nrodev+'&fecdev='+fecdev,'...','menubar=no,toolbar=no,scrollbars=yes,width=700,height=400,resizable=yes,left=400,top=120');
  }
