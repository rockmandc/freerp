<?php
// auto-generated by PropelCidesa
// date: 2008/12/08 10:20:17
?>
<?php
echo form_tag('faartpvp/save', array(
    'id' => 'sf_admin_edit_form',
    'name' => 'sf_admin_edit_form',
    'multipart' => true,
))
?>

<?php use_helper('Javascript', 'PopUp', 'Grid') ?>
<?php echo javascript_include_tag('dFilter', 'tools', 'observe', 'ajax') ?>

<?php echo object_input_hidden_tag($faartpvp, 'getId') ?>



<h2 class="h2" onclick="javascript: return $('divDatos del Artículo').toggle();"><?php echo __('Datos del Artículo') ?></h2>
<fieldset id="sf_fieldset_datos_del_art__culo" class="">

    <div class="form-row" id="divDatos del Artículo">
        <div id="divcodart">
                <?php echo label_for('faartpvp[codart]', __($labels['faartpvp{codart}']), 'class="required" Style="text-align:left; width:150px"') ?>
            <div class="content<?php if ($sf_request->hasError('faartpvp{codart}')): ?> form-error<?php endif; ?>">
                <?php if ($sf_request->hasError('faartpvp{codart}')): ?>
                <?php echo form_error('faartpvp{codart}', array('class' => 'form-error-msg')) ?>
                <?php endif; ?>

                <?php
                        $value = object_input_tag($faartpvp, 'getCodart', array(
                        'size' => 20,
                        'control_name' => 'faartpvp[codart]',
                        'readonly' => $faartpvp->getId() != '' ? true : false,
                        'onKeyDown' => "javascript:return dFilter (event.keyCode, this)",
                        'onBlur' => remote_function(array(
                            'url' => 'faartpvp/ajax',
                            'script' => true,
                            'complete' => 'AjaxJSON(request, json)',
                            'condition' => "$('faartpvp_codart').value != ''",
                            'with' => "'ajax=1&cajtexmos=faartpvp_desart&cajtexcom=faartpvp_codbar&codigo='+this.value"
                        ))
                    ));
                        echo $value ? $value : '&nbsp;' ?>
                <?php echo button_to_popup('...', cross_app_link_to('herramientas', 'catalogo') . '/metodo/Caregart_Faproalt/clase/Caregart/frame/sf_admin_edit_form/obj1/faartpvp_codart/obj2/faartpvp_desart/campo1/codart/campo2/desart', '', '', 'botoncat') ?></th>

                <?php
                        $value = object_input_tag($faartpvp, 'getDesart', array(
                                    'size' => 80,
                                    'disabled' => true,
                                    'control_name' => 'faartpvp[desart]',
                                ));
                        echo $value ? $value : '&nbsp;'
                ?>

             </div>
        </div>
        <br/>
        <div id="divcodbar">
          <?php if($labels['faartpvp{codbar}']!='.:') { ?>
          <?php echo label_for('faartpvp[codbar]', __($labels['faartpvp{codbar}' ]), 'class="required" Style="text-align:left; width:150px"') ?>
          <div class="content<?php if ($sf_request->hasError('faartpvp{codbar}')): ?> form-error<?php endif; ?>">
          <?php if ($sf_request->hasError('faartpvp{codbar}')): ?>
            <?php echo form_error('faartpvp{codbar}', array('class' => 'form-error-msg')) ?>
          <?php endif; }?>



          <?php
                $value = object_input_tag($faartpvp, 'getCodbar', array(
                            'size' => 50,
                            'maxlength' => 32,
                            'control_name' => 'faartpvp[codbar]',
                            'onBlur' => remote_function(array(
                                'url' => 'faartpvp/ajax',
                                'script' => true,
                                'complete' => 'AjaxJSON(request, json)',
                                'condition' => "$('faartpvp_codbar').value != ''",
                                'with' => "'ajax=3&codigo='+this.value"
                        ))
                        ));
                echo $value ? $value : '&nbsp;'
        ?>


          <?php if($labels['faartpvp{codbar}']!='.:') { ?>



          </div>
          <?php  } ?>

        </div>
    </div>
</fieldset>

<?php echo grid_tag_v2($obj); ?>

        <?php include_partial('edit_actions', array('faartpvp' => $faartpvp)) ?>


                <ul class="sf_admin_actions">
                    <li class="float-left"><?php if ($faartpvp->getId()): ?>
<?php
                            echo button_to(__('delete'), 'faartpvp/delete?id=' . $faartpvp->getId() . '&codart=' . $faartpvp->getCodart(), array(
                                'post' => true,
                                'confirm' => __('Are you sure?'),
                                'class' => 'sf_admin_action_delete',
                            ))
?><?php endif; ?>
                        </li>
                    </ul>

                    <script language="JavaScript" type="text/javascript">
                        var idf='<?php echo $faartpvp->getId() ?>';
    if (idf!="")
    {
        $$('.botoncat')[0].disabled=true;
    }
</script>
