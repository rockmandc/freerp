...
class faajusteActions extends autofaajusteActions
{
  ...
  public function executeList()
  {
    ...
    $this->filters = $this->getUser()->getAttributeHolder()->getAll('sf_admin/faajuste/filters');
    $filcajusu=H::getConfApp2('filcajusu', 'facturacion', 'fafactur'); 
    $loguse=sfContext::getInstance()->getUser()->getAttribute('loguse');
    $filsoldir=H::getConfApp2('filsoldir', 'compras', 'almsolegr');

     // 15    // pager
    $this->pager = new sfPropelPager('Faajuste', 15);
    $c = new Criteria();
    if ($filcajusu=='S' && $filsoldir=='S'){
            $this->sql='(tipaju = \'F\' and  codref in (select reffac from fafactur where  codcaj in 
                  (select codcaj from "SIMA_USER".segcajusu where loguse=\''.$loguse.'\'))) and (tipaju = \'F\' and  codref in (select reffac from fafactur where  coddirec in 
                  (select coddirec from "SIMA_USER".segdirusu where loguse=\''.$loguse.'\')))
                   or (tipaju = \'P\')  or (tipaju = \'NE\')';
      $c->add(FaajustePeer::TIPAJU,$this->sql,Criteria::CUSTOM);

    }else if ($filcajusu=='S') {             
      $this->sql='(tipaju = \'F\' and  codref in (select reffac from fafactur where  codcaj in 
                  (select codcaj from "SIMA_USER".segcajusu where loguse=\''.$loguse.'\')))
                   or (tipaju = \'P\')  or (tipaju = \'NE\')';
      $c->add(FaajustePeer::TIPAJU,$this->sql,Criteria::CUSTOM);
    }else if ($filsoldir=='S'){
      $this->sql='(tipaju = \'F\' and  codref in (select reffac from fafactur where  coddirec in 
                  (select coddirec from "SIMA_USER".segdirusu where loguse=\''.$loguse.'\')))
                   or (tipaju = \'P\')  or (tipaju = \'NE\')';
      $c->add(FaajustePeer::CODDIREC,$this->sql,Criteria::CUSTOM); 
    }
    $this->c ? $c=$this->c : '';
  ...
  public function executeEdit()
  {
    ...
    if (!$this->faajuste->getId()){        
        ...
        if ($filsoldir=='S'){
           ...
           if ($regt){
            if ($this->faajuste->getCoddirec()=='')
              $this->faajuste->setCoddirec($regt->getCoddirec());
           }
...
$this->Bitacora('Guardo');
            ...
            if ($this->getRequestParameter('tipaju') == 'P'){
              ...
              $desdir=H::getX('CODDIREC','cadefdirec','Desdirec',$coddirec);
                        $this->configGridDetalle('', $this->getRequestParameter('tipaju'), $codigo);
				  		...
				  		$output = '[["faajuste_codpro","'.$codpro.'",""],["faajuste_rifpro","'.$rifpro.'",""],["faajuste_nompro","'.$nompro.'",""],["faajuste_dirpro","'.$dirpro.'",""],["faajuste_telpro","'.$telpro.'",""],["faajuste_coddirec","'.$coddirec.'",""],["faajuste_desdirec","'.$desdir.'",""],["javascript","'.$javascript.'",""]]';
			         	$this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
						}
                                        }
  ...
  public function configGridDetalle($nrodev = '', $tipaju = '', $codigo='')
  {
	   ...
	   if ($codigo!="")
	   {
        ...
        $col18->setHTML('type="text" size="10"');
        $col18->setJScript('onBlur="daleformato(this.id); "');

        $col19 = clone $col6;
        $col19->setTitulo('Total Ajuste');
        $col19->setNombreCampo('montot');
        $col19->setHTML('type="text" size="10" ');
        $col19->setJScript('onBlur="daleformato(this.id); monto_total()"');
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      ...
      case '3':
        ...
        $cantival=false;
        $cantaju=H::tofloat($this->getRequestParameter('valorcol7'),6);
        $existen=0;
        $colum11=0;
        $canrealped=0;
        $canrealdes=0;
        ...
        //{
          ...
          if ($cantival)
          {
           if ($cantaju>H::tofloat($this->getRequestParameter('solicit'))) //Ajuste por arriba
           {
           	 ...
           	 {
                   $rerec=Facturacion::ajusteRecargo($this->getRequestParameter('articuloaju'),$this->getRequestParameter('referencia'),$this->getRequestParameter('descart'));
                   if ($difpreaju==0)
      ...
      case '5':
        ...
        if ($reg)
        {
           ...
           $javascript="alert_('La Direcci&oacute;n no existe'); $('faajuste_coddirec').value=''; $('faajuste_desdirec').value=''; $('faajuste_coddirec').focus();";
        }
  ...
  public function validateEdit()
  {
    if($this->getRequest()->getMethod() == sfRequest::POST)
    {
      ...
      if (Tesoreria::validarFechaPerContable($this->getRequestParameter('faajuste[fecaju]')))
      {
        $this->coderror=521;
        return false;
      }         
	  ...
	  $grid=Herramientas::CargarDatosGrid($this,$this->obj);
              $valpermes=H::getConfApp2('valpermes', 'facturacion', 'faajuste');
        if (!$this->faajuste->getId()) {
            if ($valpermes=='S')
            {
                $dateFormat = new sfDateFormat('es_VE');
                $fec = $dateFormat->format($this->getRequestParameter('faajuste[fecaju]'), 'i', $dateFormat->getInputPattern('d'));
      ...
      while ($j<count($x))
      {
        ...
        if ($this->getRequestParameter('faajuste[tipaju]')=='F') {
            $a = new Criteria();
            $a->add(FaajustePeer::CODREF,$this->getRequestParameter('faajuste[codref]'));
            $regq = FaajustePeer::doSelectOne($a);
            if ($regq){        
           $this->coderror=1162;
           return false;
  ...
  public function getLabels()
  {
    ...
    if ($cambiareti=='S')
      $labels['faajuste{coddirec}'] = 'Estado';
    return $labels;
  }   
