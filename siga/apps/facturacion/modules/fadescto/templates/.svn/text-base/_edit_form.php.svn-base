...
<div id="divtipret">
  ...
  if ($fadescto->getTipret()=='S')  {
    ?><?php echo radiobutton_tag('fadescto[tipret]', 'S', true,array('onClick' => "oculmos(this.id)"))        ."Si".'&nbsp;&nbsp;';
        echo radiobutton_tag('fadescto[tipret]', 'N', false,array('onClick' => "oculmos(this.id)"))."   No";?>
      <?

  }else{
    echo radiobutton_tag('fadescto[tipret]', 'S', false,array('onClick' => "oculmos(this.id)"))        ."Si".'&nbsp;&nbsp;';
    echo radiobutton_tag('fadescto[tipret]', 'N', true,array('onClick' => "oculmos(this.id)"))."   No";
  ...
  <th>
    <div id="divporbas">
<?php echo label_for('fadescto[porbas]', __($labels['fadescto{porbas}']), 'class="required" ') ?>
<div class="content<?php if ($sf_request->hasError('fadescto{porbas}')): ?> form-error<?php endif; ?>">
<?php if ($sf_request->hasError('fadescto{porbas}')): ?> <?php echo form_error('fadescto{porbas}', array('class' => 'form-error-msg')) ?>
<?php endif; ?>

 <?php $value = object_input_tag($fadescto, array('getPorbas',true), array (
'size' => 7,
'control_name' => 'fadescto[porbas]',
'onBlur' => "javascript:event.keyCode=13;return entermontootro(event, this.id)",
)); echo $value ? $value : '&nbsp;' ?>
</div>
</div>
...
<div id="divcomret">
  <?php if($labels['fadescto{comret}']!='.:') { ?>
  <?php echo label_for('fadescto[comret]', __($labels['fadescto{comret}' ]), 'class="required" Style="text-align:left;"') ?>
  <div class="content<?php if ($sf_request->hasError('fadescto{comret}')): ?> form-error<?php endif; ?>">
  <?php if ($sf_request->hasError('fadescto{comret}')): ?>
    <?php echo form_error('fadescto{comret}', array('class' => 'form-error-msg')) ?>
  <?php endif; }?>  
       
  
<?php echo select_tag('fadescto[comret]', options_for_select(array('I' => 'IVA', 'S' => 'ISLR', 'L' => 'LTF'),
$fadescto->getComret(),'include_blank=true')) ?>       
...
<div class="form-row">
 ...
 <?php $value = object_input_tag($fadescto, 'getCodcta', array (
  ...
  'onBlur'=> remote_function(array(
			  ...
			  'complete' => 'AjaxJSON(request, json)',
			  'condition' => "$('fadescto_codcta').value != ''",
  			  'with' => "'ajax=1&cajtexcom=fadescto_codcta&cajtexmos=fadescto_descta&codigo='+this.value"
			  )),
)); echo $value ? $value : '&nbsp;' ?>
...
if ($('fadescto_tipret_N').checked==true)
    $('divporbas').show();
  else
    $('divporbas').hide();

function oculmos(id){
  if (id=='fadescto_tipret_N')
    $('divporbas').show();
  else
    $('divporbas').hide();
