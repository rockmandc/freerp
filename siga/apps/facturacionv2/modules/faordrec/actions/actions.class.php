 ...
 */
class faordrecActions extends autofaordrecActions {
    ...
    public function executeAjax() {
        ...
        if ($this->getRequestParameter('ajax') == '1') {
            ...
            $dato = FaordcomPeer::getFecord($this->getRequestParameter('codigo'));
    ...
    public function executeAutocomplete() {
        if ($this->getRequestParameter('ajax') == '1') {
            ...
            $this->tags = Herramientas::autocompleteAjax('ORDCOM', 'Faordcom', 'Ordcom', trim($this->getRequestParameter('carcpart[ordcom]')));
        }
    }
    ...
    public function executeGrid() {
        ...
        if (trim($this->getRequestParameter('codigo')) != "") {
            if ($this->getRequestParameter('ajax') == '2') {
                $c = new Criteria();
                $c->add(FaordcomPeer::ORDCOM, $this->getRequestParameter('codigo'));
                $datos = FaordcomPeer::doSelectOne($c);
                $entro = false;
                $output = '[["carcpart_codalm","",""],["carcpart_fecord","",""],["carcpart_codpro","",""],["carcpart_nompro","",""],["carcpart_desconpag","",""],["carcpart_desforent","",""],["carcpart_desrcp","",""],["carcpart_monrcp","",""]]';
                $this->getResponse()->setHttpHeader("X-JSON", '(' . $output . ')');
                ...
                if ($datos) {
                    ...
                    if ($datos->getFecord() <= $fecreccom) {
                        ...
                        if (Herramientas::BuscarDatos($sql, &$result)) {
                            ...
                            $fecha = $datos->getFecord();
                            $fecha = date("d/m/Y", strtotime($fecha));
                            $codpro = $datos->getCedrif();
                            $codalm = $this->getRequestParameter('codalm');
                            $nompro = $datos->getNompro(); 
                            $monord = $datos->getMonord();                       
                            $des = "RECEP. " . $this->getRequestParameter('numero');
                            Facturacionv2::buscarDetalleOC($this->getRequestParameter('codigo'), &$arreglo);
                            $j=0;
                            $total=0;
                            while($j<count($arreglo))
                            {
                                $total=$total+($arreglo[$j]["canord"]*($arreglo[$j]["preart"]-$arreglo[$j]["monrgo"]));
                                $j++;
                            }
                            $total = number_format($total, 2, ',', '.');
                            $this->configGrid($this->getRequestParameter('codigo'), $this->getRequestParameter('codalm'), $arreglo);
                            $output = '[["carcpart_fecord","' . $fecha . '",""],["carcpart_nompro","' . $nompro . '",""],["carcpart_codpro","' . $codpro . '",""],["carcpart_desrcp","' . $des . '",""],["carcpart_monrcp","' . $total . '",""],["' . $cajtexcom . '","8","c"],["carcpart_codalm","' . $codalm . '",""]]';
                            $this->getResponse()->setHttpHeader("X-JSON", '(' . $output . ')');
                        }
                        else {
                            $this->configGrid();
    ...
    protected function getCarcpartOrCreate($id = 'id') {
        if (!$this->getRequestParameter($id)) {
            ...
            $refpda = H::getX_vacio('Ordcom', 'Cadetpda', $this->getRequestParameter('carcpart[ordcom]'));
                Facturacionv2::buscarDetalleOC($this->getRequestParameter('carcpart[ordcom]'), &$arreglo);
    ...
    public function configGrid($codigo='', $codalm='', $arreglo=array()) {
        ...
        if (count($arreglo) > 0) {
            ...
            while ($j < count($arreglo)) {
                ...
                $Faartordnew = new Faartord();
                $Faartordnew->setCodart($arreglo[$j]["codart"]);
                $Faartordnew->setCodunimed($arreglo[$j]["codunimed"]);
                $Faartordnew->setCanord($arreglo[$j]["canord"]);
                $Faartordnew->setCanrec($arreglo[$j]["canrec"]);
                $Faartordnew->setPreart($arreglo[$j]["preart"]);
                $Faartordnew->setMonrgo($arreglo[$j]["monrgo"]);
                $Faartordnew->setPorrgo($arreglo[$j]["porrgo"]);
            ...
            if ($manforent == 'S') {
                if ($codalm != "") {
                    $c->addJoin(FaartordPeer::ORDCOM, FaordcomPeer::ORDCOM);
                    $c->addJoin(FaordcomPeer::ORDCOM, CaentordPeer::ORDCOM);
                    $c->addJoin(FaartordPeer::CODART, CaentordPeer::CODART);
                    $c->add(CaentordPeer::ORDCOM, $codigo);
                    $c->add(CaentordPeer::CODALM, $codalm);
                }
            }
            $c->add(FaartordPeer::ORDCOM, $codigo);
            $this->sql = "Faartord.canord > Faartord.canrec ";
            $c->add(FaartordPeer::CANORD, $this->sql, Criteria::CUSTOM);
            $per = FaartordPeer::doSelect($c);
        }
    ...
    public function configGridConsulta($rcpart) {
        ...
        $col2->setAlineacionObjeto(Columna::CENTRO);
        $col2->setAlineacionContenido(Columna::CENTRO);
        $col2->setHTML('type="text" size="40x1" disabled=true');
        ...
        $col23->setAlineacionObjeto(Columna::CENTRO);
        $col23->setAlineacionContenido(Columna::CENTRO);
        $col23->setHTML('type="text" size="30" readonly=true');
        ...
        if ($this->getRequest()->getMethod() == sfRequest::POST) {
            ...
            if ($this->saveCarcpart($this->carcpart) == -1) {
                ...
                if ($this->getRequestParameter('save_and_add')) {
                    return $this->redirect('faordrec/create');
                } else if ($this->getRequestParameter('save_and_list')) {
                    return $this->redirect('faordrec/list');
                } else {
                    return $this->redirect('faordrec/edit?id=' . $this->carcpart->getId());
                }
            } else {
    ...
    protected function updateCarcpartFromRequest() {
        ...
        {
            if (isset($carcpart['codalmusu'])){
                $this->carcpart->setCodalmusu($carcpart['codalmusu']);
            }
        }
        parent::updateCarcpartFromRequest();
    ...
    protected function saveCarcpart($carcpart) {
        ...
        if ($carcpart->getId()) {
            ...
            $this->coderror = Recepcion::salvarAlmrec2($carcpart, $grid2);
        }
    ...
    protected function deleteCarcpart($carcpart) {
        $this->msgeli = "";
        if (Recepcion::eliminarAlmrec2($carcpart, &$this->msgeli))
            return true;
        else
            return false;
    }
    ...
    public function validateEdit() {
        ...
        if ($this->getRequest()->getMethod() == sfRequest::POST) {
            ...
            if ($this->carcpart->getId()) {
                ...
                if ($valor1 != "")
                    $resp1 = Herramientas::ValidarCodigo($valor1, $this->carcpart, $campo1, 'F');
    ...
    public function executeList()
    {
        ...
        $this->pager = new sfPropelPager('Carcpart', 10);
        $c = new Criteria();
        ...
        foreach($almacenes as $cod => $alm){
          $arr_or[] = CarcpartPeer::CODALMUSU."='$cod'";
        }
        $c->addOr(CarcpartPeer::CODALMUSU,implode(' OR ', $arr_or),Criteria::CUSTOM);
