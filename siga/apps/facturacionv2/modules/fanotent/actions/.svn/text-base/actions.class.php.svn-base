 ...
 * @version SVN: $Id: actions.class.php 45518 2011-08-11 19:39:07Z dmartinez $
 * 
...
$this->Bitacora('Guardo');
		...
		if ($codalm == ''){
			...
			if ($reg){
				if ($canent > $reg->getExiact()){
					...
					$totent = Facturacionv2::BuscarTotalEntregado($codart, $tipref, $codref);
  ...
  public function validateEdit()
  {
    if($this->getRequest()->getMethod() == sfRequest::POST)
    {
      ...
      if (Tesoreria::validarFechaPerContable($this->getRequestParameter('fanotent[fecnot]')))
      {
        $this->coderror1=521;
        return false;
      }
				...
				if ($reg){
					if ($x[$j]->getCanent() > 0){
						if ($x[$j]->getCanent() > $reg->getExiact()){
							...
							$totent = Facturacionv2::BuscarTotalEntregado($x[$j]->getCodart(), $this->getRequestParameter('fanotent[tipref]'), $this->getRequestParameter('fanotent[codref]'));
  ...
  public function saveFanotent($fanotent)
  {
  	if ($this->fanotent->getId() == ''){
	     ...
	     $grid=Herramientas::CargarDatosGridv2($this,$this->obj);
	     $resp=Facturacionv2::salvarFafanot($fanotent,$grid,$this->getUser()->getAttribute('loguse',null));
	    if($resp!=-1){
	      $this->coderror1 = $resp;
