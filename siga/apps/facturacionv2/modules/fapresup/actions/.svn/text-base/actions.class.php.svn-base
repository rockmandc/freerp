...
$this->Bitacora('Guardo');
  ...
  public function configGridDetPre($nropre = '', $refpre='')
  {
    ...
    $params= array('param1' => $lonarti, 'param2' => "'+$('fapedido_codprg').value+'", 'param3' => "'+$('fapedido_codtip').value+'", 'param4' => "'+$('fapedido_conpag_id').value+'");
    if (($nropre!='')&&($this->fapresup->getRefpre()!='########')){
    	...
    	$this->columns[1][0]->setCatalogov2('falisprc','sf_admin_edit_form',$obj,'Falisprc_Fapedidov2',$params);
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
        $codcli=""; $nompro= ""; $dirpro= ""; $telpro = ""; $tipcte = ""; $codtip = ""; $javascript="";
       if ($codigo!="")
       {
         $c= new Criteria();
         ...
         if ($reg)
         {
             ...
             $tipcte = H::getX('id', 'Fatipcte', 'nomtipcte', $reg->getFatipcteId());
             $codtip = $reg->getFatipcteId();
         }
         else
         {
        ...
        $output = '[["fapresup_codcli","'.$codcli.'",""],["fapresup_nompro","'.$nompro.'",""],["fapresup_dirpro","'.$dirpro.'",""],["fapresup_telpro","'.$telpro.'",""],["fapresup_tipcte","'.$tipcte.'",""],["fapresup_codtip","'.$codtip.'",""],["javascript","'.$javascript.'",""]]';
      ...
      case '5':
        $this->ajaxs='5';
        $this->precios=array();
        $this->precios=FalisprcPeer::getPreciosv2($codigo);
       break;
      case '6':
	  ...
	  if ($this->ValidarDatosVaciosenGrid($grid,&$error))
	       {
	          ...
	          return false;
               ...
               if ($this->fapresup->getConcre() == 'R'){
                   if ($this->ValidarMontoTotal($grid)) {
                       $this->coderror = 1171;
                       return false;
                }
               }
  ...
  public function ValidarMontoTotal($grid) {
       ...
       $r = new Criteria();
       $r->add(FaclientePeer::RIFPRO, $this->fapresup->getRifpro());
       $res = FaclientePeer::doSelectOne($r);
       if ($res) {
           $limite = $res->getLimcre();
           if ($acum > $limite) {
               return true;
           } else {
               return false;
  ...
  {
     $grid=Herramientas::CargarDatosGridv2($this,$this->obj);
     $resp=Facturacionv2::salvarFapresup($fapresup,$grid);
    if($resp!=-1){
      $this->coderror = $resp;
      $err = Herramientas::obtenerMensajeError($this->coderror);
      $this->getRequest()->setError('',$err);
