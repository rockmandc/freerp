...
class faemptraActions extends autofaemptraActions
{
  ...
  public function executeAjax()
  {
   $codigo = $this->getRequestParameter('codigo', '');
        ...
        switch ($ajax) {
            case '1':
                $t = new Criteria();
                $t->add(TsdefbanPeer::CODZON, $codigo);
                $reg = TsdefbanPeer::doSelectOne($t);
                if ($reg) {
                    $dato = $reg->getCodzon();
                } else {
                    $js = "alert('El Código de la Zona no Existe'); $('fadefzon_codzon').value=''; $('fadefzon_desczon').value=''; $('fadefzon_codzon').focus();";
                }
