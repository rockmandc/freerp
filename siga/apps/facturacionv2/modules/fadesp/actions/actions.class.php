...
class fadespActions extends autofadespActions {
                                ...
                                if ($reg) {
                                    if ($x[$j]->getCandesp() > 0) {
                                        if ($x[$j]->getCandesp() > $reg->getExiact()) {
                                            ...
                                            $totent = Facturacionv2::BuscarTotalEntregado($x[$j]->getCodart() , $this->getRequestParameter('cadphart[tipref]') , $this->getRequestParameter('cadphart[reqart]'));
    ...
    public function executeList() {
        ...
        if ($varemp) if (array_key_exists('aplicacion', $varemp)) if (array_key_exists('facturacion', $varemp['aplicacion'])) if (array_key_exists('modulos', $varemp['aplicacion']['facturacion'])) if (array_key_exists('fadesp', $varemp['aplicacion']['facturacion']['modulos'])) if (array_key_exists('despnotent', $varemp['aplicacion']['facturacion']['modulos']['fadesp'])) {
        ...
        if(H::getConfApp2('usualms','autenticacion','login')=='S'){
          ...
          if (count($almacenes) > 0) {
              foreach ($almacenes as $cod => $alm) {
                  $arr_or[] = CadphartPeer::CODALMUSU . "='$cod'";
              }
              $c->addOr(CadphartPeer::CODALMUSU, '(' . implode(' OR ', $arr_or) . ')', Criteria::CUSTOM);
          } else {
              $c->add(CadphartPeer::CODALMUSU, null);
    ...
    public function handleErrorEdit() {
        ...
        if ($this->coderror != - 1) {
            $err = Herramientas::obtenerMensajeError($this->coderror);
            if ($this->coderror == 1119 || $this->coderror == 1120 || $this->coderror == 1121 || $this->coderror == 1122 || $this->coderror == 1125) $this->getRequest()->setError('', $err . ' ->Articulo: ' . $this->articulo);
    ...
    protected function updateCadphartFromRequest() {
        ...
        $fields = CadphartPeer::getFieldNames();
        if (array_search('Codalmusu', $fields)) {
            if (isset($cadphart['codalmusu'])) {
                $this->cadphart->setCodalmusu($cadphart['codalmusu']);
            }
        }
    ...
    public function configGrid() {
        $this->configGridDetalle($this->getRequestParameter('cadphart[dphart]') , $this->getRequestParameter('cadphart[reqart]') , $this->getRequestParameter('cadphart[tipref]') , $this->getRequestParameter('cadphart[codalmusu]'));
    }
    /**
     * Esta función permite definir la configuración del grid de datos
     * que contiene el formulario. Esta función debe ser llamada
     ...
     *
     */
    public function configGridDetalle($nrodph = '', $codigo = '', $tipref = '', $codalm = '') {
        if ($nrodph != '') {
            $c = new Criteria();
        ...
        if ($codalm != '') {
            $p = new Criteria();
            $p->add(CaalmubiPeer::CODALM, $codalm);
            $reg = CaalmubiPeer::doSelectOne($p);
            $codubi = $reg->getCodubi();
            
            foreach ($per as $objp) {
                $objp->setCodalm($codalm);
                $objp->setCodubi($codubi);
            }
        }
        if ($tipref == 'F') {
        ...
        if ($nrodph != '') {
            ...
            $signo = "-";
            $signomas = "+";
            if ($manartlot == 'S') $col1->setJScript('onBlur="toAjaxUpdater(obtenerColumna(this.id,4,' . chr(39) . $signomas . chr(39) . '),1,getUrlModuloAjax(),this.value+' . chr(39) . '!' . chr(39) . '+$(obtenerColumna(this.id,6,' . chr(39) . $signomas . chr(39) . ')).value+' . chr(39) . '!' . chr(39) . '+obtenerColumna(this.id,1,' . chr(39) . $signomas . chr(39) . '),devuelveParVacios(),devuelveParVacios());"');
            else $col1->setJScript('onBlur="toAjax(1,getUrlModuloAjax(),this.value+' . chr(39) . '!' . chr(39) . '+$(obtenerColumna(this.id,6,' . chr(39) . $signomas . chr(39) . ')).value+' . chr(39) . '!' . chr(39) . '+obtenerColumna(this.id,1,' . chr(39) . $signomas . chr(39) . '),devuelveParVacios(),devuelveParVacios());"');
        }
        ...
        if ($nrodph != '') {
            ...
            if ($manartlot == 'S') $col3->setJScript('onKeyDown="javascript:return dFilter (event.keyCode, this,' . chr(39) . $this->mascaraubicacionalm . chr(39) . ')"  onBlur="toAjaxUpdater(obtenerColumna(this.id,2,' . chr(39) . $signomas . chr(39) . '),5,getUrlModuloAjax(),this.value+' . chr(39) . '!' . chr(39) . '+$(obtenerColumna(this.id,2,' . chr(39) . $signo . chr(39) . ')).value+' . chr(39) . '!' . chr(39) . '+$(obtenerColumna(this.id,4,' . chr(39) . $signomas . chr(39) . ')).value+' . chr(39) . '!' . chr(39) . '+obtenerColumna(this.id,4,' . chr(39) . $signomas . chr(39) . '),devuelveParVacios(),devuelveParVacios());"');
            else $col3->setJScript('onKeyDown="javascript:return dFilter (event.keyCode, this,' . chr(39) . $this->mascaraubicacionalm . chr(39) . ')"  onBlur="toAjax(5,getUrlModuloAjax(),this.value+' . chr(39) . '!' . chr(39) . '+$(obtenerColumna(this.id,2,' . chr(39) . $signo . chr(39) . ')).value+' . chr(39) . '!' . chr(39) . '+$(obtenerColumna(this.id,4,' . chr(39) . $signomas . chr(39) . ')).value+' . chr(39) . '!' . chr(39) . '+obtenerColumna(this.id,4,' . chr(39) . $signomas . chr(39) . '),devuelveParVacios(),devuelveParVacios());"');
        }
    ...
    public function executeGrid() {
        ...
        $codalmusu = $this->getRequestParameter('codalm');
        $this->mensaje = "";
        $this->setVars();
        $rifpro = "";
        $nompro = "";
        ...
        if ($this->getRequestParameter('codigo') != "") {
            if ($this->getRequestParameter('ajax') == '2') {
                if ($tipref == 'P') {
                    ...
                    $this->configGridDetalle('', $codigo, $tipref, $codalmusu);
                    $c = new Criteria();
                    $c->add(FapedidoPeer::NROPED, $codigo);
                    $datos = FapedidoPeer::doSelectOne($c);
                    if ($datos) {
                        if ($datos->getCodalmusu() != $codalmusu) {
                            $this->configGrid();
                            $this->mensaje = "Este pedido no se encuentra Asociado a este Almacen";
                        } else {
                    ...
                    if ($datos) {
                        if ($datos->getCodalmusu() != $codalmusu) {
                            $this->configGrid();
                            $this->mensaje = "Esta Factura no se encuentra Asociado a este Almacen";
                        } else {
                            if ($datos->getStatus() == 'N') {
                                $this->configGrid();
