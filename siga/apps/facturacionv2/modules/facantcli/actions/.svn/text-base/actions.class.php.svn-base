...
class facantcliActions extends autofacantcliActions
{
  ...
  {
      $this->configGrid($this->faantcli->getNroant());

  }

  public function configGrid($nroant='', $codcli='')
  {
     if ($codcli!="")
     {
         $p= new Criteria();
    ...
    switch ($ajax){
      case '1':
        ...
        if ($result)
        {
           ...
           $this->configGrid('',$dato3);
           
        }else {
            $sw=true; $dato1=""; $dato2=""; $dato3="";
  ...
  public function saving($clasemodelo)
  {
    ...
    if ($clasemodelo->getId())
    {
        ...
        Facturacionv2::SalvarAnticipoCliente($clasemodelo, $grid);
    }    
    return -1;
  }
  ...
  public function executeAjaxcomprobante()
  {
     ...
     if ($this->msjtres=="")
     {
      $x = Facturacionv2::generarComprobanteAnticipo($this->faantcli,&$comprobante,&$msjuno);
      if ($msjuno=="") {
          $concom = $concom + 1;
