...
class fadevoluActions extends autofadevoluActions
{
  ...
  public function executeEdit()
  {
    ...
    if ($this->getRequest()->getMethod() == sfRequest::POST)
    {
      ...
      if($this->saveFadevolu($this->fadevolu) ==-1){
...
*/
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      ...
      case '3':
          ...
          $candev = H::toFloat($this->getRequestParameter('candev',''));
          $precio = H::toFloat($this->getRequestParameter('precio',''));
          $idtot = $this->getRequestParameter('idtot','');
          $id = $this->getRequestParameter('id','');
          $q= new Criteria();
          $q->add(FafacturPeer::REFFAC, $factura);
          $reg= FafacturPeer::doSelectOne($q);
          if ($reg)
          {
                $ca= new Criteria();
                $ca->add(FargoartPeer::REFDOC,$factura);
                $ca->add(FargoartPeer::CODART,$codart);
                //$ca->add(FargoartPeer::DESART,$desart);          
                $trajo= FargoartPeer::doSelectOne($ca);
                if ($trajo)
                {
                    $cal=$candev*$precio;
                    $c = new Criteria();
                    $c->add(FarecargPeer :: CODRGO, $trajo->getCodrgo());
                    $resul = FarecargPeer :: doSelectOne($c);
                    if ($resul) {
                        if ($resul->getTiprgo() == 'M') {
                           $dato3 = $resul->getMonrgo();
                        } else {
                            if ($resul->getTiprgo() == 'P') {
                                $dato3 = (($cal * $resul->getMonrgo()) / 100);
                            } else {
                                $dato3 = 0;
                            }
                        }
                    ...
                    $cal= $cal + H::toFloat($dato3);
                    $js="$('".$idtot."').value='".H::FormatoMonto($cal)."'; $('".$id."').value='".H::FormatoMonto($candev)."'; sumardatosgrid();"; 
                }else {
                   $cal=$candev*$precio;
                   $js="$('".$idtot."').value='".H::FormatoMonto($cal)."'; $('".$id."').value='".H::FormatoMonto($candev)."'; sumardatosgrid();"; 
                }           
   ...
   *
   */
  function saveFadevolu($fadevolu) {

        if ($fadevolu->getTipdev() == 'C') {
            $this->coderror = Facturacion::validarUsuarioEspecial($fadevolu->getPasuse());
           if ($this->coderror == -1) {
                if ($fadevolu->getId()) {
                    $fadevolu->save();
                } else {
                    $grid2 = Herramientas::CargarDatosGrid($this, $this->obj);
                    Facturacionv2::salvarFadevolu($fadevolu, $grid2);
                }                
            }else return $this->coderror;
        }
        if ($fadevolu->getId()) {
            $fadevolu->save();
        } else {
