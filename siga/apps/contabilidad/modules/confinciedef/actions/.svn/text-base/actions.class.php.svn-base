...
/**
 * confinciedef actions.
 *
 * @package    siga
 * @subpackage confinciedef
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class confinciedefActions extends autoconfinciedefActions
{
  public function executeIndex() {
      ...
      if ($dato) {
          $id=$dato->getId();
          return $this->redirect('confinciedef/edit?id='.$id);
      }
      else {
          return $this->redirect('confinciedef/edit');
      }
  }
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
        $etadef = $this->getRequestParameter('etadef');
        $q= new Criteria();
        $q->add(ContabaPeer::CODEMP,'001');
        $result= ContabaPeer::doSelectOne($q);
        if ($result){
          if ($etadef=='C') {
            $result->setEtadef('A');
            $js="alert('La Etapa de Definicion se ha Abierto.')";
          }
          else{
            $result->setEtadef('C');
            $js="alert('La Etapa de Definicion se ha Cerrado.')";
          }
          $result->save();
        }
