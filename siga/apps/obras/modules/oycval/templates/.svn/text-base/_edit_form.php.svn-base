 ...
 function calcular_gastosree(id)
 {
   if ($(id).value!="")
   {
   	 if (validarnumero(id))
     {
       ...
       var gastos=$(id).value;
       new Ajax.Request('/obras'+getEnv()+'.php/oycval/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json)}, parameters:'ajax=8&codcon='+codcontrato+'&tipval='+codtipval+'&gastos='+gastos})
     }
     else
     {
       alert_('El Dato debe ser N&uacute;merico');
