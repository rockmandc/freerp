...
<br>
 <?php echo label_for('ocregobr[claobr]', __($labels['ocregobr{claobr}']), 'class="required"') ?>
  <div class="content<?php if ($sf_request->hasError('ocregobr{claobr}')): ?> form-error<?php endif; ?>">
  <?php if ($sf_request->hasError('ocregobr{claobr}')): ?>
    <?php echo form_error('ocregobr{claobr}', array('class' => 'form-error-msg')) ?>
  <?php endif; ?>

<?php echo select_tag('ocregobr[claobr]', options_for_select(array('N' => 'Nueva Obra', 'A' => 'Ampliación', 'R' => 'Rehabilitación'),$ocregobr->getClaobr()),array());?>
</div>
<br>

<div id="tipoproy" style="display:none">
<?php echo label_for('ocregobr[codbanpro]', __($labels['ocregobr{codbanpro}']), 'class="required" ') ?>
  <div class="content<?php if ($sf_request->hasError('ocregobr{codbanpro}')): ?> form-error<?php endif; ?>">
  <?php if ($sf_request->hasError('ocregobr{codbanpro}')): ?>
    <?php echo form_error('ocregobr{codbanpro}', array('class' => 'form-error-msg')) ?>
  <?php endif; ?>

  <?php $value = object_input_tag($ocregobr, 'getCodbanpro', array (
  'size' => 4,
  'maxlength' => 4,
  'readonly'  =>  $ocregobr->getId()!='' ? true : false ,
  'control_name' => 'ocregobr[codbanpro]',
  'onBlur'=> remote_function(array(
        'url'      => 'oycdesobr/ajax',
        'update'   => 'divgrid',
        'condition' => "$('ocregobr_codbanpro').value != '' && $('id').value == ''",
        'complete' => 'AjaxJSON(request, json)',
        'script'    => true,
        'with' => "'ajax=6&cajtexmos=ocregobr_desbanpro&cajtexcom=ocregobr_codbanpro&codigo='+this.value"
        ))
)); echo $value ? $value : '&nbsp;' ?>
&nbsp;
<?php echo  button_to_popup('...',cross_app_link_to('herramientas','catalogo').'/metodo/Ocbanpro_Oycdesobr/clase/Ocbanpro/frame/sf_admin_edit_form/obj1/ocregobr_codbanpro/obj2/ocregobr_desbanpro/campo1/codbanpro/campo2/desbanpro','','','botoncat')?>
&nbsp;
  <?php $value = object_input_tag($ocregobr, 'getDesbanpro', array (
  'disabled' => true,
  'size'=> 40,
  'control_name' => 'ocregobr[desbanpro]',
)); echo $value ? $value : '&nbsp;' ?>
    </div>
...
<script language="JavaScript" type="text/javascript">
 ...
 if (nuevo!='')
 {
   ...
   $('tipopre').hide();
     ...
     var bancopro='<?php echo $ocregobr->getManbanpro(); ?>';
     if (bancopro=='S')
         $('tipoproy').show();
