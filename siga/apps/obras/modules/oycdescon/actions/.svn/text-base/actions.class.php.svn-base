...
class oycdesconActions extends autooycdesconActions
{
  ...
  public function validateEdit()
  {
    if($this->getRequest()->getMethod() == sfRequest::POST)
    {
      ...
      $mes= date('m',strtotime($objc->getFeccie()));
      $monto=$this->ocregcon->getMoncon();
  	  if (!Obras::chequearDisponibilidadPresupuesto($this->ocregcon->getCodpre(),$anno,$mes,$grid,'1',$monto,$mondis))
  	  {
        $this->coderror1=152;
  ...
  public function executeAjax()
   {
     ...
     else if ($this->getRequestParameter('ajax')=='14')
     {
        ...
        $mes=$this->getRequestParameter('mes');
       	$grid='';
       	if (Obras::chequearDisponibilidadPresupuesto($this->getRequestParameter('codpre'),$anno,$mes,$grid,$num,$monto,$mondis))
       	{
       	  $javascript="";
       	}
       	else
