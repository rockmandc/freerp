...
protected $codigo = -1;
  ...
  public function executeList()
  {
    ...
    $c->addJoin(OpordpagPeer::TIPCAU,TsdefcajchiPeer::TIPCAU);
    $c->addJoin(OpordpagPeer::CODCAJCHI,TsdefcajchiPeer::CODCAJ);
   ...
   *
   */
  public function configGridSalida($fechades='', $fechahas='', $codcaj='', $coddirec='')
  {
    $filsoldir=H::getConfApp2('filsoldir', 'compras', 'almsolegr');
    ...
    if ($filsoldir=='S')
      $a->add(TssalcajPeer::CODDIREC,$coddirec);
  ...
  public function executeAjax()
  {
    ...
    case '4':
      ...
      case '5':
        ...
        $this->opordpag = $this->getOpordpagOrCreate();
        $this->setVars();        
        $this->configGridSalida($fechades,$fechahas,$this->getRequestParameter('codcajchi'),$this->getRequestParameter('coddirec'));
        $filsal=$this->filsal;
      ...
      case '6':
          ...
          if ($reg)
          {
             $dato=$reg->getDesdirec();         
              ...
              $dato="";
              $js="alert_('La Direcci&oacute;n no existe o no esta asociada al usuario'); $('opordpag_coddirec').value='';  $('opordpag_desdirec').value=''; $('opordpag_coddirec').focus();";
          }
      ...
      case '7'        :
          ...
          if ($resul) {
             $dato = $resul->getDescaj();
              ...
              $dato = "";
              $js = "alert('La caja Chica no existe'); $('opordpag_codcajchi').value=''; $('$cajtexmos').value=''; $('opordpag_codcajchi').focus();";
          }
           $output = '[["'.$cajtexmos.'","'.$dato.'",""],["javascript","'.$js.'",""]]';  
          $this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
          return sfView::HEADER_ONLY;
        ...
        while ($i<$concom)
        {
         ...
         if ($this->getUser()->getAttribute('grabo',null,$formulario[$i])=='S')
         {
    	  ...
    	  $per = OpdefempPeer::doSelectOne($c);
           ...
           {  $numerocomp = H::iif($per->getOrdconpre()=='t','OP'.substr($numerocomp = Comprobante::Buscar_Correlativo($feccom),2,strlen($numcom)),$numcom); }
      ...
      if ($data)
      {
        if (($data->getNumche()=='') || (strlen($data->getNumche())==0))
        {
         ...
         // Actualizar salidas de Caja Chica
          ...
          Herramientas::EliminarRegistro('Opdetord','Numord',$opordpag->getNumord());
          OrdendePago::eliminarCausado($opordpag->getNumord());
        }else { return 527;}
      }
    }else { return 526;}
   }//if ($opordpag)
  ...
  public function executeAjaxcomprobante()
  {
     ...
     if ($reg)
     {
      ...
      $this->opordpag->setCtapag($reg->getCtapag());
     }
