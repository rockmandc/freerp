...
/**
 * tesdesconmovlib actions.
 *
 * @package    Roraima
 * @subpackage tesdesconmovlib
 ...
 */
class tesdesconmovlibActions extends autotesdesconmovlibActions
{
  ...
  public function editing()
  {
	...
	$this->configGrid($this->tsdefban->getNumcue(), $this->tsdefban->getFechades(), $this->tsdefban->getFechahas());
  }
  ...
  public function transformarFecha($fecha)
  {
    ...
    if(count($arr_fecha)>1) return $arr_fecha[2].'-'.$arr_fecha[1].'-'.$arr_fecha[0];
    else return $fecha;
  }
  ...
  public function configGrid($numcue,$fechades,$fechahas,$reg = array(),$regelim = array())  //Plan de Trabajo
  {
   // $this->regelim = $regelim;
    ...
    $opc6=$c->getNewCriterion(TsmovlibPeer::FECLIB,$fechades,Criteria::GREATER_EQUAL);
    $opc66=$c->getNewCriterion(TsmovlibPeer::FECLIB,$fechahas,Criteria::LESS_EQUAL);
    $opc6->addAnd($opc66);
    $c->add($opc6);
   ...
   $this->columnas = Herramientas::getConfigGrid(sfConfig::get('sf_app_module_dir').'/tesdesconmovlib/'.sfConfig::get('sf_app_module_config_dir_name').'/grid_movimiento');
   $this->obj = $this->columnas[0]->getConfig($per);
   $this->tsdefban->setGripdesmovlib($this->obj);
  }
  ...
  public function updateError()
  {
    ...
    $grid = Herramientas::CargarDatosGrid($this,$this->gripdesmovlib);
  ...
  {
  	$grid = Herramientas::CargarDatosGridv2($this,$clasemodelo->getGripdesmovlib());
  	return Tesoreria::saveTesdesconmovlib($clasemodelo,$grid);
  }
