...
/**
 * tesdefcomban actions.
 *
 * @package    siga
 * @subpackage tesdefcomban
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class tesdefcombanActions extends autotesdefcombanActions
{
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
        $j= new Criteria();
        $j->add(ContabbPeer::CODCTA,$codigo);
        $result= ContabbPeer::doSelectOne($j);
        if ($result)
        {
            if ($result->getCargab()=='C')
              $dato=$result->getDescta();
            else
              $js="alert_('La Cuenta Contable no es Cargable'); $('tscomban_codcta').value=''; $('tscomban_descta').value=''; $('tscomban_codcta').focus();";
        }else {
            $js="alert_('La Cuenta Contable no existe'); $('tscomban_codcta').value=''; $('tscomban_descta').value=''; $('tscomban_codcta').focus();";
        }
