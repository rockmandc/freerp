    ...
    case '4':
      ...
      case '5':
       ...
       case '6':
          ...
          if ($codigo!='') {
            $c= new Criteria();
            $c->add(TsdeffonantPeer::CODFON,$codigo);
            $resul= TsdeffonantPeer::doSelectOne($c);
            if ($resul)
            {
              $dato=H::toFloat($resul->getPorrep());
              $dato1=H::toFloat($resul->getMonmax());
            }else {
              $dato=""; $javascript="alert_('El Fondo de Anticipo no existe'); $('opordpag_codfonant').value='';";
            }
          }
          $output = '[["javascript","'.$javascript.'",""],["opordpag_porcenrep","'.$dato.'",""],["opordpag_monmaxrep","'.$dato1.'",""]]';
          $this->getResponse()->setHttpHeader("X-JSON", '('.$output.')');
          return sfView::HEADER_ONLY;
          break;                       
      default:
