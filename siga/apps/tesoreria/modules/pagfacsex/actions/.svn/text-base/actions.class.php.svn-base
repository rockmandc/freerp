 ...
 *
 * @package    siga
 * @subpackage pagfacsex
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class pagfacsexActions extends autopagfacsexActions
{
  ...
  public function validateEdit()
  {
    ...
    if($this->getRequest()->getMethod() == sfRequest::POST){
      ...
      $this->opregfac = $this->getOpregfacOrCreate();
      $this->updateOpregfacFromRequest();

      if ($this->getRequestParameter('id')=='') {
        $q= new Criteria();
        $q->add(OpregfacPeer::CEDRIF,$this->getRequestParameter('opregfac[cedrif]'));
        $q->add(OpregfacPeer::NUMFAC,$this->getRequestParameter('opregfac[numfac]'));
        $regq= OpregfacPeer::doSelectOne($q);
        if ($regq){
           $this->coderr=5011;
        }      
      }
  ...
  {
    if ($clasemodelo->getRecord()=='S')
      $clasemodelo->setFecrecord('Y-m-d');
