    ...
    switch ($ajax){
      ...
      case '2':
          ...
          if (checkdate(intval($fec1[1]), intval($fec1[2]), intval($fec1[0])))
          {
           ...
           {
          	 $javascript="alert('Fecha Fuera del Período'); $('tsfonant_fecfon').value='';";
           }
           else
           {
              ...
              $javascript="alert_('El Mes ya se encuentra cerrado, por lo que no se podr&acute registrar Movimiento en esta Fecha'); $('tsfonant_fecfon').value='';";
           	...
           	{
           	  $javascript="alert('El Número de Cuenta Bancaria para Fondo en Anticipo no se encuentra Definido'); $('tsfonant_fecfon').value='';";
           	}
           }
          }
          else
      ...
      case '4':
       ...
       else
       {
         ...
         if(!$reg)
         {
         	$javascript="alert_('La C&oacute;digo Presupuestario formado por la Categoria y la Partida del articulo no existe --> $codpre'); $('$cajtexcom').value=''; $('$cajtexcom').focus();";
         }else {
           $r1= new Criteria();
           ...
           $reg1= CpasiiniPeer::doSelectOne($r1);
            if(!$reg1)
             {
               $javascript="alert_('La C&oacute;digo Presupuestario formado por la Categoria y la Partida del articulo no tiene asignaciÃ³n Inicial --> $codpre'); $('$cajtexcom').value=''; $('$cajtexcom').focus();";
             }
         }
       }
        $output = '[["javascript","'.$javascript.'",""],["","",""],["","",""]]';
