...
/**
 * tesmigmovban actions.
 *
 * @package    siga
 * @subpackage tesmigmovban
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class tesmigmovbanActions extends autotesmigmovbanActions
{

  public function executeList()
  {
    $this->redirect('tesmigmovban/create');
  }
  ...
  {
    if (!$this->getRequest()->hasErrors() && $this->getRequest()->getFileSize('tsmovban[archixls]'))
    {
        $fileName = "archivo_excel.xls";     
        $this->getRequest()->moveFile('tsmovban[archixls]', sfConfig::get('sf_upload_dir')."/assets/".$fileName);                       
    }
  ...
  public function configGrid($valor='')
  {
    ...
    $cad='';
    if ($valor!='')
      $per=Tesoreria::CargarMovban($cad);
    ...
    $opciones->setFilas(0);
    $opciones->setTabla('Tsmovban');
    $opciones->setName('a');
    $opciones->setAnchoGrid(900);
    $opciones->setAncho(800);
    $opciones->setTitulo('Movimientos Bancarios');
    ...
    $this->tsmovban->setGrid($this->obj);
  }
    ...
    switch ($ajax){
      case '1':
        ...
        try  {
          ...
          $sql = "select id,numcue,refban,fecban,tipmov,desban,monmov from tmpmovbanexc";
          if (Herramientas::BuscarDatos($sql,$result))
          {
              $tabla=true;
              if (count($result)>0) {
               $sql = "delete from tmpmovbanexc";
               Herramientas::insertarRegistros($sql);
              }
          }else
                $tabla=false;
        }catch(Exception $ex){
          $sql = "CREATE TABLE tmpmovbanexc (id int4 NOT NULL,
      ...
      if (is_file($currentFile))
      {
        ...
        foreach($data->sheets[0]['cells'] as $dat){          
          if(empty($col1) || empty($col2) || empty($col3)){             
            ...
            $fecban = $dateFormat->format($dat[3], 'i', $dateFormat->getInputPattern('d'));
            $refban=$dat[2];
            $tipmov=$dat[4];
            $desban=$dat[5];
            $monmov=H::toFloat($dat[6]);
          
            $ctanum=H::getX_vacio('NUMCUE','Tsdefban','Codcta',$numcue);
            if ($ctanum!='') {
              $exitip=H::getX_vacio('CODTIP','Tstipmov','Codtip',$tipmov);
              if ($exitip!=''){             
                $sql="INSERT INTO tmpmovbanexc(id, numcue, refban, fecban, tipmov, desban, monmov)
                      VALUES ($r, '".$numcue."', '".$refban."', '".$fecban."', '$tipmov', '".$desban."', $monmov);";
                H::insertarRegistros($sql);          
              }else {
                $pos=Tesoreria::posicion_en_el_grid_ban($perval,$numcue,$tipmov);
                if ($pos==0){
                  $b=count($perval)+1;
                  $perval[$b-1]["numcue"]=$numcue;
                  $perval[$b-1]["refban"]=$refban;   
                  $perval[$b-1]["tipmov"]=$tipmov;   
                } 
              }
            }else {
              $pos=Tesoreria::posicion_en_el_grid_ban($perval,$numcue,$refban,$tipmov);
              if ($pos==0){
                $b=count($perval)+1;
                $perval[$b-1]["numcue"]=$numcue;
                $perval[$b-1]["refban"]=$refban;   
        ...
        if (count($perval)>0) {
          ...
          while ($a<count($perval)){
            $cadena=$cadena.' '.$perval[$a]["numcue"].'-'.$perval[$a]["refban"].'-'.$perval[$a]["tipmov"];
            $a++;
          }
          $js="alert('Los siguientes Movimientos Bancarios no pueden ser migrados: $cadena .Revisar que las Cuentas Bancarias y Tipos de Movimientos Existan');";
        }
      }else {
  ...
  public function saving($clasemodelo)
  {
    $this->configGrid();
    $grid = Herramientas::CargarDatosGridv2($this,$this->obj,true);
    Tesoreria::MigrarMovBanExcel($grid);

    try  {
    //Eliminamos la Tabla Temporal
    $sql="select numcue from tmpmovbanexc";
    if (!(Herramientas::BuscarDatos($sql,$tabla)))
    {
      $sql = "drop table tmpmovbanexc";
      Herramientas::insertarRegistros($sql);
    }
  }catch(Exception $ex){}
    return -1;
