...
$this->Bitacora('Guardo');
  ...
  protected function saveContabc($contabc)
  {
    ...
    {
      if($this->contabc->getNumcom()=='########'){
      $this->contabc->setNumcom(Comprobante::Buscar_Correlativo($this->contabc->getFeccom()));
    }
    $tiptra=null;
    ...
    if ($modulo=='bienes') {
        ...
        $xf=$this->getRequestParameter('formulario');
        if ($this->getRequestParameter('formulario')=='sf_admin/bieciemesdan/confincomgen0'){
          $r= new Criteria();
          $r->add(BndepactPeer::FECDEP,$this->contabc->getFeccom());
          $trajo2=BndepactPeer::doSelectOne($r);
          if ($trajo2){
            $trajo2->setNumcom($this->contabc->getNumcom());
            $trajo2->save();
          }
        }
    }elseif ($modulo=='tesoreria')
        $tipcom='TES';
    elseif ($modulo=='almacen')
        $tipcom='ALM';
    elseif ($modulo=='contabilidad')
        $tipcom='CON';
    else
        $tipcom=null;

    if ($this->getRequestParameter('formulario')=='sf_admin/bieciemesdan/confincomgen0' && $modulo!='BIENES'){
        $tipcom='BIE';
        $tiptra=H::getX_vacio('CODINS','Bndefins','Codtiptra','001');
        $r= new Criteria();
        $r->add(BndepactPeer::FECDEP,$this->contabc->getFeccom());
      ...
      Tesoreria::Salvarconfincomgen($this->contabc->getNumcom(), $this->contabc->getReftra(),$this->contabc->getFeccom(),$this->contabc->getDescom(),$this->getRequestParameter('debito'),$this->getRequestParameter('credito'),$tipcom,$tiptra);
      Tesoreria::Salvar_asientosconfincomgen($this->contabc->getNumcom(), $this->contabc->getReftra(),$this->contabc->getFeccom(),$grid,$this->getUser()->getAttribute('grabar',null,$this->getUser()->getAttribute('formulario')));
  ...
  public function validateEdit()
  {
    if($this->getRequest()->getMethod() == sfRequest::POST)
    {
      ...
      if (Tesoreria::validaPeriodoCerrado($this->getRequestParameter('contabc[feccom]'))==true && ($this->getRequestParameter('formulario')!='sf_admin/confincie/confincomgen0' && $this->getRequestParameter('formulario')!='sf_admin/confincie/confincomgen1' && $this->getRequestParameter('formulario')!='sf_admin/confincie/confincomgen2'))
      {
        $this->coderror3=529;
        return false;
      }

      if (H::toFloat($this->getRequestParameter('debito'))!=H::toFloat($this->getRequestParameter('credito')))
      {
      	$this->error=519;
      	return false;
      }
      ...
      else
      {
        ...
        $this->coderror1=Tesoreria::validarCuentasGrid($grid,$this->getRequestParameter('contabc[feccom]'));
      	if ($this->coderror1!=-1)
        {
      	  return false;
      	}
