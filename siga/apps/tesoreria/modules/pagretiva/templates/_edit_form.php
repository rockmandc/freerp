...
<script language="JavaScript" type="text/javascript">
 function ajax2(e,id)
 {
   if ($(id).value!="")
   {
    ...
    if ($(id).value!='')
    {
    new Ajax.Request('/tesoreria'+getEnv()+'.php/pagretiva/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json), validargrid(id)}, parameters:'ajax=2&cajtexmos='+descripcion+'&cajtexcom='+id+'&tipo='+tipo+'&monto='+monto+'&partida='+partida+'&codigo='+cod})
    }
    }
 }
 ...
 function ajax(e,id)
 {
   if ($(id).value!="")
   {
     ...
     if (e.keyCode==13 || e.keyCode==9)
    {
     if ($(id).value!='')
    {
     new Ajax.Request('/tesoreria'+getEnv()+'.php/pagretiva/ajax', {asynchronous:true, evalScripts:false, onComplete:function(request, json){AjaxJSON(request, json), validar(id);}, parameters:'ajax=1&cajtexmos='+descripcion+'&cajtexcom='+id+'&codigo='+cod})
    }
    }
 }
