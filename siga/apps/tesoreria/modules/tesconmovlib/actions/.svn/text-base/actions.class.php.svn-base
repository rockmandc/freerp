 ...
 */
class tesconmovlibActions extends autotesconmovlibActions
{
  ...
  public function configGrid($numcue,$fechades,$fechahas,$reg = array(),$regelim = array())  //Plan de Trabajo
  {
   ...
   $sql = "(tsmovlib.stacon1<>'C' or tsmovlib.stacon1 isnull) and reflibpad is null and tsmovlib.reflib not in (select reflibpad from tsmovlib where numcue='".$numcue."' and reflibpad<>'')";
   $c->add(TsmovlibPeer::STACON1, $sql, Criteria :: CUSTOM);
   $c->addAscendingOrderByColumn(TsmovlibPeer :: FECLIB);
   $per = TsmovlibPeer::doSelect($c);

   $this->columnas = Herramientas::getConfigGrid(sfConfig::get('sf_app_module_dir').'/tesconmovlib/'.sfConfig::get('sf_app_module_config_dir_name').'/grid_movimiento');
   $this->obj = $this->columnas[0]->getConfig($per);
   $this->tsdefban->setGripmovlib($this->obj);
  }
  ...
  public function updateError()
  {
    ...
    $grid = Herramientas::CargarDatosGrid($this,$this->gripmovlib);
  ...
  {
  	$grid = Herramientas::CargarDatosGridv2($this,$clasemodelo->getGripmovlib());
  	return Tesoreria::saveTesconmovlib($clasemodelo,$grid);
  }
