 ...
 */
class tesconpagActions extends autotesconpagActions
{
  ...
  {
     if (!$this->opconpag->getId())
    {
       $t= new Criteria();
       $t->setLimit(1);
       $t->addDescendingOrderByColumn(OpconpagPeer::CODCONCEPTO);
       $reg= OpconpagPeer::doSelectOne($t);
       if ($reg)
       {
           $this->opconpag->setCodconcepto(str_pad(($reg->getCodconcepto()+1),4,'0',STR_PAD_LEFT));
       }else $this->opconpag->setCodconcepto('0001');
    }
