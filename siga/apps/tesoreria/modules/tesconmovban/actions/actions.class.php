 ...
 */
class tesconmovbanActions extends autotesconmovbanActions
{
  ...
  public function configGrid($numcue,$fechades,$fechahas,$reg = array(),$regelim = array())  //Plan de Trabajo
  {
   // $this->regelim = $regelim;
	...
	$fechades = $fechades=='' ? date('Y-m-d') : $fechades;
	//$fechades = $this->transformarFecha($fechades);

	$fechahas = $fechahas=='' ? date('Y-m-d') : $fechahas;
 ...
 *    $c->add(TsmovbanPeer::NUMCUE,$this->tsdefban->getNumcue());
   $c->add(TsmovbanPeer::FECBAN,$this->tsdefban->getFechades(),Criteria::GREATER_EQUAL);
   $c->add(TsmovbanPeer::FECBAN,$this->tsdefban->getFechahas(),Criteria::LESS_EQUAL);
 ...
 * */
   ...
   $this->columnas = Herramientas::getConfigGrid(sfConfig::get('sf_app_module_dir').'/tesconmovban/'.sfConfig::get('sf_app_module_config_dir_name').'/grid_movimiento');
   $this->obj = $this->columnas[0]->getConfig($per);
   $this->tsdefban->setGripmovban($this->obj);
  }
  ...
  public function executeAjax()
  {
    ...
    switch ($ajax){
      case '1':
        ...
        $fec1 = $dateFormat->format($fechades, 'i', $dateFormat->getInputPattern('d'));
                    
        $dateFormat = new sfDateFormat('es_VE');
        $fec2 = $dateFormat->format($fechahas, 'i', $dateFormat->getInputPattern('d'));                    
	    $this->tsdefban = $this->getTsdefbanOrCreate();
	    $this->updateTsdefbanFromRequest();

        $output = '[["","",""],["","",""],["","",""]]';

       $this->configGrid($numcue,$fec1,$fec2);
...
//exit('55');
  ...
  public function updateError()
  {
    ...
    $grid = Herramientas::CargarDatosGrid($this,$this->gripmovban);
  ...
  {
  	$grid = Herramientas::CargarDatosGridv2($this,$clasemodelo->getGripmovban());
  	return Tesoreria::saveTesconmovban($clasemodelo,$grid);
  }
