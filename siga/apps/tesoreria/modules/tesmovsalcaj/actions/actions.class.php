...
class tesmovsalcajActions extends autotesmovsalcajActions {
  ...
  public function executeList()
  {
    ...
    $this->filters = $this->getUser()->getAttributeHolder()->getAll('sf_admin/tssalcaj/filters');

    $loguse= sfContext::getInstance()->getUser()->getAttribute('loguse');
    $filsoldir=H::getConfApp2('filsoldir', 'compras', 'almsolegr');

     // 15    // pager
    $this->pager = new sfPropelPager('Tssalcaj', 15);
    $c = new Criteria();
    if ($filsoldir=='S'){
      $this->sql='tssalcaj.coddirec in (select coddirec from "SIMA_USER".segdirusu where loguse=\''.$loguse.'\')';
      $c->add(TssalcajPeer::CODDIREC,$this->sql,Criteria::CUSTOM); 
    }
    ...
    public function executeAjax() {
        ...
        switch ($ajax) {
            ...
            case '3':
                ...
                if ($resul) {
                    $desart=htmlspecialchars($resul->getDesart());
                    $dato1 = eregi_replace("[\n|\r|\n\r]", "", $desart);      
                    if (H::toFloat($valmonto)>0)              
                      $dato2=$valmonto;
                    else
            ...
            case '5':
                if ($codigo != "") {
                    $a = new Criteria();
                    $a->add(TsdefcajchiPeer::CODCAJ, $codigo);
                    $resul = TsdefcajchiPeer::doSelectOne($a);
                    if ($resul) {
                        $dato1 = $resul->getNumcue();
                        $dato2 = $resul->getCodtip();
                        $dato = $resul->getDescaj();
                    } else {
                        $dato1 = "";
                        $dato2 = "";
                        $dato = "";
                        $javascript = "alert('La caja Chica no existe'); $('tssalcaj_codcaj').value=''; $('tssalcaj_descaj').value=''; $('tssalcaj_codcaj').focus();";
                    }
                }
                $output = '[["tssalcaj_descaj","' . $dato . '",""],["tssalcaj_numcue","' . $dato1 . '",""],["tssalcaj_tipdoc","' . $dato2 . '",""],["javascript","' . $javascript . '",""]]';
                break;
            case '6':
                $loguse= sfContext::getInstance()->getUser()->getAttribute('loguse');
                $filsoldir=H::getConfApp2('filsoldir', 'compras', 'almsolegr');
                ...
                if ($reg)
                {
                   $dato=$reg->getDesdirec();         
                    ...
                    $dato="";
                    $js="alert_('La Direcci&oacute;n no existe o no esta asociada al usuario'); $('tssalcaj_coddirec').value='';  $('tssalcaj_desdirec').value=''; $('tssalcaj_coddirec').focus();";
                }
            ...
            case '7':
                ...
                if ($result)
                  ...
                  $js="alert('El Evento no existe'); $('tssalcaj_codeve').value=''; $('tssalcaj_deseve').value=''; $('tssalcaj_codeve').focus();";
    ...
    public function validateEdit() {
        ...
        if ($this->getRequest()->getMethod() == sfRequest::POST) {
            ...
            try {
                $this->updateTssalcajFromRequest();
            } catch (Exception $ex) {
    ...
    protected function saving($tssalcaj) {
        ...
        if ($tssalcaj->getNumcue() == '') {
            $a = new Criteria();
            $a->add(TsdefcajchiPeer::CODCAJ, $tssalcaj->getCodcaj());
            $resul = TsdefcajchiPeer::doSelectOne($a);
            if ($resul) {
                $tssalcaj->setNumcue($resul->getNumcue());
                $tssalcaj->setTipdoc($resul->getCodtip());
            } 
        }
  ...
  public function getLabels()
  {
    ...
    if ($cambiareti=='S')
      $labels['tssalcaj{coddirec}'] = 'Estado';
    return $labels;
  }    
