...
/**
 * tespropagexc actions.
 *
 * @package    siga
 * @subpackage tespropagexc
 * @author     $Auhor$ <desarrollo@cidesa.com.ve>
 * @version    SVN: $Id: actions.class.php 32371 2009-09-01 16:06:59Z lhernandez $
 * @copyright  Copyright 2007, Cidesa C.A.
 */

class tespropagexcActions extends autotespropagexcActions
{
 public function executeIndex()
  {
    return $this->forward('tespropagexc', 'edit');
  }
  ...
  public function configGrid($n1="", $n2="")
  {
    ...
    $this->columnas = Herramientas::getConfigGrid(sfConfig::get('sf_app_module_dir').'/tespropagexc/'.sfConfig::get('sf_app_module_config_dir_name').'/grid_ordenes');
      ...
      default:
        ...
        while ($j<count($x)){
          if ($x[$j]->getAprproord()=='1'){
            if ($x[$j]->getObspropag()!=''){
              $q=new Criteria();
              $q->add(OpordpagPeer::NUMORD,$x[$j]->getNumord());
              $regq= OpordpagPeer::doSelectOne($q);
              if ($regq){
                $regq->setObspropag($x[$j]->getObspropag());
                $regq->save();
              }
            }
          }
