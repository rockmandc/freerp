...
<?
  ...
  if (!empty($txtcodigo)):
      $sql="select to_char(fecdes,'dd/mm/yyyy') as fecdes , to_char(fechas,'dd/mm/yyyy') as fechas, cerrado from cppereje where pereje='".$txtcodigo."'";
      if ($tb=$tools->buscar_datos($sql))
      {
        $fecdes=$tb->fields["fecdes"];
        $fechas=$tb->fields["fechas"];
        $cerrado=$tb->fields["cerrado"];
      }else{
...
<table width="100%" align="center" class="bodyline">
          ...
          <form action="" method="post" name="form1" onsubmit="return false;">
            ...
            ?>
                          <option value="PreCiePer.php?txtcodigo=<? echo $tb->fields["pereje"]; ?>" selected="selected"><? echo $tb->fields["pereje"]; ?></option>
                <? }else{?>
                <option value="PreCiePer.php?txtcodigo=<? echo $tb->fields["pereje"]; ?>"><? echo $tb->fields["pereje"]; ?></option>
            <?
                       }
                   $tb->MoveNext();
                 }
            ...
            <input name="periodo" type="hidden" id="periodo" value="<? echo $txtcodigo; ?>" />
                ...
                <tr>
                  <td height="189" colspan="3" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                      <td><strong>Al realizar el Cierre del Per&iacute;odo, usted no podr&aacute; volver a incluir ni actualizar ning&uacute;n Registro que Afecten al Presupuesto.
...
<script language="JavaScript">
  ...
  function Limpiar()
    ...
    aToolBar.showBar();
     ...
     function Abrir()
     {
         var cerrado='<? echo $cerrado;?>';
       if (cerrado=='C'){
           document.form1.action="imecPreCiePer.php?var=N";
     ...
     function Cerrar()
     {
       ...
       if (cerrado=='N'){
         ...
         {
           document.form1.action="imecPreCiePer.php?var=<? echo 'C'; ?>";
           document.form1.submit();
         }
      }else{
