...
?>
      <script>
        alert("El Tipo de PreCompromiso No Existe.");
        var id= '<?=$id;?>';
        var donde= '<? print $donde;?>';
        opener.document.getElementById(id).value="";
        opener.document.getElementById(donde).value="";
...
?>
      <script>
        ...
        var campo4= '<? print $campo4;?>';

        var valor1= '<? print $valor1;?>';
        var valor2= '<? print $valor2;?>';
        var valor3= '<? print $valor3;?>';
        var valor4= '<? print $valor4;?>';
        ...
        opener.document.getElementById(campo4).value=valor4;
        opener.document.getElementById(foco).focus();
...
function gridatosDosSql() {
	...
	if ($tb = $z->buscar_datos($sql)) {
		//$valor = $tb->fields["campo1"];
		///////////////
            $sql3="select mondis as campo1 from cpasiini where trim(codpre) = '".$sql2."' and perpre='00'";
...
?>
      ...
      //Monto Disponible
        ...
        <?
                    ...
                    $mensaje="El Codigo Presupuestario tiene Disponibilidad negativa";
			Mensaje($mensaje);
?>
          <script>
            var id= '<?=$id;?>';
...
function Hay_Asignacion() {
	...
	//$sql = "select Sum(MonDis) as  MonDis From CPAsiIni where CodPre Like '$codigobuscar' and AnoPre='$anocierre' and PerPre='$PeriodoAsignacion'";
        $sql="select coalesce(sum(monasi +
                  coalesce(obtener_ejecucion(rtrim(codpre),'01','12','".$anocierre."','TRA'),0) +
                  coalesce(obtener_ejecucion(rtrim(codpre),'01','12','".$anocierre."','ADI'),0) -
                  coalesce(obtener_ejecucion(rtrim(codpre),'01','12','".$anocierre."','TRN'),0) -
                  coalesce(obtener_ejecucion(rtrim(codpre),'01','12','".$anocierre."','DIS'),0) -
                  coalesce(obtener_ejecucion(rtrim(codpre),'01','12','".$anocierre."','PRC'),0)),0) as mondis
                  from cpasiini where CodPre = '".$codigo."' and anopre='".$anocierre."' and perpre='00'";
	//exit();
	if ($tb = $z->buscar_datos($sql)) {
...
function MostrarDetalleMovimiento() {
    ...
    while (!$tbDetEven->EOF) {
       if ($Mostrar == "S") {
    	$codpre = $tbDetEven->fields["codpre"];
    	$mon= $tbDetEven->fields["monimp"];
    	$monto=  number_format($mon, 2, '.', ',');
		$codeve = $tbDetEven->fields["codeve"];
		$deseve = $tbDetEven->fields["deseve"];
		$moneve = $tbDetEven->fields["moneve"];		
		$montoeve = number_format($moneve, 2, '.', ',');
        ?>
        <script>
          var codpre= '<? print $codpre;?>';
          var monto= '<? print $monto;?>';
          var codeve= '<? print $codeve;?>';
          var deseve= '<? print $deseve;?>';
          var moneve= '<? print $moneve;?>';
          var l= '<? print $pos2;?>';
          var f1="f"+l+"1";
          var f2="f"+l+"2";
          var f3="f"+l+"3";
          var f4="f"+l+"4";
          var f5="f"+l+"5";
          opener.document.getElementById(f1).value=codpre;
          opener.document.getElementById(f2).value=monto;
          opener.document.getElementById(f3).value=codeve;
          opener.document.getElementById(f4).value=deseve;
          opener.document.getElementById(f5).value=moneve;
        </script>
        <?
        $pos2 = $pos2 +1;
      }
	...
	global $referencia;
        ...
        if ($validaporcentaje=='S' && $letra=='f') {
            ...
            if ($tba = $z->buscar_datos($sqla)) {
                ...
                $sqlb="select coalesce(sum(monasi +
                  coalesce(obtener_ejecucion(rtrim(codpre),'01','12','".$anocierre."','TRA'),0) +
                  coalesce(obtener_ejecucion(rtrim(codpre),'01','12','".$anocierre."','ADI'),0) -
                  coalesce(obtener_ejecucion(rtrim(codpre),'01','12','".$anocierre."','TRN'),0) -
                  coalesce(obtener_ejecucion(rtrim(codpre),'01','12','".$anocierre."','DIS'),0) -
...
?>
      <script>
        alert("El Tipo de Transacción No Existe.");
        var id= '<?=$id;?>';
        var donde= '<? print $donde;?>';
        opener.document.getElementById(id).value="";
        opener.document.getElementById(donde).value="";
