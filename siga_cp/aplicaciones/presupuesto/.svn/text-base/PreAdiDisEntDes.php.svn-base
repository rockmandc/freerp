...
$modulo="";
$forma="Créditos Adiciones/Disminuciones para Entes Descentralizados";
...
Limpiar();
 ...
 while ($i<=500)
 {
     ...
     $_POST["x".$i."5"]= number_format(0,2,'.',',');
     $_POST["x".$i."6"]= number_format(0,2,'.',',');
     $i=$i +1;
 }
 ...
 while ($j<=500)
 {
     ...
     $_POST["f".$j."5"]= number_format(0,2,'.',',');
     $_POST["f".$j."6"]= number_format(0,2,'.',',');
     $j=$j +1;
 }
          ...
          if (!$tb->EOF)
           {
            ...
            if (strtoupper(trim($status))=="N")
            {
             ...
             $block  = "S";
             $pos    = 1;
             $sqldet ="Select * From CPMovAdi Where trim(refadi)='".trim($codigo)."' and tipo='O'";
             ...
             while (!$tb2->EOF)
             {
                ...
                $_POST["x".$pos."5"] = $tb2->fields["monto"];
                $_POST["x".$pos."6"] = $tb2->fields["iva"];
                $tb2->MoveNext();
                $pos=$pos+1;
             }

             $pos2   = 1;
             $sqldet2 ="Select * From CPMovAdi Where trim(refadi)='".trim($codigo)."' and tipo='D' ";
             $tb22    = $bd->select($sqldet2);
             while (!$tb22->EOF)
             {
                $_POST["f".$pos2."1"] = $tb22->fields["codpre"];
                $_POST["f".$pos2."3"] = $tb22->fields["monmov"];
                $_POST["f".$pos2."4"] = Monto_disponible_ejecucion($anoinicio,$tb22->fields["codpre"],'00');
                $_POST["f".$pos2."5"] = $tb22->fields["monto"];
                $_POST["f".$pos2."6"] = $tb22->fields["iva"];
                $tb22->MoveNext();
                $pos2=$pos2+1;
             }
              ...
              $sql="select b.stacon, b.stagob, b.stapre, a.*,to_char(a.fecadi,'dd/mm/yyyy') as  fecadi, to_char(a.fecanu,'dd/mm/yyyy') as fecanu, to_char(a.feccon,'dd/mm/yyyy') as  feccon,
                ...
                //$status2 = Mostrar_status();
                  ...
                  if (!$tb->EOF)
                  {
                    ...
                    $sqldet = "select * From cpsolmovadi Where trim(refadi)='".trim($codigo)."' and tipo='O' ";
                    $tb2    = $bd->select($sqldet);
                    while (!$tb2->EOF)
                    {
                        $_POST["x".$pos."1"] = $tb2->fields["codpre"];
                    ...
                    $sqldet2 = "select * From cpsolmovadi Where trim(refadi)='".trim($codigo)."' and tipo='D' ";
                    $tb22    = $bd->select($sqldet2);
                    while (!$tb22->EOF)
                    {
                        $_POST["f".$pos2."1"] = $tb22->fields["codpre"];
                  ...
                  while ($i<=500)
                  {
                     ?>
                        <tr>
                          ...
                          <td  align="left" class="grid_line01_br"><input name="x<? print $i;?>1" id="x<? print $i;?>1" type="text" class="grid_txt01" size="32" maxlength="<?print strlen($_SESSION["formato"]);?>" onKeyDown="javascript:return dFilter (event.keyCode, this,'<?print $_SESSION["formato"];?>');" align="right"  value="<? print  $_POST["x".$i."1"];?>" onKeyPress="enterGRID(event,this.id,'trash','x<? print $i;?>2');" ></td>
                          <td  align="right" class="grid_line01_br"><input name="x<? print $i;?>5" id="x<? print $i;?>5" type="text" class="grid_txt02" size="16" value="<? if (empty($_POST["x".$i."5"])) {print number_format(0,2,'.',',');}  else {print number_format($_POST["x".$i."5"],2,'.',',');}  ?>" align="right"  onKeyPress="entermonto2(event,'x<? print $i;?>1',this.id,'x<? print $i;?>4','Mon')"></td>
                          <td  align="right" class="grid_line01_br"><input name="x<? print $i;?>6" id="x<? print $i;?>6" type="text" class="grid_txt02" size="16" value="<? if (empty($_POST["x".$i."6"])) {print number_format(0,2,'.',',');}  else {print number_format($_POST["x".$i."6"],2,'.',',');}  ?>" align="right"  onKeyPress="entermonto2(event,'x<? print $i;?>1',this.id,'x<? print $i;?>4','Mon')"></td>
                  ...
                  while ($j<=500)
                  {
                     ?>
                        <tr>
                          ...
                          <td  align="left" class="grid_line01_br"><input name="f<? print $j;?>1" id="f<? print $j;?>1" type="text" class="grid_txt01" size="32" maxlength="<?print strlen($_SESSION["formato"]);?>" onKeyDown="javascript:return dFilter (event.keyCode, this,'<?print $_SESSION["formato"];?>');" align="right"  value="<? print  $_POST["f".$j."1"];?>" onKeyPress="enterGRIDB(event,this.id,'trash','f<? print $j;?>2');" ></td>
                          <td  align="right" class="grid_line01_br"><input name="f<? print $j;?>5" id="f<? print $j;?>5" type="text" class="grid_txt02" size="16" value="<? if (empty($_POST["f".$j."5"])) {print number_format(0,2,'.',',');}  else {print number_format($_POST["f".$j."5"],2,'.',',');}  ?>" align="right"  onKeyPress="entermonto2b(event,'f<? print $j;?>1',this.id,'f<? print $j;?>4','Mon')"></td>
                          <td  align="right" class="grid_line01_br"><input name="f<? print $j;?>6" id="f<? print $j;?>6" type="text" class="grid_txt02" size="16" value="<? if (empty($_POST["f".$j."6"])) {print number_format(0,2,'.',',');}  else {print number_format($_POST["f".$j."6"],2,'.',',');}  ?>" align="right"  onKeyPress="entermonto2b(event,'f<? print $j;?>1',this.id,'f<? print $j;?>4','Mon')"></td>
                          <td  align="right" class="grid_line01_br"><input name="f<? print $j;?>3" id="f<? print $j;?>3" type="text" class="grid_txt02" size="16" value="<? if (empty($_POST["f".$j."3"])) {print number_format(0,2,'.',',');}  else {print number_format($_POST["f".$j."3"],2,'.',',');}  ?>" align="right"  onKeyPress="entermontob(event,'f<? print $j;?>1',this.id,'f<? print $j;?>4','Mon')"></td>
    ...
    function onButtonClick(itemId,itemValue)// TOOLBAR
    {
      ...
      else if(itemId == '0_save')
      {
        ...
        document.form1.tipo[1].disabled=false
                f.action="imecPreAdiDisEntDes.php?imec=<? print $imec?>";
                f.submit();
                }
        }
       }
                ...
                if(confirm("¿Esta seguro que desea Eliminar?"))
                  {
                    ...
                    fecpre=f.fecha.value;
                      window.open("anuPreAdiDisEntDes.php?codigo="+codigo+"&fecpre="+fecpre+"&operacion="+ope,"AnularTraslado","menubar=no,toolbar=no,scrollbars=no,width=600,height=150,resizable=no,left=200,top=250");
                  }
              }//if (eliminar=='N'){ alert(msg)}
           } //compareDate
          }//if (status=="N")
