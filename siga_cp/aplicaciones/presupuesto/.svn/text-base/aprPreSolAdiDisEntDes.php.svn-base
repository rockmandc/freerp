...
validar(array(11,15),'','presoladidisentdes.php');            //Seguridad  del Sistema
  ...
  if ($operacion=='S')   //Salvar de la pantalla de Anular
  {
    ...
    if ($tool->Validar_Periodo($fecha,"CPDefNiv"))
    {
        ...
        ?>
          <script>
             close();
             window.opener.document.location=("PreSolAdiDisEntDes.php");
          </script>
        <?
        }
    }//if ($tool->Validar_Periodo($fecha,"CPDefNiv"))
...
<html>
<head>
<title>Aprobación de Solicitud de Traslados para Entes Descentralizados</title>
...
<form name="form1" onsubmit="return false;" method="post" action="">
  <fieldset>
    ...
    <legend><span class="style3">Conformaci&oacute;n-Autorizaci&oacute;n-Aprobaci&oacute;n de Solicitud de Cr&eacute;dito Adici&oacute;n/Disminuci&oacute;n para Entes Descentralizados</span></legend>
...
function salvar()
{
  ...
  }else
  {
    codigo   = '<? print $codigo;?>';
    sw       = '<? print $sw;?>';
    f.action = "aprPreSolAdiDisEntDes.php?operacion=S&codigo="+codigo+"&sw="+sw;
    f.submit();
  }
