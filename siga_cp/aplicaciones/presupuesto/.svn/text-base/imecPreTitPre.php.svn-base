...
$novalcta=$_SESSION["configemp"]["aplicacion"]["presupuesto"]["modulos"]["PreTitPre"]["novalcta"];
...
function crearLog($valor)
{
	        ...
	        if ($tb=$z->buscar_datos($sql))
	        {
	          ...
	          if (empty($MascaraPresupuesto))
	          {
	            ...
	            //print $MascaraPresupuesto.'--'.$codcta;exit();
	                    if(strlen($MascaraPresupuesto)==strlen($codcta) && $novalcta==""){
  ...
  function Grabar_CodigoPre()
  {
	...
	$nombre =  ($configemp=='S') ? strtoupper($nompre) : $nompre;
     ...
     if ($IncMod == "M"){
       $sql="update cpdeftit set nompre='$nombre', codcta='$codcta', estatus='$tiptit', stacod='A' where codpre='$codigopre'";
       $bd->actualizar($sql);
       crearLog('A');

     }else{    //Incluir
       $sql = "insert into cpdeftit (codpre,nompre,codcta,stacod,coduni,estatus) values ('$codigopre','$nombre','$codcta','A','','$tiptit')";
       $bd->actualizar($sql);
       crearLog('G');
     }
