        ...
        if ($_POST["f".$cont2."1"]!='' && $_POST["f".$cont2."3"]!='')
        {
          if (substr(trim($_POST["f".$cont2."1"]),0,2)!="--")
          {
            if ((float)str_replace(',','',$_POST["f".$cont2."5"])!=0)
            {
              ...
              $col31[$cont2] = (float)str_replace(',','',$_POST["f".$cont2."5"]);
            }else
            {
  ...
  try
    {
      ...
      //Grabar Grid de Eventos
        $sqle="delete from cpdiseve where refdoc = '".$refcom."' and tipdoc = '".$tipcom."'";
        $bd->actualizar($sqle);
        $y=1;
        while ($y<=count($col11))
        {
            $sqlie="insert into cpdiseve (refdoc,codpre,codeve,moneve,tipdoc,tipmov)
            values ('".$refcom."','".$col11[$y]."','".$col21[$y]."',".$col31[$y].",'".$tipcom."','COM')";
