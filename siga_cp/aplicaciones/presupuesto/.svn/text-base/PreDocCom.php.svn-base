...
$modulo=$_SESSION["modulo"] . " > Def. Especificas > Documentos > ".$forma;
  ...
  {
    $var=$_POST["var"];
      if (!empty($_POST["tipdoc"])) {$tipcom=strtoupper(trim($_POST["tipdoc"]));}
   /* if ($var=='C') //CONSULTA
    {
      $tipcom=strtoupper(trim($_POST["tipdoc"]));
    }
    else */
    if ($var=='1')//PRIMERO
    {
      if ($tb=$z->primerRegistro('cpdoccom','tipcom'))
      {
        $tipcom=strtoupper(trim($tb->fields["tipcom"]));
      }
    }
    else if ($var=='2')//ANTERIOR
    {
      if (!empty($tipcom))
      {
        $aux=strtoupper(trim($tipcom));
        //chekeamos q no sea el primero
          $tb=$z->primerRegistro('cpdoccom','tipcom');
          if ($aux!=$tipcom)
          {
            $tb=$z->anteriorRegistro('cpdoccom','tipcom',$aux,'N');
          }
          else
          {
            $tb=$z->ultimoRegistro('cpdoccom','tipcom');
          }
      }
      else
      {
    ...
    else if ($var=='3')//SIGUIENTE
    {
      if (!empty($tipcom))
      {
        ...
        //chekeamos q no sea el ultimo
          ...
          if ($aux!=$tipcom)
          {
            $tb=$z->proximoRegistro('cpdoccom','tipcom',$aux,'N');
          }
           else
          {
            $tb=$z->primerRegistro('cpdoccom','tipcom');
    ...
    {
      if ($tb=$z->ultimoRegistro('cpdoccom','tipcom'))
      {
        $tipcom=strtoupper(trim($tb->fields["tipcom"]));
      }
    }
      //TRAER DATOS
      $sql="select * from cpdoccom where trim(upper(tipcom))='".$tipcom."' ";
      if ($tb=$z->buscar_datos($sql))
      {
        $tipdoc=$tb->fields["tipcom"];
        $nomext=$tb->fields["nomext"];
        $nomabr=$tb->fields["nomabr"];
        $refprc=$tb->fields["refprc"];
        $reqaut=$tb->fields["reqaut"];
      ...
      <tr>
        ...
        <tr>
          <td class="box"> <table width="100%" align="center" class="bodyline">
                      ...
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                            ...
                            <tr>
                              <td colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                                  <tr>
                                    <td width="32%">Refiere a Precompromiso</td>
                                    <td width="30%">
                  <fieldset>
                                        ...
                                        <tr>
                                          <td width="73">
                                            <div align="center">
                                              <label>
                        <? if ($refprc=='S') { ?>
                                              <input type="radio" name="radio" id="radio" value="S" checked>
                        <? } else { ?>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td width="32%"> Requeriere Autorizacion para su Causado?</td>
                            <td width="30%">
                        <fieldset>
                          <table width="167" border="0" cellpadding="0" cellspacing="0">
                                    <tr>
                                      <td width="73">
                                        <div align="center">
                                          <label>
                                <? if ($reqaut=='S') { ?>
                                          <input type="radio" name="reqaut" id="radio" value="S" checked>
                              <? } else { ?>
                                <input type="radio" name="reqaut" id="radio" value="S">
                              <? } ?>
                                            Si</label>
                                        </div></td>
                                      <td width="94">
                                        <div align="center">
                                          <label>
                                  <? if ($reqaut=='S') { ?>
                                              <input type="radio" name="reqaut" id="radio" value="N">
                                <? } else { ?>
                                  <input type="radio" name="reqaut" id="radio" value="N" checked>
                                <? } ?>
                                          No</label></div>
                                          </div></td>
                                    </tr>
...
<script language="JavaScript">
     ...
     function consulta()
      {
          ...
          var campo='nomext';
          var foco='submit';
          sql='Select nomext as Nombre_Extendido,tipcom as Tipo,nomabr as Nombre_Abreviado From CPDocCom where upper(nomext) like upper(�%�)  Order By tipcom';
          ...
          pagina='http://'+host+'/herramientas.php/generales/catalogo/metodo/Cpdoccom_Predoccom/clase/Cpdoccom/frame/form1/obj1/tipdoc/campo1/tipcom/submit/true';
     ...
     function salvar()
    {
      ...
      if (save=='S')
      {
        if(confirm("�Realmente desea Salvar?"))
        {
          ...
          {
            imec='<? print $imec;?>';
            f.action="imecPreDocCom.php?imec="+imec;
            f.submit();
          }
        }
      }
    ...
    function verificar()
    {
      ...
      if (!verificar_campo_clave(0,f.tipdoc.value,"No puede salvar sin introducir un Tipo de Compromiso"))
      {
        return false;
      }
