       ...
       Where trim(refaju) = '".trim($codigo)."'";*/
           ...
           if ($tb=$tools->buscar_datos($sql))
           {
            ...
            if (strtoupper(trim($status))=="N")
            {
             ...
             //Cargar Distribción de Eventos
             $pos2=1;
             $sqleve="select b.codpre as codpre, a.monaju as monaju, b.codeve as codeve, c.deseve as deseve, b.moneve as moneve  from cpmovaju a, cpdiseve b, cpevepre c where a.codpre=b.codpre and a.refaju=b.refdoc and b.codeve=c.codeve and b.refdoc='".$codigo."' and b.tipdoc='".$tipaju."' order by b.codpre ";
             $tb3=$bd->select($sqleve);
             while (!$tb3->EOF)
             {
                 $_POST["f".$pos2."1"]= $tb3->fields["codpre"];
                 $_POST["f".$pos2."2"]= $tb3->fields["monaju"]*(-1);
                 $_POST["f".$pos2."3"]= $tb3->fields["codeve"];
                 $_POST["f".$pos2."4"]= $tb3->fields["deseve"];
                 $_POST["f".$pos2."5"]= $tb3->fields["moneve"]*(-1);
                $tb3->MoveNext();
                $pos2=$pos2+1;
             }
             ///////////////////////////////////////////
    ...
    function MostrarDetalleGrid(id)
     {
        ...
        if (aux!="")
        {
          ...
          if (refmov=="C") {
               ...
               sql3='select b.codpre as codpre, a.monimp as monimp, b.codeve as codeve, c.deseve as deseve, b.moneve as moneve  from cpimpcom a, cpdiseve b, cpevepre c where a.codpre=b.codpre and a.refcom=b.refdoc and b.codeve=c.codeve and b.refdoc=�'+aux+'� and b.tipmov=�'+tipeve+'� order by b.codpre'; 
             }
          if (refmov=="A") {
            tipeve='CAU';
            sql2='Select * From CPImpCau Where trim (refcau) =trim(�'+aux+'�) order by Codpre';	
            sql3='select b.codpre as codpre, a.monimp as monimp, b.codeve as codeve, c.deseve as deseve, b.moneve as moneve  from cpimpcau a, cpdiseve b, cpevepre c where a.codpre=b.codpre and a.refpag=b.refdoc and b.codeve=c.codeve and b.refdoc=�'+aux+'� and b.tipmov=�'+tipeve+'� order by b.codpre';
          }
          if (refmov=="G") {
            tipeve='PAG';
            sql2='Select * From CPImpPag Where trim (refpag) =trim(�'+aux+'�) order by Codpre';	
            sql3='select b.codpre as codpre, a.monimp as monimp, b.codeve as codeve, c.deseve as deseve, b.moneve as moneve  from cpimppag a, cpdiseve b, cpevepre c where a.codpre=b.codpre and a.refcau=b.refdoc and b.codeve=c.codeve and b.refdoc=�'+aux+'� and b.tipmov=�'+tipeve+'� order by b.codpre';
          }
          if (refmov=="P") {
            tipeve='PRC';
            sql2='Select * From CPImpPrc Where trim (refprc) =trim(�'+aux+'�) order by Codpre';	
            sql3='select b.codpre as codpre, a.monimp as monimp, b.codeve as codeve, c.deseve as deseve, b.moneve as moneve  from cpimpprc a, cpdiseve b, cpevepre c where a.codpre=b.codpre and a.refprc=b.refdoc and b.codeve=c.codeve and b.refdoc=�'+aux+'� and b.tipmov=�'+tipeve+'� order by b.codpre';
          }
          pagina="gridatos.php?cuantos="+cuantos+"&refmov="+refmov+"&fecmov="+fecmov+"&fecha="+fecha+"&sql2="+sql2+"&sql3="+sql3;
...
<table width="100%" height="175" border="0" class="box">
  <!--DWLayoutTable-->
  <tr>
            <td height="20" valign="top" class="breadcrumb">FREERP<? echo $modulo; ?>
</td>
  </tr>
   <tr>
   <td>
    ...
    <div class="tabbertab">
        <h2> Imputaciones</h2>
                ...
                <tr>
                  <td width="548" class="tpButtons">								  MOVIMIENTOS DEL AJUSTE </td>
                  </tr>
            <tr>
            <td colspan="4">
            ...
            <table width="586" border="0" class="recuadro">
              ...
              <?php
                ...
                while ($j<=$numfil)
                {
                    ?>
                    <tr>
                      <td  align="left" class="grid_line01_br"><input name="f<? print $j;?>1" id="f<? print $j;?>1" type="text" class="grid_txt01" size="32" value="<? print $_POST["f".$j."1"]?>" align="right"  tabindex="2"  readonly="true"></td>
                      <td  align="right" class="grid_line01_br"><input name="f<? print $j;?>2" id="f<? print $j;?>2" type="text" class="breadcrumbv2" size="15" value="<? print number_format($_POST["f".$j."2"],2,'.',','); ?>"  align="right" tabindex=""  readonly="true"></td>
                      <td  align="right" class="grid_line01_br"><input name="f<? print $j;?>3" id="f<? print $j;?>3" type="text" class="grid_txt01" size="15" value="<? print $_POST["f".$j."3"]?>" align="right"  tabindex="2"  readonly="true"></td>
                      <td  align="right" class="grid_line01_br"><input name="f<? print $j;?>4" id="f<? print $j;?>4" type="text" class="grid_txt01" size="30" value="<? print $_POST["f".$j."4"] ?>" align="right"  tabindex="2"  readonly="true"></td>
                      <td  align="right" class="grid_line01_br"><input name="f<? print $j;?>5" id="f<? print $j;?>5" type="text" class="breadcrumbv2" size="15" value="<? print number_format($_POST["f".$j."5"],2,'.',','); ?>" align="right" onFocus="checktotal('f<? print $j;?>1','f<? print $j;?>3','f<? print $j;?>2','f<? print $j;?>5')"  onKeyPress="entermontoF(event,'f<? print $j;?>1',this.id,'f<? print $j+1;?>1')"></td>
                      </tr>
                  <?
                $j=$j+1;
                }        
...
//          pagina='http://'+host+'/herramientas.php/generales/catalogo/metodo/Precompro_Cpdoccom/clase/Cpdoccom/frame/form1/obj1/'+campo2+'/obj2/'+campo+'/obj3/'+campo3+'/campo1/tipcom/campo2/nomext/campo3/refprc/submit/false';
    ...
    function entermonto(e,id,fc)
    {
      if (e.keyCode==13)
      {
        ...
        else
        {
          ...
          str1= document.getElementById(x).value.toString();
          str1= str1.replace(',','');
          str1= str1.replace(',','');
          str1= str1.replace(',','');
    ...
    function entermontoF(e,cod,id,fc)
    {
     ...
     function actualizarEventos(idcod,idmon)
    {
       i=1;
       var acum=0;
       var am='<?php echo $numfil; ?>';
        while (i<=am)
        {
            ...
            var x2="f"+i+"2";
            var x5="f"+i+"5";
            if (document.getElementById(x).value==document.getElementById(idcod).value)
            {
               $(x2).value=document.getElementById(idmon).value
               $(x5).value="0.00";
            }
