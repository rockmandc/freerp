...
$z= new tools();
$mosrepsal=$_SESSION["configemp"]["aplicacion"]["contabilidad"]["modulos"]["confincom"]["mosrepsal"];
     ...
     ?>
        <script>
          if(confirm("¿Desea imprimir el Comprobante Contable?"))
          {
            ...
            pagina=ruta+'/reportes/reportes/contabilidad/rCOMPROBANTE.php?com1=<? echo $numero; ?>&com2=<? echo $numero; ?>&fecha1=<? echo $fecha; ?>&fecha2=<? echo $fecha; ?>&estado=T';
...
// exit;
     ...
     if ($imec=="I")
    {
          ...
          if ($fin<=1)
      {
        ...
        if ($numero=='########'){
        	$numero = Buscar_Correlativo($bd,$fecha);
        }
        $usua=$_SESSION["loguse"];
        $sql="insert into contabc (numcom,feccom,descom,moncom,stacom,tipcom,loguse)
