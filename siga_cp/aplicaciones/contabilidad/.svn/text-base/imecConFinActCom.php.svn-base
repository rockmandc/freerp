  ...
  if (!empty($_POST["i"]))
    ...
    while ($cont<=$i)
      ...
      if ($aux=='x'){
        ...
        $col5[$cant]=$_POST["x".$cont."5"];
        $col6[$cant]=$_POST["x".$cont."6"];
        }
        $cont=$cont+1;
      }
    }
        ...
        while ($cont<=$cant)
          {
            ...
            $feccom=$col3[$cont];
            $tiptra=$col5[$cont];
            $sql="update contabc set stacom = 'A', codtiptra='$tiptra' where numcom = '$numcom' and to_char(feccom,'dd/mm/yyyy') = '$feccom'";
            $bd->actualizar($sql);
