...
class tools
{
  ...
  global $bd;
    ...
    if ($tb=$x->buscar_datos($sql))
    {
      if (strlen($fecha)==10)
      {
        ...
        $explodefecini = explode('/', $fecini);
        $explodefeccie = explode('/', $feccie);

        $fecha_for = $explodefecha[2] . "-" . $explodefecha[1] . "-" . $explodefecha[0];
        $fecini_for = $explodefecini[2] . "-" . $explodefecini[1] . "-" . $explodefecini[0];
        $feccie_for = $explodefeccie[2] . "-" . $explodefeccie[1] . "-" . $explodefeccie[0];
  ...
  function formar_nivelDisponibilidad($codigo,$niveldisp='')
  {
    ...
    if($niveldisp==''){
      $sql="select nivdis from cpdefniv limit 1";
      $tb=$x->buscar_datos($sql);
      $niveldisp = $tb->fields["nivdis"];
    }
...
function fentrada2($cad){
  ...
  }else
  {
    ...
    $feccie=$periodo;
      ...
      $codigo = $x->formar_nivelDisponibilidad($codigo);
