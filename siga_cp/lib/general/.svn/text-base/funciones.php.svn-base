...
//session_start();
  ...
  function Buscar_Correlativo($con,$fecha='')
  {
    $num=0;
    $numcom='';
    $valido = false;
    ...
    if ($fecha=='') $fecha=date('Y-m-d');
     ...
     $forconuney=$_SESSION["configemp"]["generales"]["forconuney"];
     
     if ($forconuney=='S')  ///MM-B-###
     {
           $formato = substr($fecha,5,2).'-'.'5'.'-';
           $longitud='3';
           $tipo='5';
           $mes=substr($fecha,5,2);//date('m');
           $sql1="Select Max(Numcom) as correl from Contabc where Numcom Like '".$mes."-".$tipo."-%'";
           $tb1 = $con->select($sql1)->GetArray();
           if ($tb1)
           {
             $cor=substr($tb1[0]["correl"],5,3)+1;
      	   }else $cor=1;
