 ...
 function sqlp($numche1,$numche2,$numcue1,$numcue2,$status,$bendes,$benhas,$fechades,$fechahas)
  {
    ...
    if ($status=='T')
  		...
  		$estatus= "";
  	 	...
  	 	rtrim(c.nomben) as nomben,a.monche,a.numcue,b.nomcue, d.refpag
        		from tsdefban b, opbenefi c, tsmovlib d, tscheemi a
                where
        		rtrim(a.numcue)=rtrim(b.numcue) and 
			rtrim(a.numche)=rtrim(d.reflib) and
        		rtrim(a.numcue)=rtrim(d.numcue) and 
			rtrim(a.cedrif)=rtrim(c.cedrif) and
        		rtrim(a.numche)>=rtrim('".trim($numche1)."') and 
			rtrim(a.numche)<=rtrim('".trim($numche2)."') and
        		rtrim(a.numcue)>=rtrim('".trim($numcue1)."') and 
			rtrim(a.numcue)<=rtrim('".trim($numcue2)."') and
        		trim(A.CEDRIF) >= trim('".$bendes."') and
        		trim(A.CEDRIF) <= trim('".$benhas."') and 
			(a.fecemi) >= to_date('".$fechades."','dd-mm-yyyy')AND 
			(a.fecemi) <= to_date('".$fechahas."','dd-mm-yyyy') " .$estatus."
  		...
  		$estatus= "";
  	 	...
  	 	rtrim(c.nomben) as nomben,a.monche,a.numcue,b.nomcue, d.refpag
			...
			rtrim(a.numcue)<=rtrim('".trim($numcue2)."') and
        		...
        		trim(A.CEDRIF) <= trim('".$benhas."') and 
			(a.fecemi) >= to_date('".$fechades."','dd-mm-yyyy')AND 
			(a.fecemi) <= to_date('".$fechahas."','dd-mm-yyyy') " .$estatus." AND
			(refpag='' or refpag=' ')
             order by a.numcue,a.fecemi, rtrim(a.numche)";
	};
