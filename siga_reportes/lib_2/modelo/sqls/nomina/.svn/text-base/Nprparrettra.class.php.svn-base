...
//H::printr($sql);exit;
    ...
    function sqlsalariointegral($codemp){
		    	$sql= "select sum(a.monto) as monto
					from nphiscon a, npconsueldo b
					where a.codnom=b.codnom and
					 a.codcon=b.codcon and
					 a.codemp like '".($codemp)."%' and
		             fecnom in ( select max(fecnom) from nphiscon where codemp like '".($codemp)."%') ";
      //H::printr($sql);exit;
    	return $this->select($sql);
    }
