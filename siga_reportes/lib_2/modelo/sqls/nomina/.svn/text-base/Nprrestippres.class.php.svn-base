...
class Nprrestippres extends baseClases
{
 ...
 function sqlp($codtipdes,$codtiphas)
  ...
  {
			$sql="	Select Distinct(A.CodEmp) as codemp,to_char((select min(fechispre) from NPHISPRE where codemp=a.codemp and codtippre=d.codtippre),'dd/mm/yyyy') as fecini,B.NomEmp as nomemp,sum(A.Monto) as monto,sum(A.Cantidad) as cuota,
					...
					and B.StaEmp = 'A' and c.status='V' Group by a.id,a.Codemp,B.NomEmp,a.fecini,d.codtippre Order By A.CodEmp";

					//H::printR($sql);exit;

	return $this->select($sql);
  }
     function sqlfecha($codemp,$codtippre)
  {
			$sql="select min(fechispre) from NPHISPRE where codemp='".$codemp."' and codtippre='".$codtippre."'";
