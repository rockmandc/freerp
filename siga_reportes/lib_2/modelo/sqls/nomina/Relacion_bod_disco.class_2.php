...
class Federal extends BaseClases {
	...
	public function sqlp($coddes,$codhas,$codespdes,$codesphas,$banco,$especial)
    {
...

		{
			...
			$sql= "select distinct c.nacemp as nacemp, a.codemp as codemp,	c.nomemp, replace(c.numcue,'-', '' ) as cuenta_banco,
			
			(sum(CASE WHEN a.asided='A' THEN coalesce(a.saldo,0) ELSE 0 END) -
            sum(CASE WHEN a.asided='D' THEN coalesce(a.saldo,0) ELSE 0 END)) AS monto,
		    c.emaemp, c.celemp, a.fecnom
			from
			...
			group by  c.nacemp,a.codemp, c.numcue, c.nomemp, c.emaemp, c.celemp, a.fecnom
            order by codemp";
