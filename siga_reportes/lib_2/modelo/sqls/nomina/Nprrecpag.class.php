...
class Nprrecpag extends baseClases{
    ...
    function sqlcalulasueldo($codnom,$codemp)
    {
    	$sql1="select suecar as valor
						from npcargos
						where
						trim(codcar) =trim('".$codcar."')";


    	$sql="select saldo as valor from
                               npnomcal a,npconsueldo b
                               where
                               a.codcon=b.codcon and
                               a.codnom=b.codnom and
                               a.codnom= '".$codnom."' and a.codemp='".$codemp."'";
