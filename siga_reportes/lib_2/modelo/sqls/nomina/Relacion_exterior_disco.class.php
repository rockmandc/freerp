...
class Federal extends BaseClases {
	...
	public function sqlp($coddes,$codhas,$codespdes,$codesphas,$banco,$especial)
    {
		...
		if ($especial == 'S')
		{
					...
					$sql="select cedemp, NOMEMP, replace(cuenta_banco,'-',''), sum(monto) as monto  from (
		...
		(SELECT DISTINCT A.CODEMP AS CODEMP, NOMEMP, cast(A.codemp as int) AS CODEMPORD,(SUM (A.SALDO))*-1 AS monto,
					B.CEDEMP as cedemp, B.NOMEMP as nomemp, B.NUMCUE AS cuenta_banco, replace(cuenta_banco,'-','')
					FROM NPNOMCAL A left OUTER JOIN NPHOJINT B ON (A.CODEMP = B.CODEMP), NPBANCOS C
		...
		else
		{
			...
			$sql= "select distinct a.codemp, a.codcar, a.nomemp, 
...
//H::PrintR($sql);exit;
	...
	if ($especial == 'S')
		{
			$sql= "SELECT sum (a.monto) as nomina, replace(nomemp,',',''), from(SELECT A.NACEMP,
