...
class Libranza extends baseClases
{
  ...
  function generaconsolidado($codpre,$nompre,$fecha_ini,$fecha_cie)
  {
   ...
   --   B.FECPRC<='".$fecha_cie."' AND (A.STAIMP='A' OR (A.STAIMP='N' AND B.FECANU>'".$fecha_cie."'))  AND
   		  B.FECPRC<='".$fecha_cie."'   AND
      A.REFPRC = B.REFPRC)";

    #print '<pre>'; print $sql_cpimpprc;
  $this->actualizar($sql_cpimpprc);

     //Precompromisos anulados
    $sql_cpimpprcanu="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
      (SELECT 'EEF',B.TIPPRC,B.REFPRC,'NULO','NULO','NULO',B.FECANU,B.CEDRIF,A.MONIMP*(-1),B.DESPRC,'R'
      FROM CPIMPPRC A,CPPRECOM B
      WHERE
      A.CODPRE =  ('".$codpre."' ) AND
   ...
   --   B.FECPRC<='".$fecha_cie."' AND (A.STAIMP='A' OR (A.STAIMP='N' AND B.FECANU>'".$fecha_cie."'))  AND
     ...
     // print '<pre>'; print  $sql_cpimpprcanu;
  $this->actualizar($sql_cpimpprcanu);
    ...
    $sql_cpimpcomanu="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
         ...
         WHERE
             ...
             B.FECANU <= '".$fecha_cie."' AND
               A.REFCOM = B.REFCOM AND
           C.TIPCOM=B.TIPCOM)";
      //   print '<pre>';  print $sql_cpimpcomanu;
  ...
  //      print '<pre>'; print $sql_cpimpcau;
      ...
      (SELECT  'GGH',B.TIPCAU,A.REFPRC,A.REFERE,B.REFCAU,'NULO',B.FECANU,B.CEDRIF,A.MONIMP*(-1),B.DESCAU,C.AFEDIS
       ...
       WHERE
          ...
          B.FECANU<='".$fecha_cie."' AND
            A.REFCAU = B.REFCAU AND
        C.TIPCAU=B.TIPCAU)";
      //     print '<pre>'; print  $sql_cpimpcauanu;
    ...
    $sql_cpimppag="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
      (SELECT  'HHH',B.TIPPAG,A.REFPRC,A.REFCOM,A.REFERE,(case when b.tippag like '%CH%' then (numerocheque(B.REFPAG)) else B.REFPAG end) as REFPAG,B.FECPAG,B.CEDRIF,A.MONIMP,B.DESPAG,C.AFEDIS
    ...
    $sql_cpimppaganu="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
      (SELECT  'HHI',B.TIPPAG,A.REFPRC,A.REFCOM,A.REFERE,
      (case when b.tippag like 'CH%' then (numerocheque(B.REFPAG)) else B.REFPAG end) as REFPAG,B.FECANU,B.CEDRIF,A.MONIMP*(-1),B.DESPAG,C.AFEDIS
       FROM CPIMPPAG A,CPPAGOS B,CPDOCPAG C
       WHERE
          A.CODPRE =  ('".$codpre."' ) AND
          B.FECANU>='".$fecha_ini."' AND
        ...
        --  A.STAIMP='N' and
            A.REFPAG = B.REFPAG AND
        C.TIPPAG=B.TIPPAG)";
     //    print '<pre>'; print $sql_cpimppaganu;
...
/*INI ajustes*/
    ...
    $sql_cpmovajupre="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
      (SELECT 'EEI',B.TIPAJU,B.REFERE,'NULO','NULO','NULO',B.FECAJU,' ',A.MONAJU*-1,B.DESAJU,'R'
       ...
       WHERE
          ...
          C.TIPAJU=B.TIPAJU)";
        //    print '<pre>'; print  $sql_cpmovajupre;
    ...
    $sql_cpmovajucom="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
      (SELECT case when afeprc='N' then 'FFI' else 'FFE' end,
      B.TIPAJU,'NULO',B.REFERE,'NULO','NULO',B.FECAJU,' ',A.MONAJU*-1,B.DESAJU,E.AFEDIS
      FROM CPMOVAJU A,CPAJUSTE B,CPDOCAJU C,CPCOMPRO D,CPDOCCOM E
		...
		D.TIPCOM=E.TIPCOM AND
		B.STAAJU='A')";
         #print '<pre>';   print  $sql_cpmovajucom;
    ...
    $sql_cpmovajucau="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
        (SELECT case when afeprc='N' and afecom='N' then 'GGI'
                when afeprc='S' and afecom='S' then 'GGQ'
                when afeprc<>'N' then 'GGE'
                when afecom<>'N' then 'GGF' end,
        B.TIPAJU,'NULO','NULO',B.REFERE,'NULO',B.FECAJU,' ',A.MONAJU*-1,B.DESAJU,E.AFEDIS
        ...
        C.TIPAJU=B.TIPAJU AND
        D.TIPCAU=E.TIPCAU)";
             //      print '<pre>'; print  $sql_cpmovajucau;
    ...
    (SELECT case when afeprc='N' and afecom='N' and afecau='N' then 'HHA'
                 when afeprc='S' and afecom='S' and afecau='S' then 'EFG'
                 when afeprc='N' and afecom='S' and afecau='S' then 'HFG'
                 when afeprc='S' and afecom='N' and afecau='S' then 'HEG'
                 when afeprc='S' and afecom='S' and afecau='N' then 'HEF'
                 when afeprc='N' and afecom='N' and afecau='S' then 'HHG'
                 when afeprc='S' and afecom='N' and afecau='N' then 'HHE'
                 when afeprc='N' and afecom='S' and afecau='N' then 'HHF' end,
    B.TIPAJU,'NULO','NULO','NULO',B.REFERE,B.FECAJU,' ',A.MONAJU*0,B.DESAJU,E.AFEDIS
    ...
    C.TIPAJU=B.TIPAJU AND
    D.TIPPAG=E.TIPPAG)";
      #print '<pre>'; print  $sql_cpmovajupag;
    ...
    WHERE
          ...
          B.FECANU <= '".$fecha_cie."' AND
            ...
            A.REFAJU = B.REFAJU AND
          C.TIPAJU=B.TIPAJU)";
         //  print '<pre>'; print  $sql_cpmovajupreanu;
    ...
    $sql_cpmovajucomanu="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
      ...
      WHERE
        ...
        D.REFCOM = B.REFERE AND
          C.TIPAJU=B.TIPAJU AND
        D.TIPCOM=E.TIPCOM)";
    //    print '<pre>';   print   $sql_cpmovajucomanu; exit;
    ...
    WHERE
        ...
        D.REFCAU = B.REFERE AND
          C.TIPAJU=B.TIPAJU AND
        D.TIPCAU=E.TIPCAU)";
      //    print '<pre>'; print  $sql_cpmovajucauanu;
      ...
      (SELECT 'HHN',B.TIPAJU,'NULO','NULO','NULO',B.REFERE,B.FECANU,' ',A.MONAJU,B.DESAJU,E.AFEDIS
       ...
       WHERE
        ...
        D.REFPAG = B.REFERE AND
          C.TIPAJU=B.TIPAJU AND
        D.TIPPAG=E.TIPPAG)";
        //  print '<pre>'; print  $sql_cpmovajupaganu;
...
/*fin ajustes*/
    ...
    $sql_cpmovtraneganu="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
      (SELECT 'DDD','TRA-',B.REFTRA,'NULO','NULO','NULO',B.FECANU,' ',sum((A.MONMOV)*-1),B.DESTRA,'R'
       FROM CPMOVTRA A,CPTRASLA B
       WHERE
          A.CODORI =  ('".$codpre."' ) AND
           B.FECANU >= '".$fecha_ini."' AND
           B.FECANU <= '".$fecha_cie."' AND
            A.REFTRA = B.REFTRA
            group by B.REFTRA,B.FECTRA,B.DESTRA, B.FECANU)";
    ...
    $sql_cpmovtrapos="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
      (SELECT 'BBA','TRA+',B.REFTRA,'NULO','NULO','NULO',B.FECTRA,' ',sum(A.MONMOV),B.DESTRA,'S'
       FROM CPMOVTRA A,CPTRASLA B
       WHERE
          A.CODDES =  ('".$codpre."' ) AND
          B.FECTRA >= '".$fecha_ini."' AND
    ...
    $sql_cpmovtraposanu="insert into CONSOLIDADO_NUEVO (IDENTI,TIPMOV,REFPRC,REFCOM,REFCAU,REFPAG,FECMOV,CEDRIF,MONMOV,DESMOV,AFEDIS)
      (SELECT 'BBB','TRA+',B.REFTRA,'NULO','NULO','NULO',B.FECANU,' ',sum((A.MONMOV)*-1),B.DESTRA,'S'
       FROM CPMOVTRA A,CPTRASLA B
       WHERE
          A.CODDES =  ('".$codpre."' ) AND
          B.FECANU>= '".$fecha_ini."' AND
  ...
  function sql_cuerpo_cpasiini($codpre,$filtro)
  {
    $sql="SELECT MONASI as ASIGNA
      FROM CPASIINI
      WHERE CODPRE= ('".$codpre."' ) AND PERPRE='00' ";
      //print $sql; exit;
    return $this->select($sql);
    ...
    function sql_cuerpo_cpasiini_al($per,$codpre)
  {
    $hasta="trim(to_char(to_number('".$per."' ,'99')-1,'00'))";
    $sql="select monasi-
...
from CPAsiIni where  (CodPre) =  ('".$codpre."') and AnoPre='2008' and PerPre='00' and a.codpre like  (('".$filtro."'))";
  ...
  function sql_cuerpo_consolidado_nuevo($codpre,$filtro)
  {
      ...
      identi as identi,
      (case when substr(identi,1,2)='EE' or substr(identi,1,3)='FFE' or substr(identi,1,3)='GGQ' or substr(identi,1,3)='GGE' or substr(identi,1,3)='EFG'
          or substr(identi,1,3)='HEG' or substr(identi,1,3)='HEF' or substr(identi,1,3)='HEF' or substr(identi,1,3)='HHE'
          ...
          (case when substr(identi,1,2)='FF' or substr(identi,1,3)='GGQ' or substr(identi,1,3)='GGF' or substr(identi,1,3)='EFG' or substr(identi,1,3)='HFG'
          or substr(identi,1,3)='HEF' or substr(identi,1,3)='HHF'
          or tipmov=(select tipcau from cpdoccau c where c.refier='N' and c.afeprc='S' and c.afecom='S' and c.afecau='S' and c.tipcau=tipmov)
          or tipmov=(select tippag from cpdocpag c where c.refier='N' and c.afeprc='S' and c.afecom='S' and c.afecau='S' and c.tippag=tipmov)
          or tipmov=(select tipcom from cpdoccom c where c.refprc='N' and c.afeprc='S' and c.afecom='S' and c.tipcom=tipmov)
      then monmov else 0 end) as moncom,
      (case when substr(identi,1,2)='GG' or substr(identi,1,3)='EFG' or substr(identi,1,3)='HFG' or substr(identi,1,3)='HEG' or substr(identi,1,3)='HHG'
           or tipmov=(select tipcau from cpdoccau c where c.refier='N' and c.afeprc='S' and c.afecom='S' and c.afecau='S' and c.tipcau=tipmov)
           or tipmov=(select tippag from cpdocpag c where c.refier='N' and c.afeprc='S' and c.afecom='S' and c.afecau='S' and c.tippag=tipmov)
          -- or tipmov=(select tipcom from cpdoccom c where c.refprc='N' and c.afeprc='S' and c.afecom='S' and c.tipcom=tipmov)
          then monmov else 0 end) as moncau,
      (case when substr(identi,1,2)='HH' or substr(identi,1,3)='EFG' or substr(identi,1,3)='HFG' or substr(identi,1,3)='HEG' or substr(identi,1,3)='HEF'
        or tipmov=(select tippag from cpdocpag c where c.refier='N' and c.afeprc='S' and c.afecom='S' and c.afecau='S' and c.tippag=tipmov)
      ...
      monmov as monmov,a.afedis as afedis, coalesce(b.nomben,'')||' '||a.desmov as bene
      from consolidado_nuevo a left outer join opbenefi b on (a.cedrif=b.cedrif)
      where codpre= ('".$codpre."' )
      and codpre like  (('".$filtro."'))
      order by codpre,fecha,identi,refcom,refcau,/*refpag,*/identi";
