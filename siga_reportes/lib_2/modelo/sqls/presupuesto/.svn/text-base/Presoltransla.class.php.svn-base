...
class Presoltransla extends baseClases{
 ...
 function sqlp($codtrades,$codtrahas,$pertrades,$pertrahas,$fecdes,$fechas,$estatus)
  {
   ...
   if
			($estatus=="A")
				{
				$this->stacom="A";
				$t=" AND A.STATRA='".$this->stacom."' OR (A.STATRA='N' AND A.FECANU<=to_date('".$fechas."','dd/mm/yyyy'))";
				}
			elseif ($estatus=="N")
			{ 	$this->stacom="N";
				$t="AND A.STATRA='".$this->stacom."'and A.FECANU <= to_date('".$fechas."','dd/mm/yyyy')";
			}
            else
            {
            	$t="";
            }
  	$sql="SELECT A.PerTra as periodo,a.tipo,
							...
							FROM cpsoltrasla A --right outer join CPTRASLA  A on (a.reftra=c.reftra)
			WHERE
                			(
	...
	function sqlp2($codtrades,$codtrahas)
  {
	...
	{
		$sql = "SELECT CODORI AS origen, MONMOV AS monto FROM CPsolMOVTRA WHERE REFTRA = '".$codigo."'";
        //print '<pre>';print $sql;exit;
	...
	{
		$sql = "SELECT CODDES as destino, MONMOV AS monto FROM CPsolMOVTRA WHERE REFTRA = '".$codigo."'";
		 //print '<pre>';print $sql;exit;
        return $this->select($sql);
