...
exit('ActualizarSaldos');
  ...
  public static function catalogo_codctaConcatcue($objhtml,$orden)
	{
	  if ($orden=='A')
		{$sql="select codcta,descta from contabb where ( codcta like '%V_0%' AND descta like '%V_1%' ) order by codcta";}
		else { $sql="select codcta,descta from contabb where ( codcta like '%V_0%' AND descta like '%V_1%' ) order by codcta desc";}

		$catalogo = array(
		    $sql,
		    array('Código Cuenta','Nombre'),
		    array($objhtml),
		    array('codcta'),
		    100
		    );
