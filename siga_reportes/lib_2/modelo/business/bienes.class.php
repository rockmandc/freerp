...
class Bienes extends baseClases
{
	...
	{
		$sql="SELECT distinct(a.codact) as codigo, b.desact as descripción FROM bnregmue a, bndefact b where a.codact = b.codact and ( a.codact like '%V_0%' AND b.desact like '%V_1%' ) ORDER BY a.codact";
	...
	{
	   $sql="SELECT DISTINCT(a.codubi) as codigo, b.desubi as descripcion FROM bnregmue a, bnubibie b where a.codubi=b.codubi and ( codmue like '%V_0%' AND desmue like '%V_1%' ) order by a.codubi";
	...
	{
	   $sql="SELECT DISTINCT(codinm) as codigo, desinm as descripcion FROM bnreginm where ( codinm like '%V_0%' AND desinm like '%V_1%' ) order by codinm";
	...
	{
		$sql="select distinct(codemp) as codigo ,nomemp  as Nombre, nomcar as Cargo from npasicaremp where ( (nomcar like '%DIR%' OR nomcar like '%JEF%') AND codemp like '%V_0%' AND  nomemp like '%V_1%' AND nomcar like '%V_2%' ) order by codemp";
