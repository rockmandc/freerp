	...
	class pdfreporte extends fpdf
	{
		...
		function Header()
		{
			...
			//$this->SetTextColor(0,0,150);
                     $this->cab->poner_cabecera($this,$_POST["titulo"],"l","C");
			...
			foreach($this->arrp as $arrp)
			{
				if ($acodubi!=$arrp["acodubi"])
				{
					if ($acodubi)   //Fin de la linea del tipo del Mueble
					{
                         ...
                         $acum+=$this->acum_activo; $total+=$this->total_activo;
						 $this->total_activo = 0;
						 $this->acum_activo  = 0;
						 $this->AddPage();
					}
		     ...
		     $this->cell(20,5,H::FormatoMonto($total),0,0,'R');
             $this->setFont("Arial","",8);
	 		 $this->SetTextColor(0,0,150);
	 		 $this->ln();
	 		 $this->SetX(200);
			 $this->cell(45,6,"Cantidad de general:");
			 $this->cell(20,5,$acum,0,0,'R');
			 $this->total_activo = 0;
			 $this->acum_activo  = 0;
