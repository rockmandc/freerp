	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->codsem2=$_POST["codsem2"];
				...
				$this->sql="SELECT SUBSTR(A.CODACT,1,10) as acodact,A.CODSEM as acodsem,A.DESSEM as adessem,A.VALINI as avalini,A.VIDUTI as aviduti,A.VALREX as avalrex,A.STASEM as astasem
						FROM BNREGSEM A WHERE RTRIM(A.CODACT) >= RTRIM('".$this->codact1."') AND RTRIM(A.CODACT) <= RTRIM('".$this->codact2."') AND 
						RTRIM(A.CODSEM) >= RTRIM('".$this->codsem1."') AND RTRIM(A.CODSEM) <= RTRIM('".$this->codsem2."') ORDER BY A.CODSEM, A.CODACT"; 	
	...
	//		{
			while(!$tb->EOF)
			{
				...
				$this->setFont("Arial","",8); 
				 ...
				 $this->cell($this->anchos[0]-13,4,$tb->fields["acodact"]."-".$tb->fields["acodsem"]);
