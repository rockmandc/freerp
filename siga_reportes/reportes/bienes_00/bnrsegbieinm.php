        ...
        <div align="left">&nbsp;
          <table width="612"  border="1" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            <tr bordercolor="#FFFFFF">
              <td colspan="3" class="form_label_01"> <div align="center"> <font color="#000066" size="4"><strong>SEGURO
                  DE BIENES INMUEBLES
            ...
            <tr>
              ...
              <td>
                <?
			  	...
			  	$tb=$bd->select("SELECT DISTINCT(a.codact) as cod, b.desact as des FROM bnseginm a, bndefact b where a.codact = b.codact ORDER BY a.codact");
			  ?>
              ...
              <td>
                <?
			  	...
			  	$tb=$bd->select("SELECT DISTINCT(a.codact) as cod, b.desact as des FROM bnseginm a, bndefact b where a.codact = b.codact ORDER BY a.codact DESC");
			  ?>
            ...
            <tr>
              ...
              <td>
                <?
			  	...
			  	$tb=$bd->select("SELECT DISTINCT(codmue) FROM bnseginm ORDER BY codmue ASC");
               ?>


                <select name="codmue1" class="breadcrumb" id="select6">
                  <?
				  	while(!$tb->EOF)
					{
				  ?>
                  <option value="<? print $tb->fields["codmue"];?>"><? print $tb->fields["codmue"];?></option>
                  <?
					$tb->MoveNext();
					}
				?>
              ...
              <td>
                <?
			  	...
			  	$tb=$bd->select("SELECT DISTINCT(codmue) FROM bnseginm ORDER BY codmue DESC");
               ?>
                <select name="codmue2" class="breadcrumb" id="select7">
                  <?
				  	while(!$tb->EOF)
					{
				  ?>
            ...
            <tr bordercolor="#6699CC">
              <td class="form_label_01"><strong>N&uacute;mero de Poliza: </strong></td>
              <td>
                <?

			  	$tb=$bd->select("SELECT DISTINCT(nroseginm) FROM bnseginm ORDER BY nroseginm ASC");
               ?>


                <select name="numpol1" class="breadcrumb" id="numpol1">
                  <?
				  	while(!$tb->EOF)
					{
				  ?>
                  <option value="<? print $tb->fields["nroseginm"];?>"><? print $tb->fields["nroseginm"];?></option>
                  <?
					$tb->MoveNext();
					}
				?>
              ...
              <td>
                <?
			  	...
			  	$tb=$bd->select("SELECT DISTINCT(nroseginm) FROM bnseginm ORDER BY nroseginm DESC");
               ?>
                <select name="numpol2" class="breadcrumb" id="numpol2">
                  <?
				  	while(!$tb->EOF)
					{
				  ?>
            ...
            <tr bordercolor="#6699CC">
              ...
              <td> <div align="left"> </div>
                <div align="left">
			  	...
			  	$tb=$bd->select("SELECT to_char(MIN(fecseginm),'DD/MM/YYYY') as fecha FROM bnseginm");
				...
				if(!$tb->EOF)
				{
			  	...
			  	$tb2=$bd->select("SELECT to_char(MAX(fecseginm),'DD/MM/YYYY') as fecha FROM bnseginm");
				if(!$tb2->EOF)
				{
...
{
	f=document.form1;
	f.titulo.value="SEGURO DE BIENES INMUEBLES";
