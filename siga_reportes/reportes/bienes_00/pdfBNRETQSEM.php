	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->fecreg2=$_POST["fecreg2"];						
				...
				$this->sql="SELECT RTRIM(A.CODUBI) as acodubi, SUBSTR(A.CODACT,1,10) as acodact, A.CODSEM as acodsem, A.DESSEM as adessem,
							B.DESUBI as adesubi FROM BNREGSEM A,BNUBIBIE B WHERE
							RTRIM(A.CODACT) >= RTRIM('".$this->codact1."') AND
							RTRIM(A.CODACT) <= RTRIM('".$this->codact2."') AND
							RTRIM(A.CODSEM) >= RTRIM('".$this->codinm1."') AND
							RTRIM(A.CODSEM) <= RTRIM('".$this->codinm2."') AND 
							...
							RTRIM(A.CODUBI) = RTRIM(B.CODUBI) ORDER BY A.CODACT,  A.CODSEM"; 
		...
		function Cuerpo()
		{
			...
			while(!$tb->EOF)
			{
				 ...
				 $this->ln();
		  	     $this->cell(20,4," ".$tb->fields["acodact"]." - ".$tb->fields["acodsem"]);
				 $this->ln();				 
