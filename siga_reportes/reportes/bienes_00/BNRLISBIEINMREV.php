            ...
            <tr> 
              ...
              <td> 
                <?
			  	...
			  	$tb=$bd->select("SELECT a.codact as cod, b.desact as des FROM bnreginm a, bndefact b where a.codact = b.codact ORDER BY a.codact");
			  ?>
              ...
              <td> 
                <?
			  	...
			  	$tb=$bd->select("SELECT a.codact as cod, b.desact as des FROM bnreginm a, bndefact b where a.codact = b.codact ORDER BY a.codact DESC");
			  ?>
               ...
               ?>
                <select name="codinm1" class="breadcrumb" id="select6">
                  <?
					...
					{
				  ?>
                  <option value="<? print $tb->fields["codinm"];?>"><? print $tb->fields["codinm"]." / ".substr($tb->fields["desinm"],0,35);?></option>
                  <?
				  	$tb->MoveNext();
					}
				?>
...
{
	f=document.form1;
	f.titulo.value="LISTADO DE BIENES INMUEBLES REVALORIZADOS";
/*	sql='SELECT a.codubi, a.codubi,substr(a.codact,1,10) tipo,a.codact,a.codinm,a.desinm,a.feccom,';
	sql=sql+'a.fecreg,a.valini,a.viduti,a.stainm,a.ordcom,a.marinm,a.serinm,a.detinm,b.desubi ';
	sql=sql+'FROM bnreginm a,bnubibie b ';
	sql=sql+'WHERE a.codact >= rtrim("'+f.codact1.value+'") and rtrim(a.codact) <= rtrim("'+f.codact2.value+'") ';
	sql=sql+'and rtrim(a.codinm) >= ("'+f.codinm1.value+'") and rtrim(a.codinm) <= rtrim("'+f.codinm2.value+'") ';
	sql=sql+'and a.fecreg >= ("'+f.fecreg1.value+'") and a.fecreg <= ("'+f.fecreg2.value+'") and ';
	sql=sql+'a.stainm<>'d' and rtrim(a.codubi) = rtrim(b.codubi) order by a.codinm, a.codact ';
	f.sqls.value=sql; */
//	alert(f.codact1.value); 
	f.action="rBNRLISBIEINMREV.php";
	f.submit();
}
function cerrar()
{
