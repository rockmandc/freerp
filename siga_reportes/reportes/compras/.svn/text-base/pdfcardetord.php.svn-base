...
//print '<pre>'; print $this->sql;exit;
	...
	function llenartitulos2()
		...
		function Header()
		{
			...
			$this->cab->poner_cabecera_f($this,H::GetPost("titulo")." DE COMPRA: "."DESDE   ".$this->fecreg1."   HASTA   ".$this->fecreg2,"l","s","n");
			elseif(eregi("OC",$this->nrord1) and eregi("OS",$this->nrord2))
			$this->cab->poner_cabecera_f($this,H::GetPost("titulo")." DE COMPRA Y SERVICIO ","l","s","n");
			elseif(eregi("OS",$this->nrord1) and eregi("OS",$this->nrord2))
			$this->cab->poner_cabecera_f($this,H::GetPost("titulo")." DE SERVICIO ","l","s","n");
			else
			$this->cab->poner_cabecera_f($this,H::GetPost("titulo"."   Desde   ".$this->fecreg1."   Hasta   ".$this->fecreg2),"l","s","n");
...
#			}
		...
		//	$this->cell(45,5,"Al:  ".$fec);
			//	$this->cell(45,5,"Desde   ".$this->fecreg1."   Hasta   ".$this->fecreg2);
    	//	$this->ln();
 			$this->SetTextColor(0,0,0);
...
//				 $this->ln();
			 $this->SetLineWidth(0.4);
				 ...
				 $this->cell($this->anchos2[7],5,"");
				  $this->cell($this->anchos2[8],5,"".H::FormatoMonto($this->acum6),0,0,'R');
...
//-----------------------fin--------------------------
				...
				//Detalle
				 ...
				 $this->acum6=($this->acum6+$tb->fields["brgoart"]);
				 ...
				 $this->acum5t=($this->acum5t+$tb->fields["btotart"]);
				  $this->acum6t=($this->acum6t+$tb->fields["brgoart"]);
				 $this->ln();
				 $tb->MoveNext();
				 if($this->GetY()>=170){
				 	$this->AddPage();
			 ...
			 $this->SetLineWidth(0.4);
				 ...
				 $this->cell($this->anchos2[8],4,"".H::FormatoMonto($this->acum6t),0,0,'R');
