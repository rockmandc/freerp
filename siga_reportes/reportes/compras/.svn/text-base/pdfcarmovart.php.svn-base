	...
	$this->arrp=array("no_vacio");
			...
			$this->codart2=trim(H::GetPost("codart2"));
				...
				$this->sql2="SELECT A.CODART as acodart, A.DESART as adesart, A.EXITOT as aexitot FROM CAREGART A WHERE
							...
							LENGTH(RTRIM(CODART))=(LenGTH(Trim('".$forma."'))) and
							(
                                                        A.CODART=(select  codart from CAARTDPH f where f.codart=a.codart limit 1 ) or
							--A.CODART=(select  codart from CAARTREQ f where f.codart=a.codart limit 1 ) or
							A.CODART=(select  codart from CAARTRCP f where f.codart=a.codart limit 1 ) --or
							--A.CODART=(select  codart from CAARTORD f where f.codart=a.codart limit 1 )
							)ORDER BY CODART";
		...
		while(!$tb2->EOF)
			{
				...
				$this->cell(7,5,"");
  			    ...
  			    $this->cell(20,5,"");
  			    $this->cell(7,5,"");
  			    $acum+=H::FormatoMonto($tb3->fields["canord"],0);
  			    $this->cell(20,5,"".H::FormatoMonto($acum),0,0,'R');
				$this->ln();
				$tb3->MoveNext();
				}*/
...
//							 	print $this->sql6; exit;
				...
				$this->setFont("Arial","",6);
  			   ...
  			   // $this->cell(20,5,"".H::FormatoMonto($tb6->fields["canrec"]*$tb6->fields["cosrec"]),0,0,'R');
  			     $acum+=H::FormatoMonto($tb6->fields["canrec"],0);
  			    $this->cell(20,5,"".H::FormatoMonto($acum),0,0,'R');
				$this->ln();
				$tb6->MoveNext();
				}
				...
				$this->setFont("Arial","",6);
  			    ...
  			    $this->cell(20,5,"");
  			    $this->cell(7,5,"");
  			    $acum+=H::FormatoMonto($tb4->fields["canreq"],0);
  			    $this->cell(20,5,"".H::FormatoMonto($acum),0,0,'R');
			    ...
			    $this->cell(30,5,"".$tb5->fields["nomcatdph"]);
  			    ...
  			    $this->cell(7,5,"");  		;
  			    $acum-=H::FormatoMonto($tb5->fields["candph"],0);
  			    $this->cell(20,5,"".H::FormatoMonto($acum),0,0,'R');
				$this->ln();
				$tb5->MoveNext();
