          ...
          <table width="612" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            ...
            <tr> 
              <td class="form_label_01"> <div align="left"><strong>N&uacute;mero 
                  Requisici&oacute;n :</strong></div></td>
              <td> <div align="left"> 
                  <?
			  	
			  	$tb=$bd->select("select reqart from careqart order by reqart");
			  ?>
                  <select name="req1" class="breadcrumb" id="req1">
                    <?
				  	while(!$tb->EOF)	
					{
				  ?>
                    <option value="<? print $tb->fields["reqart"];?>"><? print $tb->fields["reqart"];?></option>
                    <?
					$tb->MoveNext();
					}
				?>
              ...
              <td> 
                <?
			  	...
			  	$tb2=$bd->select("select reqart from careqart order by reqart desc");
			  ?>
                <select name="req2" class="breadcrumb" id="req2">
                  <?
				  while(!$tb2->EOF)	
				{
			  ?>
                  <option value="<? print $tb2->fields["reqart"];?>"><? print $tb2->fields["reqart"];?></option>
                  <?
				  $tb2->MoveNext();
					}
				?>
            ...
            <tr> 
              ...
              <td> <div align="left"> 
			  	...
			  	$tb=$bd->select("select distinct(codart) from caartreq order by codart");
			  ?>
                  <select name="art1" class="breadcrumb" id="art1">
                    <?
				  	while(!$tb->EOF)	
					{
				  ?>
              ...
              <td> 
                <?
			  	...
			  	$tb2=$bd->select("select distinct(codart) from caartreq order by codart desc");
			  ?>
                <select name="art2" class="breadcrumb" id="art2">
                  <?
				  while(!$tb2->EOF)	
				{
			  ?>
                  <option value="<? print $tb2->fields["codart"];?>"><? print $tb2->fields["codart"];?></option>
                  <?
				  $tb2->MoveNext();
					}
				?>
            ...
            <tr> 
              ...
              <td> <div align="left"> </div>
                <div align="left"> 
			  	...
			  	$tb=$bd->select("select to_char(MIN(fecreq),'DD/MM/YYYY') as fecha1 from careqart");
				if(!$tb->EOF)
				{
					$fecha1=$tb->fields["fecha1"];
				}
	  	        
			  	$tb2=$bd->select("select to_char(MAX(fecreq),'DD/MM/YYYY') as fecha2 from careqart");
				if(!$tb2->EOF)
				{
...
{
	f=document.form1;
	f.titulo.value="DESPACHOS Y DIFERENCIAS";
	f.action="rCARREQDPH.php";
	f.submit();
}
function cerrar()
{
