        ...
        <div align="left">&nbsp;
          <table width="612" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            <tr bordercolor="#FFFFFF">
              <td colspan="3" class="form_label_01"> <div align="center"> <font color="#000066" size="4"><strong>Relaci&oacute;n Inventario Almacen
...
//prueba
         ...
         <!--   <tr bordercolor="#6699CC">
              <td class="form_label_01"><strong>Fecha Inventario : </strong></td>
			  <?
			  	$tr=$bd->select("SELECT DISTINCT(to_char(FECINV,'dd/mm/yyyy')) as fecha FROM CAINVFIS");
				if(!$tr->EOF)
				{
					$fecha=$tr->fields["fecha"];
				}
			  ?>
              <td><input name="fecinv" type="text" class="breadcrumb" id="fecinv" value="<? print $fecha;?>" datepicker="true"></td>
              <td>&nbsp;</td>
            </tr>-->
              ...
              <td>
				 <?
			  	$tb=$bd->select("SELECT DISTINCT(FECINV) as fecha FROM CAINVFIS order by fecha desc");
			  ?>
                  <select name="fecinv" class="breadcrumb" id="fecinv">
                    <?
				  	while(!$tb->EOF)
					{
				  ?>
                    <option value="<? print trim(substr($tb->fields["fecha"],8,2).'/'.substr($tb->fields["fecha"],5,2).'/'.substr($tb->fields["fecha"],0,4));?>"><? print trim(substr($tb->fields["fecha"],8,2).'/'.substr($tb->fields["fecha"],5,2).'/'.substr($tb->fields["fecha"],0,4));?></option>
                    <?
					$tb->MoveNext();
					}
				?>
              ...
              <tr>
              <td class="form_label_01"> <div align="left"><strong>Tipos de Alamacen:</strong></div></td>
              <td>
                <?
...
{
	f=document.form1;
	f.titulo.value="Relacion Inventario Almacen";
