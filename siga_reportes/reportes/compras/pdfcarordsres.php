				...
				if ($this->status=='t')
				{
			  	...
			  	$this->sql= "select
			 			A.ORDCOM as aordcom,
							...
							A.MONORD as amonord,
							A.STAORD as staord
							FROM CAORDCOM A, casolart b,    npcatpre d,  CAPROVEE E
							WHERE
							a.ORDCOM LIKE 'O%'AND
							(A.CODPRO) = (E.CODPRO) and
					    		a.refsol=b.reqart and
					    		b.unires=d.codcat and
							...
							B.unires LIKE '".$this->unidad."%' 
							-- and c.codpar  LIKE '%".$this->partida."%'
							GROUP BY A.ORDCOM , A.FECORD, A.CODPRO, E.NOMPRO,
							E.RIFPRO, B.unires, D.NOMCAT,  A.DESORD,
							A.CRECON, A.PLAENT, A.TIECAN, A.MONORD,
							A.STAORD order by a.ordcom";
//H::PrintR($this->sql);exit;
		...
		else
			{
					$this->sql=" select
							...
							E.RIFPRO, B.unires, D.NOMCAT,  A.DESORD,
							A.CRECON, A.PLAENT, A.TIECAN, A.MONORD,
							A.STAORD order by a.ordcom"; //print $this->sql;exit;
			 }
			 		//
//print '<pre>'; print $this->sql;exit;
	...
	function getAncho($pos)
	{
		...
		function Header()
		{
			...
			$this->cab->poner_cabecera_f($this,"LISTADO RESUMEN DE ORDENES DE COMPRAS","l","s","n"); 
			...
			$this->cab->poner_cabecera_f($this,"LISTADO RESUMEN DE ORDENES DE SERVICIO: "."DESDE   ".$this->fecreg1."   HASTA   ".$this->fecreg2,"l","s","n");
			else
	 ...
	 $this->Row(array($tb->fields["aordcom"],trim($tb->fields["adesord"]),$tb->fields["afecord"], $tb->fields["staord"],trim($tb->fields["dnomcat"]),trim($tb->fields["enompro"]),H::FormatoMonto($tb->fields["amonord"])));
				...
				//Detalle
				$this->acum5=($this->acum5+$tb->fields["amonord"]);
			    $this->acum5t=($this->acum5t+$tb->fields["amonord"]);
							 $tb->MoveNext();
				 if($this->GetY()>=170){
				 	$this->AddPage();
				 }
			...
			$this->ln();
			$this->SetWidths(array(20,80,20,20,35,50,35));
			$this->Setaligns(array('C','l','C','C','l','R','R'));
			$this->Row(array("","","","","","TOTAL GENERAL",H::FormatoMonto($this->acum5t)));
			$this->ln();
