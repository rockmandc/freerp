        ...
        <div align="left">&nbsp;
          <table width="612" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            <tr bordercolor="#FFFFFF">
              <td colspan="3" class="form_label_01"> <div align="center"> <font color="#000066" size="4"><strong>DESPACHOS
            ...
            <tr>
              <td class="form_label_01"><strong>C&oacute;digo Despacho:</strong></td>
              <td>
                <?

          $tb=$bd->select("select min(dphart) as valor from cadphart");
        ?>
        <input type="text" name="dphart1" class="breadcrumb" id="dphart1" value="<? print $tb->fields["valor"];?>"/>
        <input type="button" name="Button" value="..." onclick="catalogo1('dphart1');"/>
        </td>
              <td>
        <?
          $tb=$bd->select("select max(dphart) as valor from cadphart");
        ?>
        <input type="text" name="dphart2" class="breadcrumb" id="dphart2" value="<? print $tb->fields["valor"];?>"/>
        <input type="button" name="Button" value="..." onclick="catalogo1('dphart2');"/>
      </td>
            </tr>
            <tr>
          ...
          $tb=$bd->select("select min(codart) as valor from caartdph");
        ?>
              <input type="text" name="codart1" class="breadcrumb" id="codart1" value="<? print $tb->fields["valor"];?>"/>
        <input type="button" name="Button" value="..." onclick="catalogo2('codart1');"/>
      </td>
              <td>
                      <?

          $tb=$bd->select("select max(codart) as valor from caartdph");
        ?>
              <input type="text" name="codart2" class="breadcrumb" id="codart2" value="<? print $tb->fields["valor"];?>"/>
        <input type="button" name="Button" value="..." onclick="catalogo2('codart2');"/>
      </td>
            </tr>
            <tr>
          ...
          $tb=$bd->select("SELECT to_char(MIN(FECDPH),'DD/MM/YYYY') as fecha FROM CADPHART");
        if(!$tb->EOF)
        {
          $fecini=$tb->fields["fecha"];
        }

          $tb2=$bd->select("SELECT to_char(MAX(FECDPH),'DD/MM/YYYY') as fecha FROM CADPHART");
        if(!$tb2->EOF)
        {
...
{
    pagina="../../lib/general/catalogoobj.php?campo="+campo+"&sql=select dphart as dph from cadphart order by dphart";
...
{
    pagina="../../lib/general/catalogoobj.php?campo="+campo+"&sql=select distinct(codart) as codart from caartdph order by codart";
  window.open(pagina,"Catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,heigth=400,resizable=yes,left=50,top=50");
}
</script>
<? $bd->closed(); ?>
