      ...
      <td rowspan="2" valign="top" class="cell_padding_01"> <p class="breadcrumb">Reportes 
        ...
        <div align="left">&nbsp; 
          <table width="612" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            ...
            <tr> 
              ...
              <td colspan="2"> 
                <?
			  	...
			  	$tb=$bd->select("SELECT NOMPRO as nom FROM CAPROVEE ORDER BY NOMPRO");
			  ?>
                <select name="codpro1" class="breadcrumb" id="codpro1">
                  <?
				  	while(!$tb->EOF)	
					{
				  ?>
                  <option value="<? print $tb->fields["nom"];?>"><? print $tb->fields["nom"];?></option>
                  <?
										$tb->MoveNext();
					}
            ...
            <tr> 
              ...
              <td colspan="2"> 
                <?
			  	...
			  	$tb=$bd->select("SELECT NOMPRO as nom FROM CAPROVEE ORDER BY NOMPRO DESC");
			  ?>
                <select name="codpro2" class="breadcrumb" id="codpro2">
                  <?
				  	while(!$tb->EOF)	
					{
				  ?>
            ...
            <tr> 
              <td class="form_label_01"><strong>Orden Servicio:</strong></td>
              <td> 
                <?
			  	
			  	$tb=$bd->select("SELECT DISTINCT(ORDCOM) as ord FROM CAORDCOM WHERE ORDCOM LIKE 'os%' ORDER BY ORDCOM");
			  ?>
                <select name="nrord1" class="breadcrumb" id="nrord1">
                  <?
				  	while(!$tb->EOF)	
					{
				  ?>
                  <option value="<? print $tb->fields["ord"];?>"><? print $tb->fields["ord"];?></option>
                  <?
										$tb->MoveNext();
					}
              ...
              <td> 
                <?
			  	...
			  	$tb=$bd->select("SELECT DISTINCT(ORDCOM) as ord FROM CAORDCOM WHERE ORDCOM LIKE 'os%' ORDER BY ORDCOM DESC");
			  ?>
                <select name="nrord2" class="breadcrumb" id="nrord2">
                  <?
				  	while(!$tb->EOF)	
					{
				  ?>
            ...
            <tr> 
              <td class="form_label_01"><strong>Servicio:</strong></td>
              <td> 
                <?
			  	
			  	$tb=$bd->select("SELECT DISTINCT(CODART) as codart FROM CAARTORD ORDER BY CODART");
			  ?>
                <select name="codart1" class="breadcrumb" id="codart1">
                  <?
				  	while(!$tb->EOF)	
					{
				  ?>
                  <option value="<? print $tb->fields["codart"];?>"><? print $tb->fields["codart"];?></option>
                  <?
										$tb->MoveNext();
					}
              ...
              <td> 
                <?
			  	...
			  	$tb=$bd->select("SELECT DISTINCT(CODART) as codart FROM CAARTORD ORDER BY CODART DESC");
			  ?>
                <select name="codart2" class="breadcrumb" id="codart2">
                  <?
				  	while(!$tb->EOF)	
					{
				  ?>
...
{
	f=document.form1;
	f.titulo.value="ORDEN DE COMPRA";
	f.action="rCARDETORDS.php";
	f.submit();
}
function cerrar()
{
