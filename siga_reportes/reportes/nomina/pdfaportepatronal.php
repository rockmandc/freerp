...
require_once("../../lib/modelo/sqls/nomina/Aportepatronal.class.php");
...
//	  $this->consecutivo=strtoupper(H::GetPost('consedes'));
      ...
      $this->objPrenomina = new Aportepatronal();
...
//	 print "<pre>";
	 ...
	 for($i=0;$i<=$ncampos;$i++)
	 {
	  $this->titulos[$i+4]=substr($asig[$i]['nomcon'],0,60);
      $this->columna[$i]=$asig[$i]['codcon'];
     }

     //print "<pre>";
	 //print_r($this->columna);exit;
	 $this->titulos[$ncampos+4]="TOTAL APORTE PATRONAL";
//	 $this->titulos[$ncampos+5]="";
	...
	//			$this->total[$i+1][0][0]=$totalG;
				...
				//print_r($this->total);exit;
				 ...
				 {if ($i==0)
			      ...
			      $this->setXY(73,180);
			      $this->cell(50,4,'Coordinadora de Recursos Humanos',0,0,'C');
			      $this->setXY(218,180);
