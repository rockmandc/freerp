		...
		function pdfreporte()
		{
			...
			$this->sql="SELECT DISTINCT
				...
				to_char(B.FECNAC,'dd/mm/yyyy') as fechijo,
				 Extract(year from age(now(),B.FECNAC)) as EDAFAM, B.sexfam as sexo
				FROM
				...
				( Extract(year from age(now(),B.FECNAC))) >= ".$this->edad1." AND
				( Extract(year from age(now(),B.FECNAC))) <= ".$this->edad2." AND ".$sexo."
				E.STATUS='V'
				AND C.CODNIV=F.CODNIV
				AND C.CODEMP=B.CODEMP
				AND (B.PARFAM = '004' or B.PARFAM = '009')
				ORDER BY C.CODEMP";
		//	echo "<pre>".$this->sql;exit;
		...
		function Cuerpo()
		{
			...
			while(!$tbg->EOF)
			{
				if($refemp!=$tbg->fields["cedemp"])
				{
					...
					$this->SetTextColor(0,0,128);
					$this->Cell(30,4,"Ubicación Admin.:");
					$this->SetTextColor(0,0,0);
					$y=$this->gety();
					$x=$this->getx();
					$this->cell(50,4,"");
					...
					$this->setxy($x,$y);
					$this->multicell(50,4,(($tbg->fields["desniv"])));
			    ...
			    if ($tbg->fields["sexo"]=='F') { $tbg->fields["sexo"]='Femenino'; $fem++;} else { $tbg->fields["sexo"]='Masculino'; $mas++;}
			...
			$this->Line(10,$this->GetY(),200,$this->GetY());
			$this->ln(2);
			$this->multicell(100,4,'TOTAL DE FEMENINO: '.$fem);
			$this->ln(2);
			$this->multicell(100,4,'TOTAL DE MASCULINO: '.$mas);
			$this->ln(2);
			$this->Line(10,$this->GetY(),200,$this->GetY());
			$this->ln(3);
			$this->cell(10);
