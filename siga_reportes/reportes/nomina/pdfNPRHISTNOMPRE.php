	...
	class pdfreporte extends fpdf
	{
	  ...
	  /*and
			...
			$this->sql="SELECT distinct A.CODPRE,b.nompre,A.ASIGNA as ASIGNA,A.DEDUCI as DEDUCI, A.APORTE as APORTE,a.codtipgas,a.destipgas FROM
		        ...
		        trim(A.CODCON) <= trim('".$this->conhas."') AND  A.CODCON<>trim('995') and A.CODCON<>trim('006') and ".$especial."
		     ...
		     --   B.STATUS='V' AND
				...
				A.CODNOM = ('".$this->tipnomdes."') AND  ".$especial."
				--trim(B.CODEMP)=trim(A.CODEMP) AND
			--	trim(B.CODCAR)=trim(A.CODCAR) AND
				a.fecnomdes>=to_date('".$this->fechades."','dd/mm/yyyy') and
				a.fecnom<=to_date('".$this->fechahas."','dd/mm/yyyy') and
 ...
 //print '<pre>'; print $this->sql;exit;
		...
		function Header()
		{
			...
			//-----------------------sql2--------------------------
			 ...
			 $this->cell(45,5,"Nómina: ".$this->tipnomdes." - ".strtoupper($this->NombNom($this->tipnomdes)));
			//-----------------------fin--------------------------
			$this->ln();
			//-----------------------sql3--------------------------
			...
			$this->bd->closed();
               ...
               $this->ln();
				...
				if(!(strrpos($dir['path'],".php")))
			    	...
			    	$this->PutLink(trim($dir).'?tipnomdes='.$_POST["tipnomdes"].'&condes='.$_POST["condes"].'&conhas='.$_POST["conhas"].'&especial='.$_POST["especial"].
'&codnomesp1='.$_POST["codnomesp1"].'&codnomesp2='.$_POST["codnomesp2"].'&loncodpre='.$_POST['codpre'].'&fechades='.$_POST["fechades"].'&fechahas='.$_POST["fechahas"].
'&tipnomesp='.$_POST["tipnomesp"].'&schema='.$_SESSION["schema"],'Descargar Reporte en Excel');
