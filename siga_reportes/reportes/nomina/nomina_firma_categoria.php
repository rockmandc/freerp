      ...
      <td rowspan="2" valign="top" class="cell_padding_01"> <p class="breadcrumb">Reportes
        ...
        <div align="left">&nbsp;
          <table width="647"  border="0" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            ...
            <tr bordercolor="#6699CC">
              ...
              <td colspan="2">
                <select name="codcardes" class="breadcrumb" id="codcardes">
                  <?
				  $tb2=$bd->select("SELECT distinct(CODcar),UPPER(nomcar) as nombre FROM NPASICAREMP order by codcar");
				  	while(!$tb2->EOF)
					{
				  ?>
            ...
            <tr>
              ...
              <td colspan="2">
                  <select name="codcarhas" class="breadcrumb" id="codcarhas">
                    <?
					$tb2=$bd->select("SELECT distinct(CODcar),UPPER(nomcar) as nombre FROM NPASICAREMP order by codcar DESC");
				  	while(!$tb2->EOF)
					{
				  ?>
                    <option value="<? print $tb2->fields["codcar"];?>"><? print $tb2->fields["codcar"].' - '.$tb2->fields["nombre"];?></option>
            ...
            <tr>
              <td class="form_label_01"><strong>C&oacute;digo Categor&iacute;a Desde :</strong></td>
              <td colspan="2">                <select name="codcatdes" size="1" class=breadcrumb id="codcatdes">
                    <? $tb=$bd->select("SELECT codcat, UPPER(nomcat) as nomcat FROM NPCATPRE ORDER BY CODCAT");
				       while(!$tb->EOF){
					   ?>
            ...
            <tr>
              <td class="form_label_01"><strong>C&oacute;digo <strong>Categor&iacute;a</strong> Hasta :</strong></td>
              <td colspan="2">                <select name="codcathas" size="1" class=breadcrumb id="codcathas">
                    <? $tb=$bd->select("SELECT codcat, UPPER(nomcat) as nomcat FROM NPCATPRE ORDER BY CODCAT desc");
				       while(!$tb->EOF){
					   ?>
                    <option value="<? print $tb->fields["codcat"]; ?>"><? print $tb->fields["codcat"]." - ".ucwords($tb->fields["nomcat"]); ?></option>
                    <? $tb->MoveNext(); } ?>
                  </select></td>
            </tr>
            <tr>
              ...
              <td colspan="2">
                  <select name="codcondes" class="breadcrumb" id="codcondes">
                    <?
					$tb2=$bd->select("SELECT distinct(CODCON),upper(NOMCON) as nombre FROM NPASICONEMP order by CODCON");
				  	while(!$tb2->EOF)
					{
				  ?>
            ...
            <tr>
              ...
              <td colspan="2">
                  <select name="codconhas" class="breadcrumb" id="codconhas">
                    <?
					$tb2=$bd->select("SELECT distinct(CODCON),upper(NOMCON) as nombre FROM NPASICONEMP order by CODCON DESC");
				  	while(!$tb2->EOF)
					{
				  ?>
                    <option value="<? print $tb2->fields["codcon"];?>"><? print $tb2->fields["codcon"].' - '.$tb2->fields["nombre"];?></option>
            ...
            <tr>
              ...
              <td colspan="2">
                  <select name="tipnom" class="breadcrumb" id="tipnom">
                    <?
					$tb2=$bd->select("SELECT distinct(CODNOM),upper(NOMNOM) as nombre FROM npnomina order by CODNOM");
				  	while(!$tb2->EOF)
					{
				  ?>
...
{
	f=document.form1;
	f.titulo.value="Nómina para la Firma";
	f.action="rnomina_firma_categoria.php";
	f.submit();
}
function cerrar()
{
