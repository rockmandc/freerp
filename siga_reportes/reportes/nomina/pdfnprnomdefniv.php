...
/*
$this->sql="SELECT DISTINCT C.CODEMP as codemp, to_char(C.FECRET,'dd/mm/yyyy') as fecret,
C.NOMEMP as nomemp, d.codcat as codcta, d.nomcat as nomcat,
to_char(C.FECING,'dd/mm/yyyy') as fecing, C.NUMCUE as CUENTA_BANCO,
C.CODNIV as codniv, C.CEDEMP as cedemp, f.nomban as nomban,
LTRIM(RTRIM(B.CODCAR)) as CODCAR, B.NOMCAR as nomcar, E.DESNIV as desniv,
(CASE WHEN C.STAEMP='A' THEN 'ACTIVO' WHEN C.STAEMP='S' THEN 'SUSPENDIDO'
WHEN C.STAEMP='V' THEN 'VACACIONES' ELSE '' END) as ESTATUS
FROM NPHOJINT as C LEFT OUTER JOIN npbancos f ON (C.CODBAN=f.codban),
NPASICAREMP B, NPCATPRE D, NPESTORG E
where (B.CODNOM) = RPAD('".$this->tipnomdes."',3,' ') AND
D.CODCAT=B.CODCAT AND E.CODNIV=C.CODNIV AND
B.CODEMP=C.CODEMP
AND C.CODEMP>= RPAD('".$this->empdes."',16,' ') AND
C.CODEMP <= RPAD('".$this->emphas."',16,' ') AND
B.CODCAR>= RPAD('".$this->cardes."',16,' ') AND
B.CODCAR <= RPAD('".$this->carhas." ',16,' ') AND
C.CODNIV >= RPAD('".$this->nivdes."',16,' ') AND
C.CODNIV <= RPAD('".$this->nivhas."',16,' ') AND
B.STATUS='V'
ORDER BY C.CODNIV,C.CODEMP";*/
		...
		function Header()
		{
			...
			$this->cell(0,5,'NÓMINA: '.$this->tipnomdes.' - '.$this->nombre.' PERIODO DEL: '.$fechad.' AL '.$fechah,0,0,'');
			...
			//	 $this->cell(25,5,number_format($acumulado,2,',','.'));
				...
				$this->setFont("Arial","B",8);
				 ...
				 $this->setFont("Arial","BU",8);
                          //  $this->SetXY(49,205);
				 $this->cell(106,5,$this->revisado2);
			        $this->cell(150,5,$this->revisado3);

				  $this->ln(4);
				  $this->setFont("Arial","B",8);
                            // $this->SetY(212);
				 $this->cell(106,5, $this->revisado2car);
				 $this->cell(45,5,$this->revisado3car);
