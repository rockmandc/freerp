 ...
 ";
 		...
 		$this->sqlz= " SELECT DISTINCT(A.CODCON) AS CODCON, B.NOMCON as nomcon, A.CODNOM as codnom ,A.NOMNOM as nomnom, A.ASIDED,
							...
							B.IMPCPT, b.codpar,to_char(a.fecnomdes,'dd/mm/yyyy') as fecnomdes,
                                                        to_char(a.fecnom,'dd/mm/yyyy') as fecnom
							FROM NPNOMCAL A, NPDEFCPT B, NPHOJINT C
							...
							WHERE
									...
									A.SALDO > 0  AND c.staemp='A'  and
									(B.CODCON) = (A.CODCON) and a.codemp=c.codemp and c.staemp='A'
									GROUP BY A.CODCON,B.NOMCON,A.CODNOM,A.NOMNOM,A.ASIDED, B.IMPCPT,b.codpar,a.fecnomdes,a.fecnom
		...
		function Cuerpo()
	{
		...
		// 	ESto son los titulos
					...
					$this->cell(20,5,"NÓMINA: ");
							...
							$fecnomdes=$tbz->fields["fecnomdes"];
                                                        $fecnom=$tbz->fields["fecnom"];

            $this->SetTextColor(0,0,128);
			$this->SetXY(140,35);
			$this->cell(20,5,"NÓMINA DEL ".$fecnomdes.' AL '.$fecnomdes);

			$this->Line(10,$this->GetY()+5,200,$this->GetY()+5);

						$this->sql="SELECT COUNT(DISTINCT(a.CODEMP)) as numero FROM NPNOMCAL a,  NPHOJINT C WHERE (a.CODNOM)=('".$codnom."') and a.codemp=c.codemp and ".$this->especial." c.staemp='A' ";
						...
						$neto=0;
							...
							$this->SetTextColor(0,0,128);
							$this->SetXY(10,50);
							$this->cell(17,5,"CÓDIGO:");

							$this->SetXY(30,50);
							$this->cell(17,5,"NOMBRE DEL CONCEPTO");

							$this->SetXY(90,50);
							$this->cell(17,5,"ASIGNACIÓN");

							$this->SetXY(120,50);
							$this->cell(17,5,"DEDUCIÓN");
...
$this->Line(10,55,200,55);
		...
		while (!$tbz->EOF)
            ...
            $this->SetTextColor(0,0,128);
			$this->SetXY(140,35);
			$this->cell(20,5,"NOMINA DEL ".$fecnomdes.' AL '.$fecnom);
			...
			$this->Line(10,$this->GetY()+5,200,$this->GetY()+5);
		                ...
		                $this->sql="SELECT COUNT(DISTINCT(a.CODEMP)) as numero FROM NPNOMCAL a,  NPHOJINT C WHERE (a.CODNOM)=('".$codnom."') and a.codemp=c.codemp and c.staemp='A'";
						$tbx=$this->bd->select($this->sql);
						$this->SetXY(10,40);
						$this->cell(17,5,"NRO. TRABAJADORES:");
						$this->SetXY(60,40);
