		...
		function pdfreporte()
		{
			...
			print $this->sqlx;*/
					...
					if ($this->especial == 'S')
					{
						...
						impcpt='S' and a.codcon>=('".$this->con1."')   and  a.codcon<=('".$this->con2."')
						order by a.opecon,a.codcon";
						$this->especial=" especial = 'S' and   (CODNOMESP) = TRIM('".$this->tipnomesp1."') AND";
...
*/
		...
		//	$this->cell(30,5,"SEMANA: ".$sem);
			...
			//if ($this->nom<>'008')
			{
				$this->sqlz="select count(distinct(codemp)) as numero from npnomcal where ".$this->especial." rtrim(codnom)= rtrim('".$this->nom."')  ";
  		...
  		/*    else
			...
			$this->ln(7);
			$this->cell(72,5,"");
			$this->cell(60,5,"PERIODO ANTERIOR");
			$this->cell(78,5,"PERIODO ACTUAL ".$tbx->fields["ultfec"]." al ".$tbx->fields["profec"],0,0,'l');
		...
		function Cuerpo()
		{
			...
			if ($this->fechades==''){
				$this->sql3="select to_char(max(fecnom),'DD/MM/YYYY') as fecha from nphiscon where ".$this->especial." codnom='".$this->nom."'";
			//print '<pre>'; print $this->sql3;
			$tb3=$this->bd->select($this->sql3);
			}else $tb3->fields["fecha"]=$this->fechades;
            $x=$this->getx();
            $y=$this->gety();
           if ($this->aver){
           	$this->setFont("Arial","B",7);
           	$this->setxy(110,47);
			$this->cell(20,5,$tb3->fields["fecha"],0,0,'L');
			$this->aver=false;
			$this->setFont("Arial","",7);
           }
            $this->setxy($x,$y);
			$this->fecha=substr($tb3->fields["fecha"],3,7);
				$this->fecha2=$tb3->fields["fecha"];
            	$this->dia=substr($tb3->fields["fecha"],0,2);
				$this->sql33="select max(fecnom) as fecha from npnomcal where ".$this->especial." codnom='".$this->nom."'";
			//print '<pre>'; print $this->sql33;
			$tb33=$this->bd->select($this->sql33);
			$this->sql2="select count (codemp) as numero
						from nphiscon a, npdefcpt b
						where
						a.codcon = b.codcon and
						a.codcon = '".$tb->fields["codcon"]."' and
						a.fecnom=to_date('".$tb3->fields["fecha"]."','dd/mm/yyyy') and ".$this->especial."
						a.monto > 0
			...
			///periodo actual
			$this->sql66="select sum(CASE WHEN monto=0 THEN 0 ELSE 1 END) as numero
						from npnomcal a, npdefcpt b
						where
						a.codcon = b.codcon and
						a.codcon = '".$tb->fields["codcon"]."' and
						a.fecnom='".$tb33->fields["fecha"]."' and ".$this->especial."
						a.saldo > 0
						group by a.codcon,a.codnom, b.impcpt";

						$this->sql6="select count (codemp) as numero
						from npnomcal a, npdefcpt b
						where
						a.codcon = b.codcon and
						a.codcon = '".$tb->fields["codcon"]."' and ".$this->especial."
						a.fecnom='".$tb33->fields["fecha"]."' --and
						--a.saldo > 0
						group by a.codcon,a.codnom, b.impcpt";
						//print '<pre>'; print $this->sql6;
			$tb6=$this->bd->select($this->sql6); //if ($tb6->fields["numero"]==''){$tb6->fields["numero"]=0;}
			...
			$this->sql9="select count(distinct(a.codemp)) as totemping
						...
						a.codnom='".$this->nom."' and ".$this->especial."
			...
			$this->cell(55,5,"TOTAL EMPLEADOS INGRESADOS PERIODO ACTUAL: ".$tb9->fields["totemping"]);
			...
			$this->ln();
			$this->cell(55,5,"TOTAL EMPLEADOS EGRESADOS PERIODO ACTUAL: ".$tb10->fields["totempegr"]);
			//$this->cell(5,5,$tb10->fields["totempegr"],0,0,"R");

			////////////////////////////////////////////////////////

			if ($this->dia>'15'){
						$this->fecha3='16/'.$this->fecha;
				$fecha="b.fecret >= to_date('".$this->fecha3."','dd/mm/yyyy') and  b.fecret <= to_date('".$this->fecha2."','dd/mm/yyyy') and";
				$fechai="b.fecing >= to_date('".$this->fecha3."','dd/mm/yyyy') and  b.fecing <= to_date('".$this->fecha2."','dd/mm/yyyy') and";
			}
			if ($this->dia<='15'){
				$fecha=" b.fecret <= to_date('".$this->fecha2."','dd/mm/yyyy') and";
				$fechai=" b.fecing <= to_date('".$this->fecha2."','dd/mm/yyyy') and";

			}

			$this->ln();
			$this->sql99="select count(distinct(a.codemp)) as totemping
						from nphiscon a, nphojint b
						where
						a.codemp=b.codemp and
						a.codnom='".$this->nom."' and ".$this->especial."
						to_char(b.fecing,'MM/YYYY') ='".$this->fecha."' and
						b.fecing>=a.fecnomdes and  $fechai
						b.staemp='A'";

					//	print '<pre>'; print $this->sql99;
			$tb99=$this->bd->select($this->sql99);
			$this->cell(55,5,"TOTAL EMPLEADOS INGRESADOS AL ".$this->fecha2.' : '.$tb99->fields["totemping"]);
			//$this->cell(5,5,$tb99->fields["totemping"],0,0,"R");
			$this->sql100="select count(distinct(a.codemp)) as totempegr
						from nphiscon a, nphojint b
						where
						a.codemp=b.codemp and
						a.codnom='".$this->nom."' and ".$this->especial."
						to_char(b.fecret,'MM/YYYY') ='".$this->fecha."' and
						b.fecret>=a.fecnomdes and $fecha
						(b.staemp='R' or b.staemp='E') ";

						//print '<pre>'; print $this->sql100;
			$tb100=$this->bd->select($this->sql100);
			$this->ln();
			$this->cell(55,5,"TOTAL EMPLEADOS EGRESADOS AL ".$this->fecha2.' : '.$tb100->fields["totempegr"]);
		//	$this->cell(5,5,$tb100->fields["totempegr"],0,0,"R");
