<?
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->sqll2="	SELECT DISTINCT(C.CODEMP) as codemp, to_char(C.FECRET,'dd/mm/yyyy') as fecret,
						...
						f.nomban as nomban,	LTRIM(RTRIM(G.CODCAR)) as CODCAR, cast(G.CODCAR as int ) as codcarord,I.NOMCAR as nomcar,
						E.DESNIV as desniv,
						...
						NPASICAREMP B,	NPCATPRE D,	NPESTORG E,	nphiscon G,	NPDEFCPT H,  NPCARGOS I
						WHERE
						C.CODEMP <= '".$this->emphas."' AND
						C.CODEMP >= '".$this->empdes."' AND
						G.CODNIV >= '".$this->nivdes."' AND
						G.CODNIV <= '".$this->nivhas."' AND
						G.CODCAR >= '".$this->cardes."' AND
					    	G.CODCAR <= '".$this->carhas."' AND
						...
						f.nomban ,	LTRIM(RTRIM(G.CODCAR)) , cast(G.CODCAR as int ) ,I.NOMCAR ,
						E.DESNIV ,
						C.STAEMP,
						G.CODCON ,H.NOMCON,
						H.OPECON
