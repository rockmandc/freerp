		...
		function pdfreporte()
		{
			...
			$this->sql="SELECT
					...
					--	AND B.STATUS='V' and
						...
						AND (G.CODNOM) >= ('".$this->tipnomdes."')
						AND (G.CODNOM) <= ('".$this->tipnomhas."')".$especial."

						AND rtrim(G.CODCAR)=rtrim(g.CODCAR)
						AND G.CODEMP=g.CODEMP and  g.fecnom>=to_date('".$this->fecdes."','DD/MM/YYYY')
                        and g.fecnom<=to_date('".$this->fechas."','DD/MM/YYYY')
