...
$this->especial=$especial;
			$this->sql="
                       SELECT d.codnom ,d.nomnom
					   FROM npnomina d WHERE
					   d.CODNOM='".$this->tipnom."'
						 ";
		...
		function Header()
		{
            ...
            $mes2=substr($this->fecreg2,3,2);
			  //  $this->celda(15,43,'RECURSOS HUMANOS');
				...
				$this->setFont("Arial","B",9);
				$this->setXY(10,40);
				$this->Multicell(0,4,"TXT PRESTAMOS",0,'C',0);
		...
		function Cuerpo()
		{
			while (!$this->tb->EOF)
			{
				if ($this->cab1=='1'){
						$this->setFont("Arial","B",9);
				$this->setX(20);
				$this->cell(70,0,'Nomina:   '.$this->tb->fields["codnom"].' - '.$this->tb->fields["nomnom"]);
				$this->ln(4);
                ...
                $this->setborder(1);
		        $this->row(array("Cedula de Identidad","Funcionario","Monto Cuota Prestamo"));
				$this->setY(90);
				$this->pri=true;
					...
					--	A.CODCAR as codcar,
						...
						B.CODNIV, cast(REPLACE(b.cedemp,'.', '') as int )
						 ...
						 WHERE
						  ".$this->especial."
						B.CODEMP = A.CODEMP
						AND a.codcon=('".$this->codcondes."')
						AND  A.CODNOM = ('".$this->tb->fields["codnom"]."')
						AND  B.CODEMP >=  ('".$this->codempdes."')
						AND  B.CODEMP <= ('".$this->codemphas."')
						and a.codcat >= ('".$this->catdes."')
						and a.codcat <= ('".$this->cathas."') 
						and A.fecnom >= to_date('".$this->fecreg1."','dd/mm/yyyy') AND A.fecnom <= to_date('".$this->fecreg2."','dd/mm/yyyy') and
						a.codcon=e.codcon
						GROUP BY A.CODEMP, B.CEDEMP, A.CODNOM, to_char(coalesce(FECREI,FECING),'dd/mm/yyyy') ,
						B.NOMEMP , B.NACEMP, to_char(B.FECNAC,'dd/mm/yyyy') , B.SEXEMP ,B.CODNIV
...
";
							...
							$this->celda(180,'',number_format($tb2->fields["monto"],2,'.',',').'',0,0,'R');
							if (!$this->pri){
								 $this->line(20,$this->gety(),203,$this->gety());
							}

							$this->pri=false;
							$cs_apo = $cs_apo +  $tb2->fields["monto"];
     ...
     $this->sety(150);
				...
				if(!(strrpos($dir['path'],".php")))
			    	...
			    	$this->PutLink(trim($dir).'?codempdes='.$_POST["codempdes"].'&codemphas='.$_POST["codemphas"].'&tipnom='.$_POST["tipnom"].'&tipnom2='.$_POST["tipnom2"].'&codcatdes='.$_POST["codcatdes"].'&codcathas='.$_POST["codcathas"].'&especial='.$_POST["especial"].'&tipnomesp='.$_POST["tipnomesp"].'&codtipapodes='.$_POST["codtipapodes"].'&fec1='.$_POST["fecreg1"].'&fec2='.$_POST["fecreg2"].'&codcondes='.$this->codcondes.'&schema='.$_SESSION["schema"],'Descargar TXT Prestamos');
