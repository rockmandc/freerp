<?
	...
	class pdfreporte extends fpdf
	{
		...
		$this->setXY(30,17);
		$this->cell(50,4,"Contraloria Municipal de Chacao",0,0,'L');
		$this->setXY(30,21);
	//	$this->cell(50,4,"Dirección de Recursos Humanos",0,0,'L');
		$this->setXY(30,25);

	//$this->cab->poner_cabecera_f($this,"","p","s","n","n");
	$this->ln();
		//PrimerCuadro
		$this->setXY(150,25);
		...
		$y=$this->gety();
		$this->setxy(40,30);
		$this->cell(200,4,H::GetPost("titulo"),0,0,'l');
			$this->setxy($x,$y);
		...
		foreach($this->arrp as $dato)
		{
			if($dato["codemp"]!=$codemp){
				...
				$this->setBorder(1);
				$this->row(array("Nombre: ".$dato["nombre"],"C.I: ".$dato["cedemp"]));
				$this->nombre=strtoupper($dato["nombre"]);
				$this->cedula=strtoupper($dato["cedemp"]);
				$this->setwidths(array(60,60,80));
				$this->setaligns(array('L','L','L'));
				$this->setBorder(1);
				$this->row(array("CARGO: ".$dato["nomcar"],"UBICACIÓN: ".$dato["ubicacion"],"MOTIVO: ".$dato["desret"]));
				$this->setwidths(array(35,30,35,30,40,30));
				$this->setaligns(array('L','C','L','C','L','C'));
				$this->setBorder(1);
				$this->sqlsueldiconv="	SELECT
							(ultsue/30) as saldiario, (montoinci/30) as convnecion
							FROM NPVACLIQUIDACION
							WHERE codemp= '".$dato["codemp"]."' and perini=(select max(perini) from npvacliquidacion)";


					$tbSueldiconv=$this->bd->select($this->sqlsueldiconv);
				$this->row(array("SALARIO MENSUAL NORMAL: ",H::FormatoMonto($this->salint[0]['campo']),"SALARIO DIARIO NORMAL: ",H::FormatoMonto($this->salint[0]['campo']/30),"SALARIO CONVENCION COLECTIVA: ",H::FormatoMonto($tbSueldiconv->fields["convnecion"])));
				/*
				$this->ln(2);
				...
				$this->setBorder(1);
				$this->setFont("Arial","B",6);
				$this->row(array("SALARIO ACTUAL: ","SALARIO DIARIO: ","SALARIO CONVENCION COLECTIVA BS.: "));*/
				$this->ln(2);
...

				{
					...
					$this->row(array($asig["concepto"],H::FormatoMonto($asig["dias"]),H::FormatoMonto($asig["monto"])));
				}
				...
				$this->montoescrito=montoescrito(($this->total-$totaldeduc),$this->bd);
				$this->ln(1);
				$this->setFont("Arial","B",6);
				...
				$this->rect(10,$y1,200,15);$this->rect(110,$y1,200,15);
				$this->setY($y1+15);
				...
				$y=$this->gety();
				$this->sety(260);
				$this->cell(50,3,"cc. Funcionario (a) ",0,0,'L');
				$this->ln();
				$this->cell(50,3,"Expediente",0,0,'L');$this->ln();
				$this->cell(50,3,"Dirección de Administracion",0,0,'L');
				$this->setxy(40,260);
				$this->multicell(170,3,"Yo, ".$this->nombre."Titular de la cédula de identidad Nº ".$this->cedula.", hago constar que he recibido de la Contraloria Municipal del Municipio Chacao, la cantidad de ".$this->montoescrito." , por concepto de liquidación de prestaciones sociales a la fecha de retiro.",1,'l');
