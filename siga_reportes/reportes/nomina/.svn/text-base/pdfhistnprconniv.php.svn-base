	...
	class pdfreporte extends fpdf
	{
		...
		C.CODNOMESP = '".$this->tipnomesp."' AND	";
			...
			$this->sql="SELECT
						...
						WHERE
						C.CODNOM = '".$this->tipnom."' AND c.codnom=E.codnom and
						C.CODCON >= '".$this->codcondes."' AND
						C.CODCON <= '".$this->codconhas."' AND".$especial."
						c.CODCAT >= '".$this->codnivdes."' AND
						c.CODCAT <= '".$this->codnivhas."' AND   c.fecnom>=to_date('".$this->fechades."','dd/mm/yyyy') and
							 c.fecnom<=to_date('".$this->fechahas."','dd/mm/yyyy') and
		...
		function Header()
		{
			...
			$this->cell(15,5,$this->fechades);
			$this->SetTextColor(0,0,128);
			$this->cell(15,5,"  AL  ");
			$this->SetTextColor(0,0,0);
			$this->cell(15,5,$this->fechahas);
			$this->ln(1);
			$this->setFont("Arial","B",8);
