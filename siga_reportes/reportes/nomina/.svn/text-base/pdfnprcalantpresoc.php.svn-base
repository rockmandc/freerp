	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->codemphas=$_POST["emp2"];
						...
						'' as feccal,
						'' as feccor
						from NPANTPRE A,NPHOJINT C, NPASIEMPCONT B
						where
						a.codemp=c.codemp and
						A.CODEMP=B.CODEMP AND
						a.codemp>= ('".$this->codempdes."') AND
						a.codemp <= ('".$this->codemphas."')
						order by a.codemp";//H::PrintR($this->tsql);exit;
                            ...
                            $this->sql= "SELECT A.*,
                                                    ...
                                                    a.codemp>= ('".$this->codempdes."') AND
                                                    a.codemp <= ('".$this->codemphas."')
                                                    order by a.codemp,A.fecant";//H::PrintR($this->sql);exit;
		...
		function Cuerpo()
		{
			...
			while (!$tb->EOF) // while
			{
				...
				{
					if($tb2->fields["codemp"]!=$ref and $tb2->fields["monant"]<>0)
					{
						$cont=$cont+1;
						$ok=false;
					}
