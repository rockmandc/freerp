...
class pdfreporte extends fpdf
	{
		...
		function Header()
		{
			...
			$this->cell(20,5,"Contralor�-a Municipal del Municipio Chacao");
			$this->cell(130,5,"");
			...
			$this->SetX(40);
			$this->setFont("Arial","B",9);
			$this->cell(20,5,"Relación de Asignaciones y Deducciones");
			$this->setFont("Arial","",9);
			$this->ln(6);
			$this->cell(20,5,"Trabajador: ");
			$this->ln(8);
			$this->setFont("Arial","B",10);
			$this->cell(190,5,"Relación de Asignaciones y Deducciones Ejercicio Fiscal",0,0,'C');
			$this->ln(10);
			$this->setFont("Arial","B",9);
		...
		function Cuerpo()
	    {
			...
			foreach ($this->arrp4 as $arrp4)
			{
				...
				if($arrp4["codemp"]==$ref4)
				{
					...
					$this->setFont("Arial","",9);
					$this->SetXY(10,225);
					$this->Cell(50,5,'Dirección: Av. Francisco de Miranda, Centro Plaza Torre C, Piso 20, Urb, Los Palos Grandes ',0,0,'L');
					$this->ln();
					$this->Cell(50,5,'Agente de Retención: CONTRALORIA MUNICIPAL DEL MUNICIPIO CHACAO',0,0,'L');
					$this->ln();
					$this->Cell(50,5,'Periodo: '.$arrp["ano"],0,0,'L');
					$this->ln();
					$this->Cell(50,5,'Nombre del Empleado: '.$arrp4["nomemp"],0,0,'L');
					$this->ln();
					$this->Cell(50,5,'Cédula del Empleado: '.$arrp4["cedemp"],0,0,'L');
					$this->ln();
					$this->Cell(50,5,'Dirección del Trabajador: '.$arrp4["dirhab"],0,0,'L');
