<?
	...
	class pdfreporte extends fpdf
	{
			...
			while(!$tbg->EOF)
			{
				if($refemp!=$tbg->fields["codemp"])
				{
					...
					if($this->sw)
						{
							...
							$this->cell(80);
                                                 $this->SetTextColor(128,0,0);
                                                 $this->Cell (60,6,' TOTAL RETENCIÓN EMPLEADO:                    ');
					...
					$this->Cell(18,5,"Anual",0,0,'C');
                                   $this->Cell(22,5,"Retención",0,0,'C');
		...
		///////// INFO FAMILIAR
					...
					$this->sql010="select codemp, nomfam, sexfam as sexo,
                            		edafam,
                           			seghcm,
                            		parfam
                                          from npinffam
                            	 where 
                            		codemp='".$tbg->fields["codemp"]."'";
				...
				//DETALLE
                            while(!$tbg010->EOF)
			       {
                            	...
                            	if ($tbg010->fields["seghcm"]=='S')
                            	{
					$this->Cell(5);
                            	$this->Cell(62,5,$tbg010->fields["nomfam"]);
			       	$this->Cell(10,5,$tbg010->fields["edafam"],0,0,'C');
			       		if ($tbg010->fields["sexo"]=='F') 
                                    	{ $tbg010->fields["sexo"]='Femenino'; $fem++;} 
                                     	else 
                                    	{ $tbg010->fields["sexo"]='Masculino'; $mas++;}
			       	$this->Cell(22,5,$tbg010->fields["sexo"],0,0,'');
                            	$this->Cell(17,5,$tbg010->fields["seghcm"],0,0,'');
                        
                            	$this->sql222="SELECT  edaddes, edadhas, monto	
                                 		from
         			     	npseghcm
    		 	            	where
                                     	edadDES <=  '".$tbg010->fields["edafam"]."' and
                                     	edadhas >=  '".$tbg010->fields["edafam"]."' and
                                     	RTRIM(TIPPAR)  =  '".$tbg010->fields["parfam"]."' and
