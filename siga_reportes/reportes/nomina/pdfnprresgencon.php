<?
 ...
 ";
		...
		function Header()
		{
			...
			$this->Row(array("CONCEPTO","DESCRIPCIÓN","FUNCIONARIOS","MONTO"));
	...
	function Cuerpo()
	{
		...
		while(!$this->tbg->EOF)
		{
			...
			if ($this->ref!=$this->tbg->fields['codnom']){
				...
				$this->Ln(5);
				$this->Cell(170,5,"GASTOS DE NÓMINA");
			...
			$cuantos = $this->bd->select("SELECT COUNT(DISTINCT(CODEMP)) as numero FROM NPNOMCAL 
			WHERE 
			trim(codnom)=trim('".$this->tbg->fields["codnom"]."') AND
			trim(asided)=trim('".$this->tbg->fields["asided"]."') and 
			trim(codcon) = trim('".$this->tbg->fields["codcon"]."')")->fields["numero"];
