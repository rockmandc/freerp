...
$this->ln(3);
			...
			$this->setWidths(array(30,95,95,30));
			...
			$this->ln();
			$this->ln(2);
			$this->setWidths(array(40,90,40,40,40));
			$this->setAligns(array('C','L','R','R','C'));
			//$this->setBorder(1);
			$this->rowM(array(" Fecha","Descripción","Cuota Prestamo","Saldo Prestamo","Nro.Cuotas Pendientes"));
			$this->ln(2);
			$this->Line(10,$this->getY(),270,$this->getY());
			$this->ln(2);
			...
			foreach($this->arrp as $dato){
				...
				$this->rowM(array($dato["fechispre"],$dato["deshispre"],H::FormatoMonto(abs($dato["monpre"])),H::FormatoMonto($dato["saldo"])."     ",round($dato["saldo"]/$this->arrdet[0]["cuota"])));
				if ($this->gety()>155){
					$this->addpage();
					$this->setWidths(array(40,90,40,40,40));
			        $this->setAligns(array('C','L','R','R','C'));
