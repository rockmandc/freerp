...
class pdfreporte extends fpdf
{
	...
	function Cuerpo()
	{
		 ...
		 $this->ln();
		 $this->cell(20,5,"NIVEL ORGANIZACIONAL: ".$this->arrp[$this->i]['desniv']);
		 $this->ln();
	 	 $this->setFont("Arial","B",10);
         $this->ln();
		 ...
		 $this->setX(42);
		 $this->codniv=$this->arrp[$this->i]['codniv'];
			...
			if($ref!=$r['codemp'])
			{
				...
				if($ref)
				{
					if ($this->gety()>180){ $this->AddPage();}
	                   ...
	                   if ($this->codniv!=$r['codniv']){
  $this->AddPage();
							 $this->ln();
							 $this->setFont("Arial","B",10);
							 $this->cell(20,5,"NIVEL ORGANIZACIONAL: ".$r['desniv']);
							 $this->setFont("Arial","",10);
							 $this->ln();
