...
//print '<pre>'; print $this->sql; exit;
		...
		function Header()
		{
			...
			$this->cell(0,5,'NÓMINA: '.$this->tipnom.' - '.$this->nombre. '  PERIODO DEL: '.$fechad.' AL '.$fechah);
		//	$this->cell(60,5,);
			if($band==true)
			{
			...
			while (!$tb->EOF) // while primerito
			{
				...
				// $totacum3=$totacum3+$acum3;
				 ...
				 $this->cell(30,5,number_format($totacum1-$totacum2,2,',','.'));
				  $this->setx(10);
				 $this->multicell(90,4,'TOTAL:  '.$nomcat);
				 $total1=$total1+$totacum1;
				 $total2=$total2+$totacum2;
				// $total3=$total3+$totacum3;
				 $totacum1=0;
				...
				// $totacum3=0;
				 ...
				 $acum2=0;
				 $this->cell(30,5,number_format($total1-$total2,2,',','.'));
				 $this->setx(10);
				 $this->multicell(90,3,'TOTAL NÓMINA:  '.$this->nombre);
				 $this->ln(4);
				 $this->line(10,$this->getY(),200,$this->getY());
