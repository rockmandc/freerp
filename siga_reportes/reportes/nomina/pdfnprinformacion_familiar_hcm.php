	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			from NPASICAREMP C, nphojint b left outer join npinffam a on (a.codemp=b.codemp)
						WHERE  C.CODEMP=B.CODEMP AND 
							C.CODNOM='".$this->nom."'  AND
                                                 c.status='V' and
							b.seghcm='S' AND
							trim(B.CODEMP) >= trim('".$this->codesde."') AND
							trim(B.CODEMP) <= trim('".$this->codhasta."') and 
							(b.staemp='A' or b.staemp='O') 
						ORDER BY B.CODEMP
		...
		function Cuerpo()
		{
			...
			while(!$tb->EOF)
			{
 			  ...
 			  $this->ln(8);
                       ...
                       $this->cell(60,10,'TOTAL EMPLEADOS: '.$emp);
			  $this->ln(10);	
                       $this->cell(60,10,'TOTAL BENEFICIARIOS HCM: '.$hcm);
