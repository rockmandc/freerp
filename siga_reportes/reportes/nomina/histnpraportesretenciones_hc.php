...
if(H::GetPost("codtipapodes")=="0003" or H::GetPost("codtipapodes")=="0009")
{
    ...
    $fecreg2=H::GetPost("fec2");
				...
				if($codtipapodes=='0004' or $codtipapodes=='0010')
			{
				...
				$cond="";
				if($tipnom=='001' and $especial == 'N')
					{
					$cond="(A.CODCON='A01' or A.CODCON='A34' ) and";
					}
				
				if($tipnom=='002' and $especial == 'N')
					{
					$cond="(A.CODCON='A48' or A.CODCON='A42' ) and";
					}
				$sql="SELECT
						DISTINCT
						A.CODEMP as codemp,
						...
						B.CODNIV, cast(REPLACE(B.CEDEMP,'.', '') as int )
					     FROM
						   nphiscon  A,NPHOJINT B, npdefcpt e, npconfon d
						 WHERE
...
while(!$arrp->EOF)
{
	...
	//APORTE
		...
		//condicion para quitar a los trabajadore que tengan retencion igual a cero
        if (number_format($tb4->fields["elmonto"],2,'.',',')>0){
		 echo trim(strtoupper(trim(str_replace(".","",str_replace("-","",$arrp->fields["cedemp"]))))).'|'.$fecreg2.'|'.trim(substr($arrp->fields["nomemp"],0,30)).'|15072101|'.number_format($arrp->fields["monto"],2,',','').'|'.number_format($tb4->fields["elmonto"],2,',','').'|'.number_format($tb5->fields["elmonto"],2,',','').chr(10);
        }
