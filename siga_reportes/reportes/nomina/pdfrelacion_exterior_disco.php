<?
    ...
    require_once("../../lib/modelo/sqls/nomina/Relacion_exterior_disco.class.php");
...
function Cuerpo()
		...
		$nombre_archivo="/tmp/bancoexterior_"."_".strftime('%d_%m_%Y',time()).".txt";
		...
		//print $this->arrp2[0][nomina];exit;
         ...
         ////PRIMERA LINEA
         $lineatxt1 = "G200002395".str_pad($this->cuenta,20,'0',STR_PAD_LEFT).str_pad($cuantos-1,4,'0',STR_PAD_LEFT).str_pad($monto1,13,'0',STR_PAD_LEFT).str_pad($fecha,8,'0',STR_PAD_LEFT)."02".chr(13);
		...
		foreach($this->arrp as $dato)
         {
         	$numemp++;
				...
				//$acumulado = $acumulado + $dato["monto"];
				 ...
				 $monto=str_replace('.','',$monto);
				//$lineatxt = str_pad($dato["nomemp"],60,' ',STR_PAD_LEFT).str_pad($monto,13,'0',STR_PAD_LEFT).str_pad($dato["cedemp"],60,' ',STR_PAD_LEFT);
                //LINEA DE EMPLEADOS
               

 $lineatxt = str_pad($dato["replace"],60,' ',STR_PAD_left).str_pad($monto,12,'0',STR_PAD_LEFT)."V".str_pad($dato["cedemp"],10,'0',STR_PAD_LEFT)."PAGO DE NOMINA ".str_pad($this->quincena,45,' ',STR_PAD_RIGHT)."115".str_pad($dato["cuenta_banco"],20,'0',STR_PAD_LEFT);

                $lineatext2 = $lineatxt.chr(13);
