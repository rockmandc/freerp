<?
  ...
  class pdfreporte extends fpdf
  {
    ...
    function Cuerpo()
    {
      $this->setWidths(array(40,70,35,20,25,25));
      ...
      for ($i=0;$i<count($this->arrp);$i++)
      {
        ...
        if ($this->arrp[$i]["aporte"]>0)
          ...
          $aporte = '';
	$presu='';
          $presu=($this->arrp[$i]["codpre"]);
	   
	
         $asignac=$asigna;
		

		$this->sqla="select mondis  
                               from cpasiini
                	               where 
               		   codpre='".$presu."' and
				   perpre='00'";
		    ...
		    $mondis_t=H::FormatoMonto(($tba->fields["mondis"]),2,'.',',');



	             $this->Row(array($this->arrp[$i]["codpre"],$this->arrp[$i]["nompre"],$asigna,$deduci,$aporte,$mondis_t));
