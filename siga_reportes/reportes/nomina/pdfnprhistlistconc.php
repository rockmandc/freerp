<?
		...
		function pdfreporte()
		{
			...
			$this->anchos=array();
			$this->anchos2=array();
			$this->emp1=$_POST["codempmin"];
			$this->emp2=$_POST["codempmax"];
			$this->con1=$_POST["codconmin"];
			$this->con2=$_POST["codconmax"];
			$this->car1=$_POST["codcarmin"];
			$this->car2=$_POST["codcarmax"];
			$this->cat1=$_POST["codcatmin"];
			$this->cat2=$_POST["codcatmax"];
			$this->per1=$_POST["codnommin"];
			$this->per2=$_POST["codnommax"];
			$this->nom=$_POST["codnommin"];
			$this->nomesp=$_POST["nomesp"];
			//$this->tipcon=strtoupper($_POST["tipcon"]);
			  if ($this->tipcon=='T') //SE COMENTO PORQ EN LA PANTALLA NO EXISTE NINGUN tipcon
			  {
			     $EST1='A';
			     $EST2='D';
			     $EST3='P';
			...
			$this->sql="SELECT DISTINCT
				...
				--(D.OPECON = '".$EST1."'  OR
				 --D.OPECON = '".$EST2."'  OR
				-- D.OPECON = '".$EST3."')
				ORDER BY A.CODCON,A.CODEMP";//H::PrintR($this->sql);exit;
		...
		function Header()
		{
			...
			$this->ln(-1);
			$this->sqlx="select codnom, nomnom, to_char(ultfec,'dd/mm/yyyy') as ultfec, to_char(profec,'dd/mm/yyyy') as profec from npnomina";//esto iba en el where ..  where codnom='".$this->nom."'
