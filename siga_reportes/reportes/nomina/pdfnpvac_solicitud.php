...
class pdfreporte extends fpdf
{
	...
	function pdfreporte()
	{
		 ...
		 $this->elaporcar=H::GetPost("elaporcar");
		 $this->aprporcar=H::GetPost("aprporcar");
		 $this->revporcar=H::GetPost("revporcar");
	...
	//	H::PrintR($this->arrp);exit;
		for($y=0; $y < $cuantos; $y++)
		{
			...
			if($ref!=$this->arrp[$y]['codemp'])
			{
				...
				$this->setAligns('C');
				$this->setborder(true);
				$this->rowm(array('Ced. Identidad','Apellidos y Nombres','Tipo de Nómina'));
				$this->setFont("Arial","",10);
				$this->rowm(array($this->arrp[$y]['cedemp'],$this->arrp[$y]['nomemp'],$this->arrp[$y]['nomina']));
				$this->setwidths(array(20,160));
				$this->setAligns('C');
				$this->setborder(true);
				$this->setFont("Arial","B",10);
				$this->rowm(array('Fecha Ingreso','Cargo'));
				$this->setFont("Arial","",10);
				$this->rowm(array(date('d/m/Y',strtotime($this->arrp[$y]['fecing'])),$this->arrp[$y]['codcar'].' - '.$this->arrp[$y]['nomcar']));
				$this->setFont("Arial","B",10);
				...
				//"segundo quinquenio; de veintiun días hábiles durante el tercer quinquenio y de veinticinco días ".
	...
	//$this->setwidths(array(30,30,30));
				...
				$this->setFont("Arial","B",10);
				$this->setX(55);
				$this->rowm(array('Periodo','Días de disfrute'));
				$this->setFont("Arial","",10);
			...
			$this->setX(55);
			$this->rowm(array($this->arrp[$y]['perini']."-".$this->arrp[$y]['perfin'],number_format($this->arrp[$y]['diadis'],0,'.',',')));
			$this->ln(2);
