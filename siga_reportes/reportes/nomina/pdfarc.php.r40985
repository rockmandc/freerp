	...
	class pdfreporte extends fpdf
		...
		function pdfreporte()
		{
			...
			to_char(a.fecret,'dd/mm/yyyy') as fecret
						 ...
						 where a.codemp >= '".$this->codempdes."' and a.codemp <= '".$this->codemphas."' and a.codemp=b.codemp
						 and b.codnom='".$this->codnommin."' and b.status='V'	order by a.cedemp,a.codemp";
		...
		{   $tb=$this->arrp;
			...
			while (!$tb->EOF)
			{
				$this->formato($tb->fields['codemp'],$tb->fields['nomemp'],$tb->fields['cedemp'],$tb->fields['codemp'],$tb->fields['nacemp'],$this->perdes,$this->perhas);
			      ...
			      $this->RowM(array('','','','','','','INFORMACIÓN SEGUN ARI','REMUNERACION ANUAL','DESGRAVAMEN EN Bs.'));
			     // $Y=$this->GetY();
			      $this->setFont("Arial","",5);
			      $this->SetXY(173,$y);
				...
				$this->sqlmax= "select (to_char(max(a.fecnom),'dd/mm/yyyy')) as maximo from nphiscon a, nphojint b
                                where a.codemp=b.codemp and a.codemp='".$tb->fields['codemp']."' and a.codnom='".$this->codnommin."' ";
...
$this->sqlD2=("select sum(b.monto) as monto
							 ...
							 b.codemp='".$tb->fields['codemp']."' and
							    ...
							    to_char(b.fecnom,'mm')='".$i2."' and
							    to_char(b.fecnom,'yyyy')>=trim('".$this->anodes."') and
								to_char(b.fecnom,'yyyy')<=trim('".$this->anohas."') GROUP BY b.codcon "); //print $this->sqlD2;exit;
				...
				$this->sqlA="
								...
								a.codnom=b.codnom and a.codnom='".$this->codnommin."'
								union
								...
								to_char(fecnom,'mm')=('".str_pad($i, 2, '0', STR_PAD_LEFT)."') and
							    to_char(fecnom,'yyyy')>=trim('".$this->anodes."') and
								to_char(fecnom,'yyyy')<=trim('".$this->anohas."') and a.codnom='".$this->codnommin."' and
								a.codcon=b.codcon  and (a.codcon='".$this->codvacmin."' or a.codcon='".$this->codfinmin."' or a.codcon='".$this->codisrlmin."')
								 )a";
...
$asi=0;
				$i++;
			     ...
			     // $acumD=$acumD+$dec->fields['monto'];
			      $this->RowM(array('TOTAL',H::FormatoMonto($acumA),H::FormatoMonto($acumcant),H::FormatoMonto($totaldec),H::FormatoMonto($acumA),H::FormatoMonto($acumD)));
                  $acumA=0;
                  $acumcant=0;
                  $totaldec=0;
                  $acumD=0;
