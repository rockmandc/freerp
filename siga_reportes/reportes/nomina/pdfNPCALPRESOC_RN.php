	...
	class pdfreporte extends fpdf
	{
		...
		function Cuerpo()
		{
			...
			while (!$tb->EOF)
			{
				...
				$this->SetTextColor(0,0,190);//azul
				$this->SetX(10);
				$this->cell(5,5,"TOTAL DIAS DE ANTIGUEDAD DEPOSITADOS:");
				$this->SetTextColor(0,0,0);
				$this->SetX(60);
				$this->cell(20,5,$acumdiasanti,0,0,'C');
				$this->ln(4);
				$this->setFont("Arial","B",7);
				$this->SetTextColor(0,0,190);//azul
				$this->SetX(10);
				$this->cell(5,5,"AJUSTE DIAS ANTIGUEDAD NO DEPOSITADOS:");
				$this->SetTextColor(0,0,0);
				$this->SetX(60);
				$sqldiaant="select coalesce(sum(diaart108),0) as diaant from npimppresoc where codemp='".$tb->fields["codemp"]."' and (tipo='A')";
				$tbdiaant=$this->bd->select($sqldiaant);
				$this->cell(20,5,$tbdiaant->fields['diaant'],0,0,'C');
				//$this->Line(10,$this->GetY()+5,200,$this->GetY()+5);
				$this->ln(4);

				$this->SetTextColor(0,0,190);//azul				
				$this->SetX(10);
				$this->cell(5,5,"AJUSTE DIAS ADICIONALES NO DEPOSITADOS:");
				$this->SetTextColor(0,0,0);
				$this->SetX(60);
				$sqldiaadi="select coalesce(sum(diaart108),0) as diaadi from npimppresoc where codemp='".$tb->fields["codemp"]."' and (tipo='P')";
				$tbdiaadi=$this->bd->select($sqldiaadi);
				$this->cell(20,5,$tbdiaadi->fields['diaadi'],0,0,'C');
				$this->Line(10,$this->GetY()+5,200,$this->GetY()+5);
				$this->ln(6);
				...
				$this->SetTextColor(0,0,0);
				$this->SetX(190);
				$this->cell(5,5,H::FormatoMonto($acumanti),0,0,'R');
				$a1=$acumanti;
				$this->ln(4);
				$this->SetTextColor(0,0,190);//azul
				$this->SetX(10);
				...
				/*$sql2="Select *,to_char(fecfin,'dd/mm/yyyy') as fecfin, fecfin as fec2
							...
							diadif<=0 and salempdia>0 and (valart108>0 or adeant>0)
							order by fec2";*/
				$sql2="Select coalesce(sum(valart108),0) as valant from npimppresoc where trim(codemp)=trim('".$tb->fields["codemp"]."')
				       and tipo='A'";
				$tb2=$this->bd->select($sql2);
				if (!$tb2->EOF)
				{
					$a2=$tb2->fields["valant"];
					$this->cell(5,5,H::FormatoMonto($tb2->fields["valant"]),0,0,'R');
				}
				else
				{
					$a2=0;
				...
				/*$sql2="Select *,to_char(fecfin,'dd/mm/yyyy') as fecfin, fecfin as fec2
							...
							diadif<=0 and salempdia=0 and (valart108>0 or adeant>0)
							order by fec2";*/
				$sql2="Select coalesce(sum(valart108),0) as valadi from npimppresoc where trim(codemp)=trim('".$tb->fields["codemp"]."')
				       and tipo='P'";
				$tb2=$this->bd->select($sql2);
				if (!$tb2->EOF)
				{
					$a3=$tb2->fields["valadi"];
					$this->cell(5,5,H::FormatoMonto($tb2->fields["valadi"]),0,0,'R');
				}
				else
				{
					$a3=0;
			...
			$this->cell(5,5,"",0,0,'C');
			$this->SetX(173);
			$this->cell(5,5,"Director(a) de Recursos",0,0,'C');
			$this->ln(3);
			$this->SetX(97);
			...
			$this->cell(5,5,"",0,0,'C');
			$this->SetX(173);
			$this->cell(5,5,"Autorizado por",0,0,'C');
