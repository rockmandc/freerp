<?
	...
	$this->arrp=array("no_vacio");
			...
			$this->anchos=array();
			$this->anchos2=array();
			$this->fecdesde=H::GetPost("fecdesde");
			$this->fechasta=H::GetPost("fechasta");
			$this->maxcodalm2=H::GetPost("maxcodalm2");
			$this->mincodalm=H::GetPost("mincodalm");
			$this->sql="SELECT
							to_char(A.FECINV,'dd/mm/yyyy') as fecha,
							A.CODALM as codalm,
							B.NOMALM as nomalm,
							A.CODART as codart,
							C.DESART as desart,
							C.COSULT as cosult,
							A.EXIACT as exiact
						FROM CAINVFIS A, CADEFALM B, CAREGART C
						WHERE
							(A.CODALM)=(B.CODALM) AND
							(A.CODART)=(C.CODART) AND
							A.FECINV >= to_date('".$this->fecdesde."','dd/mm/yyyy') AND
							A.FECINV <= to_date('".$this->fechasta."','dd/mm/yyyy')  and
		                   a.codalm>= ('".$this->mincodalm."') and a.codalm<= ('".$this->maxcodalm2."')
						ORDER BY A.CODALM, C.DESART";
		...
		function Cuerpo()
		{
			...
			if(!$tb2->EOF)
			{
				 ...
				 $this->cell(15,-5,$tb2->fields["fecha"]);
				 $this->ln(1);
				 $this->setFont("Arial","B",8);
