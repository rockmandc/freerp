<?
  ...
  $this->arrp=array("no_vacio");
      ...
      B.PTOREO as ptoreo, B.eximin, B.eximax, E.nomubi
      FROM CAREGART A, CAARTALM B, caartalmubi C,CADEFUBI D, CADEFUBI E
      WHERE
      (A.CODART)= (B.CODART) AND
      (A.CODART) >= ('".$this->codesde."') AND
      (A.CODART) <= ('".$this->codhasta."') AND
      (C.CODUBI) = ('".$this->codubides."') AND C.CODUBI=E.CODUBI AND
...
";
    ...
    function Header()
    {
      ...
      $this->cab->poner_cabecera($this,"INVENTARIO",$this->conf,"s");
