  ...
  class pdfreporte extends FPDF
  {
    ...
    function pdfreporte()
    {
     ...
     // $empresa = H::getEmpresa();
	  ...
	  $this->cont=str_replace('*',' ',H::GetPost("cont"));
         $this->cumpl=str_replace('*',' ',H::GetPost("cumpl"));
         $this->penal=str_replace('*',' ',H::GetPost("penal"));
    ...
    //  $this->SetXY(22,56);
	 ...
	 //Vamos a buscar la requsision
      ...
      //Vamos a buscar la Forma de Entrega
...
/////////////////
	  ...
	  $this->SetFont("Arial","B",8);
	  $this->SetXY(40,66);
	  $this->Cell(10,3,'CONTRALORIA MUNICIPAL DE CHACAO - G20000239-5');
	  $this->ln();
	  $this->SetFont("Arial","",7);
	  $this->Cell(10,3,'');
	  $this->Cell(60,3,'Centro Comercial Centro Plaza, Nivel Fco. de Miranda, Torre C, piso 20, Municipio Chacao');
	  $this->ln();
	  $this->Cell(20,3,'');
	  $this->Cell(60,3,'Edo. Miranda',0,0,'C');
	  $this->ln();
	  $this->Cell(20,3,'');
	  $this->Cell(60,3,'Telefonos: 285.32.10 / 34.77',0,0,'C');
	  $this->SetFont("Arial","",7);
         $this->SetXY(130,64);
         $this->Cell(10,3,'  CONDICIONES DE PAGO');
	  $this->SetXY(132,66);
	  //$this->Cell(3,2,'',1,0,'C');
	  $this->SetFont("Arial","",7);
	  ...
	  //$this->Cell(15,3,'Cheque');
         $this->SetXY(170,64);
         $this->Cell(10,3,'FORMA DE ENTREGA');
	  $this->SetXY(168,66);
...
//////////////////DETALLE
	  ...
	  $this->SetFont("Arial","",6);
      $this->SetXY(127,81);
      $this->Cell(90,3,'CANT. SOLIC');
      $this->Line(144,80,144,189);
      $this->SetXY(144,81);
      $this->Cell(90,3,'CANT. AJUST.');
      $this->Line(160,80,160,189);
      $this->SetXY(162,81);
	  $this->SetFont("Arial","",7);
      $this->Cell(90,3,'P. UNITARIO');
	  $this->SetFont("Arial","",6);
      $this->Line(182,80,182,189);
      $this->SetXY(188,81);
      ...
      $this->SetXY(10,196);
      $this->Cell(90,3,'OBSERVACIONES Y/O');
      $this->SetXY(10,200);
      $this->Cell(90,3,'ESPECIFICACIONES:');
	  $this->SetFont("Arial","",9);
 ...
 $this->SetFont("Arial","",7);
      $this->SetXY(11,216);
      $this->MultiCell(60,3,'AL APROBARSE ESTA ORDEN AL BENEFICIARIO FIANZA DE FIEL CUMPLIMIENTO EQUIVALENTE AL ' .$this->cumpl.'% DEL MONTO DE LA ORDEN OTORGADA POR BANCO O COMPAÑIA DE SEGUROS, AUTENTICADA Y VIGENTE HASTA LA TOTAL RECEPCION DE LO SOLICITADO');
      $this->SetXY(78,216);
      $this->MultiCell(65,3,'QUEDA ESTABLECIDO QUE EL PROVEEDOR PAGARA EN CALIDAD DE CLAUSULA PENAL UN '.$this->penal.'% DEL MONTO DE LA PRESENTE ORDEN DE COMPRA POR CADA DIA HABIL DE RETRASO EN LA ENTREGA DE LO SOLICITADO');
      $this->SetXY(153,216);
  ...
  //  $this->line(10,234, 205, 234);
    ...
    $this->SetXY(15,238);
    $this->Cell(60,4,'OFICINA DE PLANIFICACIÓN Y PRESUPUESTO',0,0,'C');
    $this->Cell(60,4,'    DIRECCION DE ADMINISTRACION Y FINANZAS',0,0,'C');
    $this->Cell(60,4,'',0,0,'C');

    $this->line(10,238, 205, 238);//DEBAJO DE OBSERVACIONES


	$this->line(42,241,42,255);//la primera


	 $this->SetLineWidth(0.2);

	$this->line(75,238,75,255);
	$this->SetLineWidth(0.2);

	$this->line(108,241,108,255);

	 $this->SetLineWidth(0.2);

	$this->line(140,238,140,255);

	$this->line(175,241,175,255);//la ultima
	 ...
	 $this->SetLineWidth(0.2);
				...
				$this->Cell(35,4,'DIRECTOR ',0,0,'C');
				$this->Cell(35,4,'ANALISTA ',0,0,'C');
				$this->Cell(30,4,'DIRECTOR ',0,0,'C');
				$this->Cell(35,4,'CONTRALOR MUNICIPAL ',0,0,'C');
                            $this->Cell(30,4,'PROVEEDOR ',0,0,'C');

				$this->setFont("Arial","",6);
				$this->ln();
				$this->Cell(30,4,$this->responsable,0,0,'C');
				$this->Cell(30,4,$this->elaborado,0,0,'C');
				$this->Cell(30,4,$this->conformado,0,0,'C');
				$this->Cell(30,4,$this->aprobado,0,0,'C');
				$this->Cell(52,4,$this->presidencia,0,0,'C');


	     $this->SetXY(10,257);
      $this->Cell(68,3,"FORMA 030",0,0,'L');
      $this->Cell(128,3,"RR /LVF /LVR /FT /CO /LR /IG ",0,0,'R');
     ...
     while($this->i < $eof and $ref==$this->arrp[$this->i]["ordcom"])
      {
        ...
        if($lineas>$lineasrestante){
          ...
          $this->SetFont("Arial","",7);
          $this->SetWidths(array(6,30,55,30,13,14,23,23));
          $this->SetAligns(array('C','C','J','C','C','C','R','R'));
          $paginaantes=$this->PageNo();
          $this->SetX(10);
          //if(strlen(rtrim($this->arrp[$this->i]["desres"]))>95) $this->SetFont("Arial","",6);
          $this->Row(array((string)$contador,substr( $this->arrp[$this->i]["codcat"],6,2).'-'.$this->arrp[$this->i]["codpar1"],rtrim($this->arrp[$this->i]["desres"]),$this->arrp[$this->i]["unimed"],H::FormatoMonto($this->arrp[$this->i]["cantot"],0,0),H::FormatoMonto($this->arrp[$this->i]["canaju"],0,0),H::FormatoMonto($this->arrp[$this->i]["preart"]),H::FormatoMonto(($this->arrp[$this->i]["cantot"]*$this->arrp[$this->i]["preart"]))));
...
//              $tb1=$this->bd->select($this->sql);
		$ivaacum=0;
         ...
         // $this->Cell(10,5,'TOTAL:');
          ...
          $this->SetXY(48,189);
		  $this->SetFont("Arial","",7);
          $this->MultiCell(95,3,H::obtenermontoescrito($subtotal+$ivaacum),0,'J',0);
          $this->SetXY(174,153);

          $this->SetFont("Arial","",7);
          $this->SetXY(45,196);
          $this->multiCell(160,2.5,$observ);
          $this->SetFont("Arial","",7);
          $subtotal2=0;
