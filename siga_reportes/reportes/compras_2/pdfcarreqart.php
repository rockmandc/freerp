<?
	...
	$this->arrp=array("no_vacio");
			...
			$this->status=H::GetPost("status");
				...
				$this->sql="SELECT A.REQART as areqart, to_char(A.FECREQ,'DD/MM/YYYY') as afecreq, A.DESREQ as adesreq, B.CODCAT as bcodcat, E.NOMCAT as enomcat, A.MONREQ as amonreq, A.aprreq as STAREQ,
				...
				if ($this->status=='R' or $this->status=='S')

			{
				$this->sql="SELECT A.REQART as areqart, to_char(A.FECREQ,'DD/MM/YYYY') as afecreq, A.DESREQ as adesreq, B.CODCAT as bcodcat, E.NOMCAT as enomcat, A.MONREQ as amonreq, A.aprreq as STAREQ,
							...
							upper(A.aprreq) ='".strtoupper($this->status)."' ORDER BY A.REQART";
...

			{
				$this->sql="SELECT A.REQART as areqart, to_char(A.FECREQ,'DD/MM/YYYY') as afecreq, A.DESREQ as adesreq, B.CODCAT as bcodcat, E.NOMCAT as enomcat, A.MONREQ as amonreq, A.STAREQ as STAREQ,
							...
							A.FECREQ >= to_date('".$this->fecreg1."','dd/mm/yyyy') AND
							A.FECREQ <= to_date('".$this->fecreg2."','dd/mm/yyyy') AND
							upper(A.aprreq) is null ORDER BY A.REQART";
			...
			function Cuerpo()
		{
			...
			if(!$tb2->EOF)
			{
				$ref=$tb2->fields["areqart"];
				 ...
				 if	($tb2->fields["stareq"] == 'R')
				 {
				 $estado="Rechazada";
				 }
				 elseif	($tb2->fields["stareq"] == 'S')
				 {
				 $estado="Aprobada";
				 }
				 else
