	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->sql="select
						...
						numche,
						ctaban,
						to_char(fecpag,'dd/mm/yyyy') as fecpag,
					...
					where
						...
						(trim(ctaban) >= trim('".$this->bandes."') or ctaban is null) and
						(trim(ctaban) <= trim('".$this->banhas."') or ctaban is null) and
		...
		function Header()
		{
			...
			$this->cell(50,5,'Beneficiario');
			$this->cell(60,5,'Concepto');
			$this->cell(15,5,'Fecha',0,0,'R');
			$this->cell(15,5,'Numero',0,0,'R');
			$this->cell(15,5,'Fecha',0,0,'R');
			...
			$this->cell(50,5,' ');
			$this->cell(60,5,' ');
			$this->cell(15,5,'Emision',0,0,'R');
			$this->cell(15,5,'Cheque',0,0,'R');
			$this->cell(15,5,'Cheque',0,0,'R');
			$this->cell(30,5,'Cuenta Bancaria',0,0,'R');
			//$this->cell(30,5,' ',0,0,'R');
			$this->cell(20,5,'Original',0,0,'R');
			$this->cell(20,5,'Retenido',0,0,'R');
		    $this->ln(4);
			$this->line(10,$this->getY(),270,$this->getY());
		}
		function Cuerpo()
		{
		    ...
		    $tb=$this->bd->select($this->sql);
			 ...
			 while(!$tb->EOF){
					...
					$this->SetY($y);
					 ...
					 $this->cell(60,4," ");
					 $this->cell(52,4," ");
					 $this->cell(15,4,$tb->fields["fecemi"],0,0,'R');
					 $this->cell(15,4,$tb->fields["numche"]);
					 $this->cell(15,4,$tb->fields["fecpag"]);
					 $this->cell(30,4,$tb->fields["status1"],0,10,'C');
					 $this->cell(30,4,$tb->fields["ctaban"]);
					 $this->cell(20,4,number_format($tb->fields["monord"],2,'.',','),0,'R',0);
					 $this->cell(20,4,number_format($tb->fields["monret"],2,'.',','),0,'R',0);
					 $tot=$tb->fields["monord"]-$tb->fields["monret"]-$tb->fields["mondes"];
					 $this->cell(20,4,number_format($tot,2,'.',','),0,'R',0);

					 $this->setXY(25,$y);
					 $this->Multicell(50,4,substr($tb->fields["nomben"],0,50)); //Beneficiario
					 $this->setY($y);
					 $this->setX(75);
					 $this->Multicell(60,4,$tb->fields["desord"],0,50); //Concepto

					 //Acumuladores//
					 	 $sum_canord=$sum_canord+1;
			...
			// Impresion de Totales
					 $this->cell(3,6," ");
					 $this->cell(27,6,'Total Ordenes ===>');
					 $this->cell(20,6,number_format($sum_canord,0,'',','),0,'R',0);
					 $this->cell(125,6," ");
					 $this->cell(27,6,'Totales Bolivares ==>');
					 $this->cell(20,6,number_format($sum_monord,2,'.',','),0,'R',0);
					 $this->cell(20,6,number_format($sum_monret,2,'.',','),0,'R',0);
					 $this->cell(20,6,number_format($sum_tot,2,'.',','),0,'R',0);
			//
		}
	}
?>