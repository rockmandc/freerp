...
{
      mysql='select distinct a.numord as Orden,b.nomben as Proveedor,to_char(a.fecemi,¿dd/mm/yyy¿) as Fecha from opordpag a,opbenefi b where a.cedrif=b.cedrif ';
	  pagina="../../lib/general/catalogoobj.php?sql="+mysql+"&campo="+campo;
	  window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=400,resizable=yes,left=50,top=50");
}
function catalogo2(campo)
{
      mysql='SELECT distinct CEDRIF as cedula, NOMBEN as nombre FROM opbenefi ORDER BY (CEDRIF) ASC';
	  pagina="../../lib/general/catalogoobj.php?sql="+mysql+"&campo="+campo;
	  window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=400,resizable=yes,left=50,top=50");
}
...
{
    pagina="../../lib/general/catalogoobj.php?campo="+campo+"&sql=SELECT DISTINCT(CODPRE) as Codigo FROM cpimpcau order by Codigo limit 450";
