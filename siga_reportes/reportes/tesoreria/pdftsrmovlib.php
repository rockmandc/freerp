	...
	class pdfreporte extends fpdf
	{
		...
		function Cuerpo()
		{
			...
			$this->sql3="select  a.numcue,(b.tipmov) as tipmov,to_char(b.feclib,'dd/mm/yyyy') as feclib,c.orden,b.reflib,c.destip,
					case when c.codtip like 'CH%' then coalesce(f.nomben||' - ','')||b.deslib  else b.deslib end as deslib,
					substr(to_char(b.feclib,'dd/mm/yyyy'),4,2) as mes,
					b.status, b.stacon, coalesce(b.monmov,0) as mon, c.debcre
					from tsdefban a, tstipmov c,tsmovlib b left outer join tscheemi d on(b.reflib like d.numche||'%' and d.numcue = '".$this->numcue."' )
			...
			$this->sql3="select  a.numcue,(b.tipmov) as tipmov,to_char(b.feclib,'dd/mm/yyyy') as feclib,c.orden,b.reflib,c.destip,
					...
					where
					b.numcue = '".$this->numcue."' and
					trim(b.tipmov) >= trim('".$this->mov1."')  and trim(b.tipmov) <= trim('".$this->mov2."') and trim(b.tipmov) <> trim('SAL')  and
					(b.feclib >= to_date('".$this->fecham1."','dd/mm/yyyy')) and (b.feclib <= to_date('".$this->fecham2."','dd/mm/yyyy')) and
					b.reflib >= '".$this->ref1."' and b.reflib <= '".$this->ref2."' and
					b.reflibpad <> ''  and
					a.numcue=b.numcue and b.tipmov=c.codtip
...
# Reporte de movimientos según libros donde se muestren totales por tipo de movimiento, primero los movimientos que suman al saldo y después los movimientos que restan y al final el saldo actual.
						...
						$this->ln();
							...
							$this->ln();
								if($tb4->EOF)
								{
									$this->cell(60,5,"SALDO ANTERIOR");
									$this->line(100,$this->GetY(),190,$this->GetY());
									$this->SetX(150);
									$this->cell(35,5,number_format($this->ANTERIOR,2,'.',','),0,0,'R');
									$this->ln();
								}
							$this->ln();
					...
					//-----------------------
							$this->sql4="select substr(to_char(a.feclib,'dd/mm/yyyy'),4,2) as mesmov,
									...
									where
									(substr(to_char(a.feclib,'dd/mm/yyyy'),4,2))= '".$this->mes."' and
									trim(a.tipmov) >= trim('".$this->mov1."')  and trim(a.tipmov) <= trim('".$this->mov2."') and  trim(a.tipmov)<>'SAL'
									and
							...
							$this->ln();
								if($tb4->EOF)
								{
									$this->cell(120,5,"SALDO ANTERIOR");
									$this->line(100,$this->GetY(),190,$this->GetY());
									$this->SetX(150);
									$this->cell(40,5,number_format($this->ANTERIOR,2,'.',','),0,0,'R');
								
									$this->ln();
								}
							$this->Line(10,$this->GetY(),200,$this->GetY());
							$this->cell(84,5,"");
							$this->cell(18,5,"TOTALES");
							$this->sql5="select sum(a.monmov) as mondeb
										from tsmovlib a,tstipmov b
										where
										a.numcue = '".$this->numcue."' and
										b.debcre='D' and
										trim(a.tipmov) >= trim('".$this->mov1."')  and trim(a.tipmov) <= trim('".$this->mov2."') 
										and trim(a.tipmov) <> trim('SAL')  and
							...
							$this->cell(28,5,number_format($totdeb,2,'.',','),0,0);
							...
							$this->cell(28,5,number_format($totcre,2,'.',','),0,0);


							$this->SetX(170);
							$tottotgen+=$this->ANTERIOR+$totdeb-$totcre;				
							
							//$this->cell(30,5,H::FormatoMonto(abs($monto)),0,0,'R');
