<?
	...
	require_once("../../lib/modelo/sqls/tesoreria/Tsrchetran.class.php");
	...
	class pdfreporte extends fpdf
	{
		function pdfreporte()
		{
			...
			$this->objTsrchetran = new Tsrchetran();
			$this->arrp = $this->objTsrchetran->SQLp($this->numchedes,$this->numchehas,$this->numcuedes,$this->numcuehas,$this->bendes,$this->benhas,$this->fecregdes,$this->fecreghas);
			$this->arrp3=$this->objTsrchetran->sqlpx($this->numchedes,$this->numchehas);
			//$this->cargarEstatus();
			$this->cab=new cabecera();
		}
		...
		function header()
		{
			...
			$ubica = count($parte)-2;
			$this->SetLineWidth(0);
			$this->cab->poner_cabecera_f($this,"","p","s","n");
             $this->sety(30);
			$this->cell(0,7,$_POST["titulo"],0,0,'C');
			$this->ln();
			$this->cell(0,7,"DESDE: ".$this->fecregdes."  HASTA: ".$this->fecreghas,0,0,'C');
			$this->ln();
			$this->ln();
			...
			$this->ln();
	  		...
	  		$this->setTextColor(155,0,0);
	  		$this->setFont("Arial","B",7);
	  		$this->cell(30,4,"ASIENTO Nro",1,0,'C');
	  		$this->cell(25,4,"FECHA",1,0,'C');
	  		$this->cell(28,4,"REFERENCIA",1,0,'C');
...
//Reporte Original
			...
			$this->setWidths(array(0,30,25,28,75,30));
			$this->setAligns(array("L","C","C","C","L","R"));
			$this->setBorder(array(1,1,1,1,1,1));
			...
			foreach($this->arrp as $objtb)
			{
				...
				$y2=$this->GetY();
				$this->Row(array('',(string)$contador,date("d/m/Y",strtotime($objtb["afecemi"])),$objtb["anumche"],trim($objtb["anomben"]),H::FormatoMonto($objtb["amonche"])));
			 ...
			 $this->SetLineWidth(0.5);
			 $this->ln();
			 $this->Line(142,$this->GetY(),198,$this->GetY());
	 		 $this->setFont("Arial","B",7);
			 $this->cell(10,4);
			 $this->cell(130,4,"TOTAL CHEQUES POR BANCO:         ".$this->cont);
			 $this->cell(48,4,"TOTAL BANCO                        ".H::FormatoMonto($this->acum2),0,0,'R');
