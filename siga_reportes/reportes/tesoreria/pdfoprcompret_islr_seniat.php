		...
		function pdfreporte()
		{
			...
			$this->diradm=$_POST["diradm"];
				...
				$this->sql="select a.numord,b.cedrif,b.nomben,to_char(b.fecemi,'dd/mm/yyyy') as fecemi,to_char(a.fecfac,'dd/mm/yyyy') as fecfac,a.numfac,
	                        ...
	                        from opfactur a,opordpag b left outer join tsmovlib c on b.numche=c.reflib  and b.cedrif=c.cedrif
							where
...
//-----------------------------------------------------------------------------------------------------------------------//
		$this->sql1="select comretislr,feccomretislr, to_char(feccomretislr,'dd/mm/yyyy') as feccomretc from opordpag where numord='".$this->orde."'";
	...
	//----GENERAR CORRELATIVO
		...
		$this->setxy(71,190);
		$this->cell(85,5,"Director de Administración y Finanzas");
		$this->line(75,187,120,187);
		$this->setxy(146,190);
//		$this->cell(30,5,"Contralor Municipal");
//		$this->line(140,187,180,187);
		$this->setxy(135,187);
		...
		function Header()
		{
			...
			$this->setFont("Arial","B",9);
			$this->cell(260,5,"COMPROBANTE DE RETENCION del I.S.L.R ",0,0,'C');
			$this->ln();
	...
	//	 $this->cell(65);
		...
		while (!$this->tb->EOF)
		{
			if ($this->tb->fields["numord"]!=$ref)///	NO ESTA HACIENDO NADA
			{
    	...
    	$this->Row(array(number_format($cont),$this->tb->fields["fecemi"],$this->tb->fields["numord"],$this->tb->fields["fecfac"],$this->tb->fields["numfac"],$this->tb->fields["numctr"],H::FormatoMonto($this->tb->fields["totfac"]),H::FormatoMonto($this->tb->fields["basimp"]),H::FormatoMonto($this->tb->fields["porislr"]),H::FormatoMonto($this->tb->fields["monislr"])));
		...
		$this->cell(27,5,H::FormatoMonto($basimp),0,0,"R");
		$this->ln();
