        ...
        <div align="left">&nbsp;
          <table width="647" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            <tr bordercolor="#FFFFFF">
              <td colspan="3" class="form_label_01"> <div align="center"> <font color="#000066" size="4"><strong>Ordenes de Pago Emitidas con Datos Bancarios
            ...
            <tr bordercolor="#6699CC">
              <td class="form_label_01"><strong>N&uacute;mero Orden  de Pago: </strong></td>
              <td><div align="left"> </div>
                  <div align="left">
				...
				?>" class="breadcrumb" id="numorddes" size="20" maxlength="20">
                    <input type="button" name="Button" value="..." onClick="catalogo('numorddes');">
                </div></td>
              <td colspan="2"><input name="numordhas" type="text" value="<?
				$sql="Select max(numord) as numord from opordpag";
                LlenarTextoSql($sql,"numord",$bd);
				?>" class="breadcrumb" id="numordhas" size="20" maxlength="20">
                  <input type="button" name="Button2" value="..." onClick="catalogo('numordhas');"></td>
            </tr>
            <tr>
              <td class="form_label_01"><strong>C.I./RIF del Beneficiario:</strong></td>
              <td colspan="2">
                  <select name="bendes" class="breadcrumb" id="bendes">
                    <?
					$tb2=$bd->select("select distinct cedrif,nomben from opordpag order by cedrif asc");
				  	while(!$tb2->EOF)
					{
				  ?>
                    <option value="<? print $tb2->fields["cedrif"];?>"><? print substr($tb2->fields["cedrif"]." - ".$tb2->fields["nomben"],0,45);?></option>
                    <?
				  		$tb2->MoveNext();
					}
				  ?>
            ...
            <tr>
              <td class="form_label_01">&nbsp;</td>
              <td colspan="2"><select name="benhas" class="breadcrumb" id="benhas">
                <?
					$tb2=$bd->select("select distinct cedrif,nomben from opordpag order by cedrif desc");
				  	while(!$tb2->EOF)
					{
				  ?>
                <option value="<? print $tb2->fields["cedrif"];?>"><? print substr($tb2->fields["cedrif"]." - ".$tb2->fields["nomben"],0,45);?></option>
            ...
            <tr>
              <td class="form_label_01"><strong>Cuenta Bancaria:</strong></td>
              <td><select name="bandes" class="breadcrumb" id="bandes">
                  <?
					$tb2=$bd->select("SELECT  distinct ctaban FROM opordpag ORDER BY ctaban asc");
				  	while(!$tb2->EOF)
					{
				  ?>
                  <option value="<? print $tb2->fields["ctaban"];?>"><? print $tb2->fields["ctaban"];?></option>
                  <?
				  		$tb2->MoveNext();
					}
				  ?>
              </select></td>
              <td><select name="banhas" class="breadcrumb" id="banhas">
                  <?
					$tb2=$bd->select("SELECT  distinct ctaban FROM opordpag ORDER BY ctaban desc");
				  	while(!$tb2->EOF)
					{
				  ?>
                  <option value="<? print $tb2->fields["ctaban"];?>"><? print $tb2->fields["ctaban"];?></option>
            ...
            <tr>
              ...
              <td colspan="2"><select name="estatus" class="breadcrumb" id="estatus">
				...
				<option value="C">En Contralorias</option>
				<option value="PP">Pendiente por Pagar</option>
				<option value="E">Elaborada</option>
              </select></td>
            </tr>
            <tr bordercolor="#6699CC">
...
{
	f=document.form1;
	f.titulo.value="Listado de Ordenes de Pagos Emitidas";
	f.action="roprordpag.php";
	f.submit();
}
function cerrar()
{
