<?
	...
	class pdfreporte extends fpdf
	{
		...
		function Header()
		{
			...
			$this->SetY(20);
			$fecha=date("d/m/Y");
      		       //$this->Cell(470,5,'',0,0,'C');
			$this->ln(14);
			$this->setFont("Arial","B",9);
			$this->Cell(20,5,'NOMBRE DEL ENTE PUBLICO: CONTRALORIA MUNICIPAL DE CHACAO');
			$this->ln();
			$this->Cell(200,5,'DIRECCION: AV. FRANCISCO DE MIRANDA, URBANIZACION LOS PALOS GRANDES, EDIFICIO CENTRO PLAZA, TORRE C, PISO 20, OFICINA UNICA, ALTAMIRA.');
			$this->ln();
			...
			$this->setFont("Arial","B",9);
            $y=$this->gety();
			$this->multiCell(25,3,'Fecha Orden de Pago',0,'C');
			$this->setxy(35,$y);
			$this->multiCell(25,3,'Número Orden de Pago',0,'C');
			$this->setxy(55,$y);
			$this->multiCell(60,3,'Beneficiario',0,'C');
			$this->setxy(110,$y);
			$this->multiCell(30,3,'C.I O RIF',0,'C');
			$this->setxy(140,$y);
			$this->multiCell(30,3,'Monto Bruto de la Orden de Pago',0,'C');
			$this->setxy(170,$y);
			$this->multiCell(30,3,'Monto de Retención',0,'C');
			$this->setxy(200,$y);
			$this->multiCell(30,3,'Monto Neto de la Orden de Pago',0,'C');
			$this->setxy(235,$y);
			$this->multiCell(30,3,'Status ',0,'C');

			/*$this->Cell(25,3,'  de Pago');
			$this->Cell(33,3,'  de Pago');
			$this->Cell(50,3,'');
			$this->Cell(30,3,'');
			$this->Cell(30,3,'la Operación');
			$this->Cell(30,3,' Retención');
                     $this->Cell(30,3,'la Operación - Retención');
...
$this->i=0;
			...
			foreach($this->arrp as $op)
			{
				...
				$this->Row(array('',date("d/m/Y",strtotime($op["fecemi"])),$op["numord"],trim($op["nomben"]),'',($op["cedrif"]),H::FormatoMonto($op["monord"]),
H::FormatoMonto($op["monret"]), H::FormatoMonto($op["monord"]-$op["monret"]),trim($op["status"])));
			    ...
			    $monnet=$monnet+($op["monord"]-$op["monret"]);
			...
			$this->Cell(30,3,H::FormatoMonto($monret),0,'R',0);
				$this->SetX(200);
			$this->Cell(30,3,H::FormatoMonto($monnet),0,'R',0);
			$this->ln(25);
			$this->SetXY(20,185);
