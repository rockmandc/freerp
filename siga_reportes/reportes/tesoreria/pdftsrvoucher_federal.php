<?
	...
	/*		$this->rect(20,205,170,60);
		...
		function Cuerpo()
		{
			...
			foreach($this->arrp as $cheque)//while 
			{
				...
				$this->SetXY(30,39);
				$this->MultiCell(130,5,(str_pad(H::obtenermontoescrito($cheque["monche"]),75, "*",STR_PAD_RIGHT)));
        ...
        foreach($tbdesord as $desor)
        {
				...
				$this->sqlparch="select sum(monord) as monto from opordpag where numche='".$cheque["numche"]."'  and status='I' ";
	...
	//parche
				if ($this->ret>0 and $total==$cheque["monche"])
				{
				$this->cell(23,5,number_format($total,2,',','.'),0,0,'R');
				$this->SetXY(23,240);
				$this->cell(143,5,"",0,0,'C');
				...
				$this->SetXY(23,245);
				$this->cell(143,5,"",0,0,'C');
				$this->cell(23,5,number_format($total-$this->ret,2,',','.'),0,0,'R');
				}else
				{
				//$this->cell(23,5,number_format($cheque["monche"],2,',','.'),0,0,'R');
				$this->cell(23,5,number_format($this->parch,2,',','.'),0,0,'R');
				$this->SetXY(23,240);
				$this->cell(143,5,"",0,0,'C');
				$this->cell(23,5,number_format($this->ret,2,',','.'),0,0,'R');
                            $this->auxd=$total-$cheque["monche"];
