<?
	...
	class pdfreporte extends fpdf
	{
		...
		function Cuerpo()
		{
			...
			$fechacortada=substr($this->fecha,3,10);
			$mescon=substr($this->fecha,3,2);
			$fechacompleta='01/'.$fechacortada;
				$this->sql2="select rtrim(a.numcue) as numcue,a.refere,
					...
					a.result,c.debcre
					from tsconcilhis a,tstipmov c
					where rtrim(CASE WHEN a.monlib=0 THEN a.movban ELSE a.movlib END)=rtrim(c.codtip) and rtrim(a.movlib) <> 'SAL' and
					rtrim(a.numcue) = rtrim('".$this->numcue."') and

					--****(CASE WHEN a.monlib=0 THEN a.fecban ELSE a.feclib END) >= to_date('".$fechacompleta."','dd/mm/yyyy') and***
					(CASE WHEN a.monlib=0 THEN a.fecban ELSE a.feclib END)  <= to_date('".$this->fecha."','dd/mm/yyyy') and
					rtrim(UPPER(a.result)) <> 'CONCILIADO' and
					a.mescon='".$mescon."'
					...
					$this->ln(7);
						if($this->gety()>130){
						$this->addpage();
							$this->Line(10,$this->GetY()+9,200,$this->GetY()+9);
					}else
					$this->Line(10,$this->GetY()+2,200,$this->GetY()+2);
					$this->ln();
					...
					$this->cell(18,5,number_format($this->bancost,2,',','.'),0,0,'R');
					$this->ln(2);
					$this->cell(39,5,"");
					$this->cell(5,5,"+");
					$this->ln(-2);
					...
					$this->cell(18,5,number_format($this->librost,2,',','.'),0,0,'R');
					$this->ln(2);
					$this->cell(80,5,"");
					$this->cell(5,5,"+");
					$this->ln(-2);
					...
					//------
					$this->ln();
					$this->cell(19,5,"PNC Bancos");
					$this->cell(18,5,number_format(abs($this->acumban2),2,',','.'),0,0,'R');
					$this->ln(3);
					$this->ln(-3);
					$this->cell(43,5,"");
					$this->cell(17,5,"PNC Libros");
					$this->cell(18,5,number_format(abs($this->acumlib2),2,',','.'),0,0,'R');
					$this->SetLineWidth(0.2);
					...
					$this->cell(18,5,number_format($totalb,2,',','.'),0,0,'R');
					$this->ln(3);
					$this->ln(-3);
					$this->cell(43,5,"");
					$this->cell(17,5,"Total Libros");
					$this->cell(18,5,number_format($totall,2,',','.'),0,0,'R');
					$this->ln(-3);
					$this->cell(92,5,"");
					...
					$this->cell(5,5,"=");
					$dif=abs($totalb-$totall);
					$this->cell(21,5,number_format($dif,2,',','.'));
					$this->ln(3);

					//AKIIIIIIIIIIIIIIIIIII VOYYYYYYYYYYYYYYYYYYYYY!!!!!!!!
					//------
					$this->Line(10,$this->GetY()-11,10,$this->GetY()+7);
					$this->Line(200,$this->GetY()-11,200,$this->GetY()+7);
