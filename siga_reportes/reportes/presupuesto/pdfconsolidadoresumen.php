<?
	...
	$this->SetTextColor(0,0,0);
	 ...
	 //$this->cab->poner_cabeceraT($this,H::GetPost("titulo"),"p","s");
	 ...
	 $this->ln();
	 $this->Line($this->GetX(),$this->GetY()+1,$this->GetX()+180,$this->GetY()+1);
	 $this->ln(6);
	 $this->SetWidths(array(15,15,20,20,20,20,40,20,20));
     $this->SetAligns(array("L","L","C","C","C","C","L","R","R"));
    }
     ...
     foreach($this->arrp as $datos)
	 {
      ...
      /* if ($t>=45 and $t<=55){
	   ...
	   $this->ln(7);
		...
		$this->RowM(array($datos2["fecmov"],$datos2["tipmov"],$pre,$com,$cau,$pag,$datos2["bene"]." ".$datos2["cedrif"],H::FormatoMonto($datos2["monmov"]),H::FormatoMonto($disponible)));
		//Sumatorias
	...
	$this->setFont("Arial","B",9);
	$this->SetX(15);
	$this->cell(10,6,$datos["codpreasiini"]);
	$this->SetX(60);
	$this->multicell(110,6,'Disponible en : '. $datos["nompreasiini"]);
	$this->SetTextColor(0,0,0);
   ...
   //H::PrintR($cf_dismin);
	...
	$this->Cell(70,3,'TOTAL COMPROMETIDO:    '.H::FormatoMonto($cf_totcompro),0,0,'J');
	$this->Cell(70,3,'TOTAL AJUSTES COMPROMISOS:    '.H::FormatoMonto($cf_ajucom[0][0]),0,0,'J');
	$this->Cell(70,3,'TOTAL COMPR. AJUSTADOS:    '.H::FormatoMonto($cf_comaju),0,0,'J');
	$this->Ln(6);
	$this->SetX(10);
	$this->Cell(70,3,'TOTAL CAUSADO:    '.H::FormatoMonto($cf_totcau),0,0,'J');
	$this->Cell(70,3,'TOTAL AJUSTES CAUSADOS:    '.H::FormatoMonto($cf_ajucau[0][0]),0,0,'J');
	$this->Cell(70,3,'TOTAL CAUSA AJUSTADOS:    '.H::FormatoMonto($cf_cauaju),0,0,'J');
	$this->Ln(6);
	$this->SetX(10);
	$this->Cell(70,3,'TOTAL PAGADOS:    '.H::FormatoMonto($cf_pagado[0][0]),0,0,'J');
	$this->Cell(70,3,'TOTAL AJUSTES PAGADOS:    '.H::FormatoMonto($cf_ajupag[0][0]),0,0,'J');
	$this->Cell(70,3,'TOTAL PAGOS AJUSTADOS:    '.H::FormatoMonto($cf_pagaju),0,0,'J');
	$this->Ln(6);
	$this->SetX(10);
	$this->Cell(70,3,'TOTAL TRANSFERENCIAS:    '.H::FormatoMonto($cf_trades[0][0]),0,0,'J');
	$this->Cell(70,3,'TOTAL ADICIONES:    '.H::FormatoMonto($cf_adidis),0,0,'J');
	$this->Cell(70,3,'TOTAL ASIGNACION REAL:    '.H::FormatoMonto($cf_asireal),0,0,'J');
   }
 }
?>