...
$this->tpartida=$tpartida+$inipartida-1;

$this->sql="select (case when length(b.codpre)>$this->tpartida then partida else '' end) as partida,B.CODPRE,SUM(asignacion) AS ASIGNACION,SUM(PRECOMPROMISO) AS PRECOMPROMISO,SUM(COMPROMISO) AS COMPROMISO, SUM(CAUSADO) AS CAUSADO, SUM(PAGADO) AS PAGADO, SUM(MODIFICADO) AS MODIFICADO  FROM (
    SELECT ".$parchesql." MIN(CPDEFTIT.NOMPRE) AS NOMPRE, SUM(MONASI) AS ASIGNACION,SUM(PRECOMPROMISO) AS PRECOMPROMISO,SUM(COMPROMISO) AS COMPROMISO, SUM(CAUSADO) AS CAUSADO, SUM(PAGADO) AS PAGADO, SUM(MODIFICACION) AS MODIFICADO  FROM
      ...
      EJECUCION, CPDEFTIT WHERE EJECUCION.CODPRE=CPDEFTIT.CODPRE AND EJECUCION.CODPRE>=('".$this->codpredes."') AND EJECUCION.CODPRE<=('".$this->codprehas."') AND EJECUCION.CODPRE LIKE '".$this->comodin."'
       ...
       ORDER BY ".$parchesqlg." )a, cpdeftit b where
            ...
            group by b.codpre,(case when length(b.codpre)>$this->tpartida then partida else '' end)
            ORDER BY b.codpre";
...
//			$this->bd->actualizar('SET search_path TO "SIMA055"');
      ...
      while(!$tb->EOF)
      {
        ...
        $this->setFont("Arial","",7);
         if ($partida!=$tb->fields["partida"] and $partida!="-1" and strlen($tb->fields["codpre"])>=$this->tpartida  and $partida!="")
         {
            $this->ln();
      $this->Line(10,$this->GetY(),340,$this->GetY());
      $this->cell(78,7,"TOTAL PARTIDA " .$partida,0,0,'R');
      $this->cell(28,7,H::FormatoMonto($totalasipar),0,0,'R');
      $this->cell(28,7,H::FormatoMonto($totalmodpar),0,0,'R');
      $this->cell(28,7,H::FormatoMonto($totalasipar+$totalmodpar),0,0,'R');
      $this->cell(28,7,H::FormatoMonto($totalprcpar),0,0,'R');
      $this->cell(28,7,H::FormatoMonto($totalasipar+$totalmodpar-$totalprcpar),0,0,'R');
      $this->cell(28,7,H::FormatoMonto($totalcompar),0,0,'R');
      $this->cell(28,7,H::FormatoMonto($totalcaupar),0,0,'R');
      $this->cell(28,7,H::FormatoMonto($totalpagpar),0,0,'R');
      $this->cell(28,7,H::FormatoMonto($totalcaupar-$totalpagpar),0,0,'R');
      $this->Line(10,$this->GetY()+6,340,$this->GetY()+6);
           $this->ln();
     ...
     $totalejepar=0;
        ...
        //filtro de asigacion acumulado o parcial
      ...
      {
        //--parcial select sum(monasi) from cpasiini where codpre='01-03-51-401-01-09-00' and perpre>='01' and perpre<='04'
        $this->sqlasi="select sum(monasi) as monto from cpasiini where codpre like '".$tb->fields["codpre"]."%' and perpre>='".$this->perdesde."' and perpre<='".$this->perhasta."' and codpre LIKE '".$this->comodin."' and CODPRE>=('".$this->codpredes."') AND CODPRE<=('".$this->codprehas."')";
       // print $this->sqlasi;
        $tbasi=$this->bd->select($this->sqlasi);
        $tb->fields["asignacion"]=$tbasi->fields["monto"];
      $this->perasi="12";
      }
        ////
...
//print '<pre>'; print $tbcod;
        ...
        $acordado=$tb->fields["asignacion"]+$tb->fields["modificado"];
         ...
         $totalprcpar=$totalprcpar+$tb->fields["precompromiso"];
            $totalcompar=$totalcompar+$tb->fields["compromiso"];
            $totalcaupar=$totalcaupar+$tb->fields["causado"];
            $totalpagpar=$totalpagpar+$tb->fields["pagado"];
             $totalmodpar=$totalmodpar+$tb->fields["modificado"];
            $totalasipar=$totalasipar+$tb->fields["asignacion"];
            $totalacopar=$totalacopar+$tb->fields["asignacion"]+$tb->fields["modificado"];
            $totaldispar=$totaldispar+$tb->fields["asignacion"]+$tb->fields["modificado"]-$tb->fields["compromiso"];
             //$totaldeu=$totaldeu+$tb->fields["causado"]-$tb->fields["pagado"];
        $acordado=$tb->fields["asignacion"]+$tb->fields["modificado"];
      ...
      $this->setFont("Arial","B",6.5);
             // if ( $partida!="-1" and strlen($tb->fields["codpre"])>=$this->tpartida  and $partida!="")
         {
      $this->ln();
      $this->Line(10,$this->GetY(),340,$this->GetY());
      $this->cell(78,7,"TOTAL PARTIDA " .$partida,0,0,'R');
