...
//			$this->bd->validar();
	     ...
	     $tb1_1=$this->bd->select("SELECT NOMEXT  FROM CPNIVELES WHERE CONSEC='".$this->nivhasta."'");
$this->part=$tb1_1->fields["nomext"];

	/*	while(!$tb1_1->EOF)
			{
			$categoria=$tb1_1->fields["nomext"];
			$tb1_1->MoveNext();
			}*/
...
//H::printR($this->sql);exit;
		...
		function llenartitulosmaestro()
		{
				...
				$this->titulos[9]="COMPROMISOS";
				$this->titulos[10]="CAUSADO";
				$this->titulos[11]="PAGADO";
				$this->titulos[12]="DEUDA";
				$this->titulos[13]="DISPONIBILIDAD PRESUPUESTARIA";
		}
		...
		function Header()
		{
         ...
         $dig=substr($this->agrupar,0,1);


			$this->cab->poner_cabecera_f_b($this,"EJECUCIÓN PRESUPUESTARIA POR ".$this->NomCatPre($dig)." - ".$this->NomCatPre($this->nivhasta),"l","s","s");
...
//			$this->bd->actualizar('SET search_path TO "SIMA055"');
			...
			while(!$tb->EOF)
			{
				...
				if (($partida!=$tb->fields["partida"] && $partida!="-1" ) || ($categoria!=$tb->fields["categoria"])&&($categoria!="-1"))
				{
				    if (!empty($tb->fields["partida"]))
				    {
				    	$this->setFont("Arial","B",7);
						$this->ln(2);
						$this->cell(64,4,"SubT. ".$partida." ",0,0,'R');
				...
				if (($categoria!=$tb->fields["categoria"])&&($categoria!="-1") )
				{
				    $this->setFont("Arial","B",7);
					$this->ln(2);
					$this->cell(64,4,"SUB-TOTAL ".$categoria." ",0,0,'R');
				...
				$this->multicell(38,4,(strtoupper($tbcod->fields["nompre"])));
			...
			$this->cell(64,7,"TOTAL GENERAL",0,0,'R');
			...
			$this->cell(22,7,H::FormatoMonto($totaldis),0,0,'R');
