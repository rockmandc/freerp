...
//print $this->sql;exit;
		...
		function Header()
		{
			...
			$this->cell(270,4,"EJECUCIÓN PRESUPUESTARIA POR ESPECÍFICA",0,0,'C');
			$this->setFont("Arial","B",7);
			$this->setY($y);
...
//			$this->bd->actualizar('SET search_path TO "SIMA055"');
			...
			while(!$tb->EOF)
			{
				...
				if (($partida!=$tb->fields["partida"] && $partida!="-1" ) || ($categoria!=$tb->fields["categoria"])&&($categoria!="-1"))
				{
				    if (!empty($tb->fields["partida"]))
				    {
				    	$this->setFont("Arial","B",7);
						$this->ln(2);
						$this->cell(73,4,"SubT. ".$partida." ",0,0,'R');
						$this->cell(25,4,H::FormatoMonto($totalasipar),0,0,'R');
						$this->cell(23,4,H::FormatoMonto($totalmodpar),0,0,'R');
						$this->cell(23,4,H::FormatoMonto($totalacopar),0,0,'R');
						$this->cell(23,4,H::FormatoMonto($totalprcpar),0,0,'R');
						$this->cell(23,4,H::FormatoMonto($totaldispar),0,0,'R');
						$this->cell(23,4,H::FormatoMonto($totalcaupar),0,0,'R');
						$this->cell(23,4,H::FormatoMonto($totalpagpar),0,0,'R');
						...
						$totaldispar=$tb->fields["asignacion"]+$tb->fields["modificado"]-$tb->fields["compromiso"];
				...
				if (($categoria!=$tb->fields["categoria"])&&($categoria!="-1") )
				{
				    $this->setFont("Arial","B",7);
					$this->ln(2);
					$this->cell(73,4,"SUB-TOTAL ".$categoria." ",0,0,'R');
					...
					$totaldiscat=$tb->fields["asignacion"]+$tb->fields["modificado"]-$tb->fields["compromiso"];
					$acordado=$tb->fields["asignacion"]+$tb->fields["modificado"];
						if($acordado!=0)
				...
				else
				{
					...
					$totaldiscat=$totaldiscat+$tb->fields["asignacion"]+$tb->fields["modificado"]-$tb->fields["compromiso"];
				...
				$this->cell(23,4,H::FormatoMonto($tb->fields["modificado"]),0,0,'R');
				$this->cell(23,4,H::FormatoMonto($tb->fields["asignacion"]+$tb->fields["modificado"]),0,0,'R');
				$this->cell(23,4,H::FormatoMonto($tb->fields["compromiso"]),0,0,'R');
				$this->cell(23,4,H::FormatoMonto($tb->fields["asignacion"]+$tb->fields["modificado"]-$tb->fields["compromiso"]),0,0,'R');
				$this->cell(23,4,H::FormatoMonto($tb->fields["causado"]),0,0,'R');
				$this->cell(23,4,H::FormatoMonto($tb->fields["pagado"]),0,0,'R');
				$acordado=$tb->fields["asignacion"]+$tb->fields["modificado"];
				if (($acordado)==0)
				...
				$this->multicell(47,4,(strtoupper($tbcod->fields["nompre"])));
			...
			$this->cell(73,7,"TOTAL GENERAL",0,0,'R');
			...
			$this->cell(23,7,H::FormatoMonto($totalpag),0,0,'R');
