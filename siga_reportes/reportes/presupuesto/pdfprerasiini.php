  ...
  class pdfreporte extends fpdf
  {
    ...
    function pdfreporte()
    {
     ...
     // $this->codprehasta=H::GetPost("codprehasta");
	  ...
	  $this->sect1=H::GetPost("sect1");
      $this->pro1=H::GetPost("pro1");
      $this->act1=H::GetPost("act1");
      $this->par1=H::GetPost("par1");
      $this->sect2=H::GetPost("sect2");
      $this->pro2=H::GetPost("pro2");
      $this->act2=H::GetPost("act2");
      $this->par2=H::GetPost("par2");
      $this->codpredesde=$this->sect1.'-'.$this->pro1.'-'. $this->act1.'-'.$this->par1;
      $this->codprehasta=$this->sect2.'-'.$this->pro2.'-'. $this->act2.'-'.$this->par2;
      ...
      $this->nombre=$prerasiini->sql_nombre($this->nivel2);
      ...
      foreach($this->arrp as $datos)
	  {
	  	   if($ref!=substr($datos['codigopre'],0,$conref) && strlen(trim(substr($datos['codigopre'],0,$conref)))==$conref)
		   {
		   	  if($ref and $acumgru>0)
			  {
			      ...
			      $this->cell($this->anchos[0],2,' TOTAL  '.$this->nombre[0][0].'  '.substr($ref,$this->loncat[0][0]),0,0);
			      $this->cell($this->anchos[1],2,'',0,0);
			      $this->cell(17,2,H::FormatoMonto($acumgru),0,0,"R");
				  $this->ln(6);
			  	  $acumgru=0;
		   ...
		   $this->cell($this->anchos[0]+10,2,$datos["codigopre"]);
	          $x=$this->GetX();
		   $this->cell($this->anchos[1]);
		   $this->cell(7,2,H::FormatoMonto($datos["monasi"]),0,0,"R");
		   $this->SetX($x-10);
	          $this->MultiCell($this->anchos[1],2,$datos["nombre"]);
		   $this->ln();
