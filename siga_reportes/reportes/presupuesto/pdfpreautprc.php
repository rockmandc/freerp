		...
		function pdfreporte()
		{
			...
			if($_GET["refprc1"]!="")
			{
				...
				$this->sql="SELECT A.REFPRC as referencia,A.DESPRC as descripcion,A.TIPPRC as tipo,to_char(A.FECPRC,'dd/mm/yyyy') as fecha,
							...
							(A.REFPRC >= ('".$this->refprc1."') AND
							A.REFPRC<= ('".$this->refprc2."'))
			...
			if ($this->aprobado=='SI'){
				$aprobado="D.aprreq='S' and ";
				//$this->statu='Aprobado';
							}
			if ($this->aprobado=='NO'){
				$aprobado="D.aprreq  is  null and ";
				//	$this->statu='No Aprobado';
							}

							if ($this->aprobado=='Ambos'){
				$aprobado="";
			...
			if($this->stacom=="Todos")
			{
				$this->sql="SELECT A.REFPRC as referencia,A.DESPRC as descripcion,A.TIPPRC as tipo,to_char(A.FECPRC,'dd/mm/yyyy') as fecha,
							A.STAPRC as status,B.CODPRE as codigo,C.NOMPRE as nombre,B.MONIMP as imputado,B.MonCom as comprometido,
							B.MonCau as causado,B.MonPag as pagado,B.MonAju as Ajuste,(B.MonImp-B.MonAju) as Mon_Aju,
							substr(b.codpre,1,11) as partida, (case when D.aprreq='S' then 'Aprobado' else 'No Aprobado' end)  as statusa
							FROM CPPRECOM as A, CPIMPPRC as B, CPDEFTIT as C, casolart D
							WHERE
							A.REFPRC = B.REFPRC  AND
							B.CodPre = C.CodPre AND
							(A.REFPRC >= ('".$this->refprc1."') AND
							A.REFPRC<= ('".$this->refprc2."')) AND
							(A.FECPRC >=to_date('".$this->fecprc1."','dd/mm/yyyy') AND
							A.FECPRC <=to_date('".$this->fecprc2."','dd/mm/yyyy')) AND
							(A.TIPPRC >= ('".$this->tipprc1."') AND
							A.TIPPRC<= ('".$this->tipprc2."')) AND
							(B.CODPRE >=('".$this->codpre1."') AND
							B.CODPRE<=('".$this->codpre2."')) AND
							B.CODPRE LIKE RTRIM('".$this->comodin."') and a.refprc=d.reqart
							ORDER BY A.FECPRC,A.REFPRC,B.CODPRE,A.TIPPRC limit 3000";
			}
			else
			{
				...
				elseif ($this->stacom=="Anulado"){ $this->stacom="N";}
					...
					$this->sql="SELECT A.REFPRC as referencia,A.DESPRC as descripcion,A.TIPPRC as tipo,to_char(A.FECPRC,'dd/mm/yyyy') as fecha,
								...
								(B.CODPRE >=('".$this->codpre1."') AND ".$aprobado."
								B.CODPRE<=('".$this->codpre2."')) AND
								B.CODPRE LIKE RTRIM('".$this->comodin."') and a.refprc=d.reqart
		...
		function Cuerpo()
		{
			...
			while(!$tb->EOF)
			{
				if($tb->fields["referencia"]!=$ref)
				{
					 ...
					 $this->statu=$tb->fields["statusa"];
					 //if($estado == 'A')
					 {
						$this->cell(50,5,$this->statu);
					 }
					 $this->SetXY(40,46);
				...
				while(!$tb->EOF and $ref==$tb->fields["referencia"])
				{
					...
					$this->SetX(60);
					$this->MultiCell(110,3,($tb->fields["nombre"]));
