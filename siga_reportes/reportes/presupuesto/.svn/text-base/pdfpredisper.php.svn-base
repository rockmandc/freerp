...
class pdfreporte extends fpdf
{
	...
	function Cuerpo()
	{
	 ...
	 foreach($this->arrp as $datos)
	 {
      ...
      if($this->asignacion!="Acumulados")
      {
        $this->sqlasi="select sum(monasi) as monto from cpasiini where codpre like '".$datos["codpre"]."%' and perpre>='".$this->perdesde."' and perpre<='".$this->perhasta."' and codpre LIKE '".$this->comodin."' and CODPRE>=('".$this->codpredes."') AND CODPRE<=('".$this->codprehas."')";
       // print $this->sqlasi;
        $tbasi=$this->bd->select($this->sqlasi);
        $datos["asignacion"]=$tbasi->fields["monto"];
      }
	   ...
	   if (($categoria!=$datos["categoria"])&&($categoria!=""))
	   {
		...
		$this->SetX(84);
		$this->cell(23,4,H::FormatoMonto($totalasicat),0,0,'R');
		...
		$this->cell(23,4,H::FormatoMonto($totaldeucat),0,0,'R');
		$this->setxy(10,$y);
	...
	$this->Line(10,$this->GetY(),270,$this->GetY());
	$this->cell(68,7,"TOTAL");
	$this->SetX(84);
	$this->cell(23,7,H::FormatoMonto($totalasi),0,0,'R');
	...
	$this->cell(23,7,H::FormatoMonto($totaldeu),0,0,'R');
    $this->setFont("Arial","",7);
