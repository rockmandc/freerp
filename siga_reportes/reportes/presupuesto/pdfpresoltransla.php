<?
    ...
    require_once("../../lib/modelo/sqls/presupuesto/Presoltransla.class.php");
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			if($_GET["codtrades"]!="")
			{
			$this->reftra1=str_replace('*',' ',$_GET["codtrades"]);
			$this->reftra2=str_replace('*',' ',$_GET["codtrahas"]);
			$this->llenartitulosmaestro();
			$this->presoltransla = new Presoltransla();
			$this->cab=new cabecera();
			$this->arrp = $this->presoltransla->sqlp2($this->reftra1,$this->reftra2);
			}
			else
		    {
			...
			$this->presoltransla = new Presoltransla();
			$this->cab=new cabecera();
			$this->arrp = $this->presoltransla->sqlp($this->reftra1,$this->reftra2,$this->pertra1,$this->pertra2,$this->fectra1,$this->fectra2,$this->estado);
		    }
$this->i=0;
		}
		...
		function Header()
		{
			...
			$this->cab->poner_cabecera2($this,strtoupper($titulo)." (".$this->estado."s)","l","n",$this->arrp[$this->i]["fecha"]);
...
//$this->cell(103,5,$cuenta);
		    foreach ($this->arrp as $dato)
            {
                ...
                //print H::PrintR($ref);print $dato["referencia"];
                    ...
                    $this->arrp1 = $this->presoltransla->codori($dato["referencia"]);
			        ...
			        //division del codigo presupuestario de origen
						...
						$this->prueba="SELECT NOMPRE as NOMBRE FROM CPDEFTIT WHERE CODPRE=('".$dato1["origen"]."')";
						//print $this->prueba;
  ...
  if ($this->gety()>170){
                    ...
                    $this->arrp2 = $this->presoltransla->coddes($dato["referencia"]);
                    //CICLO PARA LAS CUENTAS DE ORIGEN
                    $acudes = 0;
                //    $acunori=0;
