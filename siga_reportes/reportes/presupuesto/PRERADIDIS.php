      ...
      <td rowspan="2" valign="top" class="cell_padding_01"> <p class="breadcrumb">Reportes
        ...
        <div align="left">&nbsp;
          <table width="612" align="center" cellspacing="0" bordercolor="#6699CC" class="grid_line01_tl">
            ...
            <tr bordercolor="#6699CC">
              <td class="form_label_01"> <div align="left"><strong>Adici&oacute;n/Disminuci&oacute;n:</strong></div></td>
              <td>
                <?

			  	$tb=$bd->select("select refadi as mrefadi from cpadidis order by refadi");
               ?>
                <select name="adi1" class="breadcrumb" id="adi2">
                  <?
				  	while(!$tb->EOF)
					{
				  ?>
                  <option value="<? print $tb->fields["mrefadi"];?>"><? print $tb->fields["mrefadi"];?></option>
                  <?
				  $tb->MoveNext();
					}
				?>
              ...
              <td>
                <?
			  	...
			  	$tb=$bd->select("select refadi as mrefadi from cpadidis order by refadi desc");
               ?>
                <select name="adi2" class="breadcrumb" id="adi2">
                  <?
				  	while(!$tb->EOF)
					{
				  ?>
            ...
            <tr>
              <td class="form_label_01"> <div align="left"><strong>Fecha de Adi/Dis:</strong></div></td>
              <td> <div align="left"> </div>
                <div align="left">
                  <?

			  	$tb=$bd->select("select to_char(MIN(fecadi),'DD/MM/YYYY') as fecha1 from cpadidis");
				if(!$tb->EOF)
				{
					$fecha1=$tb->fields["fecha1"];
				}

			  	$tb2=$bd->select("select to_char(MAX(fecadi),'DD/MM/YYYY') as fecha2 from cpadidis");
				if(!$tb2->EOF)
				{
            ...
            <tr bordercolor="#6699CC">
              <td class="form_label_01"><strong>Tipo de Adi/Dis:</strong></td>
            ...
            <tr>
              ...
              <td> <div align="left">
			  	...
			  	$tb=$bd->select("select distinct(codpre) as codpre from cpmovadi order by codpre asc");
			  ?>
              ...
              <td>
                <?
			  	...
			  	$tb2=$bd->select("select distinct(codpre) as codpre from cpmovadi order by codpre desc");
			  ?>
				...
				{
			  ?>
                  <option value="<? print $tb2->fields["codpre"];?>"><? print $tb2->fields["codpre"];?></option>
                  <?
				  $tb2->MoveNext();
					}
				?>
...
{
	f=document.form1;
	f.titulo.value="ADICIONES-DISMINUCION";
	f.action="rPRERADIDIS.php";
	f.submit();
}
function cerrar()
{
