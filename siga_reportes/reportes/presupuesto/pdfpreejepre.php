...
//print '<pre>'; print $this->sql;exit;
		...
		function llenartitulosmaestro()
		{
				...
				$this->titulos[4]="Total Asignado";
				$this->titulos[5]="Compromisos";
				$this->titulos[6]="%";
				$this->titulos[7]="Disponibilidad Presupuestaria";
				$this->titulos[8]="Causados";
				$this->titulos[9]="%";
				$this->titulos[10]="Pagado";
				$this->titulos[11]="%";
		...
		function Header()
		{
			...
			/*$this->Image("../../img/logo_1.jpg",10,8,19);
			$this->SetX(310);
			$this->setFont("Arial","B",8);
			$this->Cell(50,10,'Pagina '.$this->PageNo().' de {nb}',0,0,'L');
			$fecha=date("d/m/Y");
			$this->ln(4);
			$this->SetX(310);
			$this->Cell(50,10,'Fecha: '.$fecha,0,0,'L');
			...
			$this->cab->poner_cabecera_f_b($this,"Ejecución del Presupuesto","ll","s","s");
			...
			//$this->cell(340,4,"Ejecución del Presupuesto",0,0,'C');
			$this->setFont("Arial","B",8);
			$this->setY(38);
			...
			$this->setWidths(array(38,44,28,28,28,28,17,28,28,17,28,17));
			//$this->setBorder(1);
			$this->setAligns(array("C","C","C","C","C","C","C","C","C","C","C","C"));
			$this->rowM(array($this->titulos[0],$this->titulos[1],$this->titulos[2],$this->titulos[3],$this->titulos[4],$this->titulos[5],$this->titulos[6],$this->titulos[7],$this->titulos[8],$this->titulos[9],$this->titulos[10],$this->titulos[11]));
			$this->ln(3);
...
//			$this->bd->actualizar('SET search_path TO "SIMA055"');
			...
			while(!$tb->EOF)
			{
				...
				$this->cell(17,4,H::FormatoMonto(($tb->fields["compromiso"]*100)/($tb->fields["modificado"]+$tb->fields["asignacion"])),0,0,'R');
				$this->cell(28,4,H::FormatoMonto(($tb->fields["modificado"]+$tb->fields["asignacion"])-$tb->fields["compromiso"]),0,0,'R');
				$this->cell(28,4,H::FormatoMonto($tb->fields["causado"]),0,0,'R');
				$this->cell(17,4,H::FormatoMonto(($tb->fields["causado"]*100)/$tb->fields["compromiso"]),0,0,'R');
				$this->cell(28,4,H::FormatoMonto($tb->fields["pagado"]),0,0,'R');
				$this->cell(17,4,H::FormatoMonto(($tb->fields["pagado"]*100)/$tb->fields["causado"]),0,0,'R');
                ...
                if($this->lonpartida==0 )
                  ...
                  $tbcod=$this->bd->select("select max(nompre) as nompre from cpdeftit where trim(codpre) like '".$tb->fields["codpre"]."'");
                  $fg="select max(nompre) as nompre from cpdeftit where trim(codpre) like '".$tb->fields["codpre"]."'";
               //   print $fg;
		        ...
		        //$porccom=$porccom+(($tb->fields["compromiso"]*100)/($tb->fields["asignacion"]+$tb->fields["modificacion"]));
		        //$porccau=$porccau+(($tb->fields["causado"]*100)/$tb->fields["compromiso"]);
		        //$porcpag=$porcpag+(($tb->fields["pagado"]*100)/$tb->fields["causado"]);
			...
			$this->cell(17,7,H::FormatoMonto(($totalcom*100)/($totalasi+$totalmod)),0,0,'R');
			$this->cell(28,7,H::FormatoMonto(($totalasi+$totalmod)-$totalcom),0,0,'R');
			$this->cell(28,7,H::FormatoMonto($totalcau),0,0,'R');
			$this->cell(17,7,H::FormatoMonto(($totalcau*100)/$totalcom),0,0,'R');
			$this->cell(28,7,H::FormatoMonto($totalpag),0,0,'R');
			$this->cell(17,7,H::FormatoMonto(($totalpag*100)/$totalcau),0,0,'R');
