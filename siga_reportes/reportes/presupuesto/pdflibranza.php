     ...
     foreach($this->arrp as $datos)
   {
    ...
    $asignacion=0;
     ...
     if ($this->perdesde=='01'){
       ...
       $asignacion=$this->consolidado->sql_cuerpo_cpasiini_al($this->perdesde,$datos["codpreasiini"],$this->comodin);
     ...
     $this->setFont("Arial","B",9);
      if ($this->perdesde=='01'){
         ...
         $this->cell(80,5,'Disponibilidad Al 01/'.$this->perdesde.'/'.$this->ano.' :  '.H::FormatoMonto($asignacion[0][0]),0,0,'R');
...
//print $disponible;exit;
  ...
  $this->esta=$asignacion[0][0];
  }else{
     $this->esta=$disponible;
