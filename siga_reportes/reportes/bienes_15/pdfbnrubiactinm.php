<?
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->sql="SELECT c.codinm, c.desinm,
				to_char(c.feccom,'dd/mm/yyyy') as feccom,
				to_char(c.fecreg,'dd/mm/yyyy') as fecreg, trim(c.codact) as codact, b.desact,
				c.dirinm, c.avaact, c.valini, c.codubi, a.desubi
				FROM bnreginm c, bnubibie as a, bndefact as b
				whERE c.stainm='A' and RTRIM(c.codubi) >= RTRIM('".$this->ubides."') AND
		...
		function llenartitulosmaestro()
		{
				...
				$this->titulos[4]="Dirección ";
				$this->titulos[5]="Avaluo ";
		...
		// print $this->sql;
			...
			while(!$tb->EOF)
			{
				if ($ubi==$tb->fields["codubi"])
				{
					...
					$aux=$tb->fields["desinm"];
					$cantt=$cantt +1;
					$cant=$cant+1;
					$this->cell($this->anchos[0],5,$tb->fields["codinm"]);
					$this->cell($this->anchos[1],5," ");
					$this->cell($this->anchos[2],5,$tb->fields["feccom"]);
					$this->cell($this->anchos[3],5,$tb->fields["fecreg"]);
					$this->cell($this->anchos[4],5,$tb->fields["dirinm"]);
  					$this->cell($this->anchos[5],5,number_format($tb->fields["avaact"],2,'.',','));
