		...
		function pdfreporte()
		{
			...
			$this->fecreg2=$_POST["fecreg2"];
				...
				$this->sql="SELECT
							...
							bncobseg d
							WHERE   
					  A.fecsegmue>=to_date('$this->fecreg1','dd/mm/yyyy') and
					  A.fecsegmue<=to_date('$this->fecreg2','dd/mm/yyyy') and
					  A.CODACT=C.CODACT  AND
                                     A.CODMUE=C.CODMUE and
                                     A.CODACT=B.CODACT AND
                                     A.CODMUE=B.CODMUE AND
                                     A.CODACT >= '".$this->codact1."' AND 
					  A.CODACT <= '".$this->codact2."' AND
					 (A.CODMUE) >= ('".$this->codmue1."') AND 
					 (A.CODMUE) <= ('".$this->codmue2."') AND
					 (A.NROSEGMUE) = (c.NROSEGMUE)  AND 
					 c.codcob=d.codcob
					  ORDER BY A.CODACT,A.CODMUE , A.NROSEGMUE";
		...
		function Header()
		{
			...
			$this->setFont("Arial","B",9);
			 $this->setFont("Arial","B",8);
		        ...
		        $this->rowm(array("CODIGO DEL ACTIVO","DESCRIPCION","Nro DE POLIZA","FECHA DE EMISION","NOMBRE DE SEGURO","PROMOTOR","FECHA DE VENC.","OBSERVACIONES"));
			...
			while(!$tb->EOF)
			{
             ...
             /*    if($this->getY()>186){
                ...
                $this->setFont("Arial","B",8);
		 		$this->setwidths(array(30,45,30,17,40,40,20,35));
		        $this->setaligns(array("C","C","C","C","C","C","C","C"));
		        $this->rowm(array("CODIGO DEL ACTIVO","DESCRIPCION","Nro DE POLIZA","FECHA DE EMI","NOMBRE DE SEGURO","PROMOTOR","FECHA DE VEN.","OBSERVACIONES"));
				$this->Line(10,45,270,45);
			    $this->setFont("Arial","",8);
					 }*/
			     $this->setwidths(array(30,45,30,17,40,40,18,40));
                 $this->setaligns(array("C","L","C","C","C","L","L","L"));
                 $this->rowm(array($tb->fields["acodact"]."-".$tb->fields["acodmue"],$tb->fields["adesmue"]." MARCA: ".$tb->fields["marca"]." MODELO: ".$tb->fields["modelo"]." AÑO ".$tb->fields["ano"].
                 " COLOR: ".$tb->fields["color"]. " PLACAS: ". $tb->fields["placa"]. " SERIAL: ". $tb->fields["serial"],$tb->fields["anrosegmue"],$tb->fields["afecsegmue"],$tb->fields["anomsegmue"],$tb->fields["aprosegmue"],$tb->fields["afecsegven"]));
                // $this->ln();
                 ...
                 $this->setwidths(array(30,80,50));
                 $this->setaligns(array("C","L","R"));
                 $this->rowm(array("COBERTURA","DESCRIPCION COBERTURA","MONTO"));
                 $status=1;
                 $this->ln();
				 ...
				 $this->setwidths(array(30,80,50));
                 $this->setaligns(array("C","L","R"));
                 $this->rowm(array($tb->fields["codcob"],$tb->fields["descob"],H::FormatoMonto($tb->fields["amonsegmue"])));
