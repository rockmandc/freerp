		...
		function ReactualizarSaldoAnteriores(){
				...
				$this->bd->actualizar("
							...
							--UPDATE CONTABB
								...
								UNION
								SELECT SUBSTR(CODCTA,1,4) AS CUENTA,SUM(SALANT) AS SALDO
								FROM CONTABB
								WHERE CARGAB='C'
								GROUP BY SUBSTR(CODCTA,1,4)
								UNION
								...
								UNION
								SELECT SUBSTR(CODCTA,1,15) AS CUENTA,SUM(SALANT) AS SALDO
								FROM CONTABB
								WHERE CARGAB='C'
								GROUP BY SUBSTR(CODCTA,1,15)
								UNION
								...
								UNION
								SELECT SUBSTR(A.CODCTA,1,4) AS CUENTA,TO_CHAR(A.FECCOM,'MM') AS PERIODO,SUM(CASE A.DEBCRE WHEN 'D' THEN A.MONASI ELSE 0 END) AS DEBITO,SUM(CASE A.DEBCRE WHEN 'C' THEN A.MONASI ELSE 0 END) AS CREDITO
								FROM CONTABC1 A,CONTABC B
								WHERE B.STACOM='A'AND
								A.NUMCOM=B.NUMCOM AND
								A.FECCOM=B.FECCOM
								GROUP BY SUBSTR(CODCTA,1,4),TO_CHAR(A.FECCOM,'MM')
								UNION
								...
								UNION
								SELECT SUBSTR(A.CODCTA,1,15) AS CUENTA,TO_CHAR(A.FECCOM,'MM') AS PERIODO,SUM(CASE A.DEBCRE WHEN 'D' THEN A.MONASI ELSE 0 END) AS DEBITO,SUM(CASE A.DEBCRE WHEN 'C' THEN A.MONASI ELSE 0 END) AS CREDITO
								FROM CONTABC1 A,CONTABC B
								WHERE B.STACOM='A'AND
								A.NUMCOM=B.NUMCOM AND
								A.FECCOM=B.FECCOM
								GROUP BY SUBSTR(CODCTA,1,15),TO_CHAR(A.FECCOM,'MM')
								UNION
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		function Header()
		{
			...
			$this->fecha();
			$this->cell(40,8,"Desde: ".$this->fecperdes."  ");
			$this->cell(20,8,"Hasta: ".$this->fecperhas);
			$this->ln(4);
			$this->cell($this->anchos[1],8,"Código de Cuenta");
			$this->cell($this->anchos[4],8,"Descripción de Cuenta");
//			$this->cell(22,10," ");
			$this->cell(35,8,"Saldo Anterior",0,0,'R');
			$this->cell(35,8,"Debe",0,0,'R');
			$this->cell(35,8,"Haber",0,0,'R');
 			$this->cell(40,8,"Saldo Actual",0,0,'R');
			...
			while(!$tb->EOF)
			{
				if ($tb->fields["titulo"]!=" "){
					...
					$this->Buscar_Saldo_Anterior();
					 ...
					 {
 					    $this->Line(125,$this->GetY()+4,270,$this->GetY()+4);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					$this->setFont("Arial","",8);
					 ...
					 {  $sw=0;
  					    $this->cell($this->anchos[1],2,$this->cuenta);
						$y=$this->GetY();
						$x=$this->GetX()+$this->anchos[4];
						$this->MultiCell($this->anchos[4],2,strtoupper($tb->fields["nombre"]));
						$y1=$this->GetY();
					 }   //EndIF
					...
					{
						$this->SetXY($x,$y);
						$this->cell(35,2,number_format($this->salant,2,',','.'),0,0,'R');
						$this->cell(35,2,number_format($tb->fields["debito"],2,',','.'),0,0,'R');
						$this->cell(35,2,number_format($tb->fields["credito"],2,',','.'),0,0,'R');
						//$this->cell(40,10,number_format(abs($tb->fields["saldo"]),2,',','.'),0,0,'R');
						$this->cell(40,2,number_format($tb->fields["saldo"],2,',','.'),0,0,'R');
