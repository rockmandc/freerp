<?
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			{
			$this->sql="select to_char(feccom,'dd/mm/yyyy') as feccom1, numcom, ltrim(rtrim(descom)) as descripcion, moncom, stacom as estado, tipcom 
						from contabc 
		...
		function Cuerpo()
		{
			...
			while(!$tb->EOF)
			{
				...
				$this->cell($this->anchos[0],5,$tb->fields["feccom1"]);
				$this->cell($this->anchos[1],5,$tb->fields["numcom"]);
				$this->cell($this->anchos[2],5,$tb->fields["tipcom"]);
				$this->cell($this->anchos[3],5,'');
				...
				$this->cell($this->anchos[4],5,$aux);
				$this->cell($this->anchos[5],5,number_format($tb->fields["moncom"],2,'.',','),0,0,"R");
				$this->SetX(50);
				$this->multicell($this->anchos[3],4,$tb->fields["descripcion"]);
