		...
		function pdfreporte()
		{
			...
			if (strtoupper($this->estado)=="T")
			{
			    $this->sql="SELECT
						...
						FROM CONTABB A left outer join CONTABC1 C on (A.CODCTA = C.CODCTA AND 
						C.FECCOM >= to_date('".$this->feccom1."','dd/mm/yyyy') AND
        					C.FECCOM <= to_date('".$this->feccom2."','dd/mm/yyyy'))
							left outer join CONTABC B on (B.NUMCOM = C.NUMCOM AND
						...
						WHERE							
        					...
        					A.CARGAB = 'C' --AND
							--(select salact from contabb1 where pereje=to_char(to_date('".$this->feccom2."','dd/mm/yyyy'),'mm') and codcta=A.CODCTA)>0
						ORDER BY A.CODCTA,C.CODCTA,C.FECCOM,C.NUMCOM"; 
			}
			else
				{
					$this->sql="SELECT
		...
		function Cuerpo()
		{
			...
			while(!$tb->EOF)
			{
				...
				//Detalle
				 ...
				 $this->cell(35,5,$tb->fields["numcom"]);
