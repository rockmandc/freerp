...
*/
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->car_elaborado=$_POST["car_elaborado"];
			$this->revisado=$_POST["revisado"];
			$this->car_revisado=$_POST["car_revisado"];
			$this->aprobado=$_POST["aprobado"];
			$this->car_aprobado=$_POST["car_aprobado"];
			...
			$this->sql="SELECT  A.CODCTA,A.ORDEN,
								...
								--(sum(saldo)+(sum(DEBITO)-sum(CREDITO))) AS SALDO,
								A.CARGABLE
						FROM
							(SELECT A.CODCTA, RTRIM(A.CODCTA) AS ORDEN,
								...
								SELECT A.CODCTA,RTRIM(A.CODCTA)||'T' AS ORDEN,'TOTAL' AS TITULO,
								 ...
								 UNION ALL
										...
										(SELECT (SELECT SUM(A.SALACT) FROM CONTABB1 A WHERE A.CODCTA=B.CODIND AND A.PEREJE='$this->periodo') +
										(SELECT SUM(A.SALACT) FROM CONTABB1 A WHERE A.CODCTA=B.CODEGD AND A.PEREJE='$this->periodo') AS RESULT
										FROM CONTABA B) D
										where INSTR(TRIM(B.CODRES),TRIM(A.CODCTA),1,1)=1 AND
										A.CODCTA=C.CODCTA AND
										C.PEREJE='$this->periodo'
									   ) A,
						...
						WHERE								
								...
								A.CUENTA LIKE (RTRIM(B.CODCTD)||'%')  OR
                                                                A.CUENTA LIKE (RTRIM(B.CODORD)||'%')) and
								--((A.CARGABLE='C' AND ABS(A.SALDO) <> 0 ) OR (A.CARGABLE<>'C'))
								ABS(A.SALDO) <> 0
								GROUP BY A.CODCTA,A.ORDEN,
...
//		   $tb4=$this->bd->select("SELECT coalesce(LENGTH(SUBSTR(FORCTA,1,position('-' in FORCTA)-1)), 0) as LONIV1,
		...
		/*	$year = substr($this->fecha_cie,0,4);
			...
			$this->cell(180,10,"Del 01-01 al ".$dia."-".$mes."-".$year,0,0,'C');*/
			$year = substr($this->fecha_cieper,0,4);
			$mes = substr($this->fecha_cieper,5,2);
			$dia = substr($this->fecha_cieper,8,2);
			if ($this->p_fecha!=""){

				$this->cell(180,10,"Del 01-01 al  ".$this->p_fecha,0,0,'C');

				$this->ln(1);
				
				$this->cell(180,15,"(En Bolivares)",0,0,'C');
			}
             if ($this->fecha_p==""){
...
//						   $this->Line($this->GetX()+3,$this->GetY()+2,$this->GetX()-30,$this->GetY()+2);
			...
			if ($i==1){
			 ...
			 $this->cell($this->anchos[0],10,"TOTAL PASIVO + PATRIMONIO");
 			 $this->cell($this->anchos[1]+30,10,number_format(($this->total_pasivo)+($this->total_patriminio),2,',','.'),0,0,'R');
 			 //$this->cell($this->anchos[1]+30,10,number_format(($this->total_pasivo)+($this->total_patriminio)+($this->total),2,',','.'),0,0,'R');
 			 //$this->cell($this->anchos[1]+30,10,number_format(($this->total_pasivo),2,',','.'),0,0,'R');
 			 $this->total_pasivo=0;
			...
			$this->ln(4);
				...
				$this->cell(60,15,"Elaborado:",0,0,'C');
				$this->cell(60,15,"Revisado:",0,0,'C');
				$this->cell(60,15,"Aprobado:",0,0,'C');

			$this->ln(12);
				
				$this->cell(60,15,"_______________",0,0,'C');
				$this->cell(60,15,"_______________",0,0,'C');
				$this->cell(60,15,"_______________",0,0,'C');

			$this->ln(4);
				
				$this->cell(60,15,$this->elaborado,0,0,'C');
				$this->cell(60,15,$this->revisado,0,0,'C');
				$this->cell(60,15,$this->aprobado,0,0,'C');

						$this->ln(4);
				
				$this->cell(60,15,$this->car_elaborado,0,0,'C');
				$this->cell(60,15,$this->car_revisado,0,0,'C');
				$this->cell(60,15,$this->car_aprobado,0,0,'C');
