		...
		function pdfreporte()
		{
			...
			$this->aprocar=$_POST["aprocar"];
				...
				$this->sql="SELECT A.CODACT as acodact,A.CODMUE as acodmue,generar_descripcion(A.CODMUE) as adesmue,A.NRODISMUE as anrodismue,A.MOTDISMUE as amotdismue,A.TIPDISMUE as atipdismue,
							...
							RTRIM(A.NRODISMUE) >= RTRIM('".$this->nrodismue1."') AND 
							RTRIM(A.NRODISMUE) <= RTRIM('".$this->nrodismue2."') AND
							RTRIM(A.CODUBIORI) >= RTRIM('".$this->codubiori1."') AND 
							RTRIM(A.CODUBIORI) <= RTRIM('".$this->codubiori2."') AND
							RTRIM(A.TIPDISMUE) >= RTRIM('".$this->tipdismue1."') AND 
							RTRIM(A.TIPDISMUE) <= RTRIM('".$this->tipdismue2."') AND
							A.CODACT >= RTRIM('".$this->codact1."') AND 
							A.CODACT <= RTRIM('".$this->codact2."') and
							A.fecdismue>=to_date('".$this->fecdis1."','dd/mm/yyyy') and A.fecdismue<=to_date('".$this->fecdis2."','dd/mm/yyyy')
							 ORDER BY A.NRODISMUE";
	...
	//		{
			while(!$tb->EOF)
			{
				...
				$this->cell(60);
				 ...
				 $this->cell($this->anchos2[0],7,$tb->fields["acodmue"]);
