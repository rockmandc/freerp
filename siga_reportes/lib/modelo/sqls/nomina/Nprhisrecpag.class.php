...
//f.codcat
    ...
    function sqlcontador($codnomdes,$codnomhas,$codempdes,$codemphas,$codcardes,$codcarhas,$codcondes,$codconhas)
    {
    	$sql="select  count(distinct(a.codemp)) as numemp
							from nphiscon a, npcargos b, npdefcpt d,npcatpre f,
							nphojint c LEFT OUTER JOIN npbancos g ON (c.codban=g.codban)
							where
						...
						trim(a.codcar) >=trim('".$codcardes."') and trim(a.codcar) <= trim('".$codcarhas."') and
						a.codcon >= trim('".$codcondes."') and a.codcon <=trim('".$codconhas."') and
						a.codcon=d.codcon and d.impcpt = 'S' and  d.opecon <> 'P'
						";
    ...
    function sqlcalulasueldo($codnom,$codemp,$fecnomdes,$fecnomhas)
    {
    	$sql2="select suecar as valor
						from npcargos
						where
						trim(codcar) =trim('".$codcar."')";

						$sql="select sum(monto) as valor from
   							nphiscon a,npconsueldo b
   							where
							((a.codcon='AF8') or (a.codcon='AF5') or
   							a.codcon=b.codcon) and
   							a.codnom=b.codnom and
   							a.codnom= '".$codnom."' and 
  							a.codemp='".$codemp."' and 
   							a.fecnom>=to_date('$fecnomdes','DD/MM/YYYY')and 
   							a.fecnom<=to_date('$fecnomhas','DD/MM/YYYY') ";
