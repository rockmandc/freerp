...
class Nprliqpresoc extends baseClases {
  ...
  function sqlp($empdes,$emphas)
  {
	/*$sql="SELECT DISTINCT
		...
		$sql="	select distinct a.codemp,b.nomemp as nombre,b.cedemp,to_char(g.fechaingreso,'dd/mm/yyyy') as fecing,
				...
				g.anoefectivo as ano,g.mesefectivo as mes,g.diasefectivo as dia,g.anticipos,h.suecar,d.codnom,d.codcar,j.desret from npliquidacion_det a,nphojint b,
				npasicaremp d,npestorg e,empresa f,npliquidacion g,npcargos h, nptipret j
				where 
				a.codemp=b.codemp and 
				a.codemp=d.codemp and 
				e.codniv=b.codniv and 
				d.codcar=h.codcar and 
				f.codemp='001' and a.codemp=g.cedula
				and a.codemp>='".$empdes."' and a.codemp<='".$emphas."' and b.codret=j.codret and d.status='V'  ";
			//H::printR($sql);exit;
  ...
  {
  	$sql2="Select coalesce(SUM(a.monto),0) as campo from nphiscon A,
		...
		$sql3="Select coalesce(SUM(a.monto),0) as campo from nphiscon A,
		NPCONSALINT B where A.CODCON=B.CODCON AND a.CODNOM=B.CODNOM  and a.codemp='$codemp' and
		to_char(fecnom,'mm/yyyy')=to_char((select max(fecfin) from npimppresoc where codemp=a.codemp),'mm/yyyy')";

               $sql="Select coalesce(SUM(a.monasi),0) as campo from npsalint A
				 where
				a.codemp='".$codemp."' and
				to_char(fecinicon,'mm/yyyy')=to_char((select max(fecinicon ) from npsalint
				where codemp='".$codemp."' and monasi>0),'mm/yyyy')";
