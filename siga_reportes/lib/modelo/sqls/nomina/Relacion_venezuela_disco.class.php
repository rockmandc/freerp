...
//$sql="select codemp, replace(nomemp,',',' '), cuenta_banco, sum(monto) as monto from ( 
					$sql="select codemp, codemp as cedemp, nomemp, cuenta_banco, sum(monto) as monto from ( 
						(SELECT DISTINCT A.CODEMP AS CODEMP, b.nomemp as nombre, 
						cast(A.codemp as int) AS CODEMPORD,SUM (A.SALDO) AS monto,
						B.CeDEMP as cedemp, B.NOMEMP as nomemp, replace(b.numcue,'-', '' ) as cuenta_banco 
					...
					union
						...
						(SELECT DISTINCT A.CODEMP AS CODEMP, b.nomemp as nombre, 
						cast(A.codemp as int) AS CODEMPORD,(SUM (A.SALDO))*-1 AS monto,
						B.CeDEMP as cedemp, B.NOMEMP as nomemp,  replace(b.numcue,'-', '' ) as cuenta_banco
						FROM NPNOMCAL A left OUTER JOIN NPHOJINT B ON (A.CODEMP = B.CODEMP), NPBANCOS C
						...
						GROUP BY CODEMPORD, A.CODEMP, B.CEDEMP, B.NOMEMP,B.NUMCUE
						order by CODEMPORD))
                                          a group by codemp, nomemp, cuenta_banco, codempord
                                          order by codempord";
