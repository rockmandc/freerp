...
class Carordser extends baseClases
{
  ...
  function sqlp($ordcomdes,$ordcomhas,$codprodes,$codprohas,$codartdes,$codarthas,$fecorddes,$fecordhas,$status,$despacho,$tipcom)
  {
        if ($status=="Activas")
	{
	...
	$sql="SELECT DISTINCT A.ORDCOM,
			...
			B.codcat=F.codartpro and
			A.ORDCOM = F.ORDCOM AND
			A.CODPRO = E.CODPRO AND
			B.CODART  = C.CODART ".$tipcom."
