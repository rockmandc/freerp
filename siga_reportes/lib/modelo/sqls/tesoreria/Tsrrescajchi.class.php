...
class Tsrrescajchi extends baseClases
{
 ...
 function montoape()
  {
  	 $sql="select monapecajchi, porrepcajchi, sum(monord) as monren from opdefemp, opordpag b where tipcau='REND' group by monapecajchi, porrepcajchi ";
       //H::PrintR($sql);exit;
 ...
 function montotal()
  {
  	 $sql="select sum(monsal) as montotal from tssalcaj"; 
       //H::PrintR($sql);exit;
