...
class cabecera {
	...
	function poner_cabecera($objeto, $rep, $configuracion, $pagina) {
		if ($configuracion == "p") {
			...
			//Logo de la Empresa
			$objeto->Image("../../img/logo_1.jpg", 8, 10, 36);
			//33
			//fecha actual
			$fecha = date("d/m/Y");
			 ...
			 //$objeto->Cell(196, 10, 'Fecha: 27/12/2013 ', 0, 0, 'R');
			...
			//Paginas
			if ($pagina == "s") {
				$objeto->Cell(200, 10, 'Página ' . $objeto->PageNo() . ' de {nb}', 0, 0, 'R');
			}
			...
			$tab = 45;
			$objeto->setX($tab);
			$objeto->Cell(270, 10, 'Multiequipos Samoa C.A.', 0, 0, 'L');
			$objeto->Ln(5);
			$objeto->setX($tab);
			$objeto->setFont("Arial", "B", 10);
			$objeto->Cell(270, 10, 'Rif: J-40060588-1', 0, 0, 'L');
			$objeto->Ln(3);
			$objeto->setX($tab);
			...
			//$objeto->Cell(470, 10, 'Fecha: 27/12/2013 ', 0, 0, 'C');
			$objeto->ln(5);
			//Paginas
			if ($pagina == "s") {
		...
		function poner_cabecera_O($objeto, $rep, $configuracion, $pagina) {
		if ($configuracion == "p") {
			//configuro la pagina con Orientacion Vertical
			//busco la descripcion y direccion de la empresa
			$tb3 = $this->bd->select("select * from empresa where codemp='001'");
	...
	function poner_cabecera_f2($objeto, $rep, $configuracion, $pagina,$fec_ha) {
		if ($configuracion == "p") {
			...
			//Paginas
			if ($pagina == "s") {
				$objeto->Cell(610, 10, 'Página ' . $objeto->PageNo() . ' de {nb}', 0, 0, 'C');
			}
			//Titulo Descripcion de la Empresa
			$tab = 45;
			$objeto->setFont("Arial", "B", 12);
	...
	function poner_cabecera_f_b($objeto, $rep, $configuracion, $pagina,$fec_ha) {
		if ($configuracion == "p") {
			...
			if ($pagina == "s") {
					$objeto->SetX(173);
				$objeto->Cell(30, 10, 'Página ' . $objeto->PageNo() . ' de {nb}', 0, 0, 'l');
			}
			//Titulo Descripcion de la Empresa
			/*$objeto->Ln(-5);
			$objeto->Cell(180,5,$nombre,0,0,'L');
	...
	$objeto->ln(10);
			...
			if($fec_ha=='s'){
				...
				//$objeto->Cell(470, 10, 'Fecha: 26/12/2013', 0, 0, 'C');
	...
	function poner_cabecera2($objeto,$rep,$configuracion,$pagina,$fechaa)
	{
		if($configuracion=="p")
		{
			...
			if($pagina=="s")
			{
				$objeto->Cell(350,10,'Página '.$objeto->PageNo().' de {nb}',0,0,'C');
			}
	    	//Titulo Descripcion de la Empresa
    		/*$objeto->Ln(-5);
    		$objeto->Cell(180,5,$nombre,0,0,'L');
...
/////////////////////////////////////
      ...
      $fecha=date("d/m/Y");
	  //$objeto->Cell(470,10,'Fecha:  31/03/2014',0,0,'C');
